"""One-use runner for the frozen C63 VIX term-structure SPY/QQQ lane."""
...
from research.adapters.c63_vix_term_structure_compression_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c63_vix_term_structure_compression_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c63_vix_term_structure_compression_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6a9253c05060ea35eada61d989066c72e8d789192a9a46a56a2778d604c7a61d"
ADAPTER_SHA256 = "2511ba3610268cb02b1a8772b079559c8a54ee34f14e3062dbfd1cb53468cb8c"
SIGNAL_SHA256 = "1d82026d93b79c8eec075e598a094a4333293b1391507c481246800897722c1c"
RECEIPT_SHA256 = "d0107b92a11fbb5c0710e2f4e80b2a49daa6aa8a757c150c67aa3b86ba59dd2b"
...
SIGNAL_INPUT = Path(
    ...
    "c63_vix_term_structure_compression_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "VIX term-structure rotation input packet")
    expected_contract = {
        "left_symbol": "^VIX",
        "right_symbol": "^VIX3M",
        "ratio_change_window": (
            "VIX/VIX3M ratio at the last common accepted XNYS close "
            "in M-2 versus M-1 for execution month M"
        ),
        "target_rule": (
            "strict prior-month VIX/VIX3M ratio decline selects QQQ; equality "
            "or increase selects SPY"
        ),
        "threshold": 0,
    }
    if (
        record.get("schema_version")
        != "c63-vix-term-structure-compression-input-packet-v1"
        ...
        raise InputBlockedError("VIX term-structure packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("VIX term-structure signal row count mismatch")
    ...
    expected_source = {
        "^VIX": (
            ...
            "208ecc65d9ef7a1531cdedda74d8fa247dc24d2f99dbdde2769135a58e511944",
        ),
        "^VIX3M": (
            "https://query1.finance.yahoo.com/v8/finance/chart/%5EVIX3M?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "fcb4ba6cf1fd37a3ed3064ebfaed5a46f0c7f26a40f9f2597b492788cd48150f",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("VIX term-structure signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("VIX term-structure selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "VIX term-structure adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError(
                    "VIX term-structure source row identity mismatch"
                )
        ...
        try:
            cross_product = Decimal(row.get("relative_ratio_cross_product"))
        except (TypeError, ValueError, ArithmeticError) as exc:
            raise InputBlockedError(
                "VIX term-structure cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("VIX term-structure arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "VIX term-structure month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "VIX term-structure row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("VIX term-structure signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("VIX term-structure state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "VIX term-structure signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("VIX term-structure state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "VIX term-structure rotation input packet")
    receipt = _json(
        receipt_payload,
        "VIX term-structure materialization receipt",
    )
    ...
    if (
        ...
        != "c63-vix-term-structure-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "^VIX"
        or receipt.get("right_symbol") != "^VIX3M"
        or receipt.get("raw_fetch_receipt_sha256")
        != "5fbd963d354795e42384e94a3cf3eae6d88c329ec7003888e0d97e1bb56c3378"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "VIX term-structure signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c63-vix-term-structure-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c63-vix-term-structure-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
