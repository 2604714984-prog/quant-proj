"""One-use runner for frozen C88 relative return skewness."""
...
from research.adapters.c88_relative_return_skewness_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c88_relative_return_skewness_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c88_relative_return_skewness_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "26f030d9b60e9c3ad8ea44c17d6b258a30f5581e29b78cf4b3d9991511c9900b"
ADAPTER_SHA256 = "418ff52914edb78573dbc890406aa24308248aabc257aae92732cf18b64f51cc"
SIGNAL_SHA256 = "737b363ecc5fd79943486b78caccbc26d2d2c3bac1dc527dee4160a93b2f52b5"
RECEIPT_SHA256 = "1d07574909ccbb0285ee637520cf6faaaf159bbe12613960d1bacdf8f836ab81"
...
SIGNAL_INPUT = Path(
    ...
    "c88_relative_return_skewness_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C88 relative-skewness input packet")
    expected_contract = {
        "formation_close_count": 127,
        "return_count": 126,
        "return_type": "natural log return",
        "population_skewness": (
            "mean centered cube divided by population variance to the power 1.5"
        ),
        "target_rule": (
            "QQQ population skewness strictly above SPY means QQQ; "
    ...
    if (
        ...
        != "c88-relative-return-skewness-input-v1"
        ...
        raise InputBlockedError("C88 relative-skewness packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C88 relative-skewness signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        ...
        "stage",
        "qqq_mean_log_return",
        "qqq_population_skewness",
        "qqq_second_central_moment",
        "qqq_third_central_moment",
        "relative_skewness_difference",
        "return_count",
        "spy_mean_log_return",
        "spy_population_skewness",
        "spy_second_central_moment",
        "spy_third_central_moment",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C88 relative-skewness signal schema mismatch")
        numeric_fields = (
            "spy_mean_log_return",
            "spy_second_central_moment",
            "spy_third_central_moment",
        ...
        if any(
            ...
            raise InputBlockedError("C88 relative-skewness value is invalid")
        if (
            type(row["return_count"]) is not int
            or row["return_count"] != 126
        ):
            raise InputBlockedError("C88 return count mismatch")

        spy_second = float(row["spy_second_central_moment"])
        qqq_second = float(row["qqq_second_central_moment"])
        if spy_second <= 0.0 or qqq_second <= 0.0:
            raise InputBlockedError("C88 skewness variance must be positive")
        spy_skewness = float(row["spy_third_central_moment"]) / (
            spy_second ** 1.5
        )
        qqq_skewness = float(row["qqq_third_central_moment"]) / (
            qqq_second ** 1.5
        )
        difference = qqq_skewness - spy_skewness
        if not math.isclose(
            float(row["spy_population_skewness"]),
            spy_skewness,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["qqq_population_skewness"]),
            qqq_skewness,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["relative_skewness_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C88 relative-skewness arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C88 relative-skewness target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C88 relative-skewness chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C88 relative-skewness signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C88 relative-skewness stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C88 relative-skewness state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C88 relative-skewness signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C88 relative-skewness state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C88 relative-skewness input packet")
    receipt = _json(
        receipt_payload,
        "C88 relative-skewness materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c88-relative-return-skewness-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C88 relative-skewness signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c88-relative-return-skewness-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c88-relative-return-skewness-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C88 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C88 holdout one-use state is not pristine")
