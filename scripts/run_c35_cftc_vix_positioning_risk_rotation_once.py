"""One-use runner for the frozen Cycle 35 CFTC VIX-positioning SPY/QQQ lane."""
...
from research.adapters.c35_cftc_vix_positioning_risk_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c35_cftc_vix_positioning_risk_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c35_cftc_vix_positioning_risk_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "8653bbbf55f7341e9665e30195e542c42177062b9925dceae91d10561158d8ea"
ADAPTER_SHA256 = "f4fb4839082722fa433f85f863d13097b9210c68f77c059e09168a6659f4195a"
SIGNAL_SHA256 = "a5f171e1d2b9ded2906d85153ce36a312a832ed25a32c0d4045395ac1d494cb9"
RECEIPT_SHA256 = "a1e0aafda520e0f24e6a487a5a0ae218b2c13390ad33d22147af7b86f45ad450"
...
SIGNAL_INPUT = Path(
    ...
    "c35_cftc_vix_positioning_risk_rotation/"
    "c35_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "CFTC VIX positioning rotation input packet")
    if (
        ...
        != {
            "vix_contract_code": CFTC_VIX_CODE,
        ...
        raise InputBlockedError("CFTC VIX positioning packet boundary mismatch")
    ...
    for row in rows:
        ...
        for selection in (previous, current):
            ...
            contract = selection.get("vix_contract_row")
            if type(contract) is not dict:
                raise InputBlockedError("CFTC VIX contract row is required")
            raw_identity = {
                key: value for key, value in contract.items() if key != "row_sha256"
            }
            long_value = contract.get("leveraged_money_long_all")
            short_value = contract.get("leveraged_money_short_all")
            ...
            if (
                ...
                or contract.get("cftc_contract_market_code") != CFTC_VIX_CODE
                or contract.get("market_and_exchange_name")
                ...
                or selection.get("vix_contract_row_sha256") != expected_row_hash
            ):
                raise InputBlockedError("CFTC VIX contract row identity mismatch")
            observed_ratio = selection.get("vix_leveraged_money_net_pct_oi")
            ...
            if (
                ...
                raise InputBlockedError("CFTC VIX net-position ratio mismatch")
            selection_identity = {
                key: value
        ...
        current_value = current.get("vix_leveraged_money_net_pct_oi")
        previous_value = previous.get("vix_leveraged_money_net_pct_oi")
        change = row.get("delta_vix_leveraged_money_net_pct_oi")
        if any(
        ...
        expected_state = "SPY" if float(change) > 0.0 else "QQQ"
        ...
        identity = {
            ...
            "delta_vix_leveraged_money_net_pct_oi": float(change),
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c35-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    if (
        ...
        != "c35-cftc-vix-positioning-full-receipt-v1"
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c35-cftc-vix-positioning-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c35-cftc-vix-positioning-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 35 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 35 holdout one-use state is not pristine")
