"""One-use runner for the frozen Cycle 21 RSAFS SPY/QQQ rotation lane."""
...
from research.adapters.c21_rsafs_retail_demand_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c21_rsafs_retail_demand_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c21_rsafs_retail_demand_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "e14bbcdf7edb6934fc525e703f3f044a83218447b8efa370e93215d837315001"
ADAPTER_SHA256 = "99d70bf4a3e9cb09ffa72714bed06e5fee0d4420a3bd193b90f1ab5be0c53ce1"
SIGNAL_SHA256 = "4c4cbfe348573950455a5046f57c869a9e638c2dc996435a5cfb75b93eadcb7d"
RECEIPT_SHA256 = "29ffb69a24a1616debdafe44d04339ea6e891492e0b7acd8752025a2161e6000"
...
SIGNAL_INPUT = Path(
    ...
    "c21_rsafs_retail_demand_rotation_spy_qqq_v1/"
    "c21_full_input_packet.json"
)
...
def _previous_month(value: date) -> date:
    if value.day != 1:
        raise InputBlockedError("RSAFS reference month must begin on day one")
    if value.month == 1:
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "RSAFS retail-demand rotation input packet")
    if (
        record.get("schema_version")
        != "c21-rsafs-retail-demand-rotation-spy-qqq-input-packet-v1"
        ...
        != {
            "series_id": "RSAFS",
            "target_rule": "delta > 0 means SPY; delta <= 0 means QQQ",
            "threshold_millions_usd": 0.0,
        }
    ):
        raise InputBlockedError("RSAFS retail-demand rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("RSAFS signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("RSAFS signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("RSAFS selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "RSAFS"
                or selection.get("source_class") != "OFFICIAL_ALFRED_RSAFS"
                or selection.get("raw_response_sha256")
                != "ce1986eafa190ac128c7a12aad2ab1cf5b08c186944ef06e67cea0dde9c973b8"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("RSAFS source row identity mismatch")
        current_value = current.get("value_millions_usd")
        previous_value = previous.get("value_millions_usd")
        change = row.get("delta_millions_usd")
        if any(
            ...
            raise InputBlockedError("RSAFS values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("RSAFS change arithmetic mismatch")
        ...
        previous_reference_month = _date(
            row.get("previous_reference_month"), "previous RSAFS reference month"
        )
        current_reference_month = _date(
            row.get("current_reference_month"), "current RSAFS reference month"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous RSAFS available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current RSAFS available_at",
            )
            > decision_at
            ...
            raise InputBlockedError(
                "RSAFS reference-month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "RSAFS row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("RSAFS signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("RSAFS state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("RSAFS signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("RSAFS state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c21-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "RSAFS retail-demand rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "RSAFS retail-demand rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "RSAFS signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c21-rsafs-retail-demand-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c21-rsafs-retail-demand-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 21 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 21 holdout one-use state is not pristine")
