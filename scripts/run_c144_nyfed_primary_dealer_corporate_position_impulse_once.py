"""One-use runner for frozen C144 primary-dealer position impulse rotation."""
...
from research.adapters.c144_nyfed_primary_dealer_corporate_position_impulse import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / (
    ...
    "c144_nyfed_primary_dealer_corporate_position_impulse_v1.json"
)
ADAPTER = ROOT / (
    "research/adapters/"
    "c144_nyfed_primary_dealer_corporate_position_impulse.py"
)
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "fc2ba8b672da32dff3d773149e3f68e502cc794519b41f1069a3708a9a28ab2c"
ADAPTER_SHA256 = "8e2d58f30822e1f1e1c9d858788439b3300a4cd0224002573b98e07f9c159897"
SIGNAL_SHA256 = "18c49b116878fce77590485a6be3432db97182fb24ec76f73acdf6e91e61d93c"
SIGNAL_RECEIPT_SHA256 = "9c8e2ffb2a0eed2caca442615d21e082898d30861e2ad994cd32de9169ffcd4d"
RUNTIME_RECEIPT_SHA256 = "7e03588802563f71a198b983353e35412d5a6d315aa6277ee73efddabe445ba4"
VALIDATION_BUNDLE_SHA256 = "906975645f9b49a63b02674e45b816b5fc7d75f743cb825f6a93d90be18142af"
HOLDOUT_BUNDLE_SHA256 = "ebf74260507ea4af09c5d4e88f992e46c0d6150e4224ceeaf4621d4862ad2032"
...
SIGNAL_INPUT = Path(
    ...
    "c144_nyfed_primary_dealer_corporate_position_impulse_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C144 NY Fed primary-dealer position signal input")
    expected_contract = {
        "direction": "positive_weekly_position_impulse_selects_QQQ_else_SPY",
        "field": "PDPOSCS-TOT",
        "latest_allowed_observation": (
            "latest as-of Wednesday observation whose Thursday release "
            "precedes the decision"
        ),
        "measure": "current_value_minus_immediately_previous_weekly_value",
    }
    if (
        record.get("schema_version")
        != "c144-nyfed-primary-dealer-position-input-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("evidence_class")
        != "OFFICIAL_RETROSPECTIVE_SECONDARY_REVISED_HISTORY"
        ...
        raise InputBlockedError("C144 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C144 signal row count mismatch")
    expected_row_keys = {
        "current_as_of_date",
        "current_value_usd_millions",
        "decision_at",
        "decision_session",
        "position_impulse_usd_millions",
        "prior_as_of_date",
        "prior_value_usd_millions",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C144 signal schema mismatch")
        value_fields = (
            "current_value_usd_millions",
            "prior_value_usd_millions",
        )
        if any(
            ...
            for value in (row[field] for field in value_fields)
        ):
            raise InputBlockedError("C144 position value is invalid")
        impulse = (
            row["current_value_usd_millions"]
            - row["prior_value_usd_millions"]
        )
        observed_impulse = row["position_impulse_usd_millions"]
        if (
            ...
            raise InputBlockedError("C144 position impulse is invalid")
        expected_state = "QQQ" if impulse > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C144 target mismatch")
        ...
        current_date = _date(
            row["current_as_of_date"],
            "current as-of date",
        )
        prior_date = _date(
            row["prior_as_of_date"],
            "prior as-of date",
        )
        preceding_wednesday = decision_session - timedelta(
            days=decision_session.weekday() + 5
        )
        if (
            row["decision_at"]
            != f"{decision_session.isoformat()}T09:00:00 America/New_York"
            or not prior_date < current_date <= preceding_wednesday
        ):
            raise InputBlockedError("C144 signal chronology mismatch")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C144 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C144 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C144 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C144 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C144 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C144 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C144 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C144 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C144 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C144 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c144-nyfed-primary-dealer-position-receipt-v1"
        ...
        != "c144-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c144-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C144 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C144 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C144 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C144 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c144-nyfed-primary-dealer-position-impulse-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c144-nyfed-primary-dealer-position-impulse-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C144 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C144 holdout one-use state is not pristine")
