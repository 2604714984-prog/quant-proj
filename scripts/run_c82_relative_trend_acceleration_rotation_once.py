"""One-use runner for frozen C82 relative-trend acceleration."""
...
from research.adapters.c82_relative_trend_acceleration_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c82_relative_trend_acceleration_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c82_relative_trend_acceleration_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "bb1630e851a7ef22167e37ebfe2fcca49443f4721781ce074a762581fca02e66"
ADAPTER_SHA256 = "c179bfbf1ae94318b29170da58df1456b8814e99fdc09d2a3b3b19673c396f79"
SIGNAL_SHA256 = "49c591c7c163d246e5375fce26998bb40fcaece5361aa35b5fe694b764982e80"
RECEIPT_SHA256 = "e2a083d535170df5907eec1ebdcb7a6b4b04681b65fa7bc7d30f79c00155bb73"
...
SIGNAL_INPUT = Path(
    ...
    "c82_relative_trend_acceleration_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C82 relative-trend acceleration input packet")
    expected_contract = {
        "formation_close_count": 127,
        "older_block_return_count": 63,
        "recent_block_return_count": 63,
        "return_type": "natural log return",
        "relative_return": "QQQ return minus SPY return",
        "acceleration": (
            "recent 63-return relative sum minus older 63-return relative sum"
        ),
        "target_rule": (
            "strictly positive acceleration means QQQ; zero or negative means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c82-relative-trend-acceleration-input-v1"
        ...
        raise InputBlockedError("C82 relative-trend packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C82 relative-trend signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        ...
        "older_block_end",
        "older_block_start",
        "older_relative_return_sum",
        "older_row_hash_set_sha256",
        "recent_block_end",
        "recent_block_start",
        "recent_relative_return_sum",
        "recent_row_hash_set_sha256",
        "relative_trend_acceleration",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C82 relative-trend signal schema mismatch")
        numeric_fields = (
            "older_relative_return_sum",
            "recent_relative_return_sum",
            "relative_trend_acceleration",
        ...
        if any(
            ...
            raise InputBlockedError("C82 relative-trend value is invalid")
        if type(row["return_count_per_block"]) is not int or row["return_count_per_block"] != 63:
            raise InputBlockedError("C82 block return count mismatch")

        older_sum = float(row["older_relative_return_sum"])
        recent_sum = float(row["recent_relative_return_sum"])
        acceleration = recent_sum - older_sum
        if not math.isclose(
            float(row["relative_trend_acceleration"]),
            acceleration,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C82 relative-trend arithmetic mismatch")
        expected_state = "QQQ" if acceleration > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C82 relative-trend target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C82 relative-trend chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C82 relative-trend signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C82 relative-trend stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C82 relative-trend state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C82 relative-trend signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C82 relative-trend state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C82 relative-trend input packet")
    receipt = _json(
        receipt_payload,
        "C82 relative-trend materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c82-relative-trend-acceleration-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C82 relative-trend signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c82-relative-trend-acceleration-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c82-relative-trend-acceleration-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C82 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C82 holdout one-use state is not pristine")
