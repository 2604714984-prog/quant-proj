"""One-use runner for the frozen Cycle 47 copper gold growth SPY/QQQ lane."""
...
from research.adapters.c47_copper_gold_growth_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c47_copper_gold_growth_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c47_copper_gold_growth_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "64383d4adf7f03466f4a976490a948c8241121ff509c87007b91af61b7a1fb8e"
ADAPTER_SHA256 = "3a01b4edcce82bd093112a898f80e8354ea635ad2f099a41b9b07317815f15fa"
SIGNAL_SHA256 = "4e69d353b389cc024e643a40b3010def811d9ccca3993780bd08a0897fdeaf26"
RECEIPT_SHA256 = "b5683a9ba2393a099cc7446f317450f82cd189327a03673e944370e487675b18"
...
SIGNAL_INPUT = Path(
    ...
    "c47_copper_gold_growth_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "copper gold growth rotation input packet")
    expected_contract = {
        "left_symbol": "CPER",
        "right_symbol": "GLD",
        "return_window": (
        ...
        "target_rule": (
            "strict CPER prior-month total-return outperformance versus GLD "
    ...
    if (
        ...
        raise InputBlockedError("copper gold growth packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("copper gold growth signal row count mismatch")
    ...
    expected_source = {
        "CPER": (
            "https://query1.finance.yahoo.com/v8/finance/chart/CPER?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "9e429245e49602b65f0069a24a0612998d2fa9d727940fee563a7abe1f4e3e75",
        ),
        "GLD": (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("copper gold growth signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("copper gold growth selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "copper gold growth adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("copper gold growth source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "copper gold growth cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("copper gold growth arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "copper gold growth month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("copper gold growth signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("copper gold growth signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "copper gold growth row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("copper gold growth signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("copper gold growth state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "copper gold growth signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("copper gold growth state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "copper gold growth rotation input packet")
    receipt = _json(
        receipt_payload,
        "copper gold growth materialization receipt",
    )
    ...
    if (
        ...
        or receipt.get("left_symbol") != "CPER"
        or receipt.get("right_symbol") != "GLD"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "copper gold growth signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c47-copper-gold-growth-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c47-copper-gold-growth-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 47 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 47 holdout one-use state is not pristine")
