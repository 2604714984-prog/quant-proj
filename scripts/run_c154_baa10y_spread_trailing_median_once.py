"""One-use runner for the frozen C154 BAA10YM SPY/QQQ rotation lane."""
...
from research.adapters.c154_baa10y_spread_trailing_median import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c154_baa10y_spread_trailing_median_v1.json"
ADAPTER = ROOT / "research/adapters/c154_baa10y_spread_trailing_median.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "f3fc1fcb1a616f8bd5ac3f7578ebbe05879d8ec65198793e769839a109cf691f"
ADAPTER_SHA256 = "ef5f5cff46923d08d14d25b9b686369ad1550ec5694356e26c0a7d2024bc1c46"
SIGNAL_SHA256 = "b94d2b2f430306a21149262b722c1a5955574df6d41baccbf0fc84aa4ae7d67c"
RECEIPT_SHA256 = "a3e04a82d27a41da5590c5406d6d253aca6415a5e5bf7b088c99440f80f24319"
...
SIGNAL_INPUT = Path(
    ...
    "c154_baa10y_spread_trailing_median_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "BAA10YM impulse rotation input packet")
    if (
        record.get("schema_version")
        != "c154-baa10ym-spy-qqq-full-input-v1"
        ...
        != {
            "series_id": "BAA10YM",
            "target_rule": "current spread <= median of prior 12 monthly observations means QQQ; otherwise SPY",
            "trailing_months": 12,
        }
    ):
        raise InputBlockedError("BAA10YM rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("BAA10YM signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("BAA10YM signal stage mismatch")
        required = {
            "actual_vintage_date",
            ...
            "spread_minus_trailing_median",
            "stage",
            "target_symbol",
            "trailing_median_spread_percent",
            "trailing_month_count",
            "trailing_window_end",
            "trailing_window_start",
        }
        if set(row) != required:
            raise InputBlockedError("BAA10YM signal fields mismatch")
        numeric = tuple(
            row.get(key)
            for key in (
                "current_spread_percent",
        ...
        if any(
            ...
            raise InputBlockedError("BAA10YM statistics must be finite")
        current_spread, trailing_median, distance = map(float, numeric)
        if not all(-100.0 < value < 100.0 for value in (current_spread, trailing_median)):
            raise InputBlockedError("BAA10YM spreads are implausible")
        ...
        reference_month = _date(row["reference_month"], "reference month")
        ...
        vintage = _date(row["actual_vintage_date"], "BAA10YM actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "BAA10YM response SHA-256"
        )
        if (
            vintage > decision_at.date()
            or reference_month >= start
            or not (
                trailing_start.day
                == trailing_end.day
                == reference_month.day
            ...
            or not trailing_start < trailing_end < reference_month
            or row.get("trailing_month_count") != 12
            or not math.isclose(
                distance,
                current_spread - trailing_median,
                rel_tol=0.0,
                abs_tol=1e-12,
            )
            ...
            raise InputBlockedError("BAA10YM chronology or state mismatch")
        identity = {
            "stage": row["stage"],
            ...
            "spread_minus_trailing_median": distance,
            "target_symbol": expected_state,
            "trailing_median_spread_percent": trailing_median,
            "trailing_month_count": 12,
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("BAA10YM signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("BAA10YM source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "BAA10YM row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("BAA10YM signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("BAA10YM state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("BAA10YM signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("BAA10YM state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c154-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "New York Fed BAA10YM/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "New York Fed BAA10YM/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c154-baa10ym-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "BAA10YM signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c154-nyfed-baa10ym-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c154-nyfed-baa10ym-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
