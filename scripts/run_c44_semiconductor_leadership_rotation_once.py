"""One-use runner for the frozen Cycle 44 semiconductor leadership SPY/QQQ lane."""
...
from research.adapters.c44_semiconductor_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c44_semiconductor_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c44_semiconductor_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "aa32ae891c6f3d96b0ffbbaa595b64de793d67419fb925bec3351f7bce052f01"
ADAPTER_SHA256 = "6f0bf281c6eb33987b4ca3841f742010e440e1402e500061ca1e0324975bcfed"
SIGNAL_SHA256 = "fd950703df64fee4b133696fd7046ef6a87b401a81838d14884de02ccdbf9934"
RECEIPT_SHA256 = "ebe52c49782a6125672065f7ecc6a53a074164a4ec7c4ca638a9c148ee04608b"
...
SIGNAL_INPUT = Path(
    ...
    "c44_semiconductor_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "semiconductor leadership rotation input packet")
    expected_contract = {
        "left_symbol": "SMH",
        "right_symbol": "SPY",
        "return_window": (
        ...
        "target_rule": (
            "strict SMH prior-month total-return outperformance versus SPY "
    ...
    if (
        ...
        raise InputBlockedError("semiconductor leadership packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("semiconductor leadership signal row count mismatch")
    ...
    expected_source = {
        "SMH": (
            ...
            "7f8b2995a3e4fa28f6cad25ae619239d9ccf41563036fc842ded3a097182df88",
        ),
        "SPY": (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("semiconductor leadership signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("semiconductor leadership selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "semiconductor leadership adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("semiconductor leadership source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "semiconductor leadership cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("semiconductor leadership arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "semiconductor leadership month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("semiconductor leadership signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("semiconductor leadership signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "semiconductor leadership row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("semiconductor leadership signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("semiconductor leadership state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "semiconductor leadership signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("semiconductor leadership state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "semiconductor leadership rotation input packet")
    receipt = _json(
        receipt_payload,
        "semiconductor leadership materialization receipt",
    )
    ...
    if (
        ...
        or receipt.get("left_symbol") != "SMH"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "semiconductor leadership signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c44-semiconductor-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c44-semiconductor-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 44 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 44 holdout one-use state is not pristine")
