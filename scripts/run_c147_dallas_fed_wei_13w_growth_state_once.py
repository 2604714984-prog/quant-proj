"""One-use runner for the frozen C147 Dallas Fed WEI 13-week state rotation."""
...
from research.adapters.c147_dallas_fed_wei_13w_growth_state import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / (
    ...
    "c147_dallas_fed_wei_13w_growth_state_v1.json"
)
ADAPTER = ROOT / (
    "research/adapters/"
    "c147_dallas_fed_wei_13w_growth_state.py"
)
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "962940062a7eb0442ac4b911ac7025e8f62b9d9ef379790f64ab66e4b976eb57"
ADAPTER_SHA256 = "7f41067cc841dede980320a47aff66cdf394bde68fafa07323181ece147996f4"
SIGNAL_SHA256 = "a87f4ad12230239e821daf6aeda1903f16d32a7933f069c57783d89721a091b7"
SOURCE_ROWS_SHA256 = "46364b61a822e9b11fbff0717a34e0fce999ff9e6d3e7a179963e68507bb7be8"
SIGNAL_RECEIPT_SHA256 = "a3ea7ddb0b25e9331415fa1000e8aa4cb5f1e1b4bd03c23d7c128f5bfbd991a4"
RUNTIME_RECEIPT_SHA256 = "394e322d6ff02557b30cc9080b0e635bea400b5ea4c98708e971c84a7358acd6"
VALIDATION_BUNDLE_SHA256 = "953c4272035e0da604604b3d3762b3379165863d5c25d8395b4f37943cc30754"
HOLDOUT_BUNDLE_SHA256 = "a20ae12c2b7b4b229168ddb6e0c0443d79646aff5e777e6216c9704ec38bc7b2"
...
SIGNAL_INPUT = Path(
    ...
    "c147_dallas_fed_wei_13w_growth_state_rotation_spy_qqq_v1/"
    "signal_input_packet.json"
)
SIGNAL_RECEIPT = SIGNAL_INPUT.with_name("materialization_receipt.json")
SOURCE_ROWS = SIGNAL_INPUT.with_name("selected_wei_rows.jsonl")
RUNTIME_RECEIPT = SIGNAL_INPUT.with_name("runtime_materialization_receipt.json")
VALIDATION_BUNDLE = SIGNAL_INPUT.with_name("validation_runtime_bundle.json")
HOLDOUT_BUNDLE = SIGNAL_INPUT.with_name("holdout_runtime_bundle.json")
...
QQQ_ACTION_SHA256 = "50bab02358e317de28d410246ed112b8d6a997175d262cf2617c27d245fe8aaf"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C147 Dallas Fed WEI signal input")
    expected_contract = {
        "anchor": "latest_non_null_week_ending_before_selected_actual_vintage",
        "direction": (
            "current_WEI_strictly_above_same_vintage_13_week_mean_selects_QQQ_else_SPY"
        ),
        "reference": (
            "arithmetic_mean_of_exactly_13_consecutive_same_vintage_weekly_values"
        ),
        "vintage": (
            "latest_actual_ALFRED_vintage_no_later_than_conservative_Thursday_cutoff"
        ),
    }
    if (
        record.get("schema_version")
        != "c147-dallas-fed-wei-13w-growth-state-input-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("evidence_class")
        != "OFFICIAL_ALFRED_REAL_TIME_VINTAGE_RETROSPECTIVE_SECONDARY"
        ...
        or record.get("strategy_candidate_available") is not False
        or record.get("status") != "INPUT_MATERIALIZED_STATE_SUPPORT_PASS"
        or record.get("signal_contract") != expected_contract
    ):
        raise InputBlockedError("C147 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C147 signal row count mismatch")
    expected_row_keys = {
        "actual_vintage_date",
    ...
    for row in rows:
        if (
            ...
            or row.get("stage") not in parsed
        ):
            raise InputBlockedError("C147 signal schema mismatch")
        current = row["current_wei"]
        reference = row["trailing_13_week_mean"]
        if any(
            ...
            raise InputBlockedError("C147 WEI value is invalid")
        expected_state = "QQQ" if float(current) > float(reference) else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C147 target mismatch")
        decision_session = _date(row["decision_session"], "decision session")
        decision_at = datetime.combine(decision_session, time(9), tzinfo=NY)
        cutoff = _date(row["snapshot_cutoff_date"], "snapshot cutoff")
        vintage = _date(row["actual_vintage_date"], "actual vintage")
        anchor = _date(row["anchor_observation_date"], "anchor observation")
        if (
            row["decision_at"]
            != f"{decision_session.isoformat()}T09:00:00 America/New_York"
            or cutoff.weekday() != 3
            or not vintage <= cutoff < decision_session
            or not anchor < vintage
        ):
            raise InputBlockedError("C147 signal chronology mismatch")
        source_row_hash = _sha(
            row["source_row_identity_sha256"],
            "source-row identity SHA-256",
        )
        selection_hash = _sha(
            row["signal_identity_sha256"],
            "signal identity SHA-256",
        )
        identity = {
            key: value
            for key, value in row.items()
            if key != "signal_identity_sha256"
        }
        if selection_hash != _hash_bytes(
            (
                json.dumps(identity, sort_keys=True, separators=(",", ":"))
                + "\n"
            ).encode()
        ):
            raise InputBlockedError("C147 signal identity mismatch")
        if source_row_hash == selection_hash:
            raise InputBlockedError("C147 source and signal identities must differ")
    ...
    output = {stage: tuple(values) for stage, values in parsed.items()}
    if (
        len(output["validation"]) != VALIDATION_INTERVALS
        or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS
    ):
        raise InputBlockedError("C147 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C147 terminal execution sessions are required")
    output_mutable = {stage: list(values) for stage, values in output.items()}
    for stage, values in output_mutable.items():
        for index, signal in enumerate(values):
            end = (
                values[index + 1].start
                if index + 1 < len(values)
                else _date(terminal[stage], f"{stage} terminal execution")
            )
            values[index] = Signal(
                signal.stage,
                signal.start,
                end,
                signal.signal_decision_at,
                signal.target_symbol,
                signal.selection_row_hash,
            )
    output = {stage: tuple(values) for stage, values in output_mutable.items()}
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C147 state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("C147 signal intervals are not contiguous")
        states = tuple(item.target_symbol for item in group)
        observed = {
            "interval_count": len(states),
            "qqq_state_count": states.count("QQQ"),
            "spy_state_count": states.count("SPY"),
        }
        if support.get(stage) != observed:
            raise InputBlockedError("C147 state support mismatch")
    return output
...
def _execution(
    ...
    *,
    ...
    if (
        symbol not in {"SPY", "QQQ"}
        or row["symbol"] != symbol
        or row["market"] != "us"
        or row["currency"] != "USD"
    ):
        raise InputBlockedError("only frozen US SPY/QQQ execution inputs are allowed")
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c75-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    payload: bytes,
    input_payload: bytes,
    signal_receipt_payload: bytes,
    runtime_receipt_payload: bytes,
    *,
    stage: str,
    signals: tuple[Signal, ...],
) -> Bundle:
    record = _json(payload, "C147 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C147 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C147 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C147 runtime materialization receipt",
    )
    retrieved_at = datetime(
        2026, 7, 11, 14, 20, 10, tzinfo=timezone.utc
    )
    expected_bundle_sha = (
    ...
    if (
        ...
        or signal_receipt.get("research_id") != RESEARCH_ID
        or signal_receipt.get("schema_version")
        != "c147-dallas-fed-wei-13w-growth-state-receipt-v1"
        or signal_receipt.get("input_packet_sha256") != SIGNAL_SHA256
        or signal_receipt.get("source_rows_sha256") != SOURCE_ROWS_SHA256
        or signal_receipt.get("status")
        != "INPUT_MATERIALIZED_STATE_SUPPORT_PASS"
        or signal_receipt.get("state_support") != packet.get("state_support")
        or runtime_receipt.get("research_id") != RESEARCH_ID
        or runtime_receipt.get("schema_version")
        != "c147-runtime-materialization-receipt-v1"
        or runtime_receipt.get("signal_input_sha256") != SIGNAL_SHA256
        or runtime_receipt.get(f"{stage}_runtime_bundle_sha256")
        != expected_bundle_sha
        or runtime_receipt.get("status") != "RUNTIME_INPUT_MATERIALIZED"
        or runtime_receipt.get("database_sha256_before")
        != "f87722ecf19f9813bb161e365b0e5aa5069ef3d4a16471ad712abb131d7e9fdd"
        or runtime_receipt.get("database_sha256_after")
        != runtime_receipt.get("database_sha256_before")
        or runtime_receipt.get("database_wal_absent") is not True
        or runtime_receipt.get("database_write") is not False
        or runtime_receipt.get("strategy_outcomes_opened") is not False
        or runtime_receipt.get("strategy_candidate_available") is not False
        or runtime_receipt.get("source_selected_using_performance") is not False
    ):
        raise InputBlockedError("input packet and receipt binding mismatch")
    expected_points = (
        VALIDATION_INTERVALS + 1 if stage == "validation" else HOLDOUT_INTERVALS + 1
    )
    if (
        record.get("schema_version") != "c147-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("stage") != stage
        or record.get("signal_input_sha256") != SIGNAL_SHA256
        or record.get("database_write") is not False
        or record.get("strategy_outcomes_opened") is not False
        or record.get("strategy_candidate_available") is not False
        or record.get("source_selected_using_performance") is not False
        ...
        raise InputBlockedError("C147 runtime bundle identity mismatch")
    ...
    spy_actions = tuple(_action(value) for value in record["spy_corporate_actions"])
    qqq_statuses, _ = _qqq_lifecycle(record, retrieved_at)
    qqq_all_actions = _qqq_actions(record, retrieved_at)
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C147 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C147 signal does not bind the execution schedule"
            )
        try:
        ...
        spy_statuses = tuple(_status(value) for value in raw["spy_status_evidence"])
        executions_row = raw.get("executions")
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C147 execution pair is incomplete")
        spy_execution = _execution(
            executions_row["SPY"],
            "SPY",
            spy_statuses,
            spy_point_actions,
...
def _simulate(
    ...
    spy_only = Portfolio.us(INITIAL_CAPITAL)
    strategy_navs = [INITIAL_CAPITAL]
    benchmark_navs = [INITIAL_CAPITAL]
    spy_navs = [INITIAL_CAPITAL]
    strategy_stage = "0" * 64
    ...
    for session_date in bundle.daily_sessions:
        ...
        if point is None:
            ...
            _apply_actions(spy_only, session_date, daily_actions)
            continue
        ...
        spy_result = _rebalance(
            spy_only,
            point,
            {"SPY": 1.0},
            spy_stage,
        )
        spy_only, spy_stage = spy_result.portfolio, spy_result.stage_hash
        stage_hashes.append(spy_stage)
    ...
    if (
        len(strategy_navs) != expected
        or len(benchmark_navs) != expected
        or len(spy_navs) != expected
    ):
    ...
    spy_returns = _adjacent_returns(spy_navs)
    return strategy_returns, benchmark_returns, spy_returns, tuple(stage_hashes)
...
def _decision(
    ...
    spy_only: tuple[float, ...],
) -> dict[str, Any]:
    decision = (
        validation_decision(strategy, comparator, spy_only)
        if stage == "validation"
        else holdout_decision(strategy, comparator, spy_only)
    )
    record = {
        "stage": stage,
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c147-dallas-fed-wei-13w-growth-state-claim-v1",
        ...
        "signal_materialization_receipt_sha256": SIGNAL_RECEIPT_SHA256,
        "runtime_materialization_receipt_sha256": RUNTIME_RECEIPT_SHA256,
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c147-dallas-fed-wei-13w-growth-state-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if _file_hash(SOURCE_ROWS) != SOURCE_ROWS_SHA256:
        raise InputBlockedError("source-row bytes differ from the accepted identity")
    ...
    if (
        ...
        or definition.get("source_identities", {}).get("signal_source_rows_sha256")
        != SOURCE_ROWS_SHA256
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C147 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C147 holdout one-use state is not pristine")
    ...
    try:
        ...
        strategy, comparator, spy_only, stage_hashes = _simulate(bundle)
        decision = _decision(stage, strategy, comparator, spy_only)
        classification = (
            "RETROSPECTIVE_SECONDARY_VALIDATION_PASS_HOLDOUT_UNLOCKED"
