"""One-use runner for the frozen Cycle 36 Fed net-liquidity SPY/QQQ lane."""
...
from research.adapters.c36_fed_net_liquidity_13w_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c36_fed_net_liquidity_13w_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c36_fed_net_liquidity_13w_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "d7eaf4391e8d76bd9b1c81bf7e564f7d7d18ca13939060414cadb724c58fd8ca"
ADAPTER_SHA256 = "de6e924a7d695b130b0258bd9dfb71846d53ed1d637f0aba8654d7792d19eb41"
SIGNAL_SHA256 = "278ed4afcc180809de7da288dbe66620a337af11358bcf039b949036c1698d07"
RECEIPT_SHA256 = "5e81771426146df440662feef8b0db9d78c975badc08e2115f1eaa54ef716aac"
...
SIGNAL_INPUT = Path(
    ...
    "c36_fed_net_liquidity_13w_rotation/"
    "c36_full_input_packet.json"
)
...
FED_SERIES = ("WALCL", "WTREGEN", "RRPONTSYD")
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Fed net-liquidity rotation input packet")
    if (
        record.get("schema_version")
        != "c36-fed-net-liquidity-spy-qqq-input-v1"
        ...
        != {
            "series_ids": list(FED_SERIES),
            "formula": "WALCL - WTREGEN - 1000*RRPONTSYD, USD millions",
            "lookback_calendar_days": 91,
            "target_rule": "delta strictly above zero means QQQ; equal or below zero means SPY",
            "threshold_usd_millions": 0.0,
        }
    ):
        raise InputBlockedError("Fed net-liquidity packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("Fed signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("Fed signal stage mismatch")
        current = row.get("current_components")
        baseline = row.get("baseline_components")
        current_hashes = row.get("current_component_row_sha256")
        baseline_hashes = row.get("baseline_component_row_sha256")
        if (
            type(current) is not dict
            or type(baseline) is not dict
            or type(current_hashes) is not dict
            or type(baseline_hashes) is not dict
            or set(current) != set(FED_SERIES)
            or set(baseline) != set(FED_SERIES)
            or set(current_hashes) != set(FED_SERIES)
            or set(baseline_hashes) != set(FED_SERIES)
        ):
            raise InputBlockedError("Fed net-liquidity components are incomplete")
        decision_at = _datetime(row["decision_at"], "signal decision_at")
        vintage_date = _date(row.get("vintage_date"), "ALFRED vintage date")
        baseline_cutoff = _date(
            row.get("baseline_cutoff_date"), "baseline cutoff date"
        )
        if (
            vintage_date != decision_at.astimezone(NY).date() - timedelta(days=1)
            or baseline_cutoff != vintage_date - timedelta(days=91)
        ):
            raise InputBlockedError("Fed vintage or 91-day cutoff mismatch")
        raw_hashes: set[str] = set()
        for label, components, hashes, cutoff in (
            ("current", current, current_hashes, vintage_date),
            ("baseline", baseline, baseline_hashes, baseline_cutoff),
        ):
            for series_id in FED_SERIES:
                component = components[series_id]
                if type(component) is not dict:
                    raise InputBlockedError("Fed component must be an object")
                identity = {
                    key: value for key, value in component.items() if key != "row_sha256"
                }
                ...
                available_at = _datetime(
                    component.get("available_at_conservative"),
                    f"{label} Fed component available_at",
                )
                observation_date = _date(
                    component.get("observation_date"),
                    f"{label} Fed component observation date",
                )
                expected_unit = (
                    "USD_BILLIONS" if series_id == "RRPONTSYD" else "USD_MILLIONS"
                )
                if (
                    component.get("series_id") != series_id
                    or component.get("unit") != expected_unit
                    or component.get("vintage_date") != vintage_date.isoformat()
                    or available_at.astimezone(NY).date() != vintage_date
                    or available_at.astimezone(NY).time() != time.max
                    or not available_at < decision_at
                    or observation_date > cutoff
                    or component.get("source_class")
                    != "OFFICIAL_FRED_ALFRED_VINTAGE_GRAPH_EXPORT"
                    or isinstance(value, bool)
                    or not isinstance(value, (int, float))
                    or not math.isfinite(float(value))
                    or component.get("row_sha256") != expected_hash
                    or hashes.get(series_id) != expected_hash
                ):
                    raise InputBlockedError("Fed component identity mismatch")
                raw_hashes.add(_sha(component.get("raw_response_sha256"), "raw vintage SHA-256"))
        if len(raw_hashes) != 1:
            raise InputBlockedError("Fed components must share one vintage snapshot")
        current_value = row.get("current_net_liquidity_usd_millions")
        baseline_value = row.get("baseline_net_liquidity_usd_millions")
        change = row.get("delta_net_liquidity_usd_millions")
        if any(
            isinstance(value, bool)
            or not isinstance(value, (int, float))
            or not math.isfinite(float(value))
            for value in (current_value, baseline_value, change)
        ):
            raise InputBlockedError("Fed net-liquidity values must be finite")
        expected_current = (
            float(current["WALCL"]["value"])
            - float(current["WTREGEN"]["value"])
            - 1000.0 * float(current["RRPONTSYD"]["value"])
        )
        expected_baseline = (
            float(baseline["WALCL"]["value"])
            - float(baseline["WTREGEN"]["value"])
            - 1000.0 * float(baseline["RRPONTSYD"]["value"])
        )
        expected_change = expected_current - expected_baseline
        if (
            not math.isclose(float(current_value), expected_current, rel_tol=0, abs_tol=1e-9)
            or not math.isclose(
                float(baseline_value), expected_baseline, rel_tol=0, abs_tol=1e-9
            )
            or not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-9)
        ):
            raise InputBlockedError("Fed net-liquidity arithmetic mismatch")
        ...
        identity = {
            ...
            "baseline_cutoff_date": baseline_cutoff.isoformat(),
            "current_component_row_sha256": current_hashes,
            "baseline_component_row_sha256": baseline_hashes,
            "current_net_liquidity_usd_millions": float(current_value),
            "baseline_net_liquidity_usd_millions": float(baseline_value),
            "delta_net_liquidity_usd_millions": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("Fed net-liquidity signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("Fed net-liquidity signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "Fed signal SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("Fed signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("Fed state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("Fed signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("Fed state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c36-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "Fed net-liquidity rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "Fed net-liquidity rotation/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c36-fed-net-liquidity-full-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "Fed net-liquidity signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c36-fed-net-liquidity-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c36-fed-net-liquidity-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 36 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 36 holdout one-use state is not pristine")
