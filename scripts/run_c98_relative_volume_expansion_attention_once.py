"""One-use runner for frozen C98 relative volume-expansion attention."""
...
from research.adapters.c98_relative_volume_expansion_attention import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c98_relative_volume_expansion_attention_v1.json"
ADAPTER = ROOT / "research/adapters/c98_relative_volume_expansion_attention.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "9d1ded67d01d6f8f8d383dd1e511ae81436b739a1bb1ee058388751f75e8aa7f"
ADAPTER_SHA256 = "d6284f3aab62f9095e9ffd97d1709f0393240193ebdbede3f28067c143f8f67c"
SIGNAL_SHA256 = "2a1c67eccfa94138380a4843c29882a997e821dc62afe49c03d7ee109325f8b8"
RECEIPT_SHA256 = "1ea06117c51dcfe149294a9fa03ae6e73e06d01ac0362a96dd6c7c95f3215709"
...
SIGNAL_INPUT = Path(
    ...
    "c98_relative_volume_expansion_attention_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C98 relative-volume-expansion input packet")
    expected_contract = {
        ...
        "score_definition": (
            "arithmetic mean raw volume over latest 21 sessions "
            "divided by arithmetic mean raw volume over immediately "
            "preceding 63 sessions"
        ),
        "target_rule": (
            "QQQ volume-expansion score strictly greater than SPY "
            "selects QQQ; "
            "equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c98-relative-volume-expansion-attention-input-v1"
        ...
        or record.get("adjusted_prices_used_for_signal") is not False
        ...
        raise InputBlockedError("C98 volume-expansion packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C98 volume-expansion signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_baseline_mean_raw_volume",
        "qqq_recent_mean_raw_volume",
        "qqq_volume_expansion_score",
        "recent_session_count",
        "relative_volume_expansion_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_baseline_mean_raw_volume",
        "spy_recent_mean_raw_volume",
        "spy_volume_expansion_score",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C98 volume-expansion signal schema mismatch")
        numeric_fields = (
            "spy_baseline_mean_raw_volume",
            "spy_recent_mean_raw_volume",
            "spy_volume_expansion_score",
        ...
        if any(
            ...
            raise InputBlockedError("C98 volume-expansion value is invalid")
        if (
            ...
            raise InputBlockedError("C98 formation session count mismatch")
        for symbol in ("spy", "qqq"):
            baseline = float(row[f"{symbol}_baseline_mean_raw_volume"])
            recent = float(row[f"{symbol}_recent_mean_raw_volume"])
            score = float(row[f"{symbol}_volume_expansion_score"])
            if baseline <= 0.0 or recent <= 0.0 or score <= 0.0 or not math.isclose(
                ...
                raise InputBlockedError(
                    "C98 volume-expansion score arithmetic mismatch"
                )
        difference = float(row["qqq_volume_expansion_score"]) - float(
            row["spy_volume_expansion_score"]
        )
        if not math.isclose(
            float(row["relative_volume_expansion_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C98 volume-expansion arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C98 volume-expansion target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C98 volume-expansion chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C98 volume-expansion signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C98 volume-expansion stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C98 volume-expansion state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C98 volume-expansion signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C98 volume-expansion state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C98 volume-expansion input packet")
    receipt = _json(
        receipt_payload,
        "C98 volume-expansion materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c98-relative-volume-expansion-attention-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C98 volume-expansion signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c98-relative-volume-expansion-attention-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c98-relative-volume-expansion-attention-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C98 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C98 holdout one-use state is not pristine")
