"""One-use runner for frozen C132 spy-qqq-correlation rotation."""
...
from research.adapters.c132_spy_qqq_correlation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c132_spy_qqq_correlation_v1.json"
ADAPTER = ROOT / "research/adapters/c132_spy_qqq_correlation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "b19292b907663de39fbeb9b3b1ccdee31bc03d53b70e5288ad015cba6a0c1f90"
ADAPTER_SHA256 = "df5d140ca7dd94f07e31e4ba03b0e2f570acbf5af1f2705078dc7016cab4f612"
SIGNAL_SHA256 = "14b77a2c365d9ec3b9cf7f1994b85b6f89f528d01f888d020db64aeca59f23a2"
RECEIPT_SHA256 = "fcb359e1233cf088b1b8bd2c3f593b81d781c894d19b1874948db7717bd63dda"
...
SIGNAL_INPUT = Path(
    ...
    "c132_spy_qqq_63_return_correlation_state_rotation_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C132 spy-qqq-correlation input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Contemporaneous Pearson correlation of 63 SPY and QQQ simple returns "
            "formed from exactly 64 common adjusted closes"
        ),
        "target_rule": (
            "Correlation below 0.95 selects QQQ; 0.95 or above selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c132-spy-qqq-correlation-input-v1"
        ...
        raise InputBlockedError("C132 spy-qqq-correlation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C132 spy-qqq-correlation signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C132 spy-qqq-correlation signal schema mismatch")
        value_fields = ("spy_qqq_correlation",)
        if any(
            ...
            raise InputBlockedError(
                "C132 spy-qqq-correlation value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or not -1.0 <= row["spy_qqq_correlation"] <= 1.0
        ):
            raise InputBlockedError("C132 formation or correlation range mismatch")
        expected_state = "QQQ" if row["spy_qqq_correlation"] < 0.95 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C132 spy-qqq-correlation target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C132 spy-qqq-correlation chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C132 spy-qqq-correlation signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C132 spy-qqq-correlation stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C132 spy-qqq-correlation state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C132 spy-qqq-correlation signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C132 spy-qqq-correlation state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C132 spy-qqq-correlation input packet")
    receipt = _json(
        receipt_payload,
        "C132 spy-qqq-correlation materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c132-spy-qqq-correlation-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C132 spy-qqq-correlation signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c132-spy-qqq-correlation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c132-spy-qqq-correlation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C132 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C132 holdout one-use state is not pristine")
