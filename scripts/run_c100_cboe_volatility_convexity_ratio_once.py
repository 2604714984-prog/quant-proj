"""One-use runner for frozen C100 Cboe volatility-convexity ratio."""
...
from research.adapters.c100_cboe_volatility_convexity_ratio import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c100_cboe_volatility_convexity_ratio_v1.json"
ADAPTER = ROOT / "research/adapters/c100_cboe_volatility_convexity_ratio.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "7564600041cc93b7adda8c7cb5d00cc1febac19cf4b483dab71b3a1990e3d305"
ADAPTER_SHA256 = "675ba49673377073ab30ce96d090a5f6b47866bb171d1c80b0bc580f2a1a6bb6"
SIGNAL_SHA256 = "494b587d62d22fee1531f55833ea54a7f96a6b159f1f40480d8082315da5b7cc"
RECEIPT_SHA256 = "0199726cb16b6f6098ca3a33aa8ba1be6a985651c504e7850a463bbd66a2f7fd"
...
SIGNAL_INPUT = Path(
    ...
    "c100_cboe_volatility_convexity_ratio_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C100 volatility-convexity input packet")
    expected_contract = {
        "current_rule": (
    ...
    if (
        ...
        != "c100-cboe-volatility-convexity-ratio-input-v1"
        ...
        raise InputBlockedError("C100 volatility-convexity packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C100 volatility-convexity row count mismatch")
    expected_row_keys = {
        "decision_at",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C100 volatility-convexity schema mismatch")
        numeric_fields = (
            "current_ratio",
            "current_vix",
            "current_vvix",
        ...
        if any(
            ...
            raise InputBlockedError("C100 volatility-convexity value is invalid")
        if any(
            float(row[field]) <= 0.0
            for field in (
                "current_ratio",
            ...
            raise InputBlockedError("C100 volatility-convexity input is nonpositive")
        if not math.isclose(
            float(row["current_ratio"]),
            float(row["current_vvix"]) / float(row["current_vix"]),
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["prior_ratio"]),
            float(row["prior_vvix"]) / float(row["prior_vix"]),
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C100 ratio arithmetic mismatch")
        ...
        if not math.isclose(
            ...
            raise InputBlockedError("C100 ratio-change arithmetic mismatch")
        expected_state = "SPY" if difference > 0.0 else "QQQ"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C100 volatility-convexity target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C100 volatility-convexity chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C100 volatility-convexity identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C100 volatility-convexity stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C100 volatility-convexity support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C100 volatility-convexity intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C100 volatility-convexity support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C100 volatility-convexity input packet")
    receipt = _json(
        receipt_payload,
        "C100 volatility-convexity materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c100-cboe-volatility-convexity-ratio-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C100 volatility-convexity signal does not bind "
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c100-cboe-volatility-convexity-ratio-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c100-cboe-volatility-convexity-ratio-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C100 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C100 holdout one-use state is not pristine")
