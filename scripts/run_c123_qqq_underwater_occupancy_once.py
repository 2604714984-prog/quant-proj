"""One-use runner for frozen C123 qqq-underwater-occupancy rotation."""
...
from research.adapters.c123_qqq_underwater_occupancy import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c123_qqq_underwater_occupancy_v1.json"
ADAPTER = ROOT / "research/adapters/c123_qqq_underwater_occupancy.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "2f3ecd60d0d2e636bf3374f694acfc43dffdab7c3bfe746ee5fdf4c8e874d9c2"
ADAPTER_SHA256 = "e476de9a8d130535d7dfac569d2fef0e642b0d3ce016454c6caea868f0968e6b"
SIGNAL_SHA256 = "c921185fa706b7b38b240e6ad964a32c5f3a8be99a49dababf199ee11bca67a1"
RECEIPT_SHA256 = "30074703b92d87cb6d4ed9ddd4e62937b88eff0b75d075710ebb632fb0027a72"
...
SIGNAL_INPUT = Path(
    ...
    "c123_qqq_underwater_occupancy_improvement_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C123 qqq-underwater-occupancy input packet")
    expected_contract = {
        "formation_sessions": 126,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "underwater-session fraction in the later 63-session half versus "
            "the earlier 63-session half, resetting each half's running high"
        ),
        "target_rule": (
            "later underwater fraction at or below earlier selects QQQ; otherwise SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c123-qqq-underwater-occupancy-input-v1"
        ...
        raise InputBlockedError("C123 qqq-underwater-occupancy packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C123 qqq-underwater-occupancy signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_earlier_underwater_fraction",
        "qqq_later_underwater_fraction",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C123 qqq-underwater-occupancy signal schema mismatch")
        value_fields = ("qqq_earlier_underwater_fraction", "qqq_later_underwater_fraction")
        if any(
            ...
            raise InputBlockedError(
                "C123 qqq-underwater-occupancy value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 126
            or not 0.0 <= row["qqq_earlier_underwater_fraction"] <= 1.0
            or not 0.0 <= row["qqq_later_underwater_fraction"] <= 1.0
        ):
            raise InputBlockedError("C123 formation or fraction contract mismatch")
        expected_state = (
            "QQQ"
            if row["qqq_later_underwater_fraction"]
            <= row["qqq_earlier_underwater_fraction"]
            else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C123 qqq-underwater-occupancy target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C123 qqq-underwater-occupancy chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C123 qqq-underwater-occupancy signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C123 qqq-underwater-occupancy stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C123 qqq-underwater-occupancy state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C123 qqq-underwater-occupancy signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C123 qqq-underwater-occupancy state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C123 qqq-underwater-occupancy input packet")
    receipt = _json(
        receipt_payload,
        "C123 qqq-underwater-occupancy materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c123-qqq-underwater-occupancy-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C123 qqq-underwater-occupancy signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c123-qqq-underwater-occupancy-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c123-qqq-underwater-occupancy-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C119 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C119 holdout one-use state is not pristine")
