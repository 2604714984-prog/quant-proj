"""One-use runner for the frozen C160 CCSA SPY/QQQ rotation lane."""
...
from research.adapters.c160_continuing_claims_4w import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c160_continuing_claims_4w_v1.json"
ADAPTER = ROOT / "research/adapters/c160_continuing_claims_4w.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "141a5ce19d5cbc7ea6f546a0808b35597629924038e8f74ef6657b5a59b44825"
ADAPTER_SHA256 = "ffb7140fdb727e31c3ddd8593932ac8cc6dbf15a93f004a106a3e7b11c149ad7"
SIGNAL_SHA256 = "e271cdd9367fae45a9aa18712a4e74d56f3e5a8d669bddd1039bde7c1ae6e9fa"
RECEIPT_SHA256 = "b7a03d9dd1aa1d48f8e1e946f73fd31e6c2760e0997fc5e9550b6859f3c317a8"
...
SIGNAL_INPUT = Path(
    ...
    "c160_continuing_claims_4w_labor_persistence_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "CCSA labor-persistence rotation input packet")
    if (
        record.get("schema_version")
        != "c160-ccsa-spy-qqq-full-input-v1"
    ...
    for row in rows:
        ...
        if set(row) != required:
            raise InputBlockedError("CCSA signal fields mismatch")
        numeric = tuple(
            row.get(key)
            for key in (
                "current_continuing_claims",
        ...
        if any(
            ...
            raise InputBlockedError("CCSA statistics must be finite")
        current_yield, comparison_yield, impulse = map(float, numeric)
        if not all(0.0 <= value < 100_000_000.0 for value in (current_yield, comparison_yield)):
            raise InputBlockedError("CCSA values are implausible")
        ...
        vintage = _date(row["actual_vintage_date"], "CCSA actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "CCSA response SHA-256"
        )
        if (
            vintage > decision_at.date()
            or current_date >= start
            or current_date > decision_at.date()
            or current_date - comparison_date < timedelta(days=28)
            or not math.isclose(
                impulse,
            ...
            raise InputBlockedError("CCSA chronology or state mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("CCSA signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("CCSA source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "CCSA row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("CCSA signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("CCSA state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("CCSA signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("CCSA state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c160-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "ETA CCSA/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "ETA CCSA/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c160-ccsa-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "CCSA signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c160-ccsa-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c160-ccsa-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
