"""One-use runner for frozen C143 NY Fed SOFR-EFFR spread impulse rotation."""
...
from research.adapters.c143_nyfed_sofr_effr_spread_impulse import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c143_nyfed_sofr_effr_spread_impulse_v1.json"
ADAPTER = ROOT / "research/adapters/c143_nyfed_sofr_effr_spread_impulse.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "291306018fe116462d9240585ebbda8a6873410bbc9fe2c1f63007e1ffb567f5"
ADAPTER_SHA256 = "bb63dc91d2c830f50e35a7a73a6ad4de499137390a95578a9cd5546b2ebdeb4e"
SIGNAL_SHA256 = "707f921aa3f7714e393fb3e008c4b8109050425596d3761f223f48c5e269ede9"
SIGNAL_RECEIPT_SHA256 = "2549fa15cd93180666b9602fc2808de9283cc84d7294c96e725f9ce5883cd26e"
RUNTIME_RECEIPT_SHA256 = "738662d20dcd28042eae331c60506ac01f217b853750a0d90784ed539d24ba1c"
VALIDATION_BUNDLE_SHA256 = "b738cf4b8706829320442fe3fb8e3ae4c4e6e811f150b9a135470423e0fc3c82"
HOLDOUT_BUNDLE_SHA256 = "652edd28157414916856814dd0d1d3fec0e02dfae54e458e66dd8162c946ca95"
...
SIGNAL_INPUT = Path(
    ...
    "c143_nyfed_sofr_effr_spread_impulse_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C143 NY Fed SOFR-EFFR spread impulse signal input")
    expected_contract = {
        "current_anchor": (
            "latest common effective-date pair no later than the "
            "preceding Thursday"
        ),
        "direction": (
            "positive_SOFR_minus_EFFR_weekly_impulse_selects_SPY_else_QQQ"
        ),
        "prior_anchor": (
            "latest common effective-date pair no later than the "
            "preceding Thursday minus seven calendar days"
        ),
    }
    if (
        record.get("schema_version")
        != "c143-nyfed-sofr-effr-spread-input-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("evidence_class") != "OFFICIAL_HISTORICAL_REFERENCE_RATES"
        ...
        raise InputBlockedError("C143 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C143 signal row count mismatch")
    expected_row_keys = {
        "current_effective_date",
        "current_effr_percent_rate",
        "current_sofr_percent_rate",
        "decision_at",
        "decision_session",
        "prior_effective_date",
        "prior_effr_percent_rate",
        "prior_sofr_percent_rate",
        "rate_spread_impulse_percentage_points",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C143 signal schema mismatch")
        rate_fields = (
            "current_effr_percent_rate",
            "current_sofr_percent_rate",
            "prior_effr_percent_rate",
        ...
        if any(
            ...
            or float(value) < 0.0
            for value in (row[field] for field in rate_fields)
        ):
            raise InputBlockedError("C143 rate value is invalid")
        impulse = (
            row["current_sofr_percent_rate"]
            - row["current_effr_percent_rate"]
        ) - (
            row["prior_sofr_percent_rate"]
            - row["prior_effr_percent_rate"]
        )
        observed_impulse = row["rate_spread_impulse_percentage_points"]
        if (
            isinstance(observed_impulse, bool)
            or not isinstance(observed_impulse, (int, float))
            or not math.isfinite(float(observed_impulse))
            or not math.isclose(
                float(observed_impulse),
                float(impulse),
                rel_tol=0.0,
                abs_tol=1e-15,
            )
        ):
            raise InputBlockedError("C143 spread impulse is invalid")
        expected_state = "SPY" if impulse > 0.0 else "QQQ"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C143 target mismatch")
        ...
        current_date = _date(
            row["current_effective_date"],
            "current effective date",
        )
        prior_date = _date(
            row["prior_effective_date"],
            "prior effective date",
        )
        preceding_thursday = decision_session - timedelta(
            days=decision_session.weekday() + 4
        )
        prior_thursday = preceding_thursday - timedelta(days=7)
        if (
            row["decision_at"]
            != f"{decision_session.isoformat()}T09:00:00 America/New_York"
            or not prior_date < current_date <= preceding_thursday
            or prior_date > prior_thursday
        ):
            raise InputBlockedError("C143 signal chronology mismatch")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C143 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C143 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C143 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C143 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C143 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C143 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C143 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C143 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C143 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C143 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c143-nyfed-sofr-effr-spread-receipt-v1"
        ...
        != "c143-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c143-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C143 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C143 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C143 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C143 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c143-nyfed-sofr-effr-spread-impulse-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c143-nyfed-sofr-effr-spread-impulse-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C143 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C143 holdout one-use state is not pristine")
