"""One-use runner for the frozen C150 Initial Claims SPY/QQQ rotation lane."""
...
from research.adapters.c150_initial_claims_4w_impulse import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c150_initial_claims_4w_impulse_v1.json"
ADAPTER = ROOT / "research/adapters/c150_initial_claims_4w_impulse.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "d0e451a051dce25f144c36f00b7ca2f9df0944e5cbaa65ae41cf559dac6b8674"
ADAPTER_SHA256 = "d5e02d0cd6b199f070935fba557ac39c83f98a4c12a755acce81f987a1973440"
SIGNAL_SHA256 = "5a1e5a3d5cbc6e5cc4cc1ccab316d8f6b713d2f84d3c48a43cbaab27d47b5e54"
RECEIPT_SHA256 = "9bfc3ffecdff9690970d9935a22bf680453d1f2c9aa9d096f8a3d0449726c384"
...
SIGNAL_INPUT = Path(
    ...
    "c150_initial_claims_4w_impulse_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "initial-claims impulse rotation input packet")
    if (
        record.get("schema_version")
        != "c150-icsa-spy-qqq-full-input-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("evidence_class")
        != "OFFICIAL_ALFRED_AND_ACCEPTED_SPY_QQQ_EXECUTION_INPUTS"
        ...
        != {
            "series_id": "ICSA",
            "target_rule": "newest four-week mean < preceding four-week mean means QQQ; otherwise SPY",
            "threshold_claims": 0.0,
        }
    ):
        raise InputBlockedError("Initial Claims rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("Initial Claims signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("Initial Claims signal stage mismatch")
        required = {
            "actual_vintage_date",
            "current_four_end",
            "current_four_mean_claims",
            "current_four_start",
            "decision_at",
        ...
        if set(row) != required:
            raise InputBlockedError("Initial Claims signal fields mismatch")
        numeric = tuple(
            row.get(key)
            for key in (
                "current_four_mean_claims",
        ...
        if any(
            ...
            raise InputBlockedError("initial-claims statistics must be finite")
        current_mean, previous_mean, impulse = map(float, numeric)
        if current_mean < 0.0 or previous_mean < 0.0:
            raise InputBlockedError("initial-claims means must be nonnegative")
        ...
        previous_start = _date(row["previous_four_start"], "previous-four start")
        previous_end = _date(row["previous_four_end"], "previous-four end")
        current_start = _date(row["current_four_start"], "current-four start")
        current_end = _date(row["current_four_end"], "current-four end")
        vintage = _date(row["actual_vintage_date"], "Initial Claims actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "Initial Claims response SHA-256"
        )
        if (
            vintage > decision_at.date()
            or current_end >= start
            or previous_end - previous_start != timedelta(days=21)
            or current_end - current_start != timedelta(days=21)
            or current_start - previous_end != timedelta(days=7)
            or not math.isclose(
                impulse, current_mean - previous_mean, rel_tol=0.0, abs_tol=1e-12
            )
            or decision_at.astimezone(NY).time().replace(tzinfo=None) != time(20, 5)
            or row.get("target_symbol") != expected_state
        ):
            raise InputBlockedError("Initial Claims chronology or state mismatch")
        identity = {
            "stage": row["stage"],
            "holding_start_execution_session": start.isoformat(),
            "holding_end_execution_session": end.isoformat(),
            "decision_at": row["decision_at"],
            "actual_vintage_date": vintage.isoformat(),
            "previous_four_start": previous_start.isoformat(),
            "previous_four_end": previous_end.isoformat(),
            "previous_four_mean_claims": previous_mean,
            "current_four_start": current_start.isoformat(),
            "current_four_end": current_end.isoformat(),
            "current_four_mean_claims": current_mean,
            "impulse_claims": impulse,
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("Initial Claims signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("Initial Claims source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "Initial Claims row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("Initial Claims signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("Initial Claims state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("Initial Claims signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("Initial Claims state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c150-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "New York Fed Initial Claims/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "New York Fed Initial Claims/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c150-icsa-full-materialization-receipt-v1"
        or receipt.get("full_input_packet_sha256") != SIGNAL_SHA256
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "Initial Claims signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c150-nyfed-icsa-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c150-nyfed-icsa-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 23 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 23 holdout one-use state is not pristine")
