"""One-use runner for the frozen Cycle 38 Cboe VVIX SPY/QQQ rotation lane."""
...
from research.adapters.c38_cboe_vvix_change_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c38_cboe_vvix_change_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c38_cboe_vvix_change_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "cdbe14e06ecdaaa9942192ef5b7991df821e5d13c14c4bf20465f978a4e222ea"
ADAPTER_SHA256 = "a65dadb57464426b265f5a0647aff8aadb42694c316777977ea934c350215d09"
SIGNAL_SHA256 = "2d1c0991bb9ce2a214d66ed7ed9eaa0a483d889e791d13cc16a877935b8c09fe"
RECEIPT_SHA256 = "47bfadd3488080e03a5283cf5aadfc124d69d7de612ae327d23ee6d3671e5000"
...
SIGNAL_INPUT = Path(
    ...
    "c38_cboe_vvix_change_rotation_spy_qqq_v1/"
    "c38_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Cboe VVIX rotation input packet")
    expected_contract = {
        "series_id": "VVIX",
        "lookback_calendar_days": 28,
        "target_rule": "strict increase in official Cboe VVIX selects SPY; equal or decrease selects QQQ",
        "threshold_index_points": 0.0,
    }
    if (
        record.get("schema_version")
        != "c38-cboe-vvix-change-spy-qqq-input-packet-v1"
        ...
        raise InputBlockedError("VVIX packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("VVIX signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = (
        "abafc34b5b50b9cbe1657758e5c0c2065c229f9eb60a2bf66712d7e77fb3eb49"
    )
    expected_url = (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("VVIX signal stage mismatch")
        ...
        if type(current) is not dict or type(baseline) is not dict:
            raise InputBlockedError("VVIX selections are required")
        for selection in (baseline, current):
            ...
            if (
                ...
                raise InputBlockedError("VVIX source row identity mismatch")
        ...
        if (
            ...
            raise InputBlockedError("VVIX change arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError("VVIX observation or availability mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("VVIX signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("VVIX signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "VVIX row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("VVIX signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("VVIX state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("VVIX signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("VVIX state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c38-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "VVIX rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "VVIX rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "VVIX signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c38-cboe-vvix-change-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c38-cboe-vvix-change-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 38 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 38 holdout one-use state is not pristine")
