"""One-use runner for the frozen Cycle 26 A191RL1Q225SBEA SPY/QQQ rotation lane."""
...
from research.adapters.c26_real_gdp_growth_acceleration_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c26_real_gdp_growth_acceleration_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c26_real_gdp_growth_acceleration_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "04399a38af6b5bd2c3b6db3362f67432b9805cd59a98d2b55c52b21ea78625ca"
ADAPTER_SHA256 = "e006f76b174a70fcd20f3125da49dcf934b56e277b1cfacd11fc6bbd7063ccf0"
SIGNAL_SHA256 = "e94a5fea778474b0289a647b854fd446267c9092765ca2f0a6e02dbc11f7b7cf"
RECEIPT_SHA256 = "7d4a800d104513ff776bce92522cdbf62510ddadb22378edb100c9ef53963a6a"
...
SIGNAL_INPUT = Path(
    ...
    "c26_real_gdp_growth_acceleration_rotation_spy_qqq_v1/"
    "c26_full_input_packet.json"
)
...
def _previous_quarter(value: date) -> date:
    if value.day != 1 or value.month not in {1, 4, 7, 10}:
        raise InputBlockedError("real GDP reference must be a quarter start")
    if value.month == 1:
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "real-GDP growth acceleration input packet")
    if (
        record.get("schema_version")
        != "c26-real-gdp-growth-acceleration-spy-qqq-input-packet-v1"
    ...
    for row in rows:
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("A191RL1Q225SBEA selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "A191RL1Q225SBEA"
                or selection.get("source_class")
                != "OFFICIAL_ALFRED_BEA_REAL_GDP_GROWTH"
                ...
                raise InputBlockedError("A191RL1Q225SBEA source row identity mismatch")
        current_value = current.get("value_percent_saar")
        previous_value = previous.get("value_percent_saar")
        change = row.get("delta_percentage_points")
        if any(
            ...
            raise InputBlockedError("A191RL1Q225SBEA values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("A191RL1Q225SBEA change arithmetic mismatch")
        ...
        previous_reference_quarter = _date(
            row.get("previous_reference_quarter"), "previous real GDP reference quarter"
        )
        current_reference_quarter = _date(
            row.get("current_reference_quarter"), "current real GDP reference quarter"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous A191RL1Q225SBEA available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current A191RL1Q225SBEA available_at",
            )
            > decision_at
            or _date(previous.get("observation_date"), "previous observation date")
            != previous_reference_quarter
            or _date(current.get("observation_date"), "current observation date")
            != current_reference_quarter
            or previous_reference_quarter != _previous_quarter(current_reference_quarter)
        ):
            raise InputBlockedError(
                "real GDP reference-quarter identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
            ...
            "previous_reference_quarter": previous_reference_quarter.isoformat(),
            "current_reference_quarter": current_reference_quarter.isoformat(),
            "current_row_sha256": current.get("row_sha256"),
            "previous_row_sha256": previous.get("row_sha256"),
            "delta_percentage_points": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("A191RL1Q225SBEA signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("A191RL1Q225SBEA source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "A191RL1Q225SBEA row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("A191RL1Q225SBEA signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("A191RL1Q225SBEA state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("A191RL1Q225SBEA signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("A191RL1Q225SBEA state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c26-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "real-GDP growth acceleration/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "real-GDP growth acceleration/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "A191RL1Q225SBEA signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c26-real-gdp-growth-acceleration-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c26-real-gdp-growth-acceleration-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 26 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 26 holdout one-use state is not pristine")
