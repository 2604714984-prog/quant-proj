"""One-use runner for frozen C122 qqq-short-long-volatility rotation."""
...
from research.adapters.c122_qqq_short_long_volatility import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c122_qqq_short_long_volatility_v1.json"
ADAPTER = ROOT / "research/adapters/c122_qqq_short_long_volatility.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "c8c8e56319cbd29950753400d5a66b5e6313607346613084e90db00f844c30f4"
ADAPTER_SHA256 = "477d23faa4a30363e0394adb8f2f464de11c865a0dfc54114c8b362ce68840a0"
SIGNAL_SHA256 = "2eb8b7fea524993cc770ea6f2616a43ffc1208998cd2a7df91af472561b9c504"
RECEIPT_SHA256 = "d7af2c63cb3963df6c6454b38b14ccbc8735cc014c1a1b23b52068a89f250da9"
...
SIGNAL_INPUT = Path(
    ...
    "c122_qqq_short_long_realized_volatility_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C122 qqq-short-long-volatility input packet")
    expected_contract = {
        "formation_sessions": 127,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "sample standard deviation with ddof 1 of the final 21 QQQ returns "
            "versus sample standard deviation with ddof 1 of all 126 returns "
            "formed from exactly 127 adjusted closes"
        ),
        "target_rule": (
            "short volatility at or below long volatility selects QQQ; otherwise SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c122-qqq-short-long-volatility-input-v1"
        ...
        raise InputBlockedError("C122 qqq-short-long-volatility packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C122 qqq-short-long-volatility signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C122 qqq-short-long-volatility signal schema mismatch")
        value_fields = ("qqq_long_volatility", "qqq_short_volatility")
        if any(
            ...
            raise InputBlockedError(
                "C122 qqq-short-long-volatility value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 127
            or row["qqq_long_volatility"] <= 0.0
            or row["qqq_short_volatility"] <= 0.0
        ):
            raise InputBlockedError("C120 formation session count mismatch")
        expected_state = (
            "QQQ"
            if row["qqq_short_volatility"] <= row["qqq_long_volatility"]
            else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C122 qqq-short-long-volatility target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C122 qqq-short-long-volatility chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C122 qqq-short-long-volatility signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C122 qqq-short-long-volatility stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C122 qqq-short-long-volatility state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C122 qqq-short-long-volatility signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C122 qqq-short-long-volatility state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C122 qqq-short-long-volatility input packet")
    receipt = _json(
        receipt_payload,
        "C122 qqq-short-long-volatility materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c122-qqq-short-long-volatility-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C122 qqq-short-long-volatility signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c122-qqq-short-long-volatility-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c122-qqq-short-long-volatility-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
