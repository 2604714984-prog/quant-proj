"""One-use runner for the frozen C149 Empire State SPY/QQQ rotation lane."""
...
from research.adapters.c149_nyfed_empire_current_business_state import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c149_nyfed_empire_current_business_state_v1.json"
ADAPTER = ROOT / "research/adapters/c149_nyfed_empire_current_business_state.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "bcfad0752d880efd35276204ba922facef053f61c03e5b513565eb384a081451"
ADAPTER_SHA256 = "cb72ae57532c390e1335fca2f3d7f34bebdf1bcbe50511f25314dcc6e330e0a4"
SIGNAL_SHA256 = "5acfff63a2112fc3b65f0e704d3dcd8f74d152457da1c17b22152e9a76a224c0"
RECEIPT_SHA256 = "4c40f17b15237f9bdd417d86fc6bdef48a16acd938f67a0ec27653cd6f182380"
...
SIGNAL_INPUT = Path(
    ...
    "c149_nyfed_empire_current_business_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "New York Fed Empire State rotation input packet")
    if (
        record.get("schema_version")
        != "c149-empire-spy-qqq-full-input-v1"
        ...
        != {
            ...
            "target_rule": "diffusion index > 0 means QQQ; <= 0 means SPY",
            "threshold_index_points": 0.0,
        }
    ):
        raise InputBlockedError("Empire State rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("Empire State signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("Empire State signal stage mismatch")
        required = {
            "actual_vintage_date",
            "decision_at",
            "diffusion_index",
        ...
        if set(row) != required:
            raise InputBlockedError("Empire State signal fields mismatch")
        value = row.get("diffusion_index")
        if (
            isinstance(value, bool)
            or not isinstance(value, (int, float))
            or not math.isfinite(float(value))
        ):
            raise InputBlockedError("Empire State diffusion index must be finite")
        expected_state = "QQQ" if float(value) > 0.0 else "SPY"
        ...
        reference_month = _date(row["reference_month"], "Empire State reference month")
        vintage = _date(row["actual_vintage_date"], "Empire State actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "Empire State response SHA-256"
        )
        if (
            vintage > decision_at.date()
            or reference_month.day != 1
            ...
            raise InputBlockedError("Empire State chronology or state mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("Empire State signal identity mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("Empire State source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "Empire State row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("Empire State signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("Empire State state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("Empire State signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("Empire State state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c149-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "New York Fed Empire State/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "New York Fed Empire State/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c149-empire-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "Empire State signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c149-nyfed-empire-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c149-nyfed-empire-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
