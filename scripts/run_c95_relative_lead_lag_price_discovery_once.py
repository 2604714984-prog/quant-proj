"""One-use runner for frozen C95 relative lead-lag price discovery."""
...
from research.adapters.c95_relative_lead_lag_price_discovery import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c95_relative_lead_lag_price_discovery_v1.json"
ADAPTER = ROOT / "research/adapters/c95_relative_lead_lag_price_discovery.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "12324255f6193b247cb93d5346d9060d2af8f7a6b6269587b14a7c21db98bc14"
ADAPTER_SHA256 = "60df58aa16a456a198a51ce53359f83b0316483a20826e28034967db24e0bbf7"
SIGNAL_SHA256 = "7e9876da1ee3a463797f9afddead6187fff7ddbb97b7d901c0b066f6de1a1da3"
RECEIPT_SHA256 = "6960c13a70dfaa47488b714a462c9e495a70674679e8e07613fecd2d3dc649c9"
...
SIGNAL_INPUT = Path(
    ...
    "c95_relative_lead_lag_price_discovery_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C95 lead-lag input packet")
    expected_contract = {
        ...
        "lead_lag_definition": (
            "ordinary Pearson correlations corr(QQQ returns 1..62, "
            "SPY returns 2..63) and corr(SPY returns 1..62, QQQ "
            "returns 2..63), using population covariance and population "
            "standard deviations"
        ),
        "target_rule": (
            "QQQ-leading correlation strictly greater than SPY-leading "
            "correlation selects QQQ; "
            "equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c95-relative-lead-lag-price-discovery-input-v1"
        ...
        raise InputBlockedError("C95 lead-lag packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C95 lead-lag signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "lead_lag_pair_count",
        "qqq_leads_spy_correlation",
        "relative_lead_lag_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_leads_qqq_correlation",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C95 lead-lag signal schema mismatch")
        numeric_fields = (
            "spy_leads_qqq_correlation",
            "qqq_leads_spy_correlation",
            "relative_lead_lag_difference",
        ...
        if any(
            ...
            raise InputBlockedError("C95 lead-lag value is invalid")
        if (
            not -1.0 <= float(row["spy_leads_qqq_correlation"]) <= 1.0
            or not -1.0 <= float(row["qqq_leads_spy_correlation"]) <= 1.0
        ):
            raise InputBlockedError("C95 correlation is outside [-1, 1]")
        if (
            type(row["formation_return_count"]) is not int
            or row["formation_return_count"] != 63
            or type(row["lead_lag_pair_count"]) is not int
            or row["lead_lag_pair_count"] != 62
        ):
            raise InputBlockedError("C95 formation return count mismatch")
        difference = float(row["qqq_leads_spy_correlation"]) - float(
            row["spy_leads_qqq_correlation"]
        )
        if not math.isclose(
            float(row["relative_lead_lag_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C95 lead-lag arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C95 lead-lag target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C95 lead-lag chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C95 lead-lag signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C95 lead-lag stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C95 lead-lag state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C95 lead-lag signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C95 lead-lag state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C95 lead-lag input packet")
    receipt = _json(
        receipt_payload,
        "C95 lead-lag materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c95-relative-lead-lag-price-discovery-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C95 lead-lag signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c95-relative-lead-lag-price-discovery-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c95-relative-lead-lag-price-discovery-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C95 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C95 holdout one-use state is not pristine")
