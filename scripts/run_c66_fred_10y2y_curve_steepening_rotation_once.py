"""One-use runner for the frozen C66 10Y-2Y curve SPY/QQQ rotation lane."""
...
from research.adapters.c66_fred_10y2y_curve_steepening_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c66_fred_10y2y_curve_steepening_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c66_fred_10y2y_curve_steepening_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "659350b7dae3c0379980ca8841cc11adaaca535956b893363361f1c06ec81b95"
ADAPTER_SHA256 = "7cc2f3f32c3656da598a6d5b2cb8cafd54c7e51b78e02cfc6434f64b8407c3f1"
SIGNAL_SHA256 = "7b07586546a8e063b50a021815b1623de87a71b931e11bc685a4876ba7641f56"
RECEIPT_SHA256 = "7730c8e47fff0661d1eb757df86efced149b1fc9af212ad78652e1ac59d60acb"
...
SIGNAL_INPUT = Path(
    ...
    "c66_fred_10y2y_curve_steepening_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "T10Y2Y curve-steepening input packet")
    expected_contract = {
        "series_id": "FRED_T10Y2Y",
        "return_window": (
            "arithmetic mean of finite T10Y2Y daily observations in M-2 versus M-1 for "
            "execution month M"
        ),
        "target_rule": "strict 10Y2Y curve steepening selects QQQ; equality or decline selects SPY",
        "threshold_pct_points": 0.0,
    }
    if (
        record.get("schema_version")
        != "c66-t10y2y-curve-steepening-input-packet-v1"
        ...
        raise InputBlockedError("T10Y2Y packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("T10Y2Y signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = "e4b22b0dbbc9fecbd3d0c2cf31d1fa8aece72924ac5605007fb28277adfbc3ae"
    expected_url = (
        "https://fred.stlouisfed.org/graph/fredgraph.csv?"
        "id=T10Y2Y&cosd=2020-11-01&coed=2026-06-30"
    )
    expected_availability_rule = (
        "20:00 America/New_York after accepted close; "
        "retrospective-secondary current FRED history"
    )
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("T10Y2Y signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("T10Y2Y selections are required")
        for selection in (previous, current):
            ...
            value = selection.get("mean_spread_pct_points")
            count = selection.get("observation_count")
            _sha(
                selection.get("constituent_rows_sha256"),
                "T10Y2Y constituent rows SHA-256",
            )
            if (
                selection.get("series_id") != "FRED_T10Y2Y"
                ...
                raise InputBlockedError("T10Y2Y source row identity mismatch")
        current_value = float(current["mean_spread_pct_points"])
        previous_value = float(previous["mean_spread_pct_points"])
        change = row.get("delta_spread_pct_points")
        if (
            ...
            raise InputBlockedError("T10Y2Y change arithmetic mismatch")
        ...
        current_observation = _date(
            row.get("current_last_observation_date"), "current last observation date"
        )
        previous_observation = _date(
            row.get("previous_last_observation_date"), "previous last observation date"
        )
        ...
        if (
            ...
            != _date(current.get("last_observation_date"), "current source observation")
            or previous_observation
            != _date(previous.get("last_observation_date"), "previous source observation")
            ...
            raise InputBlockedError("T10Y2Y observation or availability mismatch")
        identity = {
            "stage": row["stage"],
            ...
            "current_last_observation_date": current_observation.isoformat(),
            "previous_last_observation_date": previous_observation.isoformat(),
            "current_row_sha256": current["row_sha256"],
            "previous_row_sha256": previous["row_sha256"],
            "delta_spread_pct_points": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("T10Y2Y signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("T10Y2Y signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "T10Y2Y row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("T10Y2Y signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("T10Y2Y state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("T10Y2Y signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("T10Y2Y state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c66-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "T10Y2Y curve-steepening/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "T10Y2Y curve-steepening/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c66-t10y2y-full-materialization-receipt-v1"
        ...
        != "98cc683ad03df9608beb2fd4f433dbb9392f8550fd3f578abf8e2d76a128fbd7"
        or receipt.get("raw_response_sha256")
        != "e4b22b0dbbc9fecbd3d0c2cf31d1fa8aece72924ac5605007fb28277adfbc3ae"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "T10Y2Y signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c66-t10y2y-curve-steepening-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c66-t10y2y-curve-steepening-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
