"""One-use runner for the frozen Cycle 31 RRP SPY/QQQ rotation lane."""
...
from research.adapters.c31_rrp_liquidity_release_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c31_rrp_liquidity_release_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c31_rrp_liquidity_release_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "ba49c7b5923dee22a319d80a96a606cdce6706b61ee06411ecea165fea9c4549"
ADAPTER_SHA256 = "ca99a46754b16fd8de0b8cde3201ae7b4b6c2421dc11a68045326a39ed6da33c"
SIGNAL_SHA256 = "372ae407a5838538ab451956aeac2319d16bca986dd64e206051dbc17430c47f"
RECEIPT_SHA256 = "4ff1a338ae1cae9624c061a02bc78e5465a09fedd2ecf77999436923b1d77d1e"
...
SIGNAL_INPUT = Path(
    ...
    "c31_rrp_liquidity_release_rotation_spy_qqq_v1/"
    "c31_full_input_packet.json"
)
...
def _prior_anchor_is_valid(previous: date, current: date) -> bool:
    anchor = current - timedelta(days=28)
    return anchor - timedelta(days=4) <= previous <= anchor
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "RRP liquidity-release rotation input packet")
    if (
        record.get("schema_version")
        != "c31-rrp-liquidity-release-spy-qqq-input-packet-v1"
        ...
        != {
            "series_id": "RRPONTTLD",
            "target_rule": "delta versus the newest eligible observation on or before the 28-day anchor < 0 means QQQ; delta >= 0 means SPY",
            "threshold_billions_usd": 0.0,
        }
    ):
        raise InputBlockedError("RRP liquidity-release rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("RRP signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("RRP signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("RRP selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "RRPONTTLD"
                or selection.get("source_class")
                != "OFFICIAL_ALFRED_NEW_YORK_FED_RRPONTTLD"
                or selection.get("raw_response_sha256")
                != "7de11930d081532d97127c5ea0ab1fc106870d6fedf9fb6eb6f52e9de540b317"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("RRP source row identity mismatch")
        current_value = current.get("value_billions_usd")
        previous_value = previous.get("value_billions_usd")
        change = row.get("delta_billions_usd_versus_prior_28_day_anchor")
        if any(
            ...
            raise InputBlockedError("RRP values must be finite")
        if float(current_value) < 0.0 or float(previous_value) < 0.0:
            raise InputBlockedError("RRP balances must be nonnegative")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("RRP change arithmetic mismatch")
        ...
        previous_reference_date = _date(
            row.get("previous_reference_date"), "previous RRP reference date"
        )
        current_reference_date = _date(
            row.get("current_reference_date"), "current RRP reference date"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous RRP available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current RRP available_at",
            )
            > decision_at
            or _date(previous.get("observation_date"), "previous observation date")
            != previous_reference_date
            or _date(current.get("observation_date"), "current observation date")
            != current_reference_date
            or not _prior_anchor_is_valid(
                previous_reference_date, current_reference_date
            )
        ):
            raise InputBlockedError(
                "RRP reference-date identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
            ...
            "previous_reference_date": previous_reference_date.isoformat(),
            "current_reference_date": current_reference_date.isoformat(),
            "current_row_sha256": current.get("row_sha256"),
            "previous_row_sha256": previous.get("row_sha256"),
            "delta_billions_usd_versus_prior_28_day_anchor": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("RRP signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("RRP source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "RRP row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("RRP signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("RRP state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("RRP signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("RRP state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c31-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "RRP liquidity-release rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "RRP liquidity-release rotation/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c31-rrp-liquidity-release-full-receipt-v1"
        ...
        != "e086835cfeddc615a879db2923718339b3fbc65acb352c8a9863bb7e8f396ef5"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "RRP signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c31-rrp-liquidity-release-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c31-rrp-liquidity-release-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 31 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 31 holdout one-use state is not pristine")
