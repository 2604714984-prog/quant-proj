"""One-use runner for the frozen Cycle 13 M2 SPY/QQQ rotation lane."""
...
from research.adapters.c13_m2_growth_acceleration import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c13_m2_growth_acceleration_v1.json"
ADAPTER = ROOT / "research/adapters/c13_m2_growth_acceleration.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "0dc32e279da5975f87ea3e405cb7601d8c1540fe23c5abe4b832a6901e059a45"
ADAPTER_SHA256 = "e1b285a11f5fcee86cee2c340905653fd8a51e970ec1be1a16a14d85fa1c547b"
SIGNAL_SHA256 = "e1ab6583a1b8a0711961c9386135f73fd4e981394bf72e7a170a4d982fdd08ff"
RECEIPT_SHA256 = "383a2c4899e64505874c4bb2d8838c0f764dbe58aab9c8ea11d40687efeec0dc"
...
SIGNAL_INPUT = Path(
    ...
    "c13_m2_growth_acceleration_spy_qqq_rotation_v1/"
    "m2_qqq_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "M2 growth-acceleration input packet")
    if (
        record.get("schema_version")
        != "c13-m2-growth-acceleration-spy-qqq-input-packet-v1"
    ...
    for row in rows:
        ...
        if any(
            ...
            raise InputBlockedError("M2 YoY values must be finite")
        expected_state = "QQQ" if float(current) > float(prior) else "SPY"
        if row.get("state") != expected_state:
            raise InputBlockedError("M2 signal arithmetic mismatch")
        identity = row.get("source_identity")
        if type(identity) is not dict:
            raise InputBlockedError("M2 source identity is required")
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                expected_state,
                _sha(row["row_sha256"], "M2 row SHA-256"),
            )
        )
        if identity_hash != row["row_sha256"]:
            raise InputBlockedError("M2 source identity hash mismatch")
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("M2 signal split count mismatch")
    for group in output.values():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("M2 signal intervals are not contiguous")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c13-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "M2/QQQ input packet")
    receipt = _json(receipt_payload, "M2/QQQ materialization receipt")
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError("M2 signal does not bind the execution schedule")
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c13-m2-growth-acceleration-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c13-m2-growth-acceleration-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 13 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 13 holdout one-use state is not pristine")
