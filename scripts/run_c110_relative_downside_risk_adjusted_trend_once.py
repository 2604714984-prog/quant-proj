"""One-use runner for frozen C110 relative-downside-risk-adjusted-trend rotation."""
...
from research.adapters.c110_relative_downside_risk_adjusted_trend import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c110_relative_downside_risk_adjusted_trend_v1.json"
ADAPTER = ROOT / "research/adapters/c110_relative_downside_risk_adjusted_trend.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "4feebcd718e4b7807a8c49d427b0596558801f6bd0ec368987f3b2604ac24ec7"
ADAPTER_SHA256 = "075c138d804ac101f961c4f702c456a2fc7ec2ca7b137adaf73dce5812417622"
SIGNAL_SHA256 = "acd2f025fbf9b7dee4b6992f1e17f117b54466f5dd2f42dafa1fd58f96245e15"
RECEIPT_SHA256 = "2c06023e848360885185bd7bf8da403b83f0d12c2a8603f53170f97d65ab5395"
...
SIGNAL_INPUT = Path(
    ...
    "c110_relative_downside_risk_adjusted_trend_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C110 relative-downside-risk-adjusted-trend input packet")
    expected_contract = {
        "formation_sessions": 127,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "total return over exact 127 adjusted closes divided by "
            "sqrt(mean(square(min(simple_daily_return,0)))) over exactly "
            "126 daily returns"
        ),
        "target_rule": (
            "QQQ downside-risk-adjusted trend score strictly greater than SPY "
            "selects QQQ; equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c110-relative-downside-risk-adjusted-trend-input-v1"
        ...
        or record.get("volume_values_used_for_signal") is not False
        ...
        raise InputBlockedError("C110 relative-downside-risk-adjusted-trend packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C110 relative-downside-risk-adjusted-trend signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_downside_deviation",
        "qqq_downside_risk_adjusted_trend_score",
        "qqq_total_return",
        "relative_downside_risk_adjusted_trend_score_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_downside_deviation",
        "spy_downside_risk_adjusted_trend_score",
        "spy_total_return",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C110 relative-downside-risk-adjusted-trend signal schema mismatch")
        value_fields = (
            "qqq_downside_deviation",
            "qqq_downside_risk_adjusted_trend_score",
            "qqq_total_return",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C110 relative-downside-risk-adjusted-trend value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 127
            or row["spy_downside_deviation"] <= 0.0
            or row["qqq_downside_deviation"] <= 0.0
        ):
            raise InputBlockedError("C110 formation session count mismatch")
        difference = (
            row["qqq_downside_risk_adjusted_trend_score"]
            - row["spy_downside_risk_adjusted_trend_score"]
        )
        if not math.isclose(
            float(
                row[
            ...
            raise InputBlockedError("C110 relative-downside-risk-adjusted-trend arithmetic mismatch")
        expected_state = "QQQ" if difference > 0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C110 relative-downside-risk-adjusted-trend target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C110 relative-downside-risk-adjusted-trend chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C110 relative-downside-risk-adjusted-trend signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C110 relative-downside-risk-adjusted-trend stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C110 relative-downside-risk-adjusted-trend state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C110 relative-downside-risk-adjusted-trend signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C110 relative-downside-risk-adjusted-trend state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C110 relative-downside-risk-adjusted-trend input packet")
    receipt = _json(
        receipt_payload,
        "C110 relative-downside-risk-adjusted-trend materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c110-relative-downside-risk-adjusted-trend-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C110 relative-downside-risk-adjusted-trend signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c110-relative-downside-risk-adjusted-trend-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c110-relative-downside-risk-adjusted-trend-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C110 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C110 holdout one-use state is not pristine")
