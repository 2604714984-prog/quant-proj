"""One-use runner for frozen C80 beta-adjusted residual leadership."""
...
from research.adapters.c80_beta_adjusted_residual_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c80_beta_adjusted_residual_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c80_beta_adjusted_residual_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6c2e88e4bd896a52f0b3bf01cae351f67e10efef2cbabc9a1108853353ba4d4e"
ADAPTER_SHA256 = "419bd556977aed611a65e2d0f3f1330afac408ab187c4b5ea610526213c8470e"
SIGNAL_SHA256 = "f77f02ca1fe0fe9535b20bd60496bc46828cea424526caf80e8b4000c7f1eecc"
RECEIPT_SHA256 = "c5c3f1ced8eab183aeaa1edac99db4df784aaff37a6234dc4bc09c1660988754"
...
SIGNAL_INPUT = Path(
    ...
    "c80_beta_adjusted_residual_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C80 beta-adjusted residual input packet")
    expected_contract = {
        "formation_close_count": 64,
        "estimation_block_return_count": 42,
        "evaluation_block_return_count": 21,
        "return_type": "natural log return",
        "estimator": (
            "ordinary OLS with intercept fitted on the first 42 paired returns "
            "using sample covariance and sample SPY variance with ddof=1"
        ),
        "residual_statistic": (
            "arithmetic sum over the final 21 observations of QQQ return minus "
            "(frozen alpha plus frozen beta times SPY return)"
        ),
        "target_rule": (
            "strictly positive evaluation residual sum means QQQ; zero or "
            "negative means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        ...
        != "c80-beta-adjusted-residual-leadership-input-v1"
        ...
        raise InputBlockedError("C80 beta-adjusted residual packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C80 beta-adjusted residual signal row count mismatch")
    expected_row_keys = {
        "ddof",
        "decision_at",
        "estimation_block_end",
        "estimation_block_start",
        "estimation_covariance",
        "estimation_qqq_mean",
        "estimation_return_count",
        "estimation_row_hash_set_sha256",
        "estimation_spy_mean",
        "estimation_spy_variance",
        "evaluation_block_end",
        "evaluation_block_start",
        "evaluation_residual_sum",
        "evaluation_return_count",
        "evaluation_row_hash_set_sha256",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C80 beta-adjusted residual signal schema mismatch")
        numeric_fields = (
            "estimation_spy_mean",
            "estimation_qqq_mean",
            "estimation_spy_variance",
        ...
        if any(
            ...
            raise InputBlockedError("C80 beta-adjusted residual value is invalid")
        if float(row["estimation_spy_variance"]) <= 0.0:
            raise InputBlockedError("C80 estimation SPY variance must be positive")
        if (
            type(row["estimation_return_count"]) is not int
            or row["estimation_return_count"] != 42
            or type(row["evaluation_return_count"]) is not int
            or row["evaluation_return_count"] != 21
        ):
            raise InputBlockedError("C80 return block count mismatch")
        if type(row["ddof"]) is not int or row["ddof"] != 1:
            raise InputBlockedError("C80 OLS ddof mismatch")

        expected_beta = float(row["estimation_covariance"]) / float(
            row["estimation_spy_variance"]
        )
        expected_alpha = float(row["estimation_qqq_mean"]) - expected_beta * float(
            row["estimation_spy_mean"]
        )
        if (
            not math.isclose(
                float(row["ols_beta"]),
            ...
            raise InputBlockedError("C80 beta-adjusted residual arithmetic mismatch")
        expected_state = (
            "QQQ" if float(row["evaluation_residual_sum"]) > 0.0 else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C80 beta-adjusted residual target mismatch")

        estimation_start = _date(
            row["estimation_block_start"], "estimation block start"
        )
        estimation_end = _date(
            row["estimation_block_end"], "estimation block end"
        )
        evaluation_start = _date(
            row["evaluation_block_start"], "evaluation block start"
        )
        evaluation_end = _date(
            row["evaluation_block_end"], "evaluation block end"
        )
        ...
        if (
            not estimation_start
            < estimation_end
            == evaluation_start
            < evaluation_end
            or evaluation_end != signal_session
            ...
            raise InputBlockedError("C80 beta-adjusted residual chronology mismatch")

        _sha(
            row["estimation_row_hash_set_sha256"],
            "estimation row-set SHA-256",
        )
        _sha(
            row["evaluation_row_hash_set_sha256"],
            "evaluation row-set SHA-256",
        )
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C80 beta-adjusted residual signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C80 beta-adjusted residual stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C80 beta-adjusted residual state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C80 beta-adjusted residual signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C80 beta-adjusted residual state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C80 beta-adjusted residual input packet")
    receipt = _json(
        receipt_payload,
        "C80 beta-adjusted residual materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c80-beta-adjusted-residual-leadership-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C80 beta-adjusted residual signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c80-beta-adjusted-residual-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c80-beta-adjusted-residual-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C80 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C80 holdout one-use state is not pristine")
