"""One-use runner for frozen C146 Philly Fed ADS growth-impulse rotation."""
...
from research.adapters.c146_philly_fed_ads_growth_impulse import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / (
    ...
    "c146_philly_fed_ads_growth_impulse_v1.json"
)
ADAPTER = ROOT / (
    "research/adapters/"
    "c146_philly_fed_ads_growth_impulse.py"
)
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6548fe4a637c4261763474cf3181a46db6102bff73a6f9fb28692768f80edac9"
ADAPTER_SHA256 = "5006e24606b14e00af41816637aabe6b321717dd5c9193b3c8e22fa651dd7a1b"
SIGNAL_SHA256 = "f50d5ced9caa447bba5f7029d3755265775fedb246b8129a0bcd231b4a35679a"
SIGNAL_RECEIPT_SHA256 = "9f24df8b61195d45a657205a6a530ad3d71664fe92d64491272dfca265e70ad2"
RUNTIME_RECEIPT_SHA256 = "b64eefd8e5a461df5536d24eaefde1eb0299e596dd784292d61c5aad37ba3fd0"
VALIDATION_BUNDLE_SHA256 = "e649fe4a9b6cff1262dea2ad977beaff874ada99f509b6bfddf9b17d0053a7a9"
HOLDOUT_BUNDLE_SHA256 = "9bdde03c4e999ddefceaa89a834a221f70e3a72a72df3f0f89ee6463df7886a8"
...
SIGNAL_INPUT = Path(
    ...
    "c146_philly_fed_ads_growth_impulse_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C146 Philly Fed ADS growth-impulse signal input")
    expected_contract = {
        "current_anchor": "preceding_Thursday_in_selected_vintage",
        "direction": "positive_ADS_weekly_impulse_selects_QQQ_else_SPY",
        "prior_anchor": (
            "current_anchor_minus_seven_calendar_days_in_same_vintage"
        ),
        "vintage": (
            "latest_official_vintage_dated_strictly_before_decision_session"
        ),
    }
    if (
        record.get("schema_version")
        != "c146-philly-fed-ads-growth-impulse-input-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("evidence_class")
        != "OFFICIAL_REAL_TIME_VINTAGE_RETROSPECTIVE_SECONDARY"
        ...
        raise InputBlockedError("C146 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C146 signal row count mismatch")
    expected_row_keys = {
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C146 signal schema mismatch")
        value_fields = (
            "current_value",
            "prior_value",
        )
        if any(
            ...
            raise InputBlockedError("C146 ADS value is invalid")
        impulse = row["current_value"] - row["prior_value"]
        observed_impulse = row["growth_impulse"]
        if (
            ...
            raise InputBlockedError("C146 ADS impulse is invalid")
        expected_state = "QQQ" if impulse > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C146 target mismatch")
        ...
        current_date = _date(
            row["current_observation_date"],
            "current observation date",
        )
        prior_date = _date(
            row["prior_observation_date"],
            "prior observation date",
        )
        vintage_date = _date(row["vintage_date"], "vintage date")
        ...
        if (
            ...
            or not vintage_date < decision_session
            or not prior_date < current_date <= preceding_thursday
            or prior_date > current_date - timedelta(days=7)
        ):
            raise InputBlockedError("C146 signal chronology mismatch")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C146 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C146 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C146 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C146 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C146 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C146 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C146 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C146 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C146 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C146 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c146-philly-fed-ads-growth-impulse-receipt-v1"
        ...
        != "c146-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c146-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C146 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C146 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C146 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C146 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c146-philly-fed-ads-growth-impulse-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c146-philly-fed-ads-growth-impulse-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C146 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C146 holdout one-use state is not pristine")
