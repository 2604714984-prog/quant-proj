"""One-use runner for the frozen C155 WRESBAL SPY/QQQ rotation lane."""
...
from research.adapters.c155_reserve_balances_13w_liquidity import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c155_reserve_balances_13w_liquidity_v1.json"
ADAPTER = ROOT / "research/adapters/c155_reserve_balances_13w_liquidity.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "2e857eda0774c0d528aaf440bddc6621dd6adba694dd009a4b1697423a6fb2f3"
ADAPTER_SHA256 = "0f3cb29eb9abd68d389f1a8f6d29944152ec3efa84c26df0d12c110f16ad358e"
SIGNAL_SHA256 = "e7b1e8174596fbc9f59953650afdb3fb2051a3b3f2d88451f4acc84a3714af2a"
RECEIPT_SHA256 = "44d8c65dbb7e696f99f1be3a17e6b8fb14227bc4f81ce4a4077b5548550708c4"
...
SIGNAL_INPUT = Path(
    ...
    "c155_reserve_balances_13w_liquidity_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "WRESBAL impulse rotation input packet")
    if (
        record.get("schema_version")
        != "c155-wresbal-spy-qqq-full-input-v1"
        ...
        != {
            "series_id": "WRESBAL",
            "target_rule": "current reserve balances >= latest observation at least 91 calendar days earlier means QQQ; otherwise SPY",
            "lookback_calendar_days": 91,
        }
    ):
        raise InputBlockedError("WRESBAL rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("WRESBAL signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("WRESBAL signal stage mismatch")
        required = {
            "actual_vintage_date",
            "comparison_observation_date",
            "comparison_reserve_balance_millions",
            "current_observation_date",
            "current_reserve_balance_millions",
            "decision_at",
        ...
        if set(row) != required:
            raise InputBlockedError("WRESBAL signal fields mismatch")
        numeric = tuple(
            row.get(key)
            for key in (
                "current_reserve_balance_millions",
        ...
        if any(
            ...
            raise InputBlockedError("WRESBAL statistics must be finite")
        current_balance, comparison_balance, impulse = map(float, numeric)
        if not all(0.0 <= value < 100_000_000.0 for value in (current_balance, comparison_balance)):
            raise InputBlockedError("WRESBAL balances are implausible")
        ...
        comparison_date = _date(
            row["comparison_observation_date"], "comparison observation date"
        )
        current_date = _date(
            row["current_observation_date"], "current observation date"
        )
        vintage = _date(row["actual_vintage_date"], "WRESBAL actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "WRESBAL response SHA-256"
        )
        if (
            vintage > decision_at.date()
            or current_date >= start
            or current_date > decision_at.date()
            or current_date - comparison_date < timedelta(days=91)
            or not math.isclose(
                impulse,
                current_balance - comparison_balance,
                rel_tol=0.0,
                abs_tol=1e-12,
            )
            ...
            raise InputBlockedError("WRESBAL chronology or state mismatch")
        identity = {
            "stage": row["stage"],
            ...
            "comparison_observation_date": comparison_date.isoformat(),
            "comparison_reserve_balance_millions": comparison_balance,
            "current_observation_date": current_date.isoformat(),
            "current_reserve_balance_millions": current_balance,
            "impulse_millions": impulse,
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("WRESBAL signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("WRESBAL source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "WRESBAL row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("WRESBAL signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("WRESBAL state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("WRESBAL signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("WRESBAL state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c155-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "New York Fed WRESBAL/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "New York Fed WRESBAL/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c155-wresbal-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "WRESBAL signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c155-nyfed-wresbal-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c155-nyfed-wresbal-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
