"""One-use runner for frozen C137 relative distribution-growth rotation."""
...
from research.adapters.c137_relative_distribution_growth import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c137_relative_distribution_growth_v1.json"
ADAPTER = ROOT / "research/adapters/c137_relative_distribution_growth.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "4484f48ea3aba592b11ceeb600d62c9e45614a4a4b4fd76a740062775eb30ce3"
ADAPTER_SHA256 = "3b9483d641e547125156e3e9507807ea20102e28ec7075671ec5430ce22e93ed"
SIGNAL_SHA256 = "0932666b69f497d67c9243041c354353b719590c937d7c46d55ccc722a14e5b3"
RECEIPT_SHA256 = "7e3e4070582d93bd2a6a57635297f8ec04e8e11b3369ce8a5e1dd8bad28e977b"
...
SIGNAL_INPUT = Path(
    ...
    "c137_relative_distribution_growth_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C137 relative distribution-growth input packet")
    expected_contract = {
        "formation_sessions": 16,
        "missing_rule": "fail closed; require the latest eight positive accepted distributions for both ETFs",
        "score_definition": (
            "For each ETF, sum the latest four per-share cash distributions and divide "
            "by the preceding four-distribution sum minus one; score is QQQ growth minus SPY growth"
        ),
        "target_rule": (
            "Strictly positive relative distribution growth selects QQQ; zero or negative selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c137-relative-distribution-growth-input-v1"
        ...
        raise InputBlockedError("C137 relative distribution-growth packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C137 relative distribution-growth signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_distribution_growth",
        "qqq_minus_spy_distribution_growth",
        "spy_distribution_growth",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C137 relative distribution-growth signal schema mismatch")
        value_fields = (
            "qqq_distribution_growth",
            "qqq_minus_spy_distribution_growth",
            "spy_distribution_growth",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C137 relative distribution-growth value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 16
            or row["qqq_distribution_growth"] <= -1.0
            or row["spy_distribution_growth"] <= -1.0
            or not math.isclose(
                row["qqq_minus_spy_distribution_growth"],
                row["qqq_distribution_growth"] - row["spy_distribution_growth"],
                rel_tol=0.0,
                abs_tol=1e-15,
            )
        ):
            raise InputBlockedError("C137 formation or distribution-growth identity mismatch")
        expected_state = "QQQ" if row["qqq_minus_spy_distribution_growth"] > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C137 relative distribution-growth target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C137 relative distribution-growth chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C137 relative distribution-growth signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C137 relative distribution-growth stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C137 relative distribution-growth state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C137 relative distribution-growth signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C137 relative distribution-growth state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C137 relative distribution-growth input packet")
    receipt = _json(
        receipt_payload,
        "C137 relative distribution-growth materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c137-relative-distribution-growth-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C137 relative distribution-growth signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c137-relative-distribution-growth-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c137-relative-distribution-growth-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C137 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C137 holdout one-use state is not pristine")
