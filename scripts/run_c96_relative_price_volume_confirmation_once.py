"""One-use runner for frozen C96 relative price-volume confirmation."""
...
from research.adapters.c96_relative_price_volume_confirmation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c96_relative_price_volume_confirmation_v1.json"
ADAPTER = ROOT / "research/adapters/c96_relative_price_volume_confirmation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "8b94d6be2506fbda9ed0e204c5dc125dff07ddc48c36a0f4b0a5444af4497562"
ADAPTER_SHA256 = "326264261d66762dcdafaa15bf080c352bd39d5812dcd1425c7d9e1bb692de88"
SIGNAL_SHA256 = "f1762f763235e7fff8dd6796a441737702d392a949cb581acfd0c066e873d9ed"
RECEIPT_SHA256 = "599ea72fdffc4141c712665fce2a0b71fd090fd492d373ac5320b2eab494cd19"
...
SIGNAL_INPUT = Path(
    ...
    "c96_relative_price_volume_confirmation_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C96 price-volume input packet")
    expected_contract = {
        "formation_returns": 63,
        "missing_rule": "fail closed; no alternate source or window",
        "return_definition": "raw close_t / raw close_t_minus_1 - 1",
        "volume_change_definition": (
            "natural logarithm of raw volume_t divided by raw "
            "volume_t_minus_1"
        ),
        "confirmation_definition": (
            "ordinary Pearson correlation between each asset's exact "
            "63 returns and same-session 63 log-volume changes, using "
            "population covariance and population standard deviations"
        ),
        "target_rule": (
            "QQQ return-volume-change correlation strictly greater "
            "than SPY selects QQQ; "
            "equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c96-relative-price-volume-confirmation-input-v1"
        ...
        raise InputBlockedError("C96 price-volume packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C96 price-volume signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        "formation_return_count",
        "formation_start",
        "holding_end_execution_session",
        "holding_start_execution_session",
        "correlation_pair_count",
        "qqq_return_volume_change_correlation",
        "relative_price_volume_confirmation_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_return_volume_change_correlation",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C96 price-volume signal schema mismatch")
        numeric_fields = (
            "spy_return_volume_change_correlation",
            "qqq_return_volume_change_correlation",
            "relative_price_volume_confirmation_difference",
        ...
        if any(
            ...
            raise InputBlockedError("C96 price-volume value is invalid")
        if (
            not -1.0 <= float(row["spy_return_volume_change_correlation"]) <= 1.0
            or not -1.0 <= float(row["qqq_return_volume_change_correlation"]) <= 1.0
        ):
            raise InputBlockedError("C96 correlation is outside [-1, 1]")
        if (
            type(row["formation_return_count"]) is not int
            or row["formation_return_count"] != 63
            or type(row["correlation_pair_count"]) is not int
            or row["correlation_pair_count"] != 63
        ):
            raise InputBlockedError("C96 formation return count mismatch")
        difference = float(row["qqq_return_volume_change_correlation"]) - float(
            row["spy_return_volume_change_correlation"]
        )
        if not math.isclose(
            float(row["relative_price_volume_confirmation_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C96 price-volume arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C96 price-volume target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C96 price-volume chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C96 price-volume signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C96 price-volume stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C96 price-volume state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C96 price-volume signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C96 price-volume state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C96 price-volume input packet")
    receipt = _json(
        receipt_payload,
        "C96 price-volume materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c96-relative-price-volume-confirmation-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C96 price-volume signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c96-relative-price-volume-confirmation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c96-relative-price-volume-confirmation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C96 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C96 holdout one-use state is not pristine")
