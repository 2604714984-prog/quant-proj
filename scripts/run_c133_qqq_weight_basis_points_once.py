"""One-use runner for frozen C133 qqq-weight-basis-points rotation."""
...
from research.adapters.c133_qqq_weight_basis_points import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c133_qqq_weight_basis_points_v1.json"
ADAPTER = ROOT / "research/adapters/c133_qqq_weight_basis_points.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "9af905af2e7f3cd08a9579bb09019b2eaff0b03ac80a3164cd17b8249a69a7aa"
ADAPTER_SHA256 = "b38130f61ffebdc478cb2f5de9d71cb0580b3456f69042d483d6cd0a8eea66c0"
SIGNAL_SHA256 = "ad331d0f3cb34b8a3f076a81ac47f43cc5befda71f0aa96db2660e0d573df8e2"
RECEIPT_SHA256 = "2bb9b8e58225d40650b4fb04061454573d96aa288dea0600aa1341cf8ac31d1c"
...
SIGNAL_INPUT = Path(
    ...
    "c133_static_qqq_60_spy_40_tilt_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C133 qqq-weight-basis-points input packet")
    expected_contract = {
        "formation_sessions": 0,
        "missing_rule": "fail closed; no alternate calendar, universe, or weights",
        "score_definition": (
            "Constant QQQ target weight 0.60 and SPY target weight 0.40; "
            "no price value enters the allocation"
        ),
        "target_rule": (
            "QQQ weight is 0.60 and SPY weight is 0.40 at every rebalance"
        ),
    }
    if (
        record.get("schema_version")
        != "c133-qqq-weight-basis-points-input-v1"
        ...
        raise InputBlockedError("C133 qqq-weight-basis-points packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C133 qqq-weight-basis-points signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C133 qqq-weight-basis-points signal schema mismatch")
        value_fields = ("qqq_weight_basis_points",)
        if any(
            ...
            raise InputBlockedError(
                "C133 qqq-weight-basis-points value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 0
            or row["qqq_weight_basis_points"] != 6000
        ):
            raise InputBlockedError("C133 formation or fixed-weight mismatch")
        expected_state = "QQQ"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C133 qqq-weight-basis-points target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C133 qqq-weight-basis-points chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C133 qqq-weight-basis-points signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C133 qqq-weight-basis-points stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C133 qqq-weight-basis-points state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C133 qqq-weight-basis-points signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C133 qqq-weight-basis-points state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C133 qqq-weight-basis-points input packet")
    receipt = _json(
        receipt_payload,
        "C133 qqq-weight-basis-points materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c133-qqq-weight-basis-points-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C133 qqq-weight-basis-points signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c133-qqq-weight-basis-points-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c133-qqq-weight-basis-points-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C133 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C133 holdout one-use state is not pristine")
