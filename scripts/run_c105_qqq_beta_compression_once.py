"""One-use runner for frozen C105 QQQ beta-compression rotation."""
...
from research.adapters.c105_qqq_beta_compression import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c105_qqq_beta_compression_v1.json"
ADAPTER = ROOT / "research/adapters/c105_qqq_beta_compression.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "a8125a2e699c9456b35ecd38d48cae4a52ba94eda4c5ecab300d0fa8f6a48ea6"
ADAPTER_SHA256 = "656d6b4c52faca2c918855996dd47f3b2d19b513a5f4cd37d892c33b867ffde8"
SIGNAL_SHA256 = "0080093b0e5d8218858256b4296d9e2e7c42c8bc3438fd17473001d475539399"
RECEIPT_SHA256 = "028c90be3f51e3f872435f8de072cc1f5bf65130d14aade2dc5b9385478b790f"
...
SIGNAL_INPUT = Path(
    ...
    "c105_qqq_beta_compression_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C105 QQQ-beta-compression input packet")
    expected_contract = {
        ...
        "score_definition": (
            "centered population covariance of QQQ and SPY returns "
            "divided by centered population variance of SPY returns"
        ),
        "target_rule": (
            "recent 21-return QQQ beta strictly below preceding "
            "42-return beta selects QQQ; equality or higher selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c105-qqq-beta-compression-input-v1"
        ...
        raise InputBlockedError("C105 QQQ-beta-compression packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C105 QQQ-beta-compression signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "baseline_qqq_beta_to_spy",
        "baseline_return_count",
        "beta_compression_difference",
        "recent_qqq_beta_to_spy",
        "recent_return_count",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C105 QQQ-beta-compression signal schema mismatch")
        numeric_fields = (
            "baseline_qqq_beta_to_spy",
            "recent_qqq_beta_to_spy",
            "beta_compression_difference",
        ...
        if any(
            ...
            raise InputBlockedError("C105 QQQ-beta-compression value is invalid")
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or type(row["baseline_return_count"]) is not int
            or row["baseline_return_count"] != 42
            or type(row["recent_return_count"]) is not int
            or row["recent_return_count"] != 21
        ):
            raise InputBlockedError("C105 formation session count mismatch")
        difference = float(row["recent_qqq_beta_to_spy"]) - float(
            row["baseline_qqq_beta_to_spy"]
        )
        if not math.isclose(
            float(row["beta_compression_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C105 QQQ-beta-compression arithmetic mismatch")
        expected_state = "QQQ" if difference < 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C105 QQQ-beta-compression target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C105 QQQ-beta-compression chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C105 QQQ-beta-compression signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C105 QQQ-beta-compression stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C105 QQQ-beta-compression state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C105 QQQ-beta-compression signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C105 QQQ-beta-compression state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C105 QQQ-beta-compression input packet")
    receipt = _json(
        receipt_payload,
        "C105 QQQ-beta-compression materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c105-qqq-beta-compression-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C105 QQQ-beta-compression signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c105-qqq-beta-compression-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c105-qqq-beta-compression-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C105 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C105 holdout one-use state is not pristine")
