"""One-use runner for frozen C120 qqq-obv-direction rotation."""
...
from research.adapters.c120_qqq_obv_direction import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c120_qqq_obv_direction_v1.json"
ADAPTER = ROOT / "research/adapters/c120_qqq_obv_direction.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "571a27d20d502765131b28b0ddfea2734956ca952d38fbade7aff6954c856f0e"
ADAPTER_SHA256 = "69f397adc5797135bb323c52a8adad22757f7950ac26183c3bfdea1cb8607fdb"
SIGNAL_SHA256 = "d5fce2cbf72bf00b0f8eb3af9a7b7adcbe9627f4a3b6bdbd128e04d6589dfd45"
RECEIPT_SHA256 = "42d321bdac80e4737f510b583ba3b5aa59cdf5089f36036464d4717422c0ffa5"
...
SIGNAL_INPUT = Path(
    ...
    "c120_qqq_63_session_obv_direction_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C120 qqq-obv-direction input packet")
    expected_contract = {
        "formation_sessions": 63,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "OBV starts at zero over exactly 63 QQQ adjusted close and adjusted "
            "volume observations; each of the final 62 volumes is added after "
            "an up close, subtracted after a down close, and ignored on a tie"
        ),
        "target_rule": (
            "positive QQQ OBV selects QQQ; zero or negative selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c120-qqq-obv-direction-input-v1"
        ...
        raise InputBlockedError("C120 qqq-obv-direction packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C120 qqq-obv-direction signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C120 qqq-obv-direction signal schema mismatch")
        value_fields = ("qqq_obv",)
        if any(
            ...
            raise InputBlockedError(
                "C120 qqq-obv-direction value is invalid"
            )
        if (
        ...
        expected_state = "QQQ" if row["qqq_obv"] > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C120 qqq-obv-direction target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C120 qqq-obv-direction chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C120 qqq-obv-direction signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C120 qqq-obv-direction stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C120 qqq-obv-direction state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C120 qqq-obv-direction signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C120 qqq-obv-direction state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C120 qqq-obv-direction input packet")
    receipt = _json(
        receipt_payload,
        "C120 qqq-obv-direction materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c120-qqq-obv-direction-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C120 qqq-obv-direction signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c120-qqq-obv-direction-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c120-qqq-obv-direction-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
