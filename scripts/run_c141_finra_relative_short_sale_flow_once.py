"""One-use runner for frozen C141 FINRA relative short-flow rotation."""
...
from research.adapters.c141_finra_relative_short_sale_flow import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c141_finra_relative_short_sale_flow_v1.json"
ADAPTER = ROOT / "research/adapters/c141_finra_relative_short_sale_flow.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6744c3810224b920586b87702d19260086fb105e95f86e05713db8fdde1b5689"
ADAPTER_SHA256 = "dd5743f37007f2f7f0565dd2b1fb16d27606677b519e96f7c5cd1fc20b043723"
SIGNAL_SHA256 = "02ffa0f232c315fa479ef71241b9bd338a55a8ac7984d404b3ef8159ee941b2f"
SIGNAL_RECEIPT_SHA256 = "d8aa7c7f8ba1c2be5241bb47702e3d8a1964fc76ee37cabccedf9f9a3fcfbd4a"
RUNTIME_RECEIPT_SHA256 = "51340bba76f25908b17b936c71953cb6d75631339512bfad05a774bf3c26e1a8"
VALIDATION_BUNDLE_SHA256 = "1598285df8483f082f730443cc7fd9cb8b82731e17cf63792804890a8ff49403"
HOLDOUT_BUNDLE_SHA256 = "9f054ee40f5814029a5ce7546779040235f8ef5d6e4008546d88b12b9ed87743"
...
SIGNAL_INPUT = Path(
    ...
    "c141_finra_relative_off_exchange_short_sale_flow_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C141 FINRA relative short-flow signal input")
    expected_contract = {
        "direction": "QQQ_ratio_strictly_above_SPY_ratio_selects_QQQ_else_SPY",
        "fields": [
            "ShortVolume",
            "ShortExemptVolume",
            "TotalVolume",
        ...
        "ratio": "sum(ShortVolume+ShortExemptVolume)/sum(TotalVolume)",
    }
    if (
        record.get("schema_version")
        != "c141-finra-relative-short-sale-flow-input-v1"
        ...
        raise InputBlockedError("C141 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C141 signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "decision_session",
        "observation_sessions",
        "qqq_short_sale_ratio",
        "qqq_totals",
        "signal_identity_sha256",
        "spy_short_sale_ratio",
        "spy_totals",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C141 signal schema mismatch")
        value_fields = ("qqq_short_sale_ratio", "spy_short_sale_ratio")
        if any(
            ...
            raise InputBlockedError("C141 signal value is invalid")
        expected_state = (
            "QQQ"
            if row["qqq_short_sale_ratio"] > row["spy_short_sale_ratio"]
            else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C141 target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C141 signal chronology mismatch")
        ...
        for symbol in ("qqq", "spy"):
            ...
            if (
                ...
                raise InputBlockedError("C141 signal totals are invalid")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C141 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C141 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C141 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C141 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C141 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C141 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C141 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C141 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C141 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C141 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c141-finra-relative-short-sale-flow-receipt-v1"
        ...
        != "c141-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c141-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C141 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C141 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C141 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C141 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c141-finra-relative-short-sale-flow-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c141-finra-relative-short-sale-flow-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C141 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C141 holdout one-use state is not pristine")
