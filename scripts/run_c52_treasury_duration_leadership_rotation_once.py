"""One-use runner for the frozen Cycle 52 Treasury-duration SPY/QQQ lane."""
...
from research.adapters.c52_treasury_duration_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c52_treasury_duration_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c52_treasury_duration_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "b03dc83a39d61273c5edbe172be6bb0a8ba4feed7961c1c4dccae8c2ea2bb520"
ADAPTER_SHA256 = "5aaa35ca731942d3625e2b2d4952fb24bddfe5d86e94a8bb9477c1b0a0148a89"
SIGNAL_SHA256 = "2aea7624784c6227208fd9b328647406ff8c0d37fdec449c23fb9949c3cf4dc8"
RECEIPT_SHA256 = "459b1403863a7a603d4e81dd54900e5f80ef55502cc633003f43bc3372d60615"
...
SIGNAL_INPUT = Path(
    ...
    "c52_treasury_duration_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Treasury duration leadership rotation input packet")
    expected_contract = {
        "left_symbol": "TLT",
        "right_symbol": "BIL",
        "return_window": (
        ...
        "target_rule": (
            "strict TLT prior-month total-return outperformance versus BIL "
    ...
    if (
        ...
        != "epoch9-cross-asset-leadership-input-packet-v1"
        ...
        or record.get("gld_price_values_used_for_materialization") is not False
        ...
        raise InputBlockedError("growth value style packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("growth value style signal row count mismatch")
    ...
    expected_source = {
        "TLT": (
            "https://query1.finance.yahoo.com/v8/finance/chart/TLT?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "b8e97dbc5fc577c5dd1b431167491e005986f84db20bcb8408e200f5bcb5c0db",
        ),
        "BIL": (
            "https://query1.finance.yahoo.com/v8/finance/chart/BIL?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "fb760406355347fecf703a4c1c023cd0a1d817502d56d37e6ca0be14714c5d10",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("growth value style signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("growth value style selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "growth value style adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("growth value style source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "growth value style cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("growth value style arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "growth value style month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("growth value style signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("growth value style signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "growth value style row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("growth value style signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("growth value style state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "growth value style signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("growth value style state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "growth value style leadership rotation input packet")
    receipt = _json(
        receipt_payload,
        "growth value style materialization receipt",
    )
    ...
    if (
        ...
        != "epoch9-cross-asset-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "TLT"
        or receipt.get("right_symbol") != "BIL"
        or receipt.get("raw_fetch_receipt_sha256")
        != "edacf753166a670f24559c6c4c22d9cbc2e6eaa00f52bd045c9d449c12552396"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "growth value style signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c52-treasury-duration-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c52-treasury-duration-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 46 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 46 holdout one-use state is not pristine")
