"""One-use runner for the frozen Cycle 55 quality/value SPY/QQQ lane."""
...
from research.adapters.c55_quality_value_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c55_quality_value_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c55_quality_value_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6777f8901c9e0020f07015582de92be7386165d86b399abdfe33b695aa1512dc"
ADAPTER_SHA256 = "368f6fff2c607e28decd0ccc603d65e93f28b6065a31e5e92886cfd83df2c72b"
SIGNAL_SHA256 = "4fe41f756b3f5d55642e48c2e5027babdf147d3e7237b59a89b1da7bd7a604b5"
RECEIPT_SHA256 = "e5831ef288e1e9200b711806b7dbccd42d3576adeae4fd4593d2a7984597381b"
...
SIGNAL_INPUT = Path(
    ...
    "c55_quality_value_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "quality value leadership rotation input packet")
    expected_contract = {
        "left_symbol": "QUAL",
        "right_symbol": "VTV",
        "return_window": (
        ...
        "target_rule": (
            "strict QUAL prior-month total-return outperformance versus VTV "
    ...
    if (
        ...
        raise InputBlockedError("quality value packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("quality value signal row count mismatch")
    ...
    expected_source = {
        "QUAL": (
            "https://query1.finance.yahoo.com/v8/finance/chart/QUAL?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "60aa1ac8457a668df24965da2ea74b92b4ace1bb31a8b447a0a998b521c5af95",
        ),
        "VTV": (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("quality value signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("quality value selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "quality value adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("quality value source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "quality value cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("quality value arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "quality value month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("quality value signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("quality value signal identity hash mismatch")
    ...
    if (
        ...
        raise InputBlockedError("quality value signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("quality value state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "quality value signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("quality value state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "quality value leadership rotation input packet")
    receipt = _json(
        receipt_payload,
        "quality value leadership materialization receipt",
    )
    ...
    if (
        ...
        or receipt.get("left_symbol") != "QUAL"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "quality value signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c55-quality-value-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c55-quality-value-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
