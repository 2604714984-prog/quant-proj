"""One-use runner for frozen C87 one-month relative reversal."""
...
from research.adapters.c87_one_month_relative_reversal_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c87_one_month_relative_reversal_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c87_one_month_relative_reversal_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "41002a449e1cd61cdced1105b72441740c7cc46b3a7ba68bba200de5ec7ea664"
ADAPTER_SHA256 = "a059e1ba3d1cb11c3a27da26cc2c2c8ece151a448a67d0a5b9a4ee627a15b965"
SIGNAL_SHA256 = "178742533fd8e25b2f12e51382b5a3f9dd37b10eb1404f79d8c1d6b45fdecb46"
RECEIPT_SHA256 = "c6a4f814aacdea6eca96cff7ee3da05bf77cc4e82e074e28a1c5f2ee395b5d7b"
...
SIGNAL_INPUT = Path(
    ...
    "c87_one_month_relative_reversal_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C87 relative-reversal input packet")
    expected_contract = {
        "formation_close_count": 22,
        "return_count": 21,
        "return_type": "natural log return",
        "relative_sum": "sum of QQQ log returns minus sum of SPY log returns",
        "target_rule": (
            "strictly negative relative sum means QQQ; zero or positive means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c87-one-month-relative-reversal-input-v1"
        ...
        raise InputBlockedError("C87 relative-reversal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C87 relative-reversal signal row count mismatch")
    expected_row_keys = {
        "decision_at",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C87 relative-reversal signal schema mismatch")
        numeric_fields = (
            "spy_log_return_sum",
            "qqq_log_return_sum",
            "qqq_minus_spy_log_return_sum",
        ...
        if any(
            ...
            raise InputBlockedError("C87 relative-reversal value is invalid")
        if (
            type(row["return_count"]) is not int
            or row["return_count"] != 21
        ):
            raise InputBlockedError("C87 return count mismatch")

        spy_sum = float(row["spy_log_return_sum"])
        qqq_sum = float(row["qqq_log_return_sum"])
        relative_sum = qqq_sum - spy_sum
        if not math.isclose(
            float(row["qqq_minus_spy_log_return_sum"]),
            relative_sum,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C87 relative-reversal arithmetic mismatch")
        expected_state = "QQQ" if relative_sum < 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C87 relative-reversal target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C87 relative-reversal chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C87 relative-reversal signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C87 relative-reversal stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C87 relative-reversal state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C87 relative-reversal signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C87 relative-reversal state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C87 relative-reversal input packet")
    receipt = _json(
        receipt_payload,
        "C87 relative-reversal materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c87-one-month-relative-reversal-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C87 relative-reversal signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c87-one-month-relative-reversal-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c87-one-month-relative-reversal-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C87 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C87 holdout one-use state is not pristine")
