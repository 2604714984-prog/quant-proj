"""One-use runner for the frozen Cycle 48 emerging market risk appetite SPY/QQQ lane."""
...
from research.adapters.c48_emerging_market_risk_appetite_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c48_emerging_market_risk_appetite_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c48_emerging_market_risk_appetite_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "1cef2f8021f169409aa78c7f29dc41fcdd34a7007e879c190efecea3f225e4ec"
ADAPTER_SHA256 = "710a8cb125a5e23f13d6e665454d30a39bf9865068c99eb09a11ee3dd4f55953"
SIGNAL_SHA256 = "61c42b40a49ff90e3dabee05e0f667544adec76e253e36d3edd8ccc81c76dbd6"
RECEIPT_SHA256 = "9789758c6d4101db9a227a2f776fa650e0403b2de24dd5b33d8938c6e304fb20"
...
SIGNAL_INPUT = Path(
    ...
    "c48_emerging_market_risk_appetite_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "emerging market risk appetite rotation input packet")
    expected_contract = {
        "left_symbol": "EEM",
        "right_symbol": "SPY",
        "return_window": (
        ...
        "target_rule": (
            "strict EEM prior-month total-return outperformance versus SPY "
    ...
    if (
        ...
        != "epoch7-intermarket-leadership-input-packet-v1"
        ...
        raise InputBlockedError("emerging market risk appetite packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("emerging market risk appetite signal row count mismatch")
    ...
    expected_source = {
        "EEM": (
            "https://query1.finance.yahoo.com/v8/finance/chart/EEM?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "e6b7c0a2b6be8b3c04e769aaeb66bcbe9d3daa7294776eeca5f66edc440e24fe",
        ),
        "SPY": (
            "https://query1.finance.yahoo.com/v8/finance/chart/SPY?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "24af0ab0ce6d0a344312cff49546efda4f5aafa6cdf66f93b4500d5b36974a33",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("emerging market risk appetite signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("emerging market risk appetite selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "emerging market risk appetite adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("emerging market risk appetite source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "emerging market risk appetite cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("emerging market risk appetite arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "emerging market risk appetite month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("emerging market risk appetite signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("emerging market risk appetite signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "emerging market risk appetite row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("emerging market risk appetite signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("emerging market risk appetite state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "emerging market risk appetite signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("emerging market risk appetite state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "emerging market risk appetite rotation input packet")
    receipt = _json(
        receipt_payload,
        "emerging market risk appetite materialization receipt",
    )
    ...
    if (
        ...
        != "epoch7-intermarket-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "EEM"
        or receipt.get("right_symbol") != "SPY"
        or receipt.get("raw_fetch_receipt_sha256")
        != "906820daebb2db49680c79f2b05abb6e3f8449f16a2fb5bd9ce3550dfa126199"
        or receipt.get("left_raw_response_sha256")
        != "e6b7c0a2b6be8b3c04e769aaeb66bcbe9d3daa7294776eeca5f66edc440e24fe"
        or receipt.get("right_raw_response_sha256")
        != "24af0ab0ce6d0a344312cff49546efda4f5aafa6cdf66f93b4500d5b36974a33"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "emerging market risk appetite signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c48-emerging-market-risk-appetite-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c48-emerging-market-risk-appetite-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 48 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 48 holdout one-use state is not pristine")
