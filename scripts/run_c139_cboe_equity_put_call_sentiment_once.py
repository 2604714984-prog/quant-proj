"""One-use runner for frozen C139 Cboe put/call sentiment rotation."""
...
from research.adapters.c139_cboe_equity_put_call_sentiment import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c139_cboe_equity_put_call_sentiment_v1.json"
ADAPTER = ROOT / "research/adapters/c139_cboe_equity_put_call_sentiment.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "ccac6b9bb9c8e3aef9924298bde4ad6f92a226d89509ee70ab632bbea8a45d78"
ADAPTER_SHA256 = "445c58b388661cb5f05472971f25c805831c39d3baddf84f8e5310a5689b4960"
SIGNAL_SHA256 = "6b1786096842b1c02de1eb562ba4068d6a341612c43aacee66d5f05f7578db07"
SIGNAL_RECEIPT_SHA256 = "eafab9f065dd50e2760df99f725ff6a72e43024e04ca1f9b4109e370cb770227"
RUNTIME_RECEIPT_SHA256 = "0f9ac1ce917d3bc8f9ff1e7cfc591751bf6cbaf833bb08e640d00c0b3829326c"
VALIDATION_BUNDLE_SHA256 = "9452bcf40a0214c20588626a832a40f6263e5763bcf7f0c5969fe4f11131256c"
HOLDOUT_BUNDLE_SHA256 = "7532106cb04c75e7b011238f52a417be25d0c9ebd09f153413a976310e249d66"
...
SIGNAL_INPUT = Path(
    ...
    "c139_cboe_equity_put_call_sentiment_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C139 Cboe put/call signal input")
    expected_contract = {
        "current_window_sessions": 20,
        "direction": "above_prior_52_week_median_selects_SPY_else_QQQ",
        "history_weeks": 52,
        "latest_allowed_observation": "preceding Thursday",
    }
    if (
        record.get("schema_version")
        != "c139-cboe-equity-put-call-input-v1"
        ...
        raise InputBlockedError("C139 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C139 signal row count mismatch")
    expected_row_keys = {
        "current_20_session_mean",
        "decision_at",
        "decision_session",
        "history_52_week_median",
        "signal_cutoff",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C139 signal schema mismatch")
        value_fields = (
            "current_20_session_mean",
            "history_52_week_median",
        )
        if any(
            ...
            raise InputBlockedError("C139 signal value is invalid")
        if (
            type(row["source_observation_count"]) is not int
            or row["source_observation_count"] != 1060
        ):
            raise InputBlockedError("C139 source observation count mismatch")
        expected_state = (
            "SPY"
            if row["current_20_session_mean"] > row["history_52_week_median"]
            else "QQQ"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C139 target mismatch")
        decision_session = _date(row["decision_session"], "decision session")
        cutoff = _date(row["signal_cutoff"], "signal cutoff")
        ...
        if (
            ...
            or cutoff
            != decision_session
            - timedelta(days=decision_session.weekday() + 4)
        ):
            raise InputBlockedError("C139 signal chronology mismatch")

        _sha(
            row["source_rows_sha256"],
            "source row-set SHA-256",
        )
        selection_hash = _sha(
            row["signal_identity_sha256"],
            "signal identity SHA-256",
        )
        identity = {key: value for key, value in row.items() if key != "signal_identity_sha256"}
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C139 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C139 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C139 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C139 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C139 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C139 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C139 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C139 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C139 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C139 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c139-cboe-equity-put-call-receipt-v1"
        ...
        != "c139-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c139-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C139 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C139 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C139 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C139 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c139-cboe-put-call-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c139-cboe-put-call-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C139 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C139 holdout one-use state is not pristine")
