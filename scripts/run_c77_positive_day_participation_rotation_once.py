"""One-use runner for frozen C77 positive-day participation."""
...
from research.adapters.c77_positive_day_participation_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c77_positive_day_participation_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c77_positive_day_participation_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "1170e6c8fff958ca43db16bb9c1ca3a99a6b9e1084c607961e69ab1e19ea21b8"
ADAPTER_SHA256 = "8fe48bb2fb906ee5ca2e842032d34feaac1d8bf2c3781f390ab5a858314fcf5a"
SIGNAL_SHA256 = "a5cd3e9b346c8b174c66a5081aae4d0f064c0c47d6071dcca7ec5158d638ff67"
RECEIPT_SHA256 = "b02e4319ac84e40bc841648da8c0b70009dd460dbbd8460a4e3a1d8ace18e5ca"
...
SIGNAL_INPUT = Path(
    ...
    "c77_positive_day_participation_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C77 positive-day-participation input packet")
    expected_contract = {
        "formation_close_count": 64,
        "return_count": 63,
        "return_type": "natural log return",
        "positive_day_count": (
            "count of natural-log returns strictly greater than zero; zero is not positive"
        ),
        "target_rule": (
            "QQQ positive-day count strictly above SPY means QQQ; equality or lower means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c77-positive-day-participation-input-v1"
        ...
        raise InputBlockedError("C77 positive-day-participation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C77 positive-day-participation signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_positive_day_count",
        "return_count",
        "signal_identity_sha256",
        "signal_session",
        "spy_positive_day_count",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C77 positive-day-participation signal schema mismatch")
        count_fields = ("spy_positive_day_count", "qqq_positive_day_count")
        if any(
            type(row[field]) is not int or not 0 <= row[field] <= 63
            for field in count_fields
        ):
            raise InputBlockedError("C77 positive-day count is invalid")
        if type(row["return_count"]) is not int or row["return_count"] != 63:
            raise InputBlockedError("C77 formation return count mismatch")
        expected_state = (
            "QQQ"
            if row["qqq_positive_day_count"] > row["spy_positive_day_count"]
            else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C77 positive-day-participation target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C77 positive-day-participation chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C77 positive-day-participation signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C77 positive-day-participation stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C77 positive-day-participation state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C77 positive-day-participation signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C77 positive-day-participation state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c77-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C77 positive-day-participation input packet")
    receipt = _json(
        receipt_payload,
        "C77 positive-day-participation materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c77-positive-day-participation-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C77 positive-day-participation signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c77-positive-day-participation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c77-positive-day-participation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C77 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C77 holdout one-use state is not pristine")
