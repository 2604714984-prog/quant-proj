"""One-use runner for frozen C108 relative-breakout-persistence rotation."""
...
from research.adapters.c108_relative_breakout_persistence import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c108_relative_breakout_persistence_v1.json"
ADAPTER = ROOT / "research/adapters/c108_relative_breakout_persistence.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "417051045a2386548df9add4426fd99aca1adb93431d567535256b4750d84d90"
ADAPTER_SHA256 = "acff3defc6391bba4cfd2ad452c5e01a9d4dc615988314681f910092e08dbac7"
SIGNAL_SHA256 = "68306307ab9e75dfdf2e5daa3434804691a91d703cf7dd228d56f19d70a8adc0"
RECEIPT_SHA256 = "b7b151f3de5a51ea2624e4b2cca8a8157b3fe1318a1a507014cc31c155f217f2"
...
SIGNAL_INPUT = Path(
    ...
    "c108_relative_breakout_persistence_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C108 relative-breakout-persistence input packet")
    expected_contract = {
        "formation_sessions": 82,
        "breakout_lookback_sessions": 20,
        "breakout_observations": 63,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "count of final 63 sessions whose adjusted close exactly "
            "equals the maximum of its trailing 20-session window"
        ),
        "target_rule": (
            "QQQ breakout count strictly greater than SPY selects QQQ; "
            "equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c108-relative-breakout-persistence-input-v1"
        ...
        raise InputBlockedError("C108 relative-breakout-persistence packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C108 relative-breakout-persistence signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "breakout_observation_count",
        "qqq_breakout_count",
        "relative_breakout_count_difference",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C108 relative-breakout-persistence signal schema mismatch")
        count_fields = (
            "breakout_lookback_sessions",
            "breakout_observation_count",
            "qqq_breakout_count",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C108 relative-breakout-persistence value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 82
            or row["breakout_lookback_sessions"] != 20
            or row["breakout_observation_count"] != 63
            or not 0 <= row["spy_breakout_count"] <= 63
            or not 0 <= row["qqq_breakout_count"] <= 63
        ):
            raise InputBlockedError("C108 formation session count mismatch")
        difference = row["qqq_breakout_count"] - row["spy_breakout_count"]
        if not math.isclose(
            float(row["relative_breakout_count_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C108 relative-breakout-persistence arithmetic mismatch")
        expected_state = "QQQ" if difference > 0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C108 relative-breakout-persistence target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C108 relative-breakout-persistence chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C108 relative-breakout-persistence signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C108 relative-breakout-persistence stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C108 relative-breakout-persistence state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C108 relative-breakout-persistence signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C108 relative-breakout-persistence state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C108 relative-breakout-persistence input packet")
    receipt = _json(
        receipt_payload,
        "C108 relative-breakout-persistence materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c108-relative-breakout-persistence-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C108 relative-breakout-persistence signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c108-relative-breakout-persistence-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c108-relative-breakout-persistence-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C108 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C108 holdout one-use state is not pristine")
