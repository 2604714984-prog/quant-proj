"""One-use runner for the frozen Cycle 33 BAA spread SPY/QQQ rotation lane."""
...
from research.adapters.c33_baa_spread_change_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c33_baa_spread_change_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c33_baa_spread_change_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "3ac258630573609aa9bea0214363926490da66794655dba6656033a828ad83d1"
ADAPTER_SHA256 = "49880c76e4e607408c022a61ecb289fcfe30d696c9fb6ee48888e48363901855"
SIGNAL_SHA256 = "fa42bea9f2421a725c416c8588842341b9439c9796527f2021bb6b2312e2a242"
RECEIPT_SHA256 = "c5ae2d1cf34112b1fae242f9046ad17fae37d53a29ea2033651c00c01b000b87"
...
SIGNAL_INPUT = Path(
    ...
    "c33_baa_treasury_spread_change_rotation_spy_qqq_v1/"
    "c33_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "BAA spread-change rotation input packet")
    if (
        record.get("schema_version")
        != "c33-baa10ym-spread-change-spy-qqq-input-packet-v1"
        ...
        != {
            "series_id": "BAA10YM",
            "target_rule": "latest PIT-qualified monthly spread > exact prior-calendar-month spread means SPY; equal or lower means QQQ",
            "threshold_percentage_points": 0.0,
        }
    ):
        raise InputBlockedError("BAA spread-change rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("BAA signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("BAA signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("BAA selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "BAA10YM"
                or selection.get("source_class") != "OFFICIAL_ALFRED_BAA10YM"
                or selection.get("raw_response_sha256")
                != "d0e4fe6cd2a9cefdb64416ec67f093e6459640acf2af030ba9066d6a5206e39d"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("BAA source row identity mismatch")
        current_value = current.get("value_percent")
        previous_value = previous.get("value_percent")
        change = row.get("delta_percentage_points_vs_prior_calendar_month")
        if any(
            ...
            raise InputBlockedError("BAA spread values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("BAA spread change arithmetic mismatch")
        ...
        previous_reference_month = _date(
            row.get("previous_reference_month"), "previous BAA reference month"
        )
        current_reference_month = _date(
            row.get("current_reference_month"), "current BAA reference month"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous BAA available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current BAA available_at",
            )
            > decision_at
            or _date(previous.get("observation_month"), "previous observation month")
            != previous_reference_month
            or _date(current.get("observation_month"), "current observation month")
            ...
            raise InputBlockedError(
                "BAA reference-month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
            ...
            "delta_percentage_points_vs_prior_calendar_month": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("BAA signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("BAA source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "BAA row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("BAA signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("BAA state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("BAA signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("BAA state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c33-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "BAA spread-change rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "BAA spread-change rotation/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c33-baa10ym-spread-change-full-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "BAA signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c33-baa10ym-spread-change-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c33-baa10ym-spread-change-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 33 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 33 holdout one-use state is not pristine")
