"""One-use runner for the frozen C60 technology/energy SPY/QQQ lane."""
...
from research.adapters.c60_tech_energy_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c60_tech_energy_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c60_tech_energy_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6c4d3ae2bb3af34a8993c8a782803d6d69224e41d48b70150ba08f7b30386970"
ADAPTER_SHA256 = "997fbbef3a8c7747ebd69e694f7f3d4e9db41608386244b8ee2253a48882df68"
SIGNAL_SHA256 = "f05503cb327fb33fe6627c9d8dab944800815969041d5ca3f3e5c859092f949c"
RECEIPT_SHA256 = "b839db92f372cac6aacff1af5ca9714ef2da996c877c325c5d9f8d1c314753d1"
...
SIGNAL_INPUT = Path(
    ...
    "c60_tech_energy_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "technology energy leadership rotation input packet")
    expected_contract = {
        "left_symbol": "XLK",
        "right_symbol": "XLE",
        "return_window": (
        ...
        "target_rule": (
            "strict XLK prior-month total-return outperformance versus XLE "
    ...
    if (
        ...
        != "c60-tech-energy-leadership-input-packet-v1"
        ...
        raise InputBlockedError("technology energy packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("technology energy signal row count mismatch")
    ...
    expected_source = {
        "XLK": (
            ...
            "b958a3bb7ce9d83c7a2a8fee927b723f8818836eca4da025172fcb7cdc38aaa4",
        ),
        "XLE": (
            "https://query1.finance.yahoo.com/v8/finance/chart/XLE?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "265e4300507290ee989243d220ab86eacf5f6f729ff4087112e084c06a1665b5",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("technology energy signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("technology energy selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "technology energy adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("technology energy source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "technology energy cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("technology energy arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "technology energy month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("technology energy signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("technology energy signal identity hash mismatch")
    ...
    if (
        ...
        raise InputBlockedError("technology energy signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("technology energy state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "technology energy signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("technology energy state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "technology energy leadership rotation input packet")
    receipt = _json(
        receipt_payload,
        "technology energy materialization receipt",
    )
    ...
    if (
        ...
        != "c60-tech-energy-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "XLK"
        or receipt.get("right_symbol") != "XLE"
        or receipt.get("raw_fetch_receipt_sha256")
        != "a294bda926cdfc6e87942fb4d456befec2c410954772a48597040e2ecd3b76ad"
        ...
        != "25b19157140e613b4c0c79126d60879f5aec97eebc6a8a568845d15ba89633a5"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "technology energy signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c60-tech-energy-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c60-tech-energy-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
