"""One-use runner for the frozen C156 USEPUINDXD SPY/QQQ rotation lane."""
...
from research.adapters.c156_us_epu_trailing_90d_median import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c156_us_epu_trailing_90d_median_v1.json"
ADAPTER = ROOT / "research/adapters/c156_us_epu_trailing_90d_median.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "4f507ff81bb0f03dfe5d926c094d9cb00fc9f4caf9515750206d9af2245e59ed"
ADAPTER_SHA256 = "c47e551ad2b26ca44bc78f42d21512d0bced0133c64a954918ba51224ad54b60"
SIGNAL_SHA256 = "f4e724792d09ce30ac70f90de18e8a71b919e57c314628bf73b18571d063a2d7"
RECEIPT_SHA256 = "936b4ba8cdeee82418a51b217a530db3db50364b4eaeeac6b3774c715275a72c"
...
SIGNAL_INPUT = Path(
    ...
    "c156_us_epu_trailing_90d_median_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "USEPUINDXD impulse rotation input packet")
    if (
        record.get("schema_version")
        != "c156-usepu-spy-qqq-full-input-v1"
        ...
        != {
            ...
            "target_rule": "latest EPU <= median of prior observations in trailing 90 calendar days means QQQ; otherwise SPY",
            "lookback_calendar_days": 90,
            "minimum_trailing_observations": 60,
        }
    ):
        raise InputBlockedError("USEPUINDXD rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("USEPUINDXD signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("USEPUINDXD signal stage mismatch")
        required = {
            "actual_vintage_date",
            "current_epu_index",
            ...
            "epu_minus_trailing_median",
            "signal_identity_sha256",
            "source_response_sha256",
            "stage",
            "target_symbol",
            "trailing_observation_count",
            "trailing_window_median_epu_index",
            "trailing_window_end",
            "trailing_window_start",
        }
        if set(row) != required:
            raise InputBlockedError("USEPUINDXD signal fields mismatch")
        numeric = tuple(
            row.get(key)
            for key in (
                "current_epu_index",
        ...
        if any(
            ...
            raise InputBlockedError("USEPUINDXD statistics must be finite")
        current_epu, trailing_median, distance = map(float, numeric)
        if not all(0.0 <= value < 100_000.0 for value in (current_epu, trailing_median)):
            raise InputBlockedError("USEPUINDXD values are implausible")
        ...
        current_date = _date(row["current_observation_date"], "current observation date")
        trailing_start = _date(row["trailing_window_start"], "trailing window start")
        trailing_end = _date(row["trailing_window_end"], "trailing window end")
        vintage = _date(row["actual_vintage_date"], "USEPUINDXD actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "USEPUINDXD response SHA-256"
        )
        if (
            vintage > decision_at.date()
            or current_date >= start
            or not current_date - timedelta(days=90) <= trailing_start
            or not trailing_start < trailing_end < current_date
            or type(row.get("trailing_observation_count")) is not int
            or row["trailing_observation_count"] < 60
            or not math.isclose(
                distance,
                current_epu - trailing_median,
                rel_tol=0.0,
                abs_tol=1e-12,
            )
            ...
            raise InputBlockedError("USEPUINDXD chronology or state mismatch")
        identity = {
            "stage": row["stage"],
            ...
            "current_epu_index": current_epu,
            "current_observation_date": current_date.isoformat(),
            "epu_minus_trailing_median": distance,
            "source_response_sha256": source_response_sha,
            "target_symbol": expected_state,
            "trailing_observation_count": row["trailing_observation_count"],
            "trailing_window_median_epu_index": trailing_median,
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("USEPUINDXD signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("USEPUINDXD source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "USEPUINDXD row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("USEPUINDXD signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("USEPUINDXD state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("USEPUINDXD signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("USEPUINDXD state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c156-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "New York Fed USEPUINDXD/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "New York Fed USEPUINDXD/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c156-usepu-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "USEPUINDXD signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c156-nyfed-usepu-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c156-nyfed-usepu-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
