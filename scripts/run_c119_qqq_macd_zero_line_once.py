"""One-use runner for frozen C119 qqq-macd-zero-line rotation."""
...
from research.adapters.c119_qqq_macd_zero_line import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c119_qqq_macd_zero_line_v1.json"
ADAPTER = ROOT / "research/adapters/c119_qqq_macd_zero_line.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "42f84662a812aa63ca0331897d8611e425e45fed79fe720d8228e9518a585f40"
ADAPTER_SHA256 = "19163f18536d9d17f1e557bb3d60e10f0b0902f4304bec8160d69d5b77150c12"
SIGNAL_SHA256 = "fb95d7ad2f6b21449973a86dbddb8b6557f2be38aa46d612761bc71c89ec2ac0"
RECEIPT_SHA256 = "3954b78f17a0770bc980f1c2e5ff0d6ae17ec74b508fed1c2104551b47fbf9b0"
...
SIGNAL_INPUT = Path(
    ...
    "c119_qqq_macd_zero_line_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C119 qqq-macd-zero-line input packet")
    expected_contract = {
        "formation_sessions": 126,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "EMA12 minus EMA26 after initializing both at the first of exactly "
            "126 QQQ adjusted closes and applying alphas 2/13 and 2/27 "
            "sequentially"
        ),
        "target_rule": (
            "positive QQQ MACD selects QQQ; zero or negative selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c119-qqq-macd-zero-line-input-v1"
        ...
        raise InputBlockedError("C119 qqq-macd-zero-line packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C119 qqq-macd-zero-line signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C119 qqq-macd-zero-line signal schema mismatch")
        value_fields = (
            "qqq_ema12",
            "qqq_ema26",
            "qqq_macd",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C119 qqq-macd-zero-line value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 126
            or row["qqq_ema12"] <= 0.0
            or row["qqq_ema26"] <= 0.0
        ):
            raise InputBlockedError("C119 formation session count mismatch")
        expected_macd = row["qqq_ema12"] - row["qqq_ema26"]
        if not math.isclose(
            row["qqq_macd"],
            expected_macd,
            rel_tol=1e-12,
            ...
            raise InputBlockedError("C119 qqq-macd-zero-line arithmetic mismatch")
        expected_state = "QQQ" if row["qqq_macd"] > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C119 qqq-macd-zero-line target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C119 qqq-macd-zero-line chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C119 qqq-macd-zero-line signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C119 qqq-macd-zero-line stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C119 qqq-macd-zero-line state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C119 qqq-macd-zero-line signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C119 qqq-macd-zero-line state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C119 qqq-macd-zero-line input packet")
    receipt = _json(
        receipt_payload,
        "C119 qqq-macd-zero-line materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c119-qqq-macd-zero-line-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C119 qqq-macd-zero-line signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c119-qqq-macd-zero-line-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c119-qqq-macd-zero-line-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
