"""One-use runner for frozen C102 relative illiquidity-improvement rotation."""
...
from research.adapters.c102_relative_illiquidity_improvement import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c102_relative_illiquidity_improvement_v1.json"
ADAPTER = ROOT / "research/adapters/c102_relative_illiquidity_improvement.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "ad765637b6d42dd45e928077e7fdc013c63fddda4d1985067dfc4cee65b377fd"
ADAPTER_SHA256 = "33fcf0c816ed3b3f2be97aaf9e6b9226e674650e3ef0ddc813a707c5d511dbbf"
SIGNAL_SHA256 = "86f6e1d625a58152fc85efcbb44acbb79cbc71063acf4dcd2f037f627d68b6ba"
RECEIPT_SHA256 = "fb434b87607cf3bab799a0e1eeb54fc45f1f1ba1909b8b2a0a5424930fffab38"
...
SIGNAL_INPUT = Path(
    ...
    "c102_relative_illiquidity_improvement_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C102 relative-illiquidity-improvement input packet")
    expected_contract = {
        "source_sessions": 85,
        "formation_sessions": 84,
        "baseline_sessions": 63,
        "recent_sessions": 21,
        "missing_rule": "fail closed; no alternate source or window",
        "daily_illiquidity_definition": (
            "absolute simple adjusted-close return divided by "
            "raw close times raw volume"
        ),
        "score_definition": (
            "arithmetic mean daily illiquidity over latest 21 observations "
            "divided by arithmetic mean over immediately preceding "
            "63 observations"
        ),
        "target_rule": (
            "QQQ illiquidity-improvement score strictly below SPY "
            "selects QQQ; equality or higher selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c102-relative-illiquidity-improvement-input-v1"
        ...
        or record.get("volume_values_used_for_signal") is not True
        ...
        raise InputBlockedError("C102 illiquidity-improvement packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C102 illiquidity-improvement signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_baseline_mean_illiquidity",
        "qqq_recent_mean_illiquidity",
        "qqq_illiquidity_improvement_score",
        "recent_session_count",
        "relative_illiquidity_improvement_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_baseline_mean_illiquidity",
        "spy_recent_mean_illiquidity",
        "spy_illiquidity_improvement_score",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C102 illiquidity-improvement signal schema mismatch")
        numeric_fields = (
            "spy_baseline_mean_illiquidity",
            "spy_recent_mean_illiquidity",
            "spy_illiquidity_improvement_score",
        ...
        if any(
            ...
            raise InputBlockedError("C102 illiquidity-improvement value is invalid")
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 84
            or type(row["source_session_count"]) is not int
            or row["source_session_count"] != 85
            or type(row["baseline_session_count"]) is not int
            or row["baseline_session_count"] != 63
            or type(row["recent_session_count"]) is not int
            or row["recent_session_count"] != 21
        ):
            raise InputBlockedError("C102 formation session count mismatch")
        for symbol in ("spy", "qqq"):
            baseline = float(row[f"{symbol}_baseline_mean_illiquidity"])
            recent = float(row[f"{symbol}_recent_mean_illiquidity"])
            score = float(row[f"{symbol}_illiquidity_improvement_score"])
            if baseline <= 0.0 or recent < 0.0 or score < 0.0 or not math.isclose(
                score,
                recent / baseline,
                rel_tol=1e-12,
                abs_tol=1e-15,
                ...
                raise InputBlockedError(
                    "C102 illiquidity-improvement score arithmetic mismatch"
                )
        difference = float(row["qqq_illiquidity_improvement_score"]) - float(
            row["spy_illiquidity_improvement_score"]
        )
        if not math.isclose(
            float(row["relative_illiquidity_improvement_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C102 illiquidity-improvement arithmetic mismatch")
        expected_state = "QQQ" if difference < 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C102 illiquidity-improvement target mismatch")

        formation_start = _date(row["formation_start"], "formation start")
        formation_end = _date(row["formation_end"], "formation end")
        ...
        if (
            not formation_start < formation_end
            or formation_end != signal_session
            ...
            raise InputBlockedError("C102 illiquidity-improvement chronology mismatch")

        _sha(
            row["source_row_hash_set_sha256"],
            "formation row-set SHA-256",
        )
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C102 illiquidity-improvement signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C102 illiquidity-improvement stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C102 illiquidity-improvement state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C102 illiquidity-improvement signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C102 illiquidity-improvement state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C102 illiquidity-improvement input packet")
    receipt = _json(
        receipt_payload,
        "C102 illiquidity-improvement materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c102-relative-illiquidity-improvement-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C102 illiquidity-improvement signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c102-relative-illiquidity-improvement-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c102-relative-illiquidity-improvement-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C102 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C102 holdout one-use state is not pristine")
