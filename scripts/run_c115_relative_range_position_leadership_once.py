"""One-use runner for frozen C115 relative-range-position-leadership rotation."""
...
from research.adapters.c115_relative_range_position_leadership import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c115_relative_range_position_leadership_v1.json"
ADAPTER = ROOT / "research/adapters/c115_relative_range_position_leadership.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "a5d84e4dbf55221d789756c18ddc2f2e0753bcd6b3cbf132436779db5e1c786b"
ADAPTER_SHA256 = "2aa644eecb7bbb155afede7545ca3d8933f245f8856444a209462fd20ead6cbf"
SIGNAL_SHA256 = "8800c50015417ef5c97bb5535603015d683aee53090acd155824cd05b6263f1b"
RECEIPT_SHA256 = "f49dec52ce3a3242e6ecc8a990513f1edf0d6567603b0b39ff59752e596bb9ca"
...
SIGNAL_INPUT = Path(
    ...
    "c115_relative_range_position_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C115 relative-range-position-leadership input packet")
    expected_contract = {
        "formation_sessions": 127,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "(final adjusted close - minimum adjusted close) divided by "
            "(maximum adjusted close - minimum adjusted close) over exactly "
            "127 sessions"
        ),
        "target_rule": (
            "QQQ range position strictly greater than SPY selects QQQ; "
            "equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c115-relative-range-position-leadership-input-v1"
        ...
        raise InputBlockedError("C115 relative-range-position-leadership packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C115 relative-range-position-leadership signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_range_position",
        "relative_range_position_difference",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C115 relative-range-position-leadership signal schema mismatch")
        value_fields = (
            "qqq_range_position",
            "relative_range_position_difference",
            "spy_range_position",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C115 relative-range-position-leadership value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 127
            or not 0.0 <= row["spy_range_position"] <= 1.0
            or not 0.0 <= row["qqq_range_position"] <= 1.0
        ):
            raise InputBlockedError("C115 formation session count mismatch")
        difference = row["qqq_range_position"] - row["spy_range_position"]
        if not math.isclose(
            float(row["relative_range_position_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C115 relative-range-position-leadership arithmetic mismatch")
        expected_state = "QQQ" if difference > 0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C115 relative-range-position-leadership target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C115 relative-range-position-leadership chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C115 relative-range-position-leadership signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C115 relative-range-position-leadership stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C115 relative-range-position-leadership state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C115 relative-range-position-leadership signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C115 relative-range-position-leadership state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C115 relative-range-position-leadership input packet")
    receipt = _json(
        receipt_payload,
        "C115 relative-range-position-leadership materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c115-relative-range-position-leadership-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C115 relative-range-position-leadership signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c115-relative-range-position-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c115-relative-range-position-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C115 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C115 holdout one-use state is not pristine")
