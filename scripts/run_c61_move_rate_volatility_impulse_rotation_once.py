"""One-use runner for the frozen C61 MOVE impulse SPY/QQQ rotation lane."""
...
from research.adapters.c61_move_rate_volatility_impulse_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c61_move_rate_volatility_impulse_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c61_move_rate_volatility_impulse_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "ba6e8d9fcef129fdb64c1207b0d042b9100f3aa8b997e76123b7b8536825cedb"
ADAPTER_SHA256 = "c8bfd57165d633a511a17e7439a081c5e241307c93b8d36c932448887ddb2967"
SIGNAL_SHA256 = "ab8347fd264959d6350a56faa41292bbabf9d846a0f764d02c535749a40d1e12"
RECEIPT_SHA256 = "9f87b6d6f6045475fad0a9e98186543fad9fd9da7bd8d8aaafd3f3339ab4b075"
...
SIGNAL_INPUT = Path(
    ...
    "c61_move_rate_volatility_impulse_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "MOVE rate-volatility impulse input packet")
    expected_contract = {
        "series_id": "ICE_BofA_MOVE",
        "return_window": (
            "last finite positive MOVE close in M-2 versus M-1 for "
            "execution month M"
        ),
        "target_rule": "strict MOVE decline selects QQQ; equality or increase selects SPY",
        "threshold_index_points": 0.0,
    }
    if (
        record.get("schema_version")
        != "c61-move-impulse-input-packet-v1"
        ...
        raise InputBlockedError("MOVE packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("MOVE signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = "0e6ea915738b90c09cfc0aef7b4653e86df492c113fe2c1389a6c5a840227e5f"
    expected_url = (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("MOVE signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("MOVE selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "ICE_BofA_MOVE"
                ...
                raise InputBlockedError("MOVE source row identity mismatch")
        ...
        if (
            ...
            raise InputBlockedError("MOVE change arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError("MOVE observation or availability mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("MOVE signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("MOVE signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "MOVE row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("MOVE signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("MOVE state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("MOVE signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("MOVE state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c61-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "MOVE impulse/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "MOVE impulse/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c61-move-full-materialization-receipt-v1"
        ...
        != "8dfef8a2ff505e337f978e98c1d73b332710c327ae759f63125cf651bd7cf2dd"
        or receipt.get("raw_response_sha256")
        != "0e6ea915738b90c09cfc0aef7b4653e86df492c113fe2c1389a6c5a840227e5f"
        ...
        != "f17ed27d4cf3f3e19f9ef47962ff8dfc6e00e03e1cee589cb72f14c08e149e11"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "MOVE signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c61-move-rate-volatility-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c61-move-rate-volatility-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
