"""One-use runner for the frozen Cycle 17 industrial-production SPY/QQQ rotation lane."""
...
from research.adapters.c17_industrial_production_direction import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c17_industrial_production_direction_v1.json"
ADAPTER = ROOT / "research/adapters/c17_industrial_production_direction.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "93fc2ec80d5f6dfc49e05ed1b7745bcd562cd07b22764c348a3ba27b5e2f3028"
ADAPTER_SHA256 = "e50303ad0fcc75d99e04580ba4972e79748f2295a5e9402d5f87f246a3a07cc1"
SIGNAL_SHA256 = "b3e2b1919f12538628509e19595478470ab7e6f217eae9c07a7347732cc2bdc6"
RECEIPT_SHA256 = "4c106a5dde0ebb069865636d2280fd7483081ed3084c7f83b346e8834faebb29"
...
SIGNAL_INPUT = Path(
    ...
    "c17_industrial_production_direction_spy_qqq_rotation_v1/"
    "indpro_qqq_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "industrial-production direction input packet")
    if (
        record.get("schema_version")
        != "c17-industrial-production-direction-spy-qqq-input-packet-v1"
        ...
        raise InputBlockedError("industrial-production input packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("industrial-production signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("industrial-production signal stage mismatch")
        identity = row.get("source_identity")
        if type(identity) is not dict:
            raise InputBlockedError("industrial-production source identity is required")
        current = identity.get("reference_industrial_production_index")
        prior = identity.get("prior_industrial_production_index")
        ...
        if any(
            ...
            raise InputBlockedError("industrial-production values must be finite")
        if float(current) <= 0.0 or float(prior) <= 0.0:
            raise InputBlockedError("industrial-production levels must be positive")
        expected_change = float(current) / float(prior) - 1.0
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-15):
            raise InputBlockedError("industrial-production change arithmetic mismatch")
        expected_state = "SPY" if float(current) > float(prior) else "QQQ"
        if row.get("state") != expected_state:
            raise InputBlockedError("industrial-production state arithmetic mismatch")
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                expected_state,
                _sha(row["row_sha256"], "industrial-production row SHA-256"),
            )
        )
        if identity_hash != row["row_sha256"]:
            raise InputBlockedError("industrial-production source identity hash mismatch")
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("industrial-production signal split count mismatch")
    for group in output.values():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError(
                "industrial-production signal intervals are not contiguous"
            )
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c17-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "industrial-production/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "industrial-production/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "industrial-production signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c17-industrial-production-direction-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c17-industrial-production-direction-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 17 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 17 holdout one-use state is not pristine")
