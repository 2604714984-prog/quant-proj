"""One-use runner for the frozen C62 homebuilder/utilities SPY/QQQ lane."""
...
from research.adapters.c62_homebuilder_utilities_financial_conditions_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c62_homebuilder_utilities_financial_conditions_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c62_homebuilder_utilities_financial_conditions_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "db597efa0d3d5ed48d86ff6ff8280ec719a64184dbe87d94950d1e91cd566094"
ADAPTER_SHA256 = "6a20734d7b7efd8cbb22d3b116106d6d6556ec06a43da2889897484def837ab6"
SIGNAL_SHA256 = "5fcf3071cf33486ea4e1f959b5a8fa1ce2ae7275ca06bcb562f832b33e1d6d13"
RECEIPT_SHA256 = "2acc4bf51e11f8fae16b3d0c72f0aae1457462e601f10a5b63021fa2e3ed489d"
...
SIGNAL_INPUT = Path(
    ...
    "c62_homebuilder_utilities_financial_conditions_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "homebuilder utilities leadership rotation input packet")
    expected_contract = {
        "left_symbol": "XHB",
        "right_symbol": "XLU",
        "return_window": (
        ...
        "target_rule": (
            "strict XHB prior-month total-return outperformance versus XLU "
    ...
    if (
        ...
        != "c62-homebuilder-utilities-leadership-input-packet-v1"
        ...
        raise InputBlockedError("homebuilder utilities packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("homebuilder utilities signal row count mismatch")
    ...
    expected_source = {
        "XHB": (
            "https://query1.finance.yahoo.com/v8/finance/chart/XHB?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "b8dc15ac463e004ce65d7e866071fe9f91aef401aade0e120d59a4bb996f11d5",
        ),
        "XLU": (
            "https://query1.finance.yahoo.com/v8/finance/chart/XLU?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "b70f36577cf6618d236558e982c724e60dbeee1ec1671a0e06f5b1a1e7c4baa0",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("homebuilder utilities signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("homebuilder utilities selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "homebuilder utilities adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError(
                    "homebuilder utilities source row identity mismatch"
                )
        ...
        try:
            ...
            raise InputBlockedError(
                "homebuilder utilities cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("homebuilder utilities arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "homebuilder utilities month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("homebuilder utilities signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError(
                "homebuilder utilities signal identity hash mismatch"
            )
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "homebuilder utilities row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("homebuilder utilities signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("homebuilder utilities state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "homebuilder utilities signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("homebuilder utilities state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(
        input_payload, "homebuilder utilities leadership rotation input packet"
    )
    receipt = _json(
        receipt_payload,
        "homebuilder utilities materialization receipt",
    )
    ...
    if (
        ...
        != "c62-homebuilder-utilities-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "XHB"
        or receipt.get("right_symbol") != "XLU"
        or receipt.get("raw_fetch_receipt_sha256")
        != "2e5bb66219679d8139166ab2b0e37617773cee4fef184fcfaaca7dcbcfd0785e"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "homebuilder utilities signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c62-homebuilder-utilities-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c62-homebuilder-utilities-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
