"""One-use runner for frozen C94 relative return serial dependence."""
...
from research.adapters.c94_relative_return_serial_dependence import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c94_relative_return_serial_dependence_v1.json"
ADAPTER = ROOT / "research/adapters/c94_relative_return_serial_dependence.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "564ccde44dbab27e19076b547e13ae47e65054d487b9821f5bc0c48f7b51fe8a"
ADAPTER_SHA256 = "05240a1924ba846045f86c9bf40c2402591a19ac06fb59cb3d6228fe4d2b503c"
SIGNAL_SHA256 = "0ad9f96278b6524df2f8d921d77034687a0b3f0e131c779b75cf3e3450dd1881"
RECEIPT_SHA256 = "757754f4a3cc1b9328e7d3aac90545aca4bc32a6f8af213da29a7ff09f2fb04c"
...
SIGNAL_INPUT = Path(
    ...
    "c94_relative_return_serial_dependence_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C94 serial-dependence input packet")
    expected_contract = {
        ...
        "serial_dependence_definition": (
            "ordinary Pearson correlation between returns 1..62 and "
            "returns 2..63 using population covariance and population "
            "standard deviations over the exact 62 pairs"
        ),
        "target_rule": (
            "QQQ lag-one correlation strictly greater than SPY selects QQQ; "
            "equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c94-relative-return-serial-dependence-input-v1"
        ...
        raise InputBlockedError("C94 serial-dependence packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C94 serial-dependence signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "lag_pair_count",
        "qqq_lag_one_return_correlation",
        "relative_serial_dependence_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_lag_one_return_correlation",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C94 serial-dependence signal schema mismatch")
        numeric_fields = (
            "spy_lag_one_return_correlation",
            "qqq_lag_one_return_correlation",
            "relative_serial_dependence_difference",
        ...
        if any(
            ...
            raise InputBlockedError("C94 serial-dependence value is invalid")
        if (
            not -1.0 <= float(row["spy_lag_one_return_correlation"]) <= 1.0
            or not -1.0 <= float(row["qqq_lag_one_return_correlation"]) <= 1.0
        ):
            raise InputBlockedError("C94 correlation is outside [-1, 1]")
        if (
            type(row["formation_return_count"]) is not int
            or row["formation_return_count"] != 63
            or type(row["lag_pair_count"]) is not int
            or row["lag_pair_count"] != 62
        ):
            raise InputBlockedError("C94 formation return count mismatch")
        difference = float(row["qqq_lag_one_return_correlation"]) - float(
            row["spy_lag_one_return_correlation"]
        )
        if not math.isclose(
            float(row["relative_serial_dependence_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C94 serial-dependence arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C94 serial-dependence target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C94 serial-dependence chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C94 serial-dependence signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C94 serial-dependence stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C94 serial-dependence state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C94 serial-dependence signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C94 serial-dependence state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C94 serial-dependence input packet")
    receipt = _json(
        receipt_payload,
        "C94 serial-dependence materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c94-relative-return-serial-dependence-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C94 serial-dependence signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c94-relative-return-serial-dependence-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c94-relative-return-serial-dependence-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C94 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C94 holdout one-use state is not pristine")
