"""One-use runner for frozen C90 multi-horizon relative consensus."""
...
from research.adapters.c90_multi_horizon_relative_leadership_consensus import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c90_multi_horizon_relative_leadership_consensus_v1.json"
ADAPTER = ROOT / "research/adapters/c90_multi_horizon_relative_leadership_consensus.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "88316f734600718eb8bb2cc7b7549e824ab59dd07f4afe40aece0737a26b1b65"
ADAPTER_SHA256 = "00303d317851ccccd8ecadeadd330844a2b168bc187c7cff02635d9e45891b4b"
SIGNAL_SHA256 = "20dc1eb902a8fa7d714856367da9c1e6ca16274568b8d7eabbc1a131214c6c20"
RECEIPT_SHA256 = "f0f34fb5ddd99583e3487875103129e52095e273a26b1e70d11890ca308b060e"
...
SIGNAL_INPUT = Path(
    ...
    "c90_multi_horizon_relative_leadership_consensus_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C90 multi-horizon-consensus input packet")
    expected_contract = {
        "formation_close_count": 127,
        "horizon_return_counts": [21, 63, 126],
        "return_type": "natural log return",
        "relative_return": "QQQ log-return sum minus SPY log-return sum",
        "vote_rule": (
            "strictly positive relative return casts QQQ; zero or negative "
            "casts SPY"
        ),
        "target_rule": (
            "at least two of three QQQ votes means QQQ; otherwise SPY"
        ),
        "vote_tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c90-multi-horizon-relative-leadership-consensus-input-v1"
        ...
        raise InputBlockedError("C90 multi-horizon-consensus packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C90 multi-horizon-consensus signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "holding_end_execution_session",
        "holding_start_execution_session",
        "formation_end",
        "formation_row_hash_set_sha256",
        "formation_start",
        "signal_identity_sha256",
        "signal_session",
        "stage",
        "qqq_log_return_sum_21",
        "qqq_log_return_sum_63",
        "qqq_log_return_sum_126",
        "qqq_positive_horizon_votes",
        "relative_log_return_sum_21",
        "relative_log_return_sum_63",
        "relative_log_return_sum_126",
        "return_count",
        "spy_log_return_sum_21",
        "spy_log_return_sum_63",
        "spy_log_return_sum_126",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C90 multi-horizon-consensus signal schema mismatch")
        numeric_fields = (
            "spy_log_return_sum_21",
            "qqq_log_return_sum_21",
            "relative_log_return_sum_21",
        ...
        if any(
            ...
            raise InputBlockedError("C90 multi-horizon-consensus value is invalid")
        if (
            type(row["qqq_positive_horizon_votes"]) is not int
            or row["qqq_positive_horizon_votes"] not in (0, 1, 2, 3)
        ):
            raise InputBlockedError("C90 vote count mismatch")
        if type(row["return_count"]) is not int or row["return_count"] != 126:
            raise InputBlockedError("C90 formation return count mismatch")

        votes = 0
        for horizon in (21, 63, 126):
            difference = (
                float(row[f"qqq_log_return_sum_{horizon}"])
                - float(row[f"spy_log_return_sum_{horizon}"])
            )
            if not math.isclose(
                float(row[f"relative_log_return_sum_{horizon}"]),
                difference,
                rel_tol=1e-12,
                abs_tol=1e-15,
            ):
                raise InputBlockedError("C90 horizon arithmetic mismatch")
            if difference > 0.0:
                votes += 1
        if row["qqq_positive_horizon_votes"] != votes:
            raise InputBlockedError("C90 vote arithmetic mismatch")
        expected_state = "QQQ" if votes >= 2 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C90 multi-horizon-consensus target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C90 multi-horizon-consensus chronology mismatch")

        _sha(
            row["formation_row_hash_set_sha256"],
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C90 multi-horizon-consensus signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C90 multi-horizon-consensus stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C90 multi-horizon-consensus state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C90 multi-horizon-consensus signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C90 multi-horizon-consensus state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C90 multi-horizon-consensus input packet")
    receipt = _json(
        receipt_payload,
        "C90 multi-horizon-consensus materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c90-multi-horizon-relative-leadership-consensus-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C90 multi-horizon-consensus signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c90-multi-horizon-relative-leadership-consensus-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c90-multi-horizon-relative-leadership-consensus-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C90 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C90 holdout one-use state is not pristine")
