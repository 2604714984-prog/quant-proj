"""One-use runner for frozen C97 relative close-location accumulation."""
...
from research.adapters.c97_relative_close_location_accumulation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c97_relative_close_location_accumulation_v1.json"
ADAPTER = ROOT / "research/adapters/c97_relative_close_location_accumulation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "2b2830c13668488032157681e876818354878a2657d8286435116e20457e869f"
ADAPTER_SHA256 = "788402431f3bfbe1fbdff7c640878fec823b26a978ecdfef137947087c315282"
SIGNAL_SHA256 = "dbaf7028f7c1abb37181608dadba012e30ef9c3d8f609f81b9e3aca3a0f20a4e"
RECEIPT_SHA256 = "79e3350afbbf440eee798006632ed4faa80d58c78f03022cfeb7d412b7a25b28"
...
SIGNAL_INPUT = Path(
    ...
    "c97_relative_close_location_accumulation_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C97 close-location accumulation input packet")
    expected_contract = {
        "formation_sessions": 21,
        "missing_rule": "fail closed; no alternate source or window",
        "session_multiplier_definition": (
            "((raw_close-raw_low)-(raw_high-raw_close))/"
            "(raw_high-raw_low); exactly 0 when raw_high==raw_low"
        ),
        "score_definition": (
            "sum(session_multiplier*raw_volume)/sum(raw_volume) "
            "over exactly 21 common sessions"
        ),
        "target_rule": (
            "QQQ close-location money flow strictly greater than SPY "
            "selects QQQ; "
    ...
    if (
        ...
        != "c97-relative-close-location-accumulation-input-v1"
        ...
        raise InputBlockedError(
            "C97 close-location accumulation packet boundary mismatch"
        )
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError(
            "C97 close-location accumulation signal row count mismatch"
        )
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_close_location_money_flow",
        "relative_close_location_accumulation_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_close_location_money_flow",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError(
                "C97 close-location accumulation signal schema mismatch"
            )
        numeric_fields = (
            "spy_close_location_money_flow",
            "qqq_close_location_money_flow",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C97 close-location accumulation value is invalid"
            )
        if (
            not -1.0 <= float(row["spy_close_location_money_flow"]) <= 1.0
            or not -1.0 <= float(row["qqq_close_location_money_flow"]) <= 1.0
        ):
            raise InputBlockedError(
                "C97 close-location money flow is outside [-1, 1]"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 21
        ):
            raise InputBlockedError("C97 formation session count mismatch")
        difference = float(row["qqq_close_location_money_flow"]) - float(
            row["spy_close_location_money_flow"]
        )
        if not math.isclose(
            float(row["relative_close_location_accumulation_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError(
                "C97 close-location accumulation arithmetic mismatch"
            )
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError(
                "C97 close-location accumulation target mismatch"
            )
        ...
        if (
            ...
            raise InputBlockedError(
                "C97 close-location accumulation chronology mismatch"
            )
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError(
                "C97 close-location accumulation signal identity mismatch"
            )
    ...
    if (
        ...
        raise InputBlockedError(
            "C97 close-location accumulation stage count mismatch"
        )
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError(
            "C97 close-location accumulation state support is required"
        )
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C97 close-location accumulation signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError(
                "C97 close-location accumulation state support mismatch"
            )
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(
        input_payload,
        "C97 close-location accumulation input packet",
    )
    receipt = _json(
        receipt_payload,
        "C97 close-location accumulation materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c97-relative-close-location-accumulation-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C97 close-location accumulation signal does not bind "
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c97-relative-close-location-accumulation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c97-relative-close-location-accumulation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C97 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C97 holdout one-use state is not pristine")
