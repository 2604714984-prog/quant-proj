"""One-use runner for frozen C131 qqq-current-drawdown rotation."""
...
from research.adapters.c131_qqq_current_drawdown import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c131_qqq_current_drawdown_v1.json"
ADAPTER = ROOT / "research/adapters/c131_qqq_current_drawdown.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "fcd61a7516b3f1a535b88be2521aa424b34d21c9b48a7962fc0274766f83f540"
ADAPTER_SHA256 = "71937ddb1576320a526a287703741749fcee9797c15afdd741ce207086b9f73f"
SIGNAL_SHA256 = "5a7fc5416975b1b61e84c0791289550e4cd3d6bb5827a1ac6a1ab24c6922cea5"
RECEIPT_SHA256 = "a41ca73cf27ca6dd4b29ed4692ee0f804ea32472713a798d9dd65ffd3d222efa"
...
SIGNAL_INPUT = Path(
    ...
    "c131_qqq_64_close_current_drawdown_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C131 qqq-current-drawdown input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Final adjusted close divided by maximum adjusted close minus one "
            "across exactly 64 chronologically accepted QQQ closes"
        ),
        "target_rule": (
            "Current drawdown above -0.05 selects QQQ; -0.05 or worse selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c131-qqq-current-drawdown-input-v1"
        ...
        raise InputBlockedError("C131 qqq-current-drawdown packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C131 qqq-current-drawdown signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C131 qqq-current-drawdown signal schema mismatch")
        value_fields = ("qqq_current_drawdown",)
        if any(
            ...
            raise InputBlockedError(
                "C131 qqq-current-drawdown value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or not -1.0 <= row["qqq_current_drawdown"] <= 0.0
        ):
            raise InputBlockedError("C131 formation or current-drawdown range mismatch")
        expected_state = "QQQ" if row["qqq_current_drawdown"] > -0.05 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C131 qqq-current-drawdown target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C131 qqq-current-drawdown chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C131 qqq-current-drawdown signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C131 qqq-current-drawdown stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C131 qqq-current-drawdown state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C131 qqq-current-drawdown signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C131 qqq-current-drawdown state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C131 qqq-current-drawdown input packet")
    receipt = _json(
        receipt_payload,
        "C131 qqq-current-drawdown materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c131-qqq-current-drawdown-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C131 qqq-current-drawdown signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c131-qqq-current-drawdown-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c131-qqq-current-drawdown-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C131 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C131 holdout one-use state is not pristine")
