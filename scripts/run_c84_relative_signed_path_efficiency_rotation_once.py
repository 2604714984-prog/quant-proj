"""One-use runner for frozen C84 relative signed path efficiency."""
...
from research.adapters.c84_relative_signed_path_efficiency_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c84_relative_signed_path_efficiency_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c84_relative_signed_path_efficiency_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "11eb0a7e810b7e4ca0609224fe28070413190e34c577841907ac329433623afa"
ADAPTER_SHA256 = "7cd6a8d9bbd3a242e37f213df782b61af6e587bad9c336c2ad51deb253dc58ab"
SIGNAL_SHA256 = "caa8dd52a042a9503dcade076825a024ab5a41306a17cfee4f515ccc2d6b749e"
RECEIPT_SHA256 = "29c99d1e90006382f9a7fe495f7b304c09b85997ee1749b70379ceb650346016"
...
SIGNAL_INPUT = Path(
    ...
    "c84_relative_signed_path_efficiency_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C84 signed-path-efficiency input packet")
    expected_contract = {
        "formation_close_count": 127,
        "return_count": 126,
        "return_type": "natural log return",
        "signed_path_efficiency": (
            "sum(log returns) divided by sum(abs(log returns))"
        ),
        "target_rule": (
            "QQQ signed path efficiency strictly above SPY means QQQ; "
            "equality or lower means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c84-relative-signed-path-efficiency-input-v1"
        ...
        raise InputBlockedError("C84 signed-path-efficiency packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C84 signed-path-efficiency signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        ...
        "signal_session",
        "stage",
        "qqq_absolute_log_return_sum",
        "qqq_log_return_sum",
        "qqq_signed_path_efficiency",
        "relative_efficiency_difference",
        "return_count",
        "spy_absolute_log_return_sum",
        "spy_log_return_sum",
        "spy_signed_path_efficiency",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C84 signed-path-efficiency signal schema mismatch")
        numeric_fields = (
            "spy_log_return_sum",
            "spy_absolute_log_return_sum",
            "spy_signed_path_efficiency",
        ...
        if any(
            ...
            raise InputBlockedError("C84 signed-path-efficiency value is invalid")
        if (
            type(row["return_count"]) is not int
            or row["return_count"] != 126
        ):
            raise InputBlockedError("C84 return count mismatch")

        spy_numerator = float(row["spy_log_return_sum"])
        spy_denominator = float(row["spy_absolute_log_return_sum"])
        qqq_numerator = float(row["qqq_log_return_sum"])
        qqq_denominator = float(row["qqq_absolute_log_return_sum"])
        if spy_denominator <= 0.0 or qqq_denominator <= 0.0:
            raise InputBlockedError("C84 efficiency denominator must be positive")
        spy_efficiency = spy_numerator / spy_denominator
        qqq_efficiency = qqq_numerator / qqq_denominator
        difference = qqq_efficiency - spy_efficiency
        if not math.isclose(
            float(row["spy_signed_path_efficiency"]),
            spy_efficiency,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["qqq_signed_path_efficiency"]),
            qqq_efficiency,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["relative_efficiency_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C84 signed-path-efficiency arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C84 signed-path-efficiency target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C84 signed-path-efficiency chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C84 signed-path-efficiency signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C84 signed-path-efficiency stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C84 signed-path-efficiency state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C84 signed-path-efficiency signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C84 signed-path-efficiency state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C84 signed-path-efficiency input packet")
    receipt = _json(
        receipt_payload,
        "C84 signed-path-efficiency materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c84-relative-signed-path-efficiency-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C84 signed-path-efficiency signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c84-relative-signed-path-efficiency-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c84-relative-signed-path-efficiency-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C84 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C84 holdout one-use state is not pristine")
