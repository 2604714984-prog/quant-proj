"""One-use runner for frozen C117 relative-positive-return-streak rotation."""
...
from research.adapters.c117_relative_positive_return_streak import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c117_relative_positive_return_streak_v1.json"
ADAPTER = ROOT / "research/adapters/c117_relative_positive_return_streak.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "b138ba135291a2e61cbd3466924782286a78ae613f4972449faaabaf0f98a153"
ADAPTER_SHA256 = "23432cfc0a5f3b4af2b875801bf618dd58f6adc06ae5824d49e4a09de12423d4"
SIGNAL_SHA256 = "dc8110827d82a6661a194349ccb1522ddc692aa3f28ba1c507e19370242a1f85"
RECEIPT_SHA256 = "b1f9113aacbbf29966bff827f3a416b52ee71fb72842c12350cc182c11c400e4"
...
SIGNAL_INPUT = Path(
    ...
    "c117_relative_positive_return_streak_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C117 relative-positive-return-streak input packet")
    expected_contract = {
        "formation_sessions": 63,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "maximum number of consecutive simple daily returns strictly "
            "greater than zero among exactly 62 returns"
        ),
        "target_rule": (
            "QQQ longest positive-return streak strictly greater than SPY "
            "selects QQQ; equality or lower selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c117-relative-positive-return-streak-input-v1"
        ...
        raise InputBlockedError("C117 relative-positive-return-streak packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C117 relative-positive-return-streak signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_longest_positive_return_streak",
        "relative_positive_return_streak_difference",
        "signal_identity_sha256",
        "signal_session",
        "source_row_hash_set_sha256",
        "spy_longest_positive_return_streak",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C117 relative-positive-return-streak signal schema mismatch")
        count_fields = (
            "qqq_longest_positive_return_streak",
            "relative_positive_return_streak_difference",
            "spy_longest_positive_return_streak",
        ...
        if any(
            ...
            for value in (row[field] for field in count_fields)
        ):
            raise InputBlockedError(
                "C117 relative-positive-return-streak value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 63
            or not 0 <= row["spy_longest_positive_return_streak"] <= 62
            or not 0 <= row["qqq_longest_positive_return_streak"] <= 62
        ):
            raise InputBlockedError("C117 formation session count mismatch")
        difference = (
            row["qqq_longest_positive_return_streak"]
            - row["spy_longest_positive_return_streak"]
        )
        if row["relative_positive_return_streak_difference"] != difference:
            raise InputBlockedError("C117 relative-positive-return-streak arithmetic mismatch")
        expected_state = "QQQ" if difference > 0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C117 relative-positive-return-streak target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C117 relative-positive-return-streak chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C117 relative-positive-return-streak signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C117 relative-positive-return-streak stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C117 relative-positive-return-streak state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C117 relative-positive-return-streak signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C117 relative-positive-return-streak state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C117 relative-positive-return-streak input packet")
    receipt = _json(
        receipt_payload,
        "C117 relative-positive-return-streak materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c117-relative-positive-return-streak-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C117 relative-positive-return-streak signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c117-relative-positive-return-streak-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c117-relative-positive-return-streak-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C117 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C117 holdout one-use state is not pristine")
