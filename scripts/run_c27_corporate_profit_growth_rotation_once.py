"""One-use runner for the frozen Cycle 27 CP SPY/QQQ rotation lane."""
...
from research.adapters.c27_corporate_profit_growth_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c27_corporate_profit_growth_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c27_corporate_profit_growth_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "d3bb524630bd32823bd7bdc6464f5973d92271eac44b00f8290df36933fa4633"
ADAPTER_SHA256 = "e6aa42dc3b72a784aaaab4a7573150694df7c7d416e406ed942f4d587f33722d"
SIGNAL_SHA256 = "a862a5721313c45579099be163ac4a3248e0be90cd680080df2c0c9c4cf458a0"
RECEIPT_SHA256 = "935ce27e448caa21103810ea755f36e870edad8b7bf4540991132264f91740bf"
...
SIGNAL_INPUT = Path(
    ...
    "c27_corporate_profit_growth_rotation_spy_qqq_v1/"
    "c27_full_input_packet.json"
)
...
def _previous_quarter(value: date) -> date:
    if value.day != 1 or value.month not in {1, 4, 7, 10}:
        raise InputBlockedError("corporate profit reference must be a quarter start")
    if value.month == 1:
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "corporate-profit growth input packet")
    if (
        record.get("schema_version")
        != "c27-corporate-profit-growth-spy-qqq-input-packet-v1"
    ...
    for row in rows:
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("CP selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "CP"
                or selection.get("source_class")
                != "OFFICIAL_ALFRED_BEA_CORPORATE_PROFITS_AFTER_TAX"
                or selection.get("raw_response_sha256")
                != "6f938e81902e72a57dcad52b2f0c022ea1a9c7b3bd3ddd7b87f7e718f004bb60"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("CP source row identity mismatch")
        current_value = current.get("value_billions_saar")
        previous_value = previous.get("value_billions_saar")
        change = row.get("delta_billions_saar")
        if any(
            ...
            raise InputBlockedError("CP values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("CP change arithmetic mismatch")
        ...
        previous_reference_quarter = _date(
            row.get("previous_reference_quarter"), "previous corporate profit reference quarter"
        )
        current_reference_quarter = _date(
            row.get("current_reference_quarter"), "current corporate profit reference quarter"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous CP available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current CP available_at",
            )
            > decision_at
            ...
            raise InputBlockedError(
                "corporate profit reference-quarter identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "CP row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("CP signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("CP state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("CP signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("CP state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c27-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "corporate-profit growth/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "corporate-profit growth/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "CP signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c27-corporate-profit-growth-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c27-corporate-profit-growth-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 27 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 27 holdout one-use state is not pristine")
