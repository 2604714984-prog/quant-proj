"""One-use runner for frozen C118 qqq-14-session-rsi-state rotation."""
...
from research.adapters.c118_qqq_14_session_rsi_state import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c118_qqq_14_session_rsi_state_v1.json"
ADAPTER = ROOT / "research/adapters/c118_qqq_14_session_rsi_state.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "28b08998f95cd74a36b4006d4bfdf910437f05d0ea2b7db540027b4f45283838"
ADAPTER_SHA256 = "3f6d6b0ede03c53504183acb942448eb40a746d8defb4f45fe615fa3306c94dc"
SIGNAL_SHA256 = "e8fde11790fedcd7657bec0fdb42b5d6130376d5551ba72332a9abe20375e335"
RECEIPT_SHA256 = "133013a16002feb42fb7ffdb58755beeb807168bf481dad8b236253b2d4b97bf"
...
SIGNAL_INPUT = Path(
    ...
    "c118_qqq_14_session_rsi_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C118 qqq-14-session-rsi-state input packet")
    expected_contract = {
        "formation_sessions": 15,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "RSI from arithmetic mean gains and losses over exactly 14 QQQ "
            "adjusted-close differences; zero average loss sets RSI to 100"
        ),
        "target_rule": (
            "QQQ RSI strictly greater than 50 selects QQQ; equality or lower "
            "selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c118-qqq-14-session-rsi-state-input-v1"
        ...
        raise InputBlockedError("C118 qqq-14-session-rsi-state packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C118 qqq-14-session-rsi-state signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C118 qqq-14-session-rsi-state signal schema mismatch")
        value_fields = (
            "qqq_average_gain_14",
            "qqq_average_loss_14",
            "qqq_rsi_14",
        ...
        if any(
            ...
            raise InputBlockedError(
                "C118 qqq-14-session-rsi-state value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 15
            or row["qqq_average_gain_14"] < 0.0
            or row["qqq_average_loss_14"] < 0.0
            or not 0.0 <= row["qqq_rsi_14"] <= 100.0
        ):
            raise InputBlockedError("C118 formation session count mismatch")
        expected_rsi = (
            100.0
            if row["qqq_average_loss_14"] == 0.0
            else 100.0
            - 100.0
            / (
                1.0
                + row["qqq_average_gain_14"]
                / row["qqq_average_loss_14"]
            )
        )
        if not math.isclose(
            row["qqq_rsi_14"],
            ...
            raise InputBlockedError("C118 qqq-14-session-rsi-state arithmetic mismatch")
        expected_state = "QQQ" if row["qqq_rsi_14"] > 50.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C118 qqq-14-session-rsi-state target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C118 qqq-14-session-rsi-state chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C118 qqq-14-session-rsi-state signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C118 qqq-14-session-rsi-state stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C118 qqq-14-session-rsi-state state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C118 qqq-14-session-rsi-state signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C118 qqq-14-session-rsi-state state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C118 qqq-14-session-rsi-state input packet")
    receipt = _json(
        receipt_payload,
        "C118 qqq-14-session-rsi-state materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c118-qqq-14-session-rsi-state-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C118 qqq-14-session-rsi-state signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c118-qqq-14-session-rsi-state-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c118-qqq-14-session-rsi-state-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C118 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C118 holdout one-use state is not pristine")
