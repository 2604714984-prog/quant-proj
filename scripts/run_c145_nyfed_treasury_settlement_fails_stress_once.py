"""One-use runner for frozen C145 Treasury settlement-fails stress rotation."""
...
from research.adapters.c145_nyfed_treasury_settlement_fails_stress import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / (
    ...
    "c145_nyfed_treasury_settlement_fails_stress_v1.json"
)
ADAPTER = ROOT / (
    "research/adapters/"
    "c145_nyfed_treasury_settlement_fails_stress.py"
)
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "e9832a96c8373195eff16262f9b4c62534cb5a5dc814450912214e24d5288d27"
ADAPTER_SHA256 = "0108712e31b3d0576066182296910fba1a9bd3689c1feb99c726027da726dc78"
SIGNAL_SHA256 = "0b9f37f152a64323f9c4509c8b2d6783c330680c3a296f5b666fa3797ca85c24"
SIGNAL_RECEIPT_SHA256 = "7f30777edab7a947c589888ab4deb8a04bf362f93569f6c0c2b699ce954a165d"
RUNTIME_RECEIPT_SHA256 = "dd61afd3a8c8b7f8ecaf694421d6e5c6b4616f4ad72842a3621f9b54a5ee61ae"
VALIDATION_BUNDLE_SHA256 = "0e433011df7b9b0683bc9343b19d5981635d4c96e9882f77616f01af1c971033"
HOLDOUT_BUNDLE_SHA256 = "84447bad0dae9861916f0a1b1d1950a2384706dc703a9e9bdc24a83b364f7e3c"
...
SIGNAL_INPUT = Path(
    ...
    "c145_nyfed_treasury_settlement_fails_stress_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C145 NY Fed Treasury settlement-fails signal input")
    expected_contract = {
        "direction": "positive_weekly_fails_impulse_selects_SPY_else_QQQ",
        "field": "PDFTD-USTET",
    ...
    if (
        ...
        != "c145-nyfed-treasury-settlement-fails-input-v1"
        ...
        raise InputBlockedError("C145 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C145 signal row count mismatch")
    expected_row_keys = {
        "current_as_of_date",
        "current_value_usd_millions",
        "decision_at",
        "decision_session",
        "fails_impulse_usd_millions",
        "prior_as_of_date",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C145 signal schema mismatch")
        value_fields = (
            "current_value_usd_millions",
            "prior_value_usd_millions",
        )
        if any(
            ...
            raise InputBlockedError("C145 fails value is invalid")
        impulse = (
            row["current_value_usd_millions"]
            - row["prior_value_usd_millions"]
        )
        observed_impulse = row["fails_impulse_usd_millions"]
        if (
            ...
            raise InputBlockedError("C145 fails impulse is invalid")
        expected_state = "SPY" if impulse > 0.0 else "QQQ"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C145 target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C145 signal chronology mismatch")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C145 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C145 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C145 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C145 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C145 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C145 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C145 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C145 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C145 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C145 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c145-nyfed-treasury-settlement-fails-receipt-v1"
        ...
        != "c145-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c145-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C145 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C145 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C145 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C145 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c145-nyfed-treasury-settlement-fails-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c145-nyfed-treasury-settlement-fails-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C145 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C145 holdout one-use state is not pristine")
