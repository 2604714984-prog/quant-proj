"""One-use runner for the frozen C72 SPY/QQQ 12-1 relative-momentum lane."""
...
from research.adapters.c72_spy_qqq_12_1_relative_momentum_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c72_spy_qqq_12_1_relative_momentum_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c72_spy_qqq_12_1_relative_momentum_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "294e698f61844fbc69c91ccb71531594a6d0003de0f632ad0cbe056e8e001930"
ADAPTER_SHA256 = "41ba731a269e1c8580b77f358c2b172a6fa66dc5217baeba42881e287d035256"
SIGNAL_SHA256 = "da541f24903d823b339547a66177f451c4723fe6b4e3a82c758f54d03087083d"
RECEIPT_SHA256 = "f23210133ca0d4ac43b1168b45b6a36ee74099b4f9db2f0a3bdc0b03ee47b436"
...
SIGNAL_INPUT = Path(
    ...
    "c72_spy_qqq_12_1_relative_momentum_rotation_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C72 relative-momentum input packet")
    expected_contract = {
        "lookback_base_common_sessions": 252,
        "skip_recent_common_sessions": 21,
        "target_rule": (
            "QQQ 12-1 return strictly greater than SPY means QQQ; otherwise SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c72-spy-qqq-12-1-relative-momentum-input-v1"
        ...
        raise InputBlockedError("C72 relative-momentum packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C72 relative-momentum signal row count mismatch")
    expected_row_keys = {
        "base_session",
        "decision_at",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C72 relative-momentum signal schema mismatch")
        spy_return = row["spy_12_1_return"]
        qqq_return = row["qqq_12_1_return"]
        if any(
            isinstance(value, bool)
            or not isinstance(value, (int, float))
            or not math.isfinite(float(value))
            for value in (spy_return, qqq_return)
        ):
            raise InputBlockedError("C72 relative-momentum return is invalid")
        expected_state = "QQQ" if float(qqq_return) > float(spy_return) else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C72 relative-momentum target mismatch")

        base_session = _date(row["base_session"], "base session")
        skip_session = _date(row["skip_session"], "skip session")
        ...
        if (
            not base_session < skip_session < signal_session < start < end
            ...
            raise InputBlockedError("C72 relative-momentum chronology mismatch")

        source_rows = row["source_row_hashes"]
        if type(source_rows) is not dict or set(source_rows) != {"SPY", "QQQ"}:
            raise InputBlockedError("C72 relative-momentum source identity mismatch")
        for symbol in ("SPY", "QQQ"):
            hashes = source_rows[symbol]
            if type(hashes) is not list or len(hashes) != 3:
                raise InputBlockedError("C72 relative-momentum source identity mismatch")
            for value in hashes:
                _sha(value, f"{symbol} source-row SHA-256")
        identity = {
            "signal_session": signal_session.isoformat(),
            "skip_session": skip_session.isoformat(),
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C72 relative-momentum signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C72 relative-momentum stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C72 relative-momentum state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C72 relative-momentum signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C72 relative-momentum state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c72-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C72 relative-momentum input packet")
    receipt = _json(
        receipt_payload,
        "C72 relative-momentum materialization receipt",
    )
    ...
    if (
        ...
        != "c72-relative-momentum-full-materialization-receipt-v2"
        or receipt.get("input_packet_sha256") != SIGNAL_SHA256
        or receipt.get("original_receipt_sha256")
        != "fecf8b3b538d211a96366504f020edc98510984b86d4b949c2a9f276146f0ae5"
        or receipt.get("reused_market_scaffold_sha256")
        != "7fdc5de1aa1e90d6867b78397c16e94c29be81f3d66eb2745bb0db0441d56bbc"
        ...
        or receipt.get("provider_or_network_call") is not False
        or receipt.get("source_selected_using_performance") is not False
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C72 relative-momentum signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c72-relative-momentum-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c72-relative-momentum-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C72 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C72 holdout one-use state is not pristine")
