"""One-use runner for the frozen Cycle 39 Cboe VXN/VIX SPY/QQQ lane."""
...
from research.adapters.c39_cboe_relative_ndx_spx_vol_change_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c39_cboe_relative_ndx_spx_vol_change_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c39_cboe_relative_ndx_spx_vol_change_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "401a03778ee36066e1624697b0e612c2d7ad4bc3cd4dd13e77bb757eab7d01fa"
ADAPTER_SHA256 = "4bbc8a1f2329b5bcf9d70735f4a0d9a22ef208a5e5e6b71a38cd0b8d0e1ebaae"
SIGNAL_SHA256 = "342dc1f8a6b502474b2000d3c80e62352658f1247f7a589d0013d2a85ebbe1c9"
RECEIPT_SHA256 = "c6a60c5fac1696523a5887640d07c42e50f0b350ef72f78754d4bbf9ddcc3f30"
...
SIGNAL_INPUT = Path(
    ...
    "c39_cboe_relative_ndx_spx_vol_change_rotation_spy_qqq_v1/"
    "c39_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Cboe VXN/VIX rotation input packet")
    expected_contract = {
        "series_id": "VXN_DIV_VIX",
        "lookback_calendar_days": 28,
        "target_rule": "strict increase in the official Cboe VXN/VIX ratio selects SPY; equal or decrease selects QQQ",
        "threshold_ratio_change": 0.0,
        "common_observation_dates_only": True,
    }
    if (
        record.get("schema_version")
        != "c39-cboe-relative-ndx-spx-vol-change-spy-qqq-input-packet-v1"
        ...
        raise InputBlockedError("VXN/VIX packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("VXN/VIX signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = {
        "VIX": "26ab75dc0cf0d2f375e0f615e466de890f1e60d3b63a25aa2989f18c26670851",
        "VXN": "af4b0496843ba796bfc4570b277c08d2862ab40b5993f83682e8254a83c96fdd",
    }
    expected_urls = {
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("VXN/VIX signal stage mismatch")
        ...
        if type(current) is not dict or type(baseline) is not dict:
            raise InputBlockedError("VXN/VIX selections are required")
        for selection in (baseline, current):
            ...
            components = selection.get("component_values_index_points")
            ratio = selection.get("value_ratio")
            retrieved_at = selection.get("retrieved_at")
            valid_components = type(components) is dict and set(components) == {
                "VIX",
                "VXN",
            }
            if valid_components:
                component_values = tuple(components[key] for key in ("VIX", "VXN"))
            ...
            valid_retrievals = type(retrieved_at) is dict and set(retrieved_at) == {
                "VIX",
                "VXN",
            }
            if valid_retrievals:
                for series in ("VIX", "VXN"):
                    _datetime(retrieved_at[series], f"{series} retrieved_at")
            if (
                selection.get("series_id") != "VXN_DIV_VIX"
                or selection.get("source_class")
                != "OFFICIAL_CBOE_STATIC_HISTORY_RETROSPECTIVE_ONLY"
                or selection.get("source_urls") != expected_urls
                or selection.get("raw_response_sha256") != expected_raw_sha
                or not valid_components
                or not valid_retrievals
                ...
                or not math.isclose(
                    float(ratio),
                    float(components["VXN"]) / float(components["VIX"]),
                    rel_tol=0,
                    abs_tol=1e-12,
                )
                or row_hash
                ...
                raise InputBlockedError("VXN/VIX source row identity mismatch")
        current_value = float(current["value_ratio"])
        baseline_value = float(baseline["value_ratio"])
        change = row.get("delta_ratio")
        if (
            ...
            raise InputBlockedError("VXN/VIX change arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError("VXN/VIX observation or availability mismatch")
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "VXN/VIX row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("VXN/VIX signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("VXN/VIX state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("VXN/VIX signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("VXN/VIX state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c39-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "VXN/VIX rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "VXN/VIX rotation/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c39-cboe-relative-ndx-spx-vol-full-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "VXN/VIX signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c39-cboe-relative-ndx-spx-vol-change-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c39-cboe-relative-ndx-spx-vol-change-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
