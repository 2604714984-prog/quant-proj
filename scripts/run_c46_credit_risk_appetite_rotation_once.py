"""One-use runner for the frozen Cycle 46 credit risk appetite SPY/QQQ lane."""
...
from research.adapters.c46_credit_risk_appetite_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c46_credit_risk_appetite_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c46_credit_risk_appetite_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "b4fbba5886776bb434936701d341442d6b1dd4e2beb7d672f03e5e61763975d3"
ADAPTER_SHA256 = "10d5f31067cb00adca49c7053807d8baaef22dde161e114de722e4c12339e094"
SIGNAL_SHA256 = "2997bb99db2bce63b2c204802163aeb3d33936348db2a4684e4641ac67d4a3a8"
RECEIPT_SHA256 = "2e6ae3b153638037ecb54c8a311e57180dc04195b370643b78111c3c289df7b3"
...
SIGNAL_INPUT = Path(
    ...
    "c46_credit_risk_appetite_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "credit risk appetite rotation input packet")
    expected_contract = {
        "left_symbol": "HYG",
        "right_symbol": "IEF",
        "return_window": (
        ...
        "target_rule": (
            "strict HYG prior-month total-return outperformance versus IEF "
    ...
    if (
        ...
        raise InputBlockedError("credit risk appetite packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("credit risk appetite signal row count mismatch")
    ...
    expected_source = {
        "HYG": (
            ...
            "abf200635514ef5351bfc1e636820ffffb52cbf8cb43878b533b96c8bd23e909",
        ),
        "IEF": (
            "https://query1.finance.yahoo.com/v8/finance/chart/IEF?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "9440afc3333b41c84383040e509fdf184c6a96250d424fbb9f61e8f8691e1c08",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("credit risk appetite signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("credit risk appetite selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "credit risk appetite adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("credit risk appetite source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "credit risk appetite cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("credit risk appetite arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "credit risk appetite month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("credit risk appetite signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("credit risk appetite signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "credit risk appetite row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("credit risk appetite signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("credit risk appetite state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "credit risk appetite signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("credit risk appetite state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "credit risk appetite rotation input packet")
    receipt = _json(
        receipt_payload,
        "credit risk appetite materialization receipt",
    )
    ...
    if (
        ...
        or receipt.get("left_symbol") != "HYG"
        or receipt.get("right_symbol") != "IEF"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "credit risk appetite signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c46-credit-risk-appetite-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c46-credit-risk-appetite-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
