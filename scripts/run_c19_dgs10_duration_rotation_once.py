"""One-use runner for the frozen Cycle 19 DGS10 SPY/QQQ rotation lane."""
...
from research.adapters.c19_dgs10_duration_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c19_dgs10_duration_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c19_dgs10_duration_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "206b6d41eb223554aecbc619ef6da345cc77fba1d41b451795d5dfc811ba30d7"
ADAPTER_SHA256 = "700739f69507ae0dac7b50c1dfa3da1dc9f78ea893c561189763fbf0c94fc8f7"
SIGNAL_SHA256 = "d1a34108905904db8918464f3eab7ee79307488bfd936424cb7f96921c6bc290"
RECEIPT_SHA256 = "99aff96b76ebbd9b28bb8fa5023afd72cd2951b3a8b940bd1b1ef37ccfbe8d0e"
...
SIGNAL_INPUT = Path(
    ...
    "c19_dgs10_duration_rotation_spy_qqq_v1/"
    "c19_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "DGS10 duration-rotation input packet")
    if (
        record.get("schema_version")
        != "c19-dgs10-duration-rotation-spy-qqq-input-packet-v1"
        ...
        raise InputBlockedError("DGS10 duration-rotation packet boundary mismatch")
    ...
    for row in rows:
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("DGS10 change arithmetic mismatch")
        ...
        identity = {
            ...
            "current_row_sha256": current.get("row_hash"),
            "previous_row_sha256": previous.get("row_hash"),
            "delta_percentage_points": float(change),
            "target_symbol": expected_state,
            "source_packet_sha256": "85e152675eef78c60665e8741151a90d3653f8ab4173960fb24a5b376fa55ce9",
        }
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("DGS10 source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                expected_state,
                _sha(row["signal_identity_sha256"], "DGS10 row SHA-256"),
            )
        )
    ...
    if type(support) is not dict:
        raise InputBlockedError("DGS10 state support is required")
    for stage, group in output.items():
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("DGS10 state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c19-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "DGS10 duration-rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "DGS10 duration-rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "DGS10 signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c19-dgs10-duration-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c19-dgs10-duration-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 19 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 19 holdout one-use state is not pristine")
