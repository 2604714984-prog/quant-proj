"""One-use runner for frozen C76 downside/upside-energy asymmetry."""
...
from research.adapters.c76_downside_upside_energy_asymmetry_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c76_downside_upside_energy_asymmetry_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c76_downside_upside_energy_asymmetry_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "d57d188b2d5f92e77c27be11a184230eaa59d31d0dca200d1c02d6a7453087ee"
ADAPTER_SHA256 = "9fd9fb8339417cb656bb483723db661b0531ef5ea9d8fadf22c2a8762ae2fdd5"
SIGNAL_SHA256 = "fcdce68cd4e82231849a38f376337fb8ee1aa907d9aab4c154cb8718aa22197d"
RECEIPT_SHA256 = "74973c96d10cd896b6d764476e1e6c1d5a0fb87d21355fcdd4ed3de160e2cbd5"
...
SIGNAL_INPUT = Path(
    ...
    "c76_downside_upside_energy_asymmetry_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C76 downside-upside-energy input packet")
    expected_contract = {
        "formation_close_count": 64,
        "return_count": 63,
        "return_type": "natural log return",
        "downside_energy": (
            "arithmetic mean of min(return, 0)^2 over all 63 returns"
        ),
        "upside_energy": (
            "arithmetic mean of max(return, 0)^2 over all 63 returns"
        ),
        "asymmetry_ratio": "downside energy divided by upside energy",
        "target_rule": (
            "QQQ asymmetry ratio strictly below SPY means QQQ; equality or greater means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c76-downside-upside-energy-asymmetry-input-v1"
        ...
        raise InputBlockedError("C76 downside-upside-energy packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C76 downside-upside-energy signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C76 downside-upside-energy signal schema mismatch")
        numeric_fields = (
            "spy_downside_energy",
            "spy_upside_energy",
            "spy_asymmetry_ratio",
        ...
        if any(
            ...
            raise InputBlockedError("C76 downside-upside-energy value is invalid")
        if any(float(row[field]) <= 0.0 for field in numeric_fields):
            raise InputBlockedError("C76 downside/upside energy and ratio must be positive")
        if type(row["return_count"]) is not int or row["return_count"] != 63:
            raise InputBlockedError("C76 formation return count mismatch")
        spy_ratio = float(row["spy_asymmetry_ratio"])
        qqq_ratio = float(row["qqq_asymmetry_ratio"])
        expected_spy = float(row["spy_downside_energy"]) / float(
            row["spy_upside_energy"]
        )
        expected_qqq = float(row["qqq_downside_energy"]) / float(
            row["qqq_upside_energy"]
        )
        if (
            not math.isclose(spy_ratio, expected_spy, rel_tol=1e-12, abs_tol=1e-15)
            or not math.isclose(qqq_ratio, expected_qqq, rel_tol=1e-12, abs_tol=1e-15)
        ):
            raise InputBlockedError("C76 downside/upside-energy arithmetic mismatch")
        expected_state = "QQQ" if qqq_ratio < spy_ratio else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C76 downside-upside-energy target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C76 downside-upside-energy chronology mismatch")

        _sha(row["formation_row_hash_set_sha256"], "formation row-set SHA-256")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C76 downside-upside-energy signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C76 downside-upside-energy stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C76 downside-upside-energy state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C76 downside-upside-energy signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C76 downside-upside-energy state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c76-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C76 downside-upside-energy input packet")
    receipt = _json(
        receipt_payload,
        "C76 downside-upside-energy materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c76-downside-upside-energy-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C76 downside-upside-energy signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c76-downside-upside-energy-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c76-downside-upside-energy-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C76 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C76 holdout one-use state is not pristine")
