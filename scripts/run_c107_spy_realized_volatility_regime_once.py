"""One-use runner for frozen C107 SPY realized-volatility regime rotation."""
...
from research.adapters.c107_spy_realized_volatility_regime import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c107_spy_realized_volatility_regime_v1.json"
ADAPTER = ROOT / "research/adapters/c107_spy_realized_volatility_regime.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "02422231361b73f6e65928ac01c5405e9ccba42b18bfee64106e2f1e97343bcc"
ADAPTER_SHA256 = "188d175091b048396d0ffe6cd3592d6bc0b48244aacf5981da194d4171af627c"
SIGNAL_SHA256 = "db4caecb1a3d74ce3ae66093a6dbde6e0506dfbc1bba8dd73aa0bb12c9563652"
RECEIPT_SHA256 = "cf3b4dc21e8c628d81cb3de80c54c6525f31c6c9200a3fee18cd0e0d0c300335"
...
SIGNAL_INPUT = Path(
    ...
    "c107_spy_realized_volatility_regime_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C107 SPY-realized-volatility-regime input packet")
    expected_contract = {
        "formation_sessions": 253,
        "baseline_returns": 231,
        "recent_returns": 21,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "centered population standard deviation of SPY simple "
            "daily returns, unannualized"
        ),
        "target_rule": (
            "recent 21-return SPY volatility strictly below preceding "
            "231-return volatility selects QQQ; equality or higher selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c107-spy-realized-volatility-regime-input-v1"
        ...
        raise InputBlockedError("C107 SPY-realized-volatility-regime packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C107 SPY-realized-volatility-regime signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "baseline_spy_realized_volatility",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C107 SPY-realized-volatility-regime signal schema mismatch")
        numeric_fields = (
            "baseline_spy_realized_volatility",
            "recent_spy_realized_volatility",
            "realized_volatility_difference",
        ...
        if any(
            ...
            raise InputBlockedError("C107 SPY-realized-volatility-regime value is invalid")
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 253
            or type(row["baseline_return_count"]) is not int
            or row["baseline_return_count"] != 231
            or type(row["recent_return_count"]) is not int
            or row["recent_return_count"] != 21
        ):
            raise InputBlockedError("C107 formation session count mismatch")
        baseline = float(row["baseline_spy_realized_volatility"])
        recent = float(row["recent_spy_realized_volatility"])
        difference = recent - baseline
        if baseline <= 0.0 or recent <= 0.0:
            raise InputBlockedError(
                "C107 SPY-realized-volatility-regime value is invalid"
            )
        if not math.isclose(
            float(row["realized_volatility_difference"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C107 SPY-realized-volatility-regime arithmetic mismatch")
        expected_state = "QQQ" if difference < 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C107 SPY-realized-volatility-regime target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C107 SPY-realized-volatility-regime chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C107 SPY-realized-volatility-regime signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C107 SPY-realized-volatility-regime stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C107 SPY-realized-volatility-regime state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C107 SPY-realized-volatility-regime signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C107 SPY-realized-volatility-regime state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C107 SPY-realized-volatility-regime input packet")
    receipt = _json(
        receipt_payload,
        "C107 SPY-realized-volatility-regime materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c107-spy-realized-volatility-regime-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C107 SPY-realized-volatility-regime signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c107-spy-realized-volatility-regime-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c107-spy-realized-volatility-regime-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C107 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C107 holdout one-use state is not pristine")
