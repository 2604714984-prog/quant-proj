"""One-use runner for the frozen Cycle 8 NFCI monthly impulse lane."""
...
from research.adapters.c8_nfci_monthly_impulse import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c8_nfci_monthly_impulse_v1.json"
ADAPTER = ROOT / "research/adapters/c8_nfci_monthly_impulse.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "4c929abe70f23a0d2dea2c810e8b964cf1789c51e501cc41598f0ccbd1f53bb1"
ADAPTER_SHA256 = "b515ca7c177ad4cc70ad14b35fe819e80748a2a036b4e17758c9f7bbb9578c50"
SIGNAL_SHA256 = "ded8bf9b68ac96ff6fe690727a9987970048bd1529e6239dc3ca9a37126990ce"
RECEIPT_SHA256 = "868345db03b61d5517ebd1851abbeba835132010d6d7a8883c82ba221fec7466"
...
SIGNAL_INPUT = Path(
    ...
    "c8_nfci_monthly_impulse_spy_cash_v1/nfci_monthly_impulse_signal_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "NFCI signal packet")
    if (
        record.get("schema_version") != "c8-nfci-monthly-impulse-signal-packet-v1"
    ...
    for row in rows:
        ...
        if type(previous) is not dict or type(current) is not dict:
            raise InputBlockedError("NFCI selections are required")
        ...
        if (
            ...
            raise InputBlockedError("NFCI values must be finite")
        expected_state = "SPY" if current_value <= previous_value else "CASH"
        if row.get("state") != expected_state or row.get("delta") != current_value - previous_value:
            raise InputBlockedError("NFCI signal arithmetic mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(current["row_hash"], "current NFCI row SHA-256"),
                _sha(previous["row_hash"], "previous NFCI row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("NFCI signal split count mismatch")
    for group in output.values():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("NFCI signal intervals are not contiguous")
    return output
...
def _load_bundle(payload: bytes, *, stage: str, signals: tuple[Signal, ...]) -> Bundle:
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError("NFCI signal does not bind the SPY execution schedule")
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c8-nfci-monthly-impulse-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c8-nfci-monthly-impulse-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
