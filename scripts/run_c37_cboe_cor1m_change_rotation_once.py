"""One-use runner for the frozen Cycle 37 Cboe COR1M SPY/QQQ rotation lane."""
...
from research.adapters.c37_cboe_cor1m_change_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c37_cboe_cor1m_change_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c37_cboe_cor1m_change_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "7ff1e79579d161f9aadbceed58b527837c8512bb1d16c103cc3f1ce4493eca5d"
ADAPTER_SHA256 = "4ba3bb837d0fb4bfca2628855975086fd3d0317029557a4946c0d0e692d830af"
SIGNAL_SHA256 = "e6a2b9f3eaa43b92cbffc587a14714e746b7f35007cf7f0c3605b9f909b098da"
RECEIPT_SHA256 = "ff4dbb697797e09c82a8e92a4c0e6df2975b7f6dd16844cef88fb8267bd3595b"
...
SIGNAL_INPUT = Path(
    ...
    "c37_cboe_cor1m_change_rotation_spy_qqq_v1/"
    "c37_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Cboe COR1M rotation input packet")
    expected_contract = {
        "series_id": "COR1M",
        "lookback_calendar_days": 28,
        "target_rule": "strict increase in official Cboe COR1M selects SPY; equal or decrease selects QQQ",
        "threshold_index_points": 0.0,
    }
    if (
        record.get("schema_version")
        != "c37-cboe-cor1m-change-spy-qqq-input-packet-v1"
        ...
        raise InputBlockedError("COR1M packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("COR1M signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = (
        "d3ac35e1b3504fc9dd3cca26724904b42e8ec77eddfff72516a641d24d884c61"
    )
    expected_url = (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("COR1M signal stage mismatch")
        ...
        if type(current) is not dict or type(baseline) is not dict:
            raise InputBlockedError("COR1M selections are required")
        for selection in (baseline, current):
            ...
            if (
                ...
                raise InputBlockedError("COR1M source row identity mismatch")
        ...
        if (
            ...
            raise InputBlockedError("COR1M change arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError("COR1M observation or availability mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("COR1M signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("COR1M signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "COR1M row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("COR1M signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("COR1M state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("COR1M signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("COR1M state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c37-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "COR1M rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "COR1M rotation/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c37-cboe-cor1m-full-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "COR1M signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c37-cboe-cor1m-change-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c37-cboe-cor1m-change-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
