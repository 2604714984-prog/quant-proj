"""One-use runner for frozen C124 qqq-pearson-kurtosis rotation."""
...
from research.adapters.c124_qqq_pearson_kurtosis import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c124_qqq_pearson_kurtosis_v1.json"
ADAPTER = ROOT / "research/adapters/c124_qqq_pearson_kurtosis.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "ea0cd97cbaf2b187cbee1dec9f27aa45cace1b5718ef44b49b970d2469b5d32d"
ADAPTER_SHA256 = "71bde3d3a17c07a79f1d6b43558b7d788393eb944cb8193717b021fa63c510ab"
SIGNAL_SHA256 = "cdf7b61cd0b717259d0a198313ebd101fa5c1d47e37def10ac70d71841624d96"
RECEIPT_SHA256 = "67a90f59c4155ac97efe9422b02f057b71ca8782fb39f486660e30f001c873ac"
...
SIGNAL_INPUT = Path(
    ...
    "c124_qqq_63_return_pearson_kurtosis_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C124 qqq-pearson-kurtosis input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Pearson population kurtosis m4 divided by m2 squared from 63 simple "
            "QQQ returns formed by exactly 64 adjusted closes"
        ),
        "target_rule": (
            "Pearson kurtosis at or below 3 selects QQQ; above 3 selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c124-qqq-pearson-kurtosis-input-v1"
        ...
        raise InputBlockedError("C124 qqq-pearson-kurtosis packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C124 qqq-pearson-kurtosis signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C124 qqq-pearson-kurtosis signal schema mismatch")
        value_fields = ("qqq_pearson_kurtosis",)
        if any(
            ...
            raise InputBlockedError(
                "C124 qqq-pearson-kurtosis value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or row["qqq_pearson_kurtosis"] <= 0.0
        ):
            raise InputBlockedError("C120 formation session count mismatch")
        expected_state = "QQQ" if row["qqq_pearson_kurtosis"] <= 3.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C124 qqq-pearson-kurtosis target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C124 qqq-pearson-kurtosis chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C124 qqq-pearson-kurtosis signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C124 qqq-pearson-kurtosis stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C124 qqq-pearson-kurtosis state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C124 qqq-pearson-kurtosis signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C124 qqq-pearson-kurtosis state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C124 qqq-pearson-kurtosis input packet")
    receipt = _json(
        receipt_payload,
        "C124 qqq-pearson-kurtosis materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c124-qqq-pearson-kurtosis-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C124 qqq-pearson-kurtosis signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c124-qqq-pearson-kurtosis-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c124-qqq-pearson-kurtosis-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
