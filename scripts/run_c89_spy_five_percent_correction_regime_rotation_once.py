"""One-use runner for frozen C89 SPY five-percent correction regime."""
...
from research.adapters.c89_spy_five_percent_correction_regime_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c89_spy_five_percent_correction_regime_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c89_spy_five_percent_correction_regime_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "0a15ad3254b2c1c87beb73540d3c2ae6415122603278d57d8f8cb5865cce64e1"
ADAPTER_SHA256 = "289bd985ccb89ff756a0ae3dc47a3e6c2d7845ce39b3c2c70bb048b1e8a86b82"
SIGNAL_SHA256 = "ab54e1b9910305c7070ad919143db446b44421d1592f6772281f63178d5a0202"
RECEIPT_SHA256 = "73bf8ff1635284ae96c631c25e8b2069d7c690c5cda70fb14c7814fbcc934d5a"
...
SIGNAL_INPUT = Path(
    ...
    "c89_spy_five_percent_correction_regime_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C89 SPY-correction-regime input packet")
    expected_contract = {
        "formation_close_count": 126,
        "current_drawdown": (
            "final adjusted close divided by maximum adjusted close in the "
            "126-close window minus one"
        ),
        "correction_threshold": -0.05,
        "target_rule": (
            "SPY current drawdown at or below -0.05 means SPY; strictly above "
            "-0.05 means QQQ"
        ),
        "threshold_equality": "SPY",
    }
    if (
        record.get("schema_version")
        != "c89-spy-five-percent-correction-regime-input-v1"
        ...
        raise InputBlockedError("C89 SPY-correction-regime packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C89 SPY-correction-regime signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "holding_end_execution_session",
        "holding_start_execution_session",
        "formation_end",
        "formation_close_count",
        ...
        "spy_current_drawdown",
        "spy_final_adjusted_close",
        "spy_peak_adjusted_close",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C89 SPY-correction-regime signal schema mismatch")
        numeric_fields = (
            "spy_peak_adjusted_close",
            "spy_final_adjusted_close",
            "spy_current_drawdown",
        ...
        if any(
            ...
            raise InputBlockedError("C89 SPY-correction-regime value is invalid")
        if (
            type(row["formation_close_count"]) is not int
            or row["formation_close_count"] != 126
        ):
            raise InputBlockedError("C89 formation close count mismatch")
        peak = float(row["spy_peak_adjusted_close"])
        final_close = float(row["spy_final_adjusted_close"])
        if peak <= 0.0 or final_close <= 0.0:
            raise InputBlockedError("C89 SPY prices must be positive")
        drawdown = final_close / peak - 1.0
        if not math.isclose(
            float(row["spy_current_drawdown"]),
            drawdown,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["correction_threshold"]),
            -0.05,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C89 SPY-correction-regime arithmetic mismatch")
        expected_state = "SPY" if drawdown <= -0.05 else "QQQ"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C89 SPY-correction-regime target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C89 SPY-correction-regime chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C89 SPY-correction-regime signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C89 SPY-correction-regime stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C89 SPY-correction-regime state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C89 SPY-correction-regime signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C89 SPY-correction-regime state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C89 SPY-correction-regime input packet")
    receipt = _json(
        receipt_payload,
        "C89 SPY-correction-regime materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c89-spy-five-percent-correction-regime-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C89 SPY-correction-regime signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c89-spy-five-percent-correction-regime-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c89-spy-five-percent-correction-regime-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C89 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C89 holdout one-use state is not pristine")
