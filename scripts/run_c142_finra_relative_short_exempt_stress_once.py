"""One-use runner for frozen C142 FINRA short-exempt stress rotation."""
...
from research.adapters.c142_finra_relative_short_exempt_stress import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c142_finra_relative_short_exempt_stress_v1.json"
ADAPTER = ROOT / "research/adapters/c142_finra_relative_short_exempt_stress.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "75432513cf5b190176bdd77f70f960296e6cf4f29341132312cbc100a79c4218"
ADAPTER_SHA256 = "01c6377706f6a97461de0c5bcc9f0c98c6a0afe1c0ef8b4c5ed794b46c3df558"
SIGNAL_SHA256 = "39f28adf30009c73a6fb57d031177e8662fc94188f937c23e71372e4f2513a61"
SIGNAL_RECEIPT_SHA256 = "f5e92627a061a0890392e538da6b2f102990d71e21b5e7b3c636c2464394f106"
RUNTIME_RECEIPT_SHA256 = "4010f25d0dc6e1ee68862fe576cf7089d90c2e07766e3bea212d88a09accc2a4"
VALIDATION_BUNDLE_SHA256 = "efea9baa988e563da2076f19db8d12dd5116d5a6e55951eaf5880d14099d1b9c"
HOLDOUT_BUNDLE_SHA256 = "b418864ea41d443d7268ac53eb854346077ed02fa4896f7a15cfb1cdeab79dc7"
...
SIGNAL_INPUT = Path(
    ...
    "c142_finra_relative_short_exempt_stress_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C142 FINRA short-exempt stress signal input")
    expected_contract = {
        "direction": "QQQ_exempt_ratio_strictly_above_SPY_selects_SPY_else_QQQ",
        "fields": ["ShortExemptVolume", "TotalVolume"],
        "lookback": "exactly five accepted XNYS sessions before decision",
        "ratio": "sum(ShortExemptVolume)/sum(TotalVolume)",
    }
    if (
        record.get("schema_version")
        != "c142-finra-relative-short-exempt-input-v1"
        ...
        raise InputBlockedError("C142 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C142 signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "decision_session",
        "observation_sessions",
        "qqq_short_exempt_ratio",
        "qqq_totals",
        "signal_identity_sha256",
        "spy_short_exempt_ratio",
        "spy_totals",
        "stage",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C142 signal schema mismatch")
        value_fields = ("qqq_short_exempt_ratio", "spy_short_exempt_ratio")
        if any(
            ...
            raise InputBlockedError("C142 signal value is invalid")
        expected_state = (
            "SPY"
            if row["qqq_short_exempt_ratio"] > row["spy_short_exempt_ratio"]
            else "QQQ"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C142 target mismatch")
        ...
        observed = row["observation_sessions"]
        if (
            row["decision_at"]
            ...
            raise InputBlockedError("C142 signal chronology mismatch")
        observation_dates = tuple(
            _date(value, "observation session") for value in observed
        )
        if (
            observation_dates != tuple(sorted(set(observation_dates)))
            or observation_dates[-1] >= decision_session
        ):
            raise InputBlockedError("C142 signal chronology mismatch")
        for symbol in ("qqq", "spy"):
            totals = row[f"{symbol}_totals"]
            if (
                ...
                or totals["total_volume"] <= 0.0
            ):
                raise InputBlockedError("C142 signal totals are invalid")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C142 signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C142 signal stage count mismatch")
    terminal = record.get("stage_terminal_execution_sessions")
    if type(terminal) is not dict:
        raise InputBlockedError("C142 terminal execution sessions are required")
    ...
    if type(support) is not dict:
        raise InputBlockedError("C142 state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C142 signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C142 state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    record = _json(payload, "C142 SPY/QQQ runtime bundle")
    packet = _json(input_payload, "C142 signal input packet")
    signal_receipt = _json(
        signal_receipt_payload,
        "C142 signal materialization receipt",
    )
    runtime_receipt = _json(
        runtime_receipt_payload,
        "C142 runtime materialization receipt",
    )
    ...
    if (
        ...
        != "c142-finra-relative-short-exempt-receipt-v1"
        ...
        != "c142-runtime-materialization-receipt-v1"
    ...
    if (
        record.get("schema_version") != "c142-spy-qqq-weekly-runtime-v1"
        or record.get("research_id") != RESEARCH_ID
        ...
        raise InputBlockedError("C142 runtime bundle identity mismatch")
    ...
    if len(raw_points) != len(signals) + 1:
        raise InputBlockedError("C142 execution boundary count mismatch")
    points: list[Point] = []
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C142 signal does not bind the execution schedule"
            )
        try:
        ...
        if type(executions_row) is not dict or set(executions_row) != {"SPY", "QQQ"}:
            raise InputBlockedError("C142 execution pair is incomplete")
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c142-finra-relative-short-exempt-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c142-finra-relative-short-exempt-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C142 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C142 holdout one-use state is not pristine")
