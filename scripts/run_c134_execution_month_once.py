"""One-use runner for frozen C134 tech earnings-season rotation."""
...
from research.adapters.c134_execution_month import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c134_execution_month_v1.json"
ADAPTER = ROOT / "research/adapters/c134_execution_month.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "3a431a5460569752f3eb69702e31a4a05eaf1830f8638a9937e0c4fefd930604"
ADAPTER_SHA256 = "2a827bb709d7b7116bbd59a52e890c62b3db62c3f5a41d2f371c709440a563cc"
SIGNAL_SHA256 = "02033accc39e20ade79ae5cd232445543203cf5cb8b3e48073d24613d46d0068"
RECEIPT_SHA256 = "f872a68ff676bbe0017062f341725c935a6e40fa80e96223011d38e556150ff9"
...
SIGNAL_INPUT = Path(
    ...
    "c134_tech_earnings_season_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C134 execution-month input packet")
    expected_contract = {
        "formation_sessions": 0,
        "missing_rule": "fail closed; no alternate calendar or shifted month boundary",
        "score_definition": (
            "Calendar month of the next accepted XNYS execution session; "
            "no price value enters the signal"
        ),
        "target_rule": (
            "January/April/July/October select QQQ; all other months select SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c134-execution-month-input-v1"
        ...
        raise InputBlockedError("C134 execution-month packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C134 execution-month signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C134 execution-month signal schema mismatch")
        value_fields = ("execution_month",)
        if any(
            ...
            raise InputBlockedError(
                "C134 execution-month value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 0
            or not 1 <= row["execution_month"] <= 12
        ):
            raise InputBlockedError("C134 formation or execution-month range mismatch")
        expected_state = (
            "QQQ" if row["execution_month"] in {1, 4, 7, 10} else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C134 execution-month target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C134 execution-month chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C134 execution-month signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C134 execution-month stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C134 execution-month state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C134 execution-month signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C134 execution-month state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C134 execution-month input packet")
    receipt = _json(
        receipt_payload,
        "C134 execution-month materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c134-execution-month-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C134 execution-month signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c134-execution-month-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c134-execution-month-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C134 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C134 holdout one-use state is not pristine")
