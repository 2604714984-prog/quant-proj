"""One-use runner for the frozen Cycle 23 UMCSENT SPY/QQQ rotation lane."""
...
from research.adapters.c23_umcsent_behavioral_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c23_umcsent_behavioral_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c23_umcsent_behavioral_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "48d7dc538ab1c91f2ec5ff7824ccf98880c5af44a86902f464da3b1f00856015"
ADAPTER_SHA256 = "57513444f0e43ab25b29c4605569aabaab163df8c943f023d3b60b3db5c2bce5"
SIGNAL_SHA256 = "57ff845c063dbe11927419b879d2dd52fcc31242839ed148a7ab3a82cb3d481d"
RECEIPT_SHA256 = "9b58516b9bc937271348cc8b2a63f8d061a7a3deb30aa39eb1e1ea6662852d61"
...
SIGNAL_INPUT = Path(
    ...
    "c23_umcsent_behavioral_rotation_spy_qqq_v1/"
    "c23_full_input_packet.json"
)
...
def _previous_month(value: date) -> date:
    if value.day != 1:
        raise InputBlockedError("UMCSENT reference month must begin on day one")
    if value.month == 1:
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "UMCSENT behavioral rotation input packet")
    if (
        record.get("schema_version")
        != "c23-umcsent-behavioral-rotation-spy-qqq-input-packet-v1"
    ...
    for row in rows:
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("UMCSENT selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "UMCSENT"
                or selection.get("source_class") != "OFFICIAL_ALFRED_UMCSENT"
                or selection.get("raw_response_sha256")
                != "0c7fd1c657f9c4838f71fff56744049dcc3f87b700a16fd1a67c22bebfae9c2a"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("UMCSENT source row identity mismatch")
        ...
        if any(
            ...
            raise InputBlockedError("UMCSENT values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("UMCSENT change arithmetic mismatch")
        ...
        previous_reference_month = _date(
            row.get("previous_reference_month"), "previous UMCSENT reference month"
        )
        current_reference_month = _date(
            row.get("current_reference_month"), "current UMCSENT reference month"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous UMCSENT available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current UMCSENT available_at",
            )
            > decision_at
            ...
            raise InputBlockedError(
                "UMCSENT reference-month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("UMCSENT signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("UMCSENT source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "UMCSENT row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("UMCSENT signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("UMCSENT state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("UMCSENT signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("UMCSENT state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c23-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "UMCSENT behavioral rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "UMCSENT behavioral rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "UMCSENT signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c23-umcsent-behavioral-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c23-umcsent-behavioral-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
