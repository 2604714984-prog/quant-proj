"""One-use runner for frozen C121 qqq-log-price-slope rotation."""
...
from research.adapters.c121_qqq_log_price_slope import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c121_qqq_log_price_slope_v1.json"
ADAPTER = ROOT / "research/adapters/c121_qqq_log_price_slope.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "9e9f91b4d04ca668855a7fd7ff52eda913a146243da8e5bdd8344630c89c8e71"
ADAPTER_SHA256 = "5d10d7bb2bcdf2bb3fe5e8d54516b0ba6d2f9d61a941db13f414636d84963e86"
SIGNAL_SHA256 = "d95ad5c82c8ba5b2abc3f4b44bd27bf269c69c94b3f83e6e796f511e079518de"
RECEIPT_SHA256 = "7c869bfcc0cae31c8c9dac7677529db5db7b72fb125935b4020b577d75cacfd7"
...
SIGNAL_INPUT = Path(
    ...
    "c121_qqq_63_session_log_price_slope_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C121 qqq-log-price-slope input packet")
    expected_contract = {
        "formation_sessions": 63,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "ordinary-least-squares slope of log QQQ adjusted close on integer "
            "time 0 through 62 over exactly 63 completed sessions"
        ),
        "target_rule": (
            "positive QQQ log-price slope selects QQQ; zero or negative selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c121-qqq-log-price-slope-input-v1"
        ...
        raise InputBlockedError("C121 qqq-log-price-slope packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C121 qqq-log-price-slope signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C121 qqq-log-price-slope signal schema mismatch")
        value_fields = ("qqq_log_price_slope",)
        if any(
            ...
            raise InputBlockedError(
                "C121 qqq-log-price-slope value is invalid"
            )
        if (
        ...
        expected_state = "QQQ" if row["qqq_log_price_slope"] > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C121 qqq-log-price-slope target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C121 qqq-log-price-slope chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C121 qqq-log-price-slope signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C121 qqq-log-price-slope stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C121 qqq-log-price-slope state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C121 qqq-log-price-slope signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C121 qqq-log-price-slope state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C121 qqq-log-price-slope input packet")
    receipt = _json(
        receipt_payload,
        "C121 qqq-log-price-slope materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c121-qqq-log-price-slope-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C121 qqq-log-price-slope signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c121-qqq-log-price-slope-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c121-qqq-log-price-slope-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
