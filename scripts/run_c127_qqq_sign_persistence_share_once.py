"""One-use runner for frozen C127 qqq-sign-persistence-share rotation."""
...
from research.adapters.c127_qqq_sign_persistence_share import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c127_qqq_sign_persistence_share_v1.json"
ADAPTER = ROOT / "research/adapters/c127_qqq_sign_persistence_share.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "40158330851e532d5db3c8495152a77020f5d2ca7ba66f3dc5589fa80e6f404a"
ADAPTER_SHA256 = "cb3af304edd494a4c2ce44558417a77db0cb5fe8332a22c74d91c385d5ce9cec"
SIGNAL_SHA256 = "b59681a4f525bd5de0f02aa78f849686e03b5a960a7264a5e9319c0e0bc84ec0"
RECEIPT_SHA256 = "ec3fc1bd01cc8e2964860630ab61613d990e560d4466030ec2419e452ba52a40"
...
SIGNAL_INPUT = Path(
    ...
    "c127_qqq_63_return_sign_persistence_share_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C127 qqq-sign-persistence-share input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Share of 62 adjacent return pairs with matching positive/nonpositive "
            "signs from 63 simple QQQ returns formed by exactly 64 adjusted closes"
        ),
        "target_rule": (
            "Sign-persistence share above 0.5 selects QQQ; 0.5 or below selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c127-qqq-sign-persistence-share-input-v1"
        ...
        raise InputBlockedError("C127 qqq-sign-persistence-share packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C127 qqq-sign-persistence-share signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C127 qqq-sign-persistence-share signal schema mismatch")
        value_fields = ("qqq_sign_persistence_share",)
        if any(
            ...
            raise InputBlockedError(
                "C127 qqq-sign-persistence-share value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or not 0.0 <= row["qqq_sign_persistence_share"] <= 1.0
        ):
            raise InputBlockedError("C127 formation or sign-persistence range mismatch")
        expected_state = "QQQ" if row["qqq_sign_persistence_share"] > 0.5 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C127 qqq-sign-persistence-share target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C127 qqq-sign-persistence-share chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C127 qqq-sign-persistence-share signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C127 qqq-sign-persistence-share stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C127 qqq-sign-persistence-share state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C127 qqq-sign-persistence-share signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C127 qqq-sign-persistence-share state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C127 qqq-sign-persistence-share input packet")
    receipt = _json(
        receipt_payload,
        "C127 qqq-sign-persistence-share materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c127-qqq-sign-persistence-share-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C127 qqq-sign-persistence-share signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c127-qqq-sign-persistence-share-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c127-qqq-sign-persistence-share-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C127 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C127 holdout one-use state is not pristine")
