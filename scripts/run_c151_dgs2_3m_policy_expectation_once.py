"""One-use runner for the frozen C151 DGS2 SPY/QQQ rotation lane."""
...
from research.adapters.c151_dgs2_3m_policy_expectation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c151_dgs2_3m_policy_expectation_v1.json"
ADAPTER = ROOT / "research/adapters/c151_dgs2_3m_policy_expectation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "275452ac03bc6dc58e9b338e20d7f91ef60a91f213c9c8b1b6d250c71d55a728"
ADAPTER_SHA256 = "b65afcf8ea3ebf74cdac7807eb51b389f775bb908ac9408f6690cca0b171215a"
SIGNAL_SHA256 = "f1583c926bc2bcd2c367dca78ea92cbb34f1aa42ef7cf86be03f3f253e7c7206"
RECEIPT_SHA256 = "f4507e3cf7899fe8d3f59afd1a5cdf72019c8d50215302e3989b667f5aa834ba"
...
SIGNAL_INPUT = Path(
    ...
    "c151_dgs2_3m_policy_expectation_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "DGS2 impulse rotation input packet")
    if (
        record.get("schema_version")
        != "c151-dgs2-spy-qqq-full-input-v1"
    ...
    for row in rows:
        ...
        if set(row) != required:
            raise InputBlockedError("DGS2 signal fields mismatch")
        numeric = tuple(
            row.get(key)
            for key in (
                "current_yield_percent",
        ...
        if any(
            ...
            raise InputBlockedError("DGS2 statistics must be finite")
        current_yield, comparison_yield, impulse = map(float, numeric)
        if not all(-10.0 < value < 100.0 for value in (current_yield, comparison_yield)):
            raise InputBlockedError("DGS2 yields are implausible")
        ...
        vintage = _date(row["actual_vintage_date"], "DGS2 actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "DGS2 response SHA-256"
        )
        if (
            ...
            or not math.isclose(
                impulse,
                current_yield - comparison_yield,
                rel_tol=0.0,
                abs_tol=1e-12,
            )
            ...
            raise InputBlockedError("DGS2 chronology or state mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("DGS2 signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("DGS2 source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "DGS2 row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("DGS2 signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("DGS2 state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("DGS2 signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("DGS2 state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c151-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "New York Fed DGS2/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "New York Fed DGS2/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c151-dgs2-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "DGS2 signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c151-nyfed-dgs2-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c151-nyfed-dgs2-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
