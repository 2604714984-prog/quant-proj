"""One-use runner for frozen C101 Cboe relative oil-equity volatility ratio."""
...
from research.adapters.c101_cboe_relative_oil_equity_volatility import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c101_cboe_relative_oil_equity_volatility_v1.json"
ADAPTER = ROOT / "research/adapters/c101_cboe_relative_oil_equity_volatility.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "50b4bd2d35be0d65cdabf7e053e20d57c28bbd824c175991edcc428095c9ea4a"
ADAPTER_SHA256 = "9bda01496b89d97978a24e019eeae13997e247977a73519217a04bb422dd8545"
SIGNAL_SHA256 = "bfc4e4bbbf936e51d8cf6e8b14ce3245fb1d43cd7d768452463acc986aa69484"
RECEIPT_SHA256 = "016abd4534c209e845d5f4259e885d668ca72a98015d0909502ea9441fd66d74"
...
SIGNAL_INPUT = Path(
    ...
    "c101_cboe_relative_oil_equity_volatility_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C101 relative oil-equity volatility input packet")
    expected_contract = {
        "current_rule": (
            "newest common positive finite official OVX and VIX "
        ...
        "ratio_definition": "OVX divided by VIX",
        "target_rule": (
            "strict ratio increase selects QQQ; "
            "equality or decrease selects SPY"
        ),
        "missing_rule": (
    ...
    if (
        ...
        != "c101-cboe-relative-oil-equity-volatility-input-v1"
        ...
        raise InputBlockedError("C101 relative oil-equity volatility packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C101 relative oil-equity volatility row count mismatch")
    expected_row_keys = {
        "decision_at",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C101 relative oil-equity volatility schema mismatch")
        numeric_fields = (
            "current_ratio",
            "current_vix",
            "current_ovx",
        ...
        if any(
            ...
            raise InputBlockedError("C101 relative oil-equity volatility value is invalid")
        if any(
            float(row[field]) <= 0.0
            for field in (
                "current_ratio",
            ...
            raise InputBlockedError("C101 relative oil-equity volatility input is nonpositive")
        if not math.isclose(
            float(row["current_ratio"]),
            float(row["current_ovx"]) / float(row["current_vix"]),
            rel_tol=1e-12,
            abs_tol=1e-15,
        ) or not math.isclose(
            float(row["prior_ratio"]),
            float(row["prior_ovx"]) / float(row["prior_vix"]),
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C101 ratio arithmetic mismatch")
        difference = float(row["current_ratio"]) - float(row["prior_ratio"])
        if not math.isclose(
            float(row["ratio_change"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C101 ratio-change arithmetic mismatch")
        expected_state = "QQQ" if difference > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C101 relative oil-equity volatility target mismatch")

        prior_observation = _date(
        ...
        if (
            not prior_observation < current_observation
            or current_observation > signal_session
            or prior_observation > current_observation - timedelta(days=28)
            ...
            raise InputBlockedError("C101 relative oil-equity volatility chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C101 relative oil-equity volatility identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C101 relative oil-equity volatility stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C101 relative oil-equity volatility support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C101 relative oil-equity volatility intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C101 relative oil-equity volatility support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C101 relative oil-equity volatility input packet")
    receipt = _json(
        receipt_payload,
        "C101 relative oil-equity volatility materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c101-cboe-relative-oil-equity-volatility-materialization-receipt-v1"
        ...
        or receipt.get("network_requests") != 1
        or receipt.get("network_retries") != 0
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C101 relative oil-equity volatility signal does not bind "
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c101-cboe-relative-oil-equity-volatility-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c101-cboe-relative-oil-equity-volatility-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C101 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C101 holdout one-use state is not pristine")
