"""One-use runner for the frozen Cycle 50 high-beta/low-volatility SPY/QQQ lane."""
...
from research.adapters.c50_high_beta_low_vol_risk_appetite_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c50_high_beta_low_vol_risk_appetite_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c50_high_beta_low_vol_risk_appetite_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "69ad8e62ec2ca5139da9ca9ffdb29adf5e3b804731eeb0ec6bd068f4accbb6d1"
ADAPTER_SHA256 = "8dbd7396e004d5f35867d38bff7ff273d94dcc996709bc61f66c0764d981d03c"
SIGNAL_SHA256 = "2e87b2ed9a7011dcfe73705be79ff8a2cc912406a844b23a685bdb6c05af1f34"
RECEIPT_SHA256 = "5983e11debba36adee090cfff8d51542c99e8efde657740fda11f845720bd625"
...
SIGNAL_INPUT = Path(
    ...
    "c50_high_beta_low_vol_risk_appetite_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "high beta low volatility rotation input packet")
    expected_contract = {
        "left_symbol": "SPHB",
        "right_symbol": "SPLV",
        "return_window": (
        ...
        "target_rule": (
            "strict SPHB prior-month total-return outperformance versus SPLV "
    ...
    if (
        ...
        != "epoch8-style-leadership-input-packet-v1"
        ...
        raise InputBlockedError("high beta low volatility packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("high beta low volatility signal row count mismatch")
    ...
    expected_source = {
        "SPHB": (
            "https://query1.finance.yahoo.com/v8/finance/chart/SPHB?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "617a1349f85cc158c49112ea6a7544ba98fed14fd462da81d7e05e43daa650ae",
        ),
        "SPLV": (
            "https://query1.finance.yahoo.com/v8/finance/chart/SPLV?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "39de8396e02d2c8479f7a5c219250f6a18b531f47709f4048a551f516c44d354",
        ),
    }
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "high beta low volatility rotation input packet")
    receipt = _json(
        receipt_payload,
        "high beta low volatility materialization receipt",
    )
    ...
    if (
        ...
        != "epoch8-style-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "SPHB"
        or receipt.get("right_symbol") != "SPLV"
        or receipt.get("raw_fetch_receipt_sha256")
        != "3da67e92612f2ab10e443f753a4777df92fa450533516f589977be3031d14000"
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c50-high-beta-low-vol-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c50-high-beta-low-vol-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
