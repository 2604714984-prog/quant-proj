"""One-use runner for the frozen Cycle 24 PERMIT SPY/QQQ rotation lane."""
...
from research.adapters.c24_permit_housing_financing_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c24_permit_housing_financing_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c24_permit_housing_financing_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "dcd53908c0c5fc92ac7a09a0a005d21982b023b5c0a812025ced8e68f35a47f2"
ADAPTER_SHA256 = "76e91c107a19a3ca689f67b668f08089cf3a6576556b14b5e5354c0840705d08"
SIGNAL_SHA256 = "9d640c45d1c53e74d77eac667624766e886f0d0dd3dd178e1f80fe703bcd2af6"
RECEIPT_SHA256 = "a99937126feb67458656ecd9a14fbafd1d56be13f411fa7fa65dee4cfb6d5627"
...
SIGNAL_INPUT = Path(
    ...
    "c24_permit_housing_financing_rotation_spy_qqq_v1/"
    "c24_full_input_packet.json"
)
...
def _previous_month(value: date) -> date:
    if value.day != 1:
        raise InputBlockedError("PERMIT reference month must begin on day one")
    if value.month == 1:
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "PERMIT housing-financing rotation input packet")
    if (
        record.get("schema_version")
        != "c24-permit-housing-financing-rotation-spy-qqq-input-packet-v1"
        ...
        != {
            ...
            "threshold_thousands_saar": 0.0,
        }
    ):
        raise InputBlockedError("PERMIT housing-financing rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("PERMIT signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("PERMIT signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("PERMIT selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "PERMIT"
                or selection.get("source_class") != "OFFICIAL_ALFRED_PERMIT"
                or selection.get("raw_response_sha256")
                != "3a111a593b7cdff338a8c06c690e2a8a6ac5d8086fecabe019795def7509bda5"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("PERMIT source row identity mismatch")
        current_value = current.get("value_thousands_saar")
        previous_value = previous.get("value_thousands_saar")
        change = row.get("delta_thousands_saar")
        if any(
            ...
            raise InputBlockedError("PERMIT values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("PERMIT change arithmetic mismatch")
        ...
        previous_reference_month = _date(
            row.get("previous_reference_month"), "previous PERMIT reference month"
        )
        current_reference_month = _date(
            row.get("current_reference_month"), "current PERMIT reference month"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous PERMIT available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current PERMIT available_at",
            )
            > decision_at
            ...
            or previous_reference_month != _previous_month(current_reference_month)
        ):
            raise InputBlockedError(
                "PERMIT reference-month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "PERMIT row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("PERMIT signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("PERMIT state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("PERMIT signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("PERMIT state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c24-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "PERMIT housing-financing rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "PERMIT housing-financing rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "PERMIT signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c24-permit-housing-financing-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c24-permit-housing-financing-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 24 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 24 holdout one-use state is not pristine")
