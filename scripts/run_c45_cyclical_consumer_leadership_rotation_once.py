"""One-use runner for the frozen Cycle 45 cyclical consumer leadership SPY/QQQ lane."""
...
from research.adapters.c45_cyclical_consumer_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c45_cyclical_consumer_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c45_cyclical_consumer_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "4bdacabb2e0b6f23134635c9c777f0332b026a17dd07a495574716b432bed5c9"
ADAPTER_SHA256 = "d17d32e508e3ea460434bb77faeb73ec01bc1ba6abcc973f109b590935e8890b"
SIGNAL_SHA256 = "3941f2b76470370abf2156fe21b2038568f591c33220ec23a8009e4e4561ab5f"
RECEIPT_SHA256 = "cab9664b9dad291b9e62353505b4f019d34af58b7c7131423a03d507facbd57f"
...
SIGNAL_INPUT = Path(
    ...
    "c45_cyclical_consumer_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "cyclical consumer leadership rotation input packet")
    expected_contract = {
        "left_symbol": "XLY",
        "right_symbol": "XLP",
        "return_window": (
        ...
        "target_rule": (
            "strict XLY prior-month total-return outperformance versus XLP "
    ...
    if (
        ...
        != "epoch6-intermarket-leadership-input-packet-v1"
        ...
        raise InputBlockedError("cyclical consumer leadership packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("cyclical consumer leadership signal row count mismatch")
    ...
    expected_source = {
        "XLY": (
            "https://query1.finance.yahoo.com/v8/finance/chart/XLY?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "90634aa219442460766558ad2671566f0c166ddde2884dffd3439e933f64de1b",
        ),
        "XLP": (
            "https://query1.finance.yahoo.com/v8/finance/chart/XLP?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "aa97599f4475bb2bbde38abaa48eaa864e50029120153310953f572eb55d5326",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("cyclical consumer leadership signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("cyclical consumer leadership selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "cyclical consumer leadership adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("cyclical consumer leadership source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "cyclical consumer leadership cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("cyclical consumer leadership arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "cyclical consumer leadership month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("cyclical consumer leadership signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("cyclical consumer leadership signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(
                    ...
                    "cyclical consumer leadership row SHA-256",
                ),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("cyclical consumer leadership signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("cyclical consumer leadership state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "cyclical consumer leadership signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("cyclical consumer leadership state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "cyclical consumer leadership rotation input packet")
    receipt = _json(
        receipt_payload,
        "cyclical consumer leadership materialization receipt",
    )
    ...
    if (
        ...
        != "epoch6-intermarket-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "XLY"
        or receipt.get("right_symbol") != "XLP"
        or receipt.get("raw_fetch_receipt_sha256")
        != "d9a026d5b3f08fa43e5d4319078c7cb2594eef83b526fb736299e838f6f1b0e4"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "cyclical consumer leadership signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c45-cyclical-consumer-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c45-cyclical-consumer-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 45 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 45 holdout one-use state is not pristine")
