"""One-use runner for frozen C125 qqq-lag1-autocorrelation rotation."""
...
from research.adapters.c125_qqq_lag1_autocorrelation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c125_qqq_lag1_autocorrelation_v1.json"
ADAPTER = ROOT / "research/adapters/c125_qqq_lag1_autocorrelation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "79da2bccf2c875e064cb04558802b4a853e91e6844dcc2c756d7981459bb5785"
ADAPTER_SHA256 = "8c9db6dfea6879255af3182e0cea2fa765cac915ac5da8e4906a48081069539a"
SIGNAL_SHA256 = "1d6f5b99fc36fc2fa6edd447749cc9c1b2fcc664c55e149bed5743766bca6947"
RECEIPT_SHA256 = "820ca3dbae8bc4d6495ceced9491843143b31a72e0125728abd2385257978041"
...
SIGNAL_INPUT = Path(
    ...
    "c125_qqq_63_return_lag1_autocorrelation_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C125 qqq-lag1-autocorrelation input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Pearson correlation between the first 62 and following 62 values of "
            "63 simple QQQ returns formed by exactly 64 adjusted closes"
        ),
        "target_rule": (
            "Strictly positive lag-one autocorrelation selects QQQ; zero or negative selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c125-qqq-lag1-autocorrelation-input-v1"
        ...
        raise InputBlockedError("C125 qqq-lag1-autocorrelation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C125 qqq-lag1-autocorrelation signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C125 qqq-lag1-autocorrelation signal schema mismatch")
        value_fields = ("qqq_lag1_autocorrelation",)
        if any(
            ...
            raise InputBlockedError(
                "C125 qqq-lag1-autocorrelation value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or not -1.0 <= row["qqq_lag1_autocorrelation"] <= 1.0
        ):
            raise InputBlockedError("C125 formation or autocorrelation range mismatch")
        expected_state = "QQQ" if row["qqq_lag1_autocorrelation"] > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C125 qqq-lag1-autocorrelation target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C125 qqq-lag1-autocorrelation chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C125 qqq-lag1-autocorrelation signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C125 qqq-lag1-autocorrelation stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C125 qqq-lag1-autocorrelation state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C125 qqq-lag1-autocorrelation signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C125 qqq-lag1-autocorrelation state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C125 qqq-lag1-autocorrelation input packet")
    receipt = _json(
        receipt_payload,
        "C125 qqq-lag1-autocorrelation materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c125-qqq-lag1-autocorrelation-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C125 qqq-lag1-autocorrelation signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c125-qqq-lag1-autocorrelation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c125-qqq-lag1-autocorrelation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C125 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C125 holdout one-use state is not pristine")
