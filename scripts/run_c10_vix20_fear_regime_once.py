"""One-use runner for the frozen Cycle 10 VIX20 fear-regime lane."""
...
from research.adapters.c10_vix20_fear_regime import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c10_vix20_fear_regime_v1.json"
ADAPTER = ROOT / "research/adapters/c10_vix20_fear_regime.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "d3927e200bf67896e238738005483f0f36ee01437f7f44540adb0c04ab6dbe9d"
ADAPTER_SHA256 = "2f9062980b687bdcfeeccee64847140916e2308622bdd5b3910ae296814def43"
SIGNAL_SHA256 = "a180ee203a19dcdcba9afd015f3a81d6632b24d74f6d6fde039f2fee07352ed9"
RECEIPT_SHA256 = "b0bc6168b9c91ed310e10b8bf69bae92a93e06091e0faba84ca2a5520e6181c0"
...
SIGNAL_INPUT = Path(
    ...
    "c10_vix_20_fear_regime_spy_cash_v1/"
    "vix20_monthly_level_signal_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "VIX20 signal packet")
    if (
        record.get("schema_version")
        != "c10-vix20-monthly-level-signal-packet-v1"
        ...
        raise InputBlockedError("VIX20 signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("VIX20 signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("VIX20 signal stage mismatch")
        selection = row.get("selection")
        if type(selection) is not dict:
            raise InputBlockedError("VIX20 selection is required")
        selected_value = selection.get("close")
        if (
            isinstance(selected_value, bool)
            or not isinstance(selected_value, (int, float))
            or not math.isfinite(selected_value)
            or selected_value <= 0.0
        ):
            raise InputBlockedError("VIX20 values must be finite and positive")
        if row.get("threshold") != 20.0:
            raise InputBlockedError("VIX20 threshold mismatch")
        expected_state = "SPY" if selected_value <= 20.0 else "CASH"
        if (
            row.get("state") != expected_state
        ):
            raise InputBlockedError("VIX20 signal arithmetic mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(selection["row_hash"], "VIX20 row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("VIX20 signal split count mismatch")
    for group in output.values():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("VIX20 signal intervals are not contiguous")
    return output
...
def _load_bundle(payload: bytes, *, stage: str, signals: tuple[Signal, ...]) -> Bundle:
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError("VIX20 signal does not bind the SPY execution schedule")
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c10-vix20-fear-regime-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c10-vix20-fear-regime-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
