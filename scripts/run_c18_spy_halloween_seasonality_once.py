"""One-use runner for the frozen Cycle 18 SPY Halloween lane."""
...
from research.adapters.c18_spy_halloween_seasonality import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c18_spy_halloween_seasonality_v1.json"
ADAPTER = ROOT / "research/adapters/c18_spy_halloween_seasonality.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "6374002b816a256deb8cc60bcd36c64a904e1efde9328d27976301c44b1b0c91"
ADAPTER_SHA256 = "1d7979b62bd2c051581a4cab262fae4ef380252bcf76b60467b8b47278f9cbea"
SIGNAL_SHA256 = "e20d59b3dcda61e592292c3b502a88073367d146867c6aa09cea01b48496f4f9"
RECEIPT_SHA256 = "93016a837b6221b3ad804a5fb403913802db53c90b4c38d4975cf771fc10d4f5"
...
SIGNAL_INPUT = Path(
    ...
    "c18_spy_halloween_seasonality_v1/"
    "halloween_signal_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Halloween signal packet")
    if (
        record.get("schema_version")
        != "c18-spy-halloween-signal-packet-v1"
        ...
        or record.get("source_hashes")
        != {
            "validation": VALIDATION_BUNDLE_SHA256,
            "retrospective_holdout": HOLDOUT_BUNDLE_SHA256,
        }
    ):
        raise InputBlockedError("Halloween signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("Halloween signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("Halloween signal stage mismatch")
        ...
        expected_state = "SPY" if start.month in {11, 12, 1, 2, 3, 4} else "CASH"
        expected_bundle = (
            VALIDATION_BUNDLE_SHA256
            if row["stage"] == "validation"
            else HOLDOUT_BUNDLE_SHA256
        )
        identity = row.get("source_identity")
        expected_identity = {
            "rule_id": "FIXED_NOVEMBER_THROUGH_APRIL_SPY",
            "holding_start_execution_session": start.isoformat(),
            "execution_month": start.month,
            "target": expected_state,
            "spy_runtime_bundle_sha256": expected_bundle,
        }
        if identity != expected_identity or row.get("target") != expected_state:
            raise InputBlockedError("Halloween signal arithmetic mismatch")
        expected_row_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("row_sha256") != expected_row_hash:
            raise InputBlockedError("Halloween source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                row["stage"],
                start,
                end,
                _datetime(row["signal_decision_at"], "signal decision_at"),
                1.0 if expected_state == "SPY" else 0.0,
                _sha(row["row_sha256"], "Halloween row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("Halloween signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("Halloween state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("Halloween signal intervals are not contiguous")
        states = tuple("SPY" if row.target == 1.0 else "CASH" for row in group)
        observed_support = {
            "complete_intervals": len(states),
            "spy_intervals": states.count("SPY"),
            "cash_intervals": states.count("CASH"),
            "spy_to_cash": sum(
        ...
        if support.get(stage) != observed_support:
            raise InputBlockedError("Halloween state support mismatch")
    return output
...
def _load_bundle(payload: bytes, *, stage: str, signals: tuple[Signal, ...]) -> Bundle:
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError("Halloween signal does not bind the SPY execution schedule")
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c18-spy-halloween-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c18-spy-halloween-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 18 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 18 holdout one-use state is not pristine")
