"""One-use runner for the frozen C159 CFNAIDIFF SPY/QQQ rotation lane."""
...
from research.adapters.c159_cfnai_diffusion_state import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c159_cfnai_diffusion_state_v1.json"
ADAPTER = ROOT / "research/adapters/c159_cfnai_diffusion_state.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "783dc01e11a2b3ab13923ddb779f309f6b5e38114d596c69c2626b73ccd37281"
ADAPTER_SHA256 = "b951f490464d17cb8b05d6fdd9437e1cff6e3dcd0e99e5146fb2a8ed8be0e93d"
SIGNAL_SHA256 = "25cd3006d714721a393bd137cf3be627acb2e8a3752dca290a7f454e8d49e456"
RECEIPT_SHA256 = "e19c57c752c399203e4a2b00d16bd9d5614a4c4632d2bf22c1c379f1cebdd538"
...
SIGNAL_INPUT = Path(
    ...
    "c159_cfnai_diffusion_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "CFNAIDIFF rotation input packet")
    if (
        record.get("schema_version")
        != "c159-cfnaidiff-spy-qqq-full-input-v1"
        ...
        != {
            "series_id": "CFNAIDIFF",
            "target_rule": "latest official-vintage CFNAI diffusion index > 0 means QQQ; otherwise SPY",
            "threshold": 0.0,
        }
    ):
        raise InputBlockedError("CFNAIDIFF rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("CFNAIDIFF signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    required = {
        "actual_vintage_date",
        "current_cfnai_diffusion_index",
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("CFNAIDIFF signal stage mismatch")
        if set(row) != required:
            raise InputBlockedError("CFNAIDIFF signal fields mismatch")
        value = row.get("current_cfnai_diffusion_index")
        if (
            isinstance(value, bool)
            or not isinstance(value, (int, float))
            or not math.isfinite(float(value))
            or not -100.0 < float(value) < 100.0
        ):
            raise InputBlockedError("CFNAIDIFF value is invalid")
        current_value = float(value)
        expected_state = "QQQ" if current_value > 0.0 else "SPY"
        ...
        vintage = _date(row["actual_vintage_date"], "CFNAIDIFF actual vintage")
        source_response_sha = _sha(
            row["source_response_sha256"], "CFNAIDIFF response SHA-256"
        )
        if (
            ...
            raise InputBlockedError("CFNAIDIFF chronology or state mismatch")
        identity = {
            "actual_vintage_date": vintage.isoformat(),
            "current_cfnai_diffusion_index": current_value,
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("CFNAIDIFF signal identity mismatch")
        ...
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("CFNAIDIFF source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "CFNAIDIFF row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("CFNAIDIFF signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("CFNAIDIFF state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("CFNAIDIFF signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("CFNAIDIFF state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c159-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "Chicago Fed CFNAIDIFF/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "Chicago Fed CFNAIDIFF/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c159-cfnaidiff-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "CFNAIDIFF signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c159-cfnaidiff-state-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c159-cfnaidiff-state-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
