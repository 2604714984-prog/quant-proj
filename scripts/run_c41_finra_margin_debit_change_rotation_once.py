"""One-use runner for the frozen Cycle 41 FINRA margin debit SPY/QQQ lane."""
...
from research.adapters.c41_finra_margin_debit_change_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c41_finra_margin_debit_change_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c41_finra_margin_debit_change_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "17a882badebecec3c93ef2c25ed6b810a547d57a619e4d638c96c26c73111868"
ADAPTER_SHA256 = "5cdee1637bfb0f008b4af34b2046568af9588050b13e180a7ce8e55dd79fba1e"
SIGNAL_SHA256 = "9da26a22d3302121008d221049d96f033237506f3570facde38e2f4b70a100e7"
RECEIPT_SHA256 = "ee4d7bb53f4db614969f17642558fc53609a196d6b1614a67f296846e8f49525"
...
SIGNAL_INPUT = Path(
    ...
    "c41_finra_margin_debit_change_rotation_spy_qqq_v1/"
    "c41_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "FINRA margin debit rotation input packet")
    expected_contract = {
        "availability_rule": (
            "final calendar day of following month at 23:59:59 "
            "America/New_York"
        ),
        "series_id": "FINRA_CUSTOMER_MARGIN_DEBIT_BALANCE",
        "target_rule": (
            "strict increase in consecutive eligible monthly debit balances "
            "selects QQQ; equal or decrease selects SPY"
        ),
        "threshold_usd_millions": 0,
    }
    if (
        record.get("schema_version")
        != "c41-finra-margin-debit-change-spy-qqq-input-packet-v1"
        ...
        raise InputBlockedError("FINRA packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("FINRA signal row count mismatch")
    ...
    expected_url = (
        "https://www.finra.org/sites/default/files/2021-03/"
        "margin-statistics.xlsx"
    )
    expected_xlsx_sha = "821d0a247cb646d38312994ac9a709a7a7c5cac32b9aa6c6d05c7eda4007331c"
    expected_availability_rule = (
        "23:59:59 America/New_York on the final calendar day of the month "
        "following the reference month"
    )
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("FINRA signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("FINRA selections are required")
        for selection in (previous, current):
            ...
            value = selection.get("value_usd_millions")
            if (
                selection.get("series_id")
                != "FINRA_CUSTOMER_MARGIN_DEBIT_BALANCE"
                or selection.get("source_class")
                != "OFFICIAL_FINRA_MONTHLY_AGGREGATE"
                or selection.get("source_page") != expected_page
                or selection.get("source_url") != expected_url
                or selection.get("raw_xlsx_sha256") != expected_xlsx_sha
                ...
                raise InputBlockedError("FINRA source row identity mismatch")
        current_value = current["value_usd_millions"]
        previous_value = previous["value_usd_millions"]
        change = row.get("delta_usd_millions")
        if (
            isinstance(change, bool)
            or not isinstance(change, int)
            or change != current_value - previous_value
        ):
            raise InputBlockedError("FINRA change arithmetic mismatch")
        expected_state = "QQQ" if change > 0 else "SPY"
        ...
        current_reference_month = _date(
            row.get("current_reference_month"), "current FINRA reference month"
        )
        previous_reference_month = _date(
            row.get("previous_reference_month"), "previous FINRA reference month"
        )

        def frozen_available_at(reference_month: date) -> datetime:
            if reference_month.month == 12:
                following = date(reference_month.year + 1, 1, 1)
            else:
                following = date(
                    reference_month.year, reference_month.month + 1, 1
                )
            if following.month == 12:
                after_following = date(following.year + 1, 1, 1)
            else:
                after_following = date(
                    following.year, following.month + 1, 1
                )
            final_day = after_following - timedelta(days=1)
            return datetime.combine(final_day, time(23, 59, 59), NY)

        current_available_at = _datetime(
            current["available_at_conservative"], "current FINRA available_at"
        )
        previous_available_at = _datetime(
            previous["available_at_conservative"], "previous FINRA available_at"
        )
        if (
            _date(current.get("reference_month"), "current reference month")
            != current_reference_month
            or _date(previous.get("reference_month"), "previous reference month")
            != previous_reference_month
            or not _prior_calendar_month_is_valid(
                previous_reference_month, current_reference_month
            )
            or current_available_at != frozen_available_at(current_reference_month)
            or previous_available_at
            != frozen_available_at(previous_reference_month)
            or current_available_at > decision_at
            or previous_available_at > decision_at
        ):
            raise InputBlockedError(
                "FINRA reference-month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "FINRA row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("FINRA signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("FINRA state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("FINRA signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("FINRA state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "FINRA margin debit rotation input packet")
    receipt = _json(
        receipt_payload,
        "FINRA margin debit materialization receipt",
    )
    ...
    if (
        ...
        != "c41-finra-margin-full-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "FINRA signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c41-finra-margin-debit-change-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c41-finra-margin-debit-change-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 41 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 41 holdout one-use state is not pristine")
