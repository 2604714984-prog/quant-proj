"""One-use runner for the frozen Cycle 34 CFTC positioning SPY/QQQ lane."""
...
from research.adapters.c34_cftc_relative_equity_positioning_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c34_cftc_relative_equity_positioning_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c34_cftc_relative_equity_positioning_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "0e4ff699ac5ce44d4fda91dc3556eff95f57681fb1b299502a6d5f0dfde8a752"
ADAPTER_SHA256 = "b7e8aab3ecd84f2d963721701c877cad2fd57f27513fe0f61ad99ae50940baee"
SIGNAL_SHA256 = "66dbfd8714ce4226ac50223a925244a53010967c6d7f7e46a801f5e8518e55e4"
RECEIPT_SHA256 = "ba43a42d26a39141478a3d912dc83c78a5066513d49440e38e84aee5b67ea55b"
...
SIGNAL_INPUT = Path(
    ...
    "c34_cftc_equity_index_positioning/"
    "c34_full_input_packet.json"
)
...
CFTC_SP500_CODE = "13874+"
CFTC_NASDAQ100_CODE = "20974+"
CFTC_AVAILABILITY_RULE = (
    "ordinary report date plus ten calendar days at end-of-day "
    "America/New_York; documented later CFTC special-announcement "
    "publication date controls"
)
CFTC_RAW_SHA_BY_YEAR = {
    2020: "c90d98d94db708772d669f263fb8061c75f8a7f681ba231fb21247e5c9798e6b",
    2021: "5c929991038c70ae2501661b6d442b9e784d3c29d3166ceac2e21b45ac93d56a",
    2022: "94c9c1fdee9dfbe377a09923ddfe26b88d3460605dd076081f221ad367d88601",
    2023: "43c3dbdd4d01fceefad5e457003751cdc0974c7cf46097b322e26e9e2fe44d7a",
    2024: "3f00585741a7cf76c207df640951d644dd91ff22a1106b38ca37a2469c765637",
    2025: "2ea0cda6395f7dd6501c27422be3763e1f2f5b41b768cfd36b871f092a07d438",
    2026: "db36c1e1a727610d5ad896f38ec1081d3b8f503b74fecc63bc7c4e5d7fce7211",
}
CFTC_SPECIAL_RELEASE_DATES = {
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "CFTC positioning rotation input packet")
    if (
        record.get("schema_version")
        != "c34-cftc-relative-equity-positioning-spy-qqq-input-v1"
        ...
        or record.get("signal_contract")
        != {
            "nasdaq100_contract_code": CFTC_NASDAQ100_CODE,
            "sp500_contract_code": CFTC_SP500_CODE,
            "target_rule": "delta of relative asset-manager net percent of open interest strictly above zero means QQQ; equal or below zero means SPY",
            "threshold": 0.0,
        }
    ):
        raise InputBlockedError("CFTC positioning packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("CFTC signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    prior_decision_by_stage: dict[str, datetime] = {
        "validation": datetime(2020, 12, 31, 20, 5, tzinfo=NY),
        "retrospective_holdout": datetime(2023, 6, 30, 20, 5, tzinfo=NY),
    }
    prior_selection_by_stage: dict[str, str | None] = {
        "validation": None,
        "retrospective_holdout": None,
    }
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("CFTC signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("CFTC selections are required")
        for selection in (previous, current):
            report_date = _date(selection.get("report_date"), "CFTC report date")
            available_at = _datetime(
                selection.get("available_at_conservative"),
                "CFTC available_at",
            )
            ordinary_release = report_date + timedelta(days=10)
            expected_release = max(
                ordinary_release,
                CFTC_SPECIAL_RELEASE_DATES.get(report_date, ordinary_release),
            )
            if (
                selection.get("availability_rule") != CFTC_AVAILABILITY_RULE
                or available_at.astimezone(NY).date() != expected_release
                or available_at.astimezone(NY).time() != time.max
            ):
                raise InputBlockedError("CFTC availability identity mismatch")
            contract_specs = (
                (
                    "sp500_contract_row",
                    "sp500_contract_row_sha256",
                    "sp500_asset_manager_net_pct_oi",
                    CFTC_SP500_CODE,
                    "S&P 500 Consolidated - CHICAGO MERCANTILE EXCHANGE",
                ),
                (
                    "nasdaq100_contract_row",
                    "nasdaq100_contract_row_sha256",
                    "nasdaq100_asset_manager_net_pct_oi",
                    CFTC_NASDAQ100_CODE,
                    "NASDAQ-100 Consolidated - CHICAGO MERCANTILE EXCHANGE",
                ),
            )
            ratios: dict[str, float] = {}
            for row_key, hash_key, ratio_key, code, market_name in contract_specs:
                contract = selection.get(row_key)
                if type(contract) is not dict:
                    raise InputBlockedError("CFTC contract row is required")
                raw_identity = {
                    key: value for key, value in contract.items() if key != "row_sha256"
                }
                long_value = contract.get("asset_manager_long_all")
                short_value = contract.get("asset_manager_short_all")
                open_interest = contract.get("open_interest_all")
                if any(
                    type(value) is not int
                    for value in (long_value, short_value, open_interest)
                ) or open_interest <= 0:
                    raise InputBlockedError("CFTC positions and open interest must be integers")
                expected_row_hash = _hash_bytes(
                    json.dumps(raw_identity, sort_keys=True, separators=(",", ":")).encode()
                )
                if (
                    contract.get("report_date") != report_date.isoformat()
                    or contract.get("cftc_contract_market_code") != code
                    or contract.get("market_and_exchange_name") != market_name
                    or contract.get("futures_only_or_combined") != "FutOnly"
                    or contract.get("source_class")
                    != "OFFICIAL_CFTC_TFF_FUTURES_ONLY_HISTORICAL_COMPRESSED"
                    or contract.get("raw_response_sha256")
                    != CFTC_RAW_SHA_BY_YEAR.get(report_date.year)
                    or contract.get("row_sha256") != expected_row_hash
                    or selection.get(hash_key) != expected_row_hash
                ):
                    raise InputBlockedError("CFTC contract row identity mismatch")
                observed_ratio = selection.get(ratio_key)
                expected_ratio = (long_value - short_value) / open_interest
                if (
                    isinstance(observed_ratio, bool)
                    or not isinstance(observed_ratio, (int, float))
                    or not math.isfinite(float(observed_ratio))
                    or not math.isclose(
                        float(observed_ratio), expected_ratio, rel_tol=0, abs_tol=1e-15
                    )
                ):
                    raise InputBlockedError("CFTC net-position ratio mismatch")
                ratios[ratio_key] = expected_ratio
            relative = selection.get("relative_asset_manager_net_pct_oi")
            expected_relative = (
                ratios["nasdaq100_asset_manager_net_pct_oi"]
                - ratios["sp500_asset_manager_net_pct_oi"]
            )
            if (
                ...
                or not math.isclose(
                    float(relative), expected_relative, rel_tol=0, abs_tol=1e-15
                )
            ):
                raise InputBlockedError("CFTC relative-position ratio mismatch")
            selection_identity = {
                key: value
            ...
            if selection.get("selection_row_sha256") != _hash_bytes(
                json.dumps(
                    selection_identity, sort_keys=True, separators=(",", ":")
                ).encode()
            ):
                raise InputBlockedError("CFTC selection identity mismatch")
        current_value = current.get("relative_asset_manager_net_pct_oi")
        previous_value = previous.get("relative_asset_manager_net_pct_oi")
        change = row.get("delta_relative_asset_manager_net_pct_oi")
        if any(
            ...
            raise InputBlockedError("CFTC positioning values must be finite")
        expected_change = float(current_value) - float(previous_value)
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("CFTC positioning change arithmetic mismatch")
        ...
        prior_cutoff = _datetime(row.get("prior_monthly_cutoff"), "prior monthly cutoff")
        previous_report_date = _date(
            row.get("previous_report_date"), "previous CFTC report date"
        )
        current_report_date = _date(
            row.get("current_report_date"), "current CFTC report date"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous CFTC available_at",
            )
            > prior_cutoff
            or _datetime(
                current["available_at_conservative"],
                "current CFTC available_at",
            )
            > decision_at
            or _date(previous.get("report_date"), "previous report date")
            != previous_report_date
            or _date(current.get("report_date"), "current report date")
            != current_report_date
            or prior_cutoff != prior_decision_by_stage[row["stage"]]
            or (
                prior_selection_by_stage[row["stage"]] is not None
                and previous.get("selection_row_sha256")
                != prior_selection_by_stage[row["stage"]]
            )
        ):
            raise InputBlockedError("CFTC report identity or availability mismatch")
        identity = {
            "stage": row["stage"],
            "holding_start_execution_session": start.isoformat(),
            "holding_end_execution_session": end.isoformat(),
            "decision_at": row["decision_at"],
            "prior_monthly_cutoff": row["prior_monthly_cutoff"],
            "previous_report_date": previous_report_date.isoformat(),
            "current_report_date": current_report_date.isoformat(),
            "previous_selection_row_sha256": previous.get("selection_row_sha256"),
            "current_selection_row_sha256": current.get("selection_row_sha256"),
            "delta_relative_asset_manager_net_pct_oi": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("CFTC signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("CFTC signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "CFTC signal SHA-256"),
            )
        )
        prior_decision_by_stage[row["stage"]] = decision_at
        prior_selection_by_stage[row["stage"]] = current["selection_row_sha256"]
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("CFTC signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("CFTC state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("CFTC signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("CFTC state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c34-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "CFTC positioning rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "CFTC positioning rotation/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c34-cftc-relative-equity-positioning-full-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "CFTC signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c34-cftc-relative-equity-positioning-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c34-cftc-relative-equity-positioning-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 34 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 34 holdout one-use state is not pristine")
