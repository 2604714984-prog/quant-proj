"""One-use runner for frozen C83 relative-ratio trend."""
...
from research.adapters.c83_relative_ratio_200_session_trend_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c83_relative_ratio_200_session_trend_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c83_relative_ratio_200_session_trend_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "2f2442344e6cfe55fe76954cde398d4612a2d22e1aa848955ad491ab831b95a7"
ADAPTER_SHA256 = "68b1bf62903033bb5895e6d2ecb9e741109a8f3c6a4d991ce46c4288ec8e679d"
SIGNAL_SHA256 = "e788451db863cc9b26f1cad4917e6125ad6444b8d161624eaa6cde639a717d49"
RECEIPT_SHA256 = "1a99d86964a125119f6e4306baca353a38061ea521075c410c9cdff3462c2466"
...
SIGNAL_INPUT = Path(
    ...
    "c83_relative_ratio_200_session_trend_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C83 relative-ratio trend input packet")
    expected_contract = {
        "formation_close_count": 201,
        "ratio": (
            "natural log of QQQ adjusted close divided by SPY adjusted close"
        ),
        "reference_ratio_count": 200,
        "reference": "arithmetic mean of the first 200 log ratios",
        "target_rule": (
            "final log ratio strictly above the reference means QQQ; equality "
            "or lower means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        ...
        != "c83-relative-ratio-200-session-trend-input-v1"
        ...
        raise InputBlockedError("C83 relative-ratio packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C83 relative-ratio signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        ...
        "signal_session",
        "stage",
        "reference_mean_log_ratio",
        "reference_ratio_count",
        "relative_trend_distance",
        "target_symbol",
    }
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C83 relative-ratio signal schema mismatch")
        numeric_fields = (
            "reference_mean_log_ratio",
            "final_log_ratio",
            "relative_trend_distance",
        ...
        if any(
            ...
            raise InputBlockedError("C83 relative-ratio value is invalid")
        if (
            type(row["reference_ratio_count"]) is not int
            or row["reference_ratio_count"] != 200
        ):
            raise InputBlockedError("C83 reference ratio count mismatch")

        reference = float(row["reference_mean_log_ratio"])
        final_ratio = float(row["final_log_ratio"])
        distance = final_ratio - reference
        if not math.isclose(
            float(row["relative_trend_distance"]),
            distance,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C83 relative-ratio arithmetic mismatch")
        expected_state = "QQQ" if distance > 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C83 relative-ratio target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C83 relative-ratio chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C83 relative-ratio signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C83 relative-ratio stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C83 relative-ratio state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C83 relative-ratio signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C83 relative-ratio state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C83 relative-ratio input packet")
    receipt = _json(
        receipt_payload,
        "C83 relative-ratio materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c83-relative-ratio-200-session-trend-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C83 relative-ratio signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c83-relative-ratio-200-session-trend-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c83-relative-ratio-200-session-trend-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C83 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C83 holdout one-use state is not pristine")
