"""One-use runner for frozen C129 qqq-max-drawdown rotation."""
...
from research.adapters.c129_qqq_max_drawdown import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c129_qqq_max_drawdown_v1.json"
ADAPTER = ROOT / "research/adapters/c129_qqq_max_drawdown.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "0eb1febf54c14db4cd296ed1c7b9297f87bc9c6e01a920af41b6ae8c19c2235e"
ADAPTER_SHA256 = "39cddb3a4422de09d8bbe2ead7f9e11f081390d8e34fed5c128f1c75a7d7a1ee"
SIGNAL_SHA256 = "08847dedad0fb4173e96ddf08f9f4801dad128a52872669602b51611368e19fb"
RECEIPT_SHA256 = "cd9e0ef5359ff5bc2b9e571cc1385f2e87c582436880214b82b7578c6eccbd83"
...
SIGNAL_INPUT = Path(
    ...
    "c129_qqq_64_close_max_drawdown_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C129 qqq-max-drawdown input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Minimum adjusted close divided by running peak minus one across "
            "exactly 64 chronologically accepted QQQ closes"
        ),
        "target_rule": (
            "Maximum drawdown above -0.10 selects QQQ; -0.10 or worse selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c129-qqq-max-drawdown-input-v1"
        ...
        raise InputBlockedError("C129 qqq-max-drawdown packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C129 qqq-max-drawdown signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C129 qqq-max-drawdown signal schema mismatch")
        value_fields = ("qqq_max_drawdown",)
        if any(
            ...
            raise InputBlockedError(
                "C129 qqq-max-drawdown value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or not -1.0 <= row["qqq_max_drawdown"] <= 0.0
        ):
            raise InputBlockedError("C129 formation or maximum-drawdown range mismatch")
        expected_state = "QQQ" if row["qqq_max_drawdown"] > -0.10 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C129 qqq-max-drawdown target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C129 qqq-max-drawdown chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C129 qqq-max-drawdown signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C129 qqq-max-drawdown stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C129 qqq-max-drawdown state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C129 qqq-max-drawdown signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C129 qqq-max-drawdown state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C129 qqq-max-drawdown input packet")
    receipt = _json(
        receipt_payload,
        "C129 qqq-max-drawdown materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c129-qqq-max-drawdown-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C129 qqq-max-drawdown signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c129-qqq-max-drawdown-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c129-qqq-max-drawdown-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C129 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C129 holdout one-use state is not pristine")
