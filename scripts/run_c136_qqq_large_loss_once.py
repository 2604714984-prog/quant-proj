"""One-use runner for frozen C136 QQQ large-loss rotation."""
...
from research.adapters.c136_qqq_large_loss import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c136_qqq_large_loss_v1.json"
ADAPTER = ROOT / "research/adapters/c136_qqq_large_loss.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "e167bf4b97d794a4284bc0448f816687c22fe1c8a78a8d2c4bf49b2570f7279e"
ADAPTER_SHA256 = "16b1d3375cab8a828e722563001db38d8dd11511573fe708f8db5dd5da609d99"
SIGNAL_SHA256 = "11de12a9e978b16d7d8b5a50596bacfd6db3c2acf31f6e9c6c3ebcf9b3498c75"
RECEIPT_SHA256 = "9c7a80c46a79b923a642b6fcafda91808eadcab31ff44621f5cfc2b2e43ea6c0"
...
SIGNAL_INPUT = Path(
    ...
    "c136_qqq_63_return_large_loss_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C136 QQQ large-loss input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Minimum of 63 simple QQQ returns formed from exactly 64 "
            "chronologically accepted adjusted closes"
        ),
        "target_rule": (
            "Worst daily return above -0.03 selects QQQ; -0.03 or worse selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c136-qqq-large-loss-input-v1"
        ...
        raise InputBlockedError("C136 QQQ large-loss packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C136 QQQ large-loss signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C136 QQQ large-loss signal schema mismatch")
        value_fields = ("qqq_worst_daily_return",)
        if any(
            ...
            raise InputBlockedError(
                "C136 QQQ large-loss value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or row["qqq_worst_daily_return"] <= -1.0
        ):
            raise InputBlockedError("C136 formation or worst-return range mismatch")
        expected_state = "QQQ" if row["qqq_worst_daily_return"] > -0.03 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C136 QQQ large-loss target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C136 QQQ large-loss chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C136 QQQ large-loss signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C136 QQQ large-loss stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C136 QQQ large-loss state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C136 QQQ large-loss signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C136 QQQ large-loss state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C136 QQQ large-loss input packet")
    receipt = _json(
        receipt_payload,
        "C136 QQQ large-loss materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c136-qqq-large-loss-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C136 QQQ large-loss signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c136-qqq-large-loss-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c136-qqq-large-loss-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C136 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C136 holdout one-use state is not pristine")
