"""One-use runner for the frozen Cycle 25 IC4WSA SPY/QQQ rotation lane."""
...
from research.adapters.c25_ic4wsa_labor_stress_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c25_ic4wsa_labor_stress_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c25_ic4wsa_labor_stress_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "0ac2ac961730d504bc74521b6a11cf2664db72337d8872b9fd651531eb8f78f6"
ADAPTER_SHA256 = "6deee040f80e21ccce0a3056910e761fd2d9891e56a613152c954b4bd284207b"
SIGNAL_SHA256 = "e4fe5304be0bdac433544559d0328e6fd1e1ea4275ca161529be4cb9fc0da78f"
RECEIPT_SHA256 = "fc55eef9a3ff0760a7ebe09c36d82f8e6e8c78b288341119fd9bef7c74b72693"
...
SIGNAL_INPUT = Path(
    ...
    "c25_ic4wsa_labor_stress_rotation_spy_qqq_v1/"
    "c25_full_input_packet.json"
)
...
def _prior_four_week_observation(value: date) -> date:
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "IC4WSA labor-stress rotation input packet")
    if (
        record.get("schema_version")
        != "c25-ic4wsa-labor-stress-rotation-spy-qqq-input-packet-v1"
        ...
        != {
            "series_id": "IC4WSA",
            "target_rule": "delta versus 28 days prior < 0 means QQQ; delta >= 0 means SPY",
            "threshold_claimants": 0.0,
        }
    ):
        raise InputBlockedError("IC4WSA labor-stress rotation packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("IC4WSA signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("IC4WSA signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("IC4WSA selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "IC4WSA"
                or selection.get("source_class") != "OFFICIAL_ALFRED_IC4WSA"
                or selection.get("raw_response_sha256")
                != "a1123bfc9e2e4d03c382c53ae26528817842667c1af02272c0adf5ae172df582"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("IC4WSA source row identity mismatch")
        current_value = current.get("value_claimants")
        previous_value = previous.get("value_claimants")
        change = row.get("delta_claimants_versus_28_days_prior")
        if any(
            ...
            raise InputBlockedError("IC4WSA values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("IC4WSA change arithmetic mismatch")
        ...
        previous_reference_week = _date(
            row.get("previous_reference_week"), "previous IC4WSA reference week"
        )
        current_reference_week = _date(
            row.get("current_reference_week"), "current IC4WSA reference week"
        )
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous IC4WSA available_at",
            )
            > decision_at
            or _datetime(
                current["available_at_conservative"],
                "current IC4WSA available_at",
            )
            > decision_at
            or _date(previous.get("observation_date"), "previous observation date")
            != previous_reference_week
            or _date(current.get("observation_date"), "current observation date")
            != current_reference_week
            or previous_reference_week
            != _prior_four_week_observation(current_reference_week)
        ):
            raise InputBlockedError(
                "IC4WSA reference-month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
            ...
            "previous_reference_week": previous_reference_week.isoformat(),
            "current_reference_week": current_reference_week.isoformat(),
            "current_row_sha256": current.get("row_sha256"),
            "previous_row_sha256": previous.get("row_sha256"),
            "delta_claimants_versus_28_days_prior": float(change),
            "target_symbol": expected_state,
        }
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("IC4WSA signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("IC4WSA source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "IC4WSA row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("IC4WSA signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("IC4WSA state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("IC4WSA signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("IC4WSA state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c25-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "IC4WSA labor-stress rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "IC4WSA labor-stress rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "IC4WSA signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c25-ic4wsa-labor-stress-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c25-ic4wsa-labor-stress-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 25 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 25 holdout one-use state is not pristine")
