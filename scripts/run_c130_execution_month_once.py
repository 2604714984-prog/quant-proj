"""One-use runner for frozen C130 execution-month rotation."""
...
from research.adapters.c130_execution_month import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c130_execution_month_v1.json"
ADAPTER = ROOT / "research/adapters/c130_execution_month.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "faa08387f0a6939f101036ddbba2aad4690c1c2c80d120ddb0400cf6044e69a4"
ADAPTER_SHA256 = "ee62963e644f0f038d4fe2e9aa65e8e4a5c882ce12badb6985deddf75c1cf319"
SIGNAL_SHA256 = "22afea0affb9105bade6c130a614971e88ffd431eb00015a83136289997bea94"
RECEIPT_SHA256 = "fa2a158eede9064235d62165f760c1d3aee2ad058f317bdd3ca21885cff67e5c"
...
SIGNAL_INPUT = Path(
    ...
    "c130_halloween_seasonality_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C130 execution-month input packet")
    expected_contract = {
        ...
        "target_rule": (
            "November-April selects QQQ; May-October selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c130-execution-month-input-v1"
        ...
        raise InputBlockedError("C130 execution-month packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C130 execution-month signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C130 execution-month signal schema mismatch")
        ...
        if any(
            ...
            raise InputBlockedError(
                "C130 execution-month value is invalid"
            )
        if (
            ...
            raise InputBlockedError("C130 formation or execution-month range mismatch")
        expected_state = (
            "QQQ" if row["execution_month"] in {11, 12, 1, 2, 3, 4} else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C130 execution-month target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C130 execution-month chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C130 execution-month signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C130 execution-month stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C130 execution-month state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C130 execution-month signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C130 execution-month state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C130 execution-month input packet")
    receipt = _json(
        receipt_payload,
        "C130 execution-month materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c130-execution-month-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C130 execution-month signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c130-execution-month-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c130-execution-month-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C130 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C130 holdout one-use state is not pristine")
