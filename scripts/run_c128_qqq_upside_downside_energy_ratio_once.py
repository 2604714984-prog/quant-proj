"""One-use runner for frozen C128 qqq-upside-downside-energy-ratio rotation."""
...
from research.adapters.c128_qqq_upside_downside_energy_ratio import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c128_qqq_upside_downside_energy_ratio_v1.json"
ADAPTER = ROOT / "research/adapters/c128_qqq_upside_downside_energy_ratio.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "0376b6f3a028bd94f74fc42026068e551f82bb2e9922b739bc47cbf7795b0a6a"
ADAPTER_SHA256 = "86d0aae8b178c5b6175e06d30a111152b8f5a2a28f14e7623eadd8d705009a00"
SIGNAL_SHA256 = "16ff465e35a7e4f932ceda2aeaebcaf17dec96f69d7c5521058013c257ce6b72"
RECEIPT_SHA256 = "ebdf3cb82fec4cce142c62a66694f3e8fdb81409cd12d54f799c4838a8c14bc6"
...
SIGNAL_INPUT = Path(
    ...
    "c128_qqq_63_return_upside_downside_energy_ratio_state_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C128 qqq-upside-downside-energy-ratio input packet")
    expected_contract = {
        "formation_sessions": 64,
        "missing_rule": "fail closed; no alternate source or window",
        "score_definition": (
            "Sum of squared positive returns divided by sum of squared negative returns "
            "from 63 simple QQQ returns formed by exactly 64 adjusted closes"
        ),
        "target_rule": (
            "Upside-to-downside energy ratio above 1 selects QQQ; 1 or below selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c128-qqq-upside-downside-energy-ratio-input-v1"
        ...
        raise InputBlockedError("C128 qqq-upside-downside-energy-ratio packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C128 qqq-upside-downside-energy-ratio signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "formation_end",
        ...
        "qqq_upside_downside_energy_ratio",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C128 qqq-upside-downside-energy-ratio signal schema mismatch")
        value_fields = ("qqq_upside_downside_energy_ratio",)
        if any(
            ...
            raise InputBlockedError(
                "C128 qqq-upside-downside-energy-ratio value is invalid"
            )
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 64
            or row["qqq_upside_downside_energy_ratio"] < 0.0
        ):
            raise InputBlockedError("C128 formation or energy-ratio range mismatch")
        expected_state = "QQQ" if row["qqq_upside_downside_energy_ratio"] > 1.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C128 qqq-upside-downside-energy-ratio target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C128 qqq-upside-downside-energy-ratio chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C128 qqq-upside-downside-energy-ratio signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C128 qqq-upside-downside-energy-ratio stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C128 qqq-upside-downside-energy-ratio state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C128 qqq-upside-downside-energy-ratio signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C128 qqq-upside-downside-energy-ratio state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C128 qqq-upside-downside-energy-ratio input packet")
    receipt = _json(
        receipt_payload,
        "C128 qqq-upside-downside-energy-ratio materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c128-qqq-upside-downside-energy-ratio-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C128 qqq-upside-downside-energy-ratio signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c128-qqq-upside-downside-energy-ratio-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c128-qqq-upside-downside-energy-ratio-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C128 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C128 holdout one-use state is not pristine")
