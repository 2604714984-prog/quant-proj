"""One-use runner for the frozen Cycle 20 T10YIE SPY/QQQ rotation lane."""
...
from research.adapters.c20_t10yie_inflation_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c20_t10yie_inflation_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c20_t10yie_inflation_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "bbc5b36952c6f4131eace0fbe6fc7255d92dea1124ceb3557285e2d4aba52913"
ADAPTER_SHA256 = "0a80ef3e122273cf68a83e8f06e1e0a0aa84ae3b08814e752e522c9ed5cdece7"
SIGNAL_SHA256 = "b913f660e953cdb62fbb518a92fee8d846d21d600897d96cdbdd06aa2f54e167"
RECEIPT_SHA256 = "7b1dd41a33a68366ca11be4d9761515d59a01c15a3b5610b1d17e1612ad4ab31"
...
SIGNAL_INPUT = Path(
    ...
    "c20_t10yie_inflation_expectation_rotation_spy_qqq_v1/"
    "c20_full_input_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "T10YIE inflation-expectation rotation input packet")
    if (
        record.get("schema_version")
        != "c20-t10yie-inflation-rotation-spy-qqq-input-packet-v1"
    ...
    for row in rows:
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("T10YIE selections are required")
        for selection in (previous, current):
            ...
            if (
                selection.get("series_id") != "T10YIE"
                or selection.get("source_class") != "OFFICIAL_ALFRED_T10YIE"
                or selection.get("raw_response_sha256")
                != "55d62e8a6837d822e391c3ee6d79a858e7ae7cced22f3a410f45cbc83397039d"
                or row_hash
                != _hash_bytes(
                    json.dumps(
                ...
                raise InputBlockedError("T10YIE source row identity mismatch")
        ...
        if any(
            ...
            raise InputBlockedError("T10YIE values must be finite")
        ...
        if not math.isclose(float(change), expected_change, rel_tol=0, abs_tol=1e-12):
            raise InputBlockedError("T10YIE change arithmetic mismatch")
        ...
        previous_decision_at = _datetime(
            row["previous_decision_at"], "previous signal decision_at"
        )
        decision_at = _datetime(row["decision_at"], "signal decision_at")
        if (
            _datetime(
                previous["available_at_conservative"],
                "previous T10YIE available_at",
            )
            > previous_decision_at
            or _datetime(
                current["available_at_conservative"],
                "current T10YIE available_at",
            )
            > decision_at
        ):
            raise InputBlockedError("T10YIE source row was unavailable at cutoff")
        identity = {
            "stage": row["stage"],
            "holding_start_execution_session": start.isoformat(),
            "holding_end_execution_session": end.isoformat(),
            "previous_decision_at": previous_decision_at.isoformat(),
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("T10YIE signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("T10YIE source identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "T10YIE row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("T10YIE signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("T10YIE state support is required")
    for stage, group in output.items():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("T10YIE signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("T10YIE state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c20-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "T10YIE inflation-expectation rotation/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "T10YIE inflation-expectation rotation/QQQ materialization receipt",
    )
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "T10YIE signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c20-t10yie-inflation-rotation-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c20-t10yie-inflation-rotation-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 20 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 20 holdout one-use state is not pristine")
