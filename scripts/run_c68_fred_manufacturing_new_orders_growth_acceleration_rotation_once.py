"""One-use runner for the frozen C68 manufacturing-orders SPY/QQQ rotation lane."""
...
from research.adapters.c68_fred_manufacturing_new_orders_growth_acceleration_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c68_fred_manufacturing_new_orders_growth_acceleration_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c68_fred_manufacturing_new_orders_growth_acceleration_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "70f7c3d37219c3ef6e65eb3b69f7f5147ced04258b199dbff1b451ebabcef154"
ADAPTER_SHA256 = "8d224343e73fbe3d306142500a598589942d94f585231ed20a0eca9d16a2a133"
SIGNAL_SHA256 = "7fdc5de1aa1e90d6867b78397c16e94c29be81f3d66eb2745bb0db0441d56bbc"
RECEIPT_SHA256 = "8461fe67af8236f2841d28024f00217024c56c7278fdbbe4e8a942cde7ebec12"
...
SIGNAL_INPUT = Path(
    ...
    "c68_fred_manufacturing_new_orders_growth_acceleration_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "AMTMNO growth-acceleration input packet")
    expected_contract = {
        "series_id": "FRED_AMTMNO",
        "return_window": (
            "year-over-year AMTMNO growth in M-4 versus M-3 for "
            "execution month M"
        ),
        "target_rule": "strict manufacturing-orders growth acceleration selects SPY; equality or deceleration selects QQQ",
        "threshold_yoy_growth": 0.0,
    }
    if (
        record.get("schema_version")
        != "c68-amtmno-growth-acceleration-input-packet-v1"
        ...
        raise InputBlockedError("AMTMNO packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("AMTMNO signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = "70abe9ab8194d53df8ea899698227b179dd5b6e78a7c33e08f0a842618a4ecec"
    expected_url = (
        "https://fred.stlouisfed.org/graph/fredgraph.csv?"
        "id=AMTMNO&cosd=2019-01-01&coed=2026-06-30"
    )
    expected_availability_rule = (
        "20:00 America/New_York on the 15th calendar day of the second following month; "
        "retrospective-secondary current FRED history without vintage proof"
    )
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("AMTMNO signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("AMTMNO selections are required")
        for selection in (previous, current):
            ...
            value = selection.get("current_millions_usd")
            prior_value = selection.get("prior_year_millions_usd")
            yoy_growth = selection.get("yoy_growth")
            if (
                selection.get("series_id") != "FRED_AMTMNO"
                or selection.get("source_class")
                != "FEDERAL_RESERVE_FRED_OFFICIAL_CURRENT_HISTORY_RETROSPECTIVE_SECONDARY"
                ...
                or not isinstance(prior_value, (int, float))
                or not math.isfinite(float(prior_value))
                or float(prior_value) <= 0.0
                or isinstance(yoy_growth, bool)
                or not isinstance(yoy_growth, (int, float))
                or not math.isfinite(float(yoy_growth))
                or not math.isclose(
                    float(yoy_growth),
                    float(value) / float(prior_value) - 1.0,
                    rel_tol=0,
                    abs_tol=1e-12,
                )
                or row_hash
                ...
                raise InputBlockedError("AMTMNO source row identity mismatch")
            reference = _date(selection.get("reference_month"), "AMTMNO reference month")
            prior_reference = _date(
                selection.get("prior_year_reference_month"),
                "AMTMNO prior-year reference month",
            )
            observed = _date(selection.get("observation_date"), "AMTMNO observation")
            prior_observed = _date(
                selection.get("prior_year_observation_date"),
                "AMTMNO prior-year observation",
            )
            expected_prior_ordinal = reference.year * 12 + reference.month - 13
            expected_prior = date(
                expected_prior_ordinal // 12,
                expected_prior_ordinal % 12 + 1,
                1,
            )
            available = _datetime(
            ...
            expected_available_ordinal = reference.year * 12 + reference.month + 1
            expected_available_date = date(
                expected_available_ordinal // 12,
                expected_available_ordinal % 12 + 1,
                15,
            )
            if (
                reference.day != 1
                or prior_reference != expected_prior
                or observed != reference
                or prior_observed != prior_reference
                or available.date() != expected_available_date
                or (available.hour, available.minute, available.second) != (20, 0, 0)
            ):
                raise InputBlockedError("AMTMNO source chronology mismatch")
        current_value = float(current["yoy_growth"])
        previous_value = float(previous["yoy_growth"])
        change = row.get("delta_yoy_growth")
        if (
            ...
            raise InputBlockedError("AMTMNO change arithmetic mismatch")
        ...
        expected_current_ordinal = start.year * 12 + start.month - 4
        ...
        if (
            ...
            raise InputBlockedError("AMTMNO observation or availability mismatch")
        identity = {
            "stage": row["stage"],
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "AMTMNO row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("AMTMNO signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("AMTMNO state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("AMTMNO signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("AMTMNO state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c68-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "AMTMNO growth-acceleration/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "AMTMNO growth-acceleration/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c68-amtmno-full-materialization-receipt-v1"
        ...
        != "06f4df34785076b58046de85fe3d649fdd3cd324627d23da8017ea875ea59cf9"
        or receipt.get("raw_response_sha256")
        != "70abe9ab8194d53df8ea899698227b179dd5b6e78a7c33e08f0a842618a4ecec"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "AMTMNO signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c68-amtmno-growth-acceleration-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c68-amtmno-growth-acceleration-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
