"""One-use runner for frozen C75 relative-volatility compression."""
...
from research.adapters.c75_relative_realized_volatility_compression_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c75_relative_realized_volatility_compression_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c75_relative_realized_volatility_compression_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "92152eb15e4a92fc913453c396a3f04d61474a98d9a8e5bdefdbd6aeeda51733"
ADAPTER_SHA256 = "2f96b307893be32c94b4ffd9c1914782887fdcf6d6a29b9ba52ff8598aca5129"
SIGNAL_SHA256 = "144dc7506d9b7f0637c1f61057760ba9a96637cec5082bf7e4cbd086db49cd58"
RECEIPT_SHA256 = "bfdf1c668525fbc8dba54d4684c59e2e6a0813e1c37317d326f1d7f2d42ea414"
...
SIGNAL_INPUT = Path(
    ...
    "c75_relative_realized_volatility_compression_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C75 relative-realized-volatility input packet")
    expected_contract = {
        "formation_close_count": 43,
        "older_block_return_count": 21,
        "recent_block_return_count": 21,
        "return_type": "natural log return",
        "volatility_estimator": "sample standard deviation ddof=1",
        "ratio": "QQQ realized volatility divided by SPY realized volatility",
        "target_rule": (
            "strict recent ratio compression means QQQ; equality or expansion means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        record.get("schema_version")
        != "c75-relative-realized-volatility-compression-input-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("evidence_class")
        != "RETROSPECTIVE_SECONDARY_TIINGO_CURRENT_SNAPSHOT"
        or record.get("strategy_outcomes_opened") is not False
        or record.get("price_values_used_for_signal") is not True
        or record.get("spy_price_values_used_for_materialization") is not True
        or record.get("adjusted_prices_used") is not True
        or record.get("adjusted_prices_used_for_signal") is not True
        or record.get("database_opened") is not True
        ...
        raise InputBlockedError("C75 relative-realized-volatility packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C75 relative-realized-volatility signal row count mismatch")
    expected_row_keys = {
        "ddof",
        "decision_at",
        "holding_end_execution_session",
        "holding_start_execution_session",
        "older_block_end",
        "older_block_start",
        "older_qqq_realized_volatility",
        "older_relative_volatility_ratio",
        "older_row_hash_set_sha256",
        "older_spy_realized_volatility",
        "recent_block_end",
        "recent_block_start",
        "recent_qqq_realized_volatility",
        "recent_relative_volatility_ratio",
        "recent_row_hash_set_sha256",
        "recent_spy_realized_volatility",
        "relative_volatility_ratio_change",
        "return_count_per_block",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C75 relative-realized-volatility signal schema mismatch")
        numeric_fields = (
            "older_spy_realized_volatility",
            "older_qqq_realized_volatility",
            "recent_spy_realized_volatility",
        ...
        if any(
            ...
            for value in (row[field] for field in numeric_fields)
        ):
            raise InputBlockedError("C75 relative-realized-volatility value is invalid")
        if any(float(row[field]) <= 0.0 for field in numeric_fields[:-1]):
            raise InputBlockedError("C75 realized volatility and ratio must be positive")
        if type(row["return_count_per_block"]) is not int or row["return_count_per_block"] != 21:
            raise InputBlockedError("C75 block return count mismatch")
        if type(row["ddof"]) is not int or row["ddof"] != 1:
            raise InputBlockedError("C75 volatility ddof mismatch")

        older_ratio = float(row["older_relative_volatility_ratio"])
        recent_ratio = float(row["recent_relative_volatility_ratio"])
        expected_old = float(row["older_qqq_realized_volatility"]) / float(
            row["older_spy_realized_volatility"]
        )
        expected_recent = float(row["recent_qqq_realized_volatility"]) / float(
            row["recent_spy_realized_volatility"]
        )
        if (
            not math.isclose(older_ratio, expected_old, rel_tol=1e-12, abs_tol=1e-15)
            or not math.isclose(
                recent_ratio, expected_recent, rel_tol=1e-12, abs_tol=1e-15
            )
            or not math.isclose(
                float(row["relative_volatility_ratio_change"]),
                recent_ratio - older_ratio,
                rel_tol=1e-12,
                abs_tol=1e-15,
            )
        ):
            raise InputBlockedError("C75 relative-volatility arithmetic mismatch")
        expected_state = "QQQ" if recent_ratio < older_ratio else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C75 relative-realized-volatility target mismatch")

        older_start = _date(row["older_block_start"], "older block start")
        older_end = _date(row["older_block_end"], "older block end")
        recent_start = _date(row["recent_block_start"], "recent block start")
        recent_end = _date(row["recent_block_end"], "recent block end")
        signal_session = _date(row["signal_session"], "signal session")
        ...
        local_decision = decision_at.astimezone(NY)
        if (
            not older_start < older_end == recent_start < recent_end
            or recent_end != signal_session
            or not signal_session < start < end
            or local_decision.date() != signal_session
            or (
                local_decision.hour,
                local_decision.minute,
                local_decision.second,
                local_decision.microsecond,
            )
            != (20, 5, 0, 0)
        ):
            raise InputBlockedError("C75 relative-realized-volatility chronology mismatch")

        _sha(row["older_row_hash_set_sha256"], "older row-set SHA-256")
        _sha(row["recent_row_hash_set_sha256"], "recent row-set SHA-256")
        selection_hash = _sha(
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C75 relative-realized-volatility signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C75 relative-realized-volatility stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C75 relative-realized-volatility state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C75 relative-realized-volatility signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C75 relative-realized-volatility state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C75 relative-realized-volatility input packet")
    receipt = _json(
        receipt_payload,
        "C75 relative-realized-volatility materialization receipt",
    )
    retrieved_at = datetime(
        2026, 7, 23, 13, 21, 43, 911309, tzinfo=timezone.utc
    )
    if (
        packet.get("research_id") != RESEARCH_ID
        or receipt.get("research_id") != RESEARCH_ID
        or receipt.get("schema_version")
        != "c75-relative-realized-volatility-materialization-receipt-v1"
        or receipt.get("input_packet_sha256") != SIGNAL_SHA256
        or receipt.get("source_scaffold_sha256")
        != "da541f24903d823b339547a66177f451c4723fe6b4e3a82c758f54d03087083d"
        or receipt.get("status") != "INPUT_MATERIALIZED_STATE_SUPPORT_PASS"
        or receipt.get("state_support") != packet.get("state_support")
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C75 relative-realized-volatility signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c75-relative-realized-volatility-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c75-relative-realized-volatility-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C75 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C75 holdout one-use state is not pristine")
