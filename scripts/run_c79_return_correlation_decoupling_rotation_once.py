"""One-use runner for frozen C79 return-correlation decoupling."""
...
from research.adapters.c79_return_correlation_decoupling_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c79_return_correlation_decoupling_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c79_return_correlation_decoupling_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "caea3963d5ad0e67304569e46710e02f99940bfc2240afea203fac8309fdb61b"
ADAPTER_SHA256 = "fbba3e3886fc8ab500c43c68eb1958f21529598e4572e46ce4b3a3de2c1a32ac"
SIGNAL_SHA256 = "426a68a4af47ddd43d5249c083fc80e53f2bdf6a8720e797134b6ea9b62eef05"
RECEIPT_SHA256 = "dca4b3239b30f987d1c43752df1b65643e50cb78fb4fe74d7ed5eb0e2fba3863"
...
SIGNAL_INPUT = Path(
    ...
    "c79_return_correlation_decoupling_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C79 return-correlation-decoupling input packet")
    expected_contract = {
        ...
        "correlation_estimator": (
            "ordinary sample Pearson correlation using sample covariance and "
            "marginal sample standard deviations with ddof=1"
        ),
        "target_rule": (
            "strict recent correlation decline means QQQ; equality or increase "
            "means SPY"
        ),
        "tie_break": "SPY",
    }
    if (
        ...
        != "c79-return-correlation-decoupling-input-v1"
        ...
        raise InputBlockedError("C79 return-correlation-decoupling packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C79 return-correlation-decoupling signal row count mismatch")
    expected_row_keys = {
        "ddof",
        "decision_at",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C79 return-correlation-decoupling signal schema mismatch")
        numeric_fields = (
            "older_spy_std",
            "older_qqq_std",
            "older_covariance",
        ...
        if any(
            ...
            raise InputBlockedError("C79 return-correlation-decoupling value is invalid")
        if any(
            float(row[field]) <= 0.0
            for field in (
                "older_spy_std",
            ...
            raise InputBlockedError("C79 marginal standard deviation must be positive")
        if any(
            not -1.0 <= float(row[field]) <= 1.0
            for field in ("older_correlation", "recent_correlation")
        ):
            raise InputBlockedError("C79 correlation must be within [-1, 1]")
        if type(row["return_count_per_block"]) is not int or row["return_count_per_block"] != 21:
            raise InputBlockedError("C79 block return count mismatch")
        if type(row["ddof"]) is not int or row["ddof"] != 1:
            raise InputBlockedError("C79 correlation ddof mismatch")

        older_correlation = float(row["older_correlation"])
        recent_correlation = float(row["recent_correlation"])
        expected_old = float(row["older_covariance"]) / (
            float(row["older_spy_std"]) * float(row["older_qqq_std"])
        )
        expected_recent = float(row["recent_covariance"]) / (
            float(row["recent_spy_std"]) * float(row["recent_qqq_std"])
        )
        if (
            not math.isclose(
                older_correlation, expected_old, rel_tol=1e-12, abs_tol=1e-15
            )
            or not math.isclose(
                recent_correlation, expected_recent, rel_tol=1e-12, abs_tol=1e-15
            )
            or not math.isclose(
                float(row["correlation_change"]),
                recent_correlation - older_correlation,
                rel_tol=1e-12,
                abs_tol=1e-15,
            )
        ):
            raise InputBlockedError("C79 return-correlation arithmetic mismatch")
        expected_state = (
            "QQQ" if recent_correlation < older_correlation else "SPY"
        )
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C79 return-correlation-decoupling target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C79 return-correlation-decoupling chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C79 return-correlation-decoupling signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C79 return-correlation-decoupling stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C79 return-correlation-decoupling state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C79 return-correlation-decoupling signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C79 return-correlation-decoupling state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C79 return-correlation-decoupling input packet")
    receipt = _json(
        receipt_payload,
        "C79 return-correlation-decoupling materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c79-return-correlation-decoupling-materialization-receipt-v1"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C79 return-correlation-decoupling signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c79-return-correlation-decoupling-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c79-return-correlation-decoupling-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C79 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C79 holdout one-use state is not pristine")
