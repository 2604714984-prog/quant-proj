"""One-use runner for the frozen Cycle 56 momentum/value SPY/QQQ lane."""
...
from research.adapters.c56_momentum_value_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c56_momentum_value_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c56_momentum_value_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "b20e81b23c7af7545d967b1bf73a74854522d69cf815e469e4583a1914e851b3"
ADAPTER_SHA256 = "2804c2be285da0da9cf752a8349470f66f4858e4f88bcc7599a60144a1e7a078"
SIGNAL_SHA256 = "58a0e298e332b761ebedf4c1a6cefdc5b988c78f0685be96b532de03388d7a82"
RECEIPT_SHA256 = "999e5a43e99a3049d4726211ae518e4ae6f6b5ce77988c7c2a5426694ea5050b"
...
SIGNAL_INPUT = Path(
    ...
    "c56_momentum_value_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "momentum value leadership rotation input packet")
    expected_contract = {
        "left_symbol": "MTUM",
        "right_symbol": "VTV",
        "return_window": (
        ...
        "target_rule": (
            "strict MTUM prior-month total-return outperformance versus VTV "
    ...
    if (
        ...
        raise InputBlockedError("momentum value packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("momentum value signal row count mismatch")
    ...
    expected_source = {
        "MTUM": (
            "https://query1.finance.yahoo.com/v8/finance/chart/MTUM?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "8c469aaa9b114f14c6662f3515a642ca5a07db8762befc26ddccc0b8ef641191",
        ),
        "VTV": (
            "https://query1.finance.yahoo.com/v8/finance/chart/VTV?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "618387cccc6299f7d0759b59145dbf7ca884f34b362149245497cdfb54689438",
        ),
    }
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("momentum value signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("momentum value selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "momentum value adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("momentum value source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "momentum value cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("momentum value arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "momentum value month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("momentum value signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("momentum value signal identity hash mismatch")
    ...
    if (
        ...
        raise InputBlockedError("momentum value signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("momentum value state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "momentum value signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("momentum value state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "momentum value leadership rotation input packet")
    receipt = _json(
        receipt_payload,
        "momentum value leadership materialization receipt",
    )
    ...
    if (
        ...
        != "epoch10-factor-full-materialization-receipt-v1"
        or receipt.get("left_symbol") != "MTUM"
        or receipt.get("right_symbol") != "VTV"
        or receipt.get("raw_fetch_receipt_sha256")
        != "b494419c911d18bbd86349802760b6fd2f55258e031cd2bbd407e73e961f31be"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "momentum value signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c56-momentum-value-leadership-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c56-momentum-value-leadership-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
