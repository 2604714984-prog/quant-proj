"""One-use runner for the frozen Cycle 43 small-cap breadth SPY/QQQ lane."""
...
from research.adapters.c43_small_cap_breadth_leadership_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c43_small_cap_breadth_leadership_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c43_small_cap_breadth_leadership_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "2c8cd20290beb7acfabfcbc4f7469c9bb2f8c0dd611792c1f3965b4f4b33a8b3"
ADAPTER_SHA256 = "ca27ac41a7bcaffbeeec53ec071160a307b3afca33d51ddfde04b493527a79be"
SIGNAL_SHA256 = "2fa04a629431633af01ae218406f43ca955948085e272c66070310a583c3f0d6"
RECEIPT_SHA256 = "d8bf0a1c1493265860c95ee52f5bb1ed9c810fbea640e168476b9b070a620c74"
...
SIGNAL_INPUT = Path(
    ...
    "c43_small_cap_breadth_leadership_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "small-cap breadth rotation input packet")
    expected_contract = {
        "left_symbol": "IWM",
        "right_symbol": "SPY",
        "return_window": (
        ...
        "target_rule": (
            "strict IWM prior-month total-return outperformance versus SPY "
    ...
    if (
        ...
        raise InputBlockedError("small-cap breadth packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("small-cap breadth signal row count mismatch")
    ...
    expected_source = {
        "IWM": (
            "https://query1.finance.yahoo.com/v8/finance/chart/IWM?"
            "period1=1604188800&period2=1782864000&interval=1d&"
            "events=div%2Csplits&includeAdjustedClose=true",
            "fa9bdec9ce0082e3008f87f8581d635778a708be216ffaf8e6b9450235d80569",
        ),
        "SPY": (
    ...
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("small-cap breadth signal stage mismatch")
        ...
        if any(type(selection) is not dict for selection in selections):
            raise InputBlockedError("small-cap breadth selections are required")
        for selection in selections:
            ...
            try:
                ...
                raise InputBlockedError(
                    "small-cap breadth adjusted close is invalid"
                ) from exc
            if (
                source is None
                ...
                raise InputBlockedError("small-cap breadth source row identity mismatch")
        ...
        try:
            ...
            raise InputBlockedError(
                "small-cap breadth cross product is invalid"
            ) from exc
        if not cross_product.is_finite() or cross_product != expected_cross_product:
            raise InputBlockedError("small-cap breadth arithmetic mismatch")
        ...
        if (
            ...
            raise InputBlockedError(
                "small-cap breadth month identity or availability mismatch"
            )
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("small-cap breadth signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("small-cap breadth signal identity hash mismatch")
    ...
    if (
        ...
        raise InputBlockedError("small-cap breadth signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("small-cap breadth state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "small-cap breadth signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("small-cap breadth state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "small-cap breadth rotation input packet")
    receipt = _json(
        receipt_payload,
        "small-cap breadth materialization receipt",
    )
    ...
    if (
        ...
        or receipt.get("left_symbol") != "IWM"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "small-cap breadth signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c43-small-cap-breadth-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c43-small-cap-breadth-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 43 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 43 holdout one-use state is not pristine")
