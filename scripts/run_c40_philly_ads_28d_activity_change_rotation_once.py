"""One-use runner for the frozen Cycle 40 Philadelphia Fed ADS SPY/QQQ lane."""
...
from research.adapters.c40_philly_ads_28d_activity_change_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c40_philly_ads_28d_activity_change_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c40_philly_ads_28d_activity_change_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "ec651b9999cb97b7747f3e279287e5525895decc29a5e16b7b3fe9cada53f09c"
ADAPTER_SHA256 = "05ed0f8e00c425e082f9488fe0c1bc3d85ac0bfea40d7e9e3325078f5a2ce4c9"
SIGNAL_SHA256 = "0ea0e8278bba2e387fbf5479b42bb15bce561121dba836203eaf7fe31e71cfa4"
RECEIPT_SHA256 = "bf358c8d884e2a29b2ab5f742eebe1b59a85a21c0225f47cb47b94343a4df923"
...
SIGNAL_INPUT = Path(
    ...
    "c40_philly_ads_28d_activity_change_rotation_spy_qqq_v1/"
    "c40_full_input_packet_v2.json"
)
INPUT_RECEIPT = SIGNAL_INPUT.with_name("full_materialization_receipt_v2.json")
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "Philadelphia Fed ADS rotation input packet")
    expected_contract = {
        "series_id": "ADS_REALTIME_VINTAGE",
        "lookback_calendar_days": 28,
        "vintage_rule": "latest official ADS vintage date strictly before execution date",
        "target_rule": "strict 28-calendar-day ADS increase selects QQQ; equal or decrease selects SPY",
        "threshold_index_points": 0.0,
    }
    if (
        record.get("schema_version")
        != "c40-philly-ads-28d-change-spy-qqq-input-packet-v2"
        ...
        raise InputBlockedError("ADS packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("ADS signal row count mismatch")
    ...
    expected_url = (
        ...
        "ads/ADS_All_Vintages-zip.zip"
    )
    expected_zip_sha = "fcb0317952ed9d0b83b47f653002e9bc0593583983ddf509a6e3a84ebe088b59"
    expected_xlsx_sha = "5d2f2801a3f8314cec157c9d244e064921b29b29e15614ef73a6f4b35da93be8"
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("ADS signal stage mismatch")
        current = row.get("current_selection")
        baseline = row.get("baseline_selection")
        if type(current) is not dict or type(baseline) is not dict:
            raise InputBlockedError("ADS selections are required")
        for selection in (baseline, current):
            row_hash = selection.get("row_sha256")
            identity = {
                key: value for key, value in selection.items() if key != "row_sha256"
            }
            value = selection.get("value_index_points")
            vintage_header = selection.get("vintage_header")
            vintage_date = _date(selection.get("vintage_date"), "ADS vintage date")
            expected_suffix = vintage_date.strftime("%m%d%y")
            if (
                selection.get("series_id") != "ADS_REALTIME_VINTAGE"
                or selection.get("source_class")
                != "OFFICIAL_PHILADELPHIA_FED_REALTIME_VINTAGE"
                or selection.get("source_page") != expected_page
                or selection.get("source_url") != expected_url
                or selection.get("raw_zip_sha256") != expected_zip_sha
                or selection.get("xlsx_sha256") != expected_xlsx_sha
                or vintage_header != f"ADS_INDEX_{expected_suffix}"
                ...
                raise InputBlockedError("ADS source row identity mismatch")
        current_value = float(current["value_index_points"])
        baseline_value = float(baseline["value_index_points"])
        ...
        if (
            ...
            or not math.isclose(
                float(change), current_value - baseline_value, rel_tol=0, abs_tol=1e-12
            )
        ):
            raise InputBlockedError("ADS change arithmetic mismatch")
        ...
        baseline_observation = _date(
            row.get("baseline_observation_date"), "baseline observation date"
        )
        vintage_header = row.get("vintage_header")
        vintage_date = _date(row.get("vintage_date"), "signal vintage date")
        if (
            baseline_cutoff != decision_at.date() - timedelta(days=28)
            or current.get("vintage_header") != vintage_header
            or baseline.get("vintage_header") != vintage_header
            or _date(current.get("vintage_date"), "current vintage date")
            != vintage_date
            or _date(baseline.get("vintage_date"), "baseline vintage date")
            != vintage_date
            or vintage_date >= start
            or current_observation
            != _date(current.get("observation_date"), "current source observation")
            or baseline_observation
            != _date(baseline.get("observation_date"), "baseline source observation")
            or current_observation > decision_at.date()
            or baseline_observation > baseline_cutoff
            or _datetime(current["available_at_conservative"], "current available_at")
            > decision_at
            or _datetime(baseline["available_at_conservative"], "baseline available_at")
            > decision_at
        ):
            raise InputBlockedError("ADS observation or availability mismatch")
        identity = {
            "stage": row["stage"],
            "holding_start_execution_session": start.isoformat(),
            "holding_end_execution_session": end.isoformat(),
            "decision_at": row["decision_at"],
            "vintage_header": vintage_header,
        ...
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "ADS row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("ADS signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("ADS state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("ADS signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("ADS state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "Philadelphia Fed ADS rotation input packet")
    receipt = _json(
        receipt_payload,
        "Philadelphia Fed ADS materialization receipt",
    )
    ...
    if (
        ...
        != "c40-philly-ads-full-materialization-receipt-v2"
        or receipt.get("input_packet_sha256") != SIGNAL_SHA256
        or receipt.get("status") != "MATERIALIZED_OUTCOME_FREE"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "ADS signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c40-philly-ads-28d-activity-change-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c40-philly-ads-28d-activity-change-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 39 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 39 holdout one-use state is not pristine")
