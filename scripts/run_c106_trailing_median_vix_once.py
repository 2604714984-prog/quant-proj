"""One-use runner for frozen C106 trailing-median VIX rotation."""
...
from research.adapters.c106_trailing_median_vix import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c106_trailing_median_vix_v1.json"
ADAPTER = ROOT / "research/adapters/c106_trailing_median_vix.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "dcd8540a5a606b01c2cb357ef01bb3d1fb0efcf814e1f69f94563bcd070e30ed"
ADAPTER_SHA256 = "a426d0f2def9d53640817be6fe99d82dd17caa2dd32564e19a49e5ccfb6411fd"
SIGNAL_SHA256 = "74a511af07466eabadc3adb662f0785aa9d6cfa83a10e2a919787a906a5a31a7"
RECEIPT_SHA256 = "001bb8a4dd4cf514a6fc4daac362aac0d1bcce3a457f189096a13e1db071f1c3"
...
SIGNAL_INPUT = Path(
    ...
    "c106_trailing_median_vix_risk_appetite_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "C106 trailing-median-VIX input packet")
    expected_contract = {
        "formation_sessions": 252,
        "missing_rule": "fail closed; no alternate source or window",
        "median_definition": (
            "ordinary median; arithmetic mean of two middle values "
            "for even observation count"
        ),
        "target_rule": (
            "current VIX strictly below trailing median selects QQQ; "
            "equality or higher selects SPY"
        ),
    }
    if (
        record.get("schema_version")
        != "c106-trailing-median-vix-input-v1"
        ...
        or record.get("implied_volatility_values_used_for_signal") is not True
        ...
        raise InputBlockedError("C106 trailing-median-VIX packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("C106 trailing-median-VIX signal row count mismatch")
    expected_row_keys = {
        "decision_at",
        "current_vix_close",
        "formation_end",
    ...
    for row in rows:
        if (
            ...
            raise InputBlockedError("C106 trailing-median-VIX signal schema mismatch")
        numeric_fields = (
            "current_vix_close",
            "trailing_median_vix",
            "vix_minus_trailing_median",
        ...
        if any(
            ...
            raise InputBlockedError("C106 trailing-median-VIX value is invalid")
        if (
            type(row["formation_session_count"]) is not int
            or row["formation_session_count"] != 252
        ):
            raise InputBlockedError("C106 formation session count mismatch")
        current_vix = float(row["current_vix_close"])
        trailing_median = float(row["trailing_median_vix"])
        difference = current_vix - trailing_median
        if current_vix <= 0.0 or trailing_median <= 0.0:
            raise InputBlockedError("C106 trailing-median-VIX value is invalid")
        if not math.isclose(
            float(row["vix_minus_trailing_median"]),
            difference,
            rel_tol=1e-12,
            abs_tol=1e-15,
        ):
            raise InputBlockedError("C106 trailing-median-VIX arithmetic mismatch")
        expected_state = "QQQ" if difference < 0.0 else "SPY"
        if row["target_symbol"] != expected_state:
            raise InputBlockedError("C106 trailing-median-VIX target mismatch")
        ...
        if (
            ...
            raise InputBlockedError("C106 trailing-median-VIX chronology mismatch")
        ...
        if selection_hash != _hash_bytes(
            ...
            raise InputBlockedError("C106 trailing-median-VIX signal identity mismatch")
    ...
    if (
        ...
        raise InputBlockedError("C106 trailing-median-VIX stage count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("C106 trailing-median-VIX state support is required")
    for stage, group in output.items():
        if any(
            ...
            raise InputBlockedError(
                "C106 trailing-median-VIX signal intervals are not contiguous"
            )
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("C106 trailing-median-VIX state support mismatch")
    return output
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "C106 trailing-median-VIX input packet")
    receipt = _json(
        receipt_payload,
        "C106 trailing-median-VIX materialization receipt",
    )
    retrieved_at = datetime(
    ...
    if (
        ...
        != "c106-trailing-median-vix-materialization-receipt-v1"
        ...
        or receipt.get("cboe_vix_raw_sha256")
        != "26ab75dc0cf0d2f375e0f615e466de890f1e60d3b63a25aa2989f18c26670851"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "C106 trailing-median-VIX signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c106-trailing-median-vix-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c106-trailing-median-vix-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("C106 validation one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("C106 holdout one-use state is not pristine")
