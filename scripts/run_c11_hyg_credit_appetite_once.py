"""One-use runner for the frozen Cycle 11 HYG credit-appetite lane."""
...
from research.adapters.c11_hyg_credit_appetite import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c11_hyg_credit_appetite_v1.json"
ADAPTER = ROOT / "research/adapters/c11_hyg_credit_appetite.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "9c2da02dfd6ef8c7bb1730c35a97a515cec169f90caac9d380f30acdb5180c40"
ADAPTER_SHA256 = "b4b402925adf6b909de0dce2de105dac0010fa3b1734b0c0effe92fa04684a84"
SIGNAL_SHA256 = "7831756f046b9d9c408c6efe594cb8ccac075b9693e5cea3c21785fd6f1023ab"
RECEIPT_SHA256 = "d10c392b7fe1bb766277f0ff9ba50b8f5c8845be22df868500b9030cfda37ea5"
...
SIGNAL_INPUT = Path(
    ...
    "c11_hyg_monthly_credit_risk_appetite_spy_cash_v1/"
    "hyg_monthly_credit_appetite_signal_packet.json"
)
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "HYG credit-appetite signal packet")
    if (
        record.get("schema_version")
        != "c11-hyg-monthly-credit-appetite-signal-packet-v1"
        or record.get("research_id") != RESEARCH_ID
        or record.get("strategy_outcomes_opened") is not False
        or record.get("spy_price_values_used") is not False
        or record.get("database_access") is not False
        or record.get("database_write") is not False
    ):
        raise InputBlockedError("HYG signal packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("HYG signal row count mismatch")
    parsed: dict[str, list[Signal]] = {"validation": [], "retrospective_holdout": []}
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("HYG signal stage mismatch")
        selection = row.get("selection")
        if type(selection) is not dict:
            raise InputBlockedError("HYG selection is required")
        selected_value = selection.get("monthly_total_return")
        if (
            isinstance(selected_value, bool)
            or not isinstance(selected_value, (int, float))
            or not math.isfinite(selected_value)
        ):
            raise InputBlockedError("HYG monthly total return must be finite")
        if row.get("threshold") != 0.0:
            raise InputBlockedError("HYG threshold mismatch")
        expected_state = "SPY" if selected_value > 0.0 else "CASH"
        if row.get("state") != expected_state:
            raise InputBlockedError("HYG signal arithmetic mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                _sha(selection["row_hash"], "HYG row SHA-256"),
            )
        )
    ...
    if len(output["validation"]) != VALIDATION_INTERVALS or len(output["retrospective_holdout"]) != HOLDOUT_INTERVALS:
        raise InputBlockedError("HYG signal split count mismatch")
    for group in output.values():
        if any(left.end != right.start for left, right in zip(group, group[1:], strict=False)):
            raise InputBlockedError("HYG signal intervals are not contiguous")
    return output
...
def _load_bundle(payload: bytes, *, stage: str, signals: tuple[Signal, ...]) -> Bundle:
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError("HYG signal does not bind the SPY execution schedule")
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
        "schema_version": "c11-hyg-credit-appetite-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c11-hyg-credit-appetite-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
...
def execute_stage(stage: str) -> dict[str, Any]:
    ...
    if stage == "validation":
        if any(path.exists() for path in (VALIDATION_CLAIM, VALIDATION_RESULT, HOLDOUT_CLAIM, HOLDOUT_RESULT)):
            raise InputBlockedError("Cycle 10 one-use state is not pristine")
        ...
        if HOLDOUT_CLAIM.exists() or HOLDOUT_RESULT.exists():
            raise InputBlockedError("Cycle 10 holdout one-use state is not pristine")
