"""One-use runner for the frozen C67 policy-uncertainty SPY/QQQ rotation lane."""
...
from research.adapters.c67_fred_us_economic_policy_uncertainty_impulse_rotation import (  # noqa: E402
    HOLDOUT_INTERVALS,
    INITIAL_CAPITAL,
    PROGRAM_ALPHA,
    RESEARCH_ID,
...
DEFINITION = ROOT / "research/definitions/c67_fred_us_economic_policy_uncertainty_impulse_rotation_v1.json"
ADAPTER = ROOT / "research/adapters/c67_fred_us_economic_policy_uncertainty_impulse_rotation.py"
RUNNER = Path(__file__).resolve()
DEFINITION_SHA256 = "645c6b08fb301e9a483e5c8883b73909c664d4a136f207cb700de640d67c7a54"
ADAPTER_SHA256 = "e8a6c9caed9e1b86614f3c2f9ceb538b15968041b6809edc9c6025a65f69653e"
SIGNAL_SHA256 = "28126f8188f90401aaafc5038ec42068d49ad9e22759a6dcbf1606124f70667d"
RECEIPT_SHA256 = "a148825bb758bc1591806a836bbbecbb75df14f0643e46d779da7dca187b74ef"
...
SIGNAL_INPUT = Path(
    ...
    "c67_fred_us_economic_policy_uncertainty_impulse_rotation_spy_qqq_v1/"
...
def _signals(payload: bytes) -> dict[str, tuple[Signal, ...]]:
    record = _json(payload, "USEPUINDXM uncertainty-impulse input packet")
    expected_contract = {
        "series_id": "FRED_USEPUINDXM",
        "return_window": (
            "official USEPUINDXM observation in M-3 versus M-2 for "
            "execution month M"
        ),
        "target_rule": "strict policy-uncertainty decline selects QQQ; equality or increase selects SPY",
        "threshold_index_points": 0.0,
    }
    if (
        record.get("schema_version")
        != "c67-usepuindxm-uncertainty-impulse-input-packet-v1"
        ...
        raise InputBlockedError("USEPUINDXM packet boundary mismatch")
    rows = record.get("signals")
    if type(rows) is not list or len(rows) != VALIDATION_INTERVALS + HOLDOUT_INTERVALS:
        raise InputBlockedError("USEPUINDXM signal row count mismatch")
    parsed: dict[str, list[Signal]] = {
        "validation": [],
        "retrospective_holdout": [],
    }
    expected_raw_sha = "9bb7dcce5ad4b5e528bc5bc0f085219c37822aa6440143c0ebc271e7fb800b00"
    expected_url = (
        "https://fred.stlouisfed.org/graph/fredgraph.csv?"
        "id=USEPUINDXM&cosd=2020-10-01&coed=2026-06-30"
    )
    expected_availability_rule = (
        "20:00 America/New_York on the 15th calendar day of the following month; "
        "retrospective-secondary current FRED history without vintage proof"
    )
    for row in rows:
        if type(row) is not dict or row.get("stage") not in parsed:
            raise InputBlockedError("USEPUINDXM signal stage mismatch")
        ...
        if type(current) is not dict or type(previous) is not dict:
            raise InputBlockedError("USEPUINDXM selections are required")
        for selection in (previous, current):
            ...
            value = selection.get("index_points")
            if (
                selection.get("series_id") != "FRED_USEPUINDXM"
                ...
                raise InputBlockedError("USEPUINDXM source row identity mismatch")
        current_value = float(current["index_points"])
        previous_value = float(previous["index_points"])
        ...
        if (
            ...
            raise InputBlockedError("USEPUINDXM change arithmetic mismatch")
        ...
        expected_current_ordinal = start.year * 12 + start.month - 3
        ...
        if (
            ...
            raise InputBlockedError("USEPUINDXM observation or availability mismatch")
        identity = {
            "stage": row["stage"],
        ...
        if any(row.get(key) != value for key, value in identity.items()):
            raise InputBlockedError("USEPUINDXM signal arithmetic mismatch")
        identity_hash = _hash_bytes(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        )
        if row.get("signal_identity_sha256") != identity_hash:
            raise InputBlockedError("USEPUINDXM signal identity hash mismatch")
        parsed[row["stage"]].append(
            Signal(
                ...
                decision_at,
                expected_state,
                _sha(row["signal_identity_sha256"], "USEPUINDXM row SHA-256"),
            )
        )
    ...
    if (
        ...
        raise InputBlockedError("USEPUINDXM signal split count mismatch")
    support = record.get("state_support")
    if type(support) is not dict:
        raise InputBlockedError("USEPUINDXM state support is required")
    for stage, group in output.items():
        if any(
            left.end != right.start
            for left, right in zip(group, group[1:], strict=False)
        ):
            raise InputBlockedError("USEPUINDXM signal intervals are not contiguous")
        ...
        if support.get(stage) != observed:
            raise InputBlockedError("USEPUINDXM state support mismatch")
    return output
...
def _combined_snapshot(
    ...
    *,
    ...
    source = SourceIdentity(
        ...
        available_at,
        retrieved_at,
        f"c67-spy-qqq-universe-{execution_session.isoformat()}",
    )
...
def _load_bundle(
    ...
    *,
    ...
    packet = _json(input_payload, "USEPUINDXM uncertainty-impulse/QQQ input packet")
    receipt = _json(
        receipt_payload,
        "USEPUINDXM uncertainty-impulse/QQQ materialization receipt",
    )
    ...
    if (
        ...
        != "c67-usepuindxm-full-materialization-receipt-v1"
        ...
        != "02d3dc508c9c3fbb1c82e12697d80e954730222ba62deec1d12a7400a993424d"
        or receipt.get("raw_response_sha256")
        != "9bb7dcce5ad4b5e528bc5bc0f085219c37822aa6440143c0ebc271e7fb800b00"
    ...
    for index, raw in enumerate(raw_points):
        ...
        if signal is not None and (
            ...
            raise InputBlockedError(
                "USEPUINDXM signal does not bind the execution schedule"
            )
        try:
...
def _claim(stage: str, runner_sha: str, bundle_sha: str, validation_result_sha: str | None) -> dict[str, Any]:
    return {
            "schema_version": "c67-usepuindxm-uncertainty-impulse-claim-v1",
...
def _result(
    ...
    *,
    ...
    return {
        "schema_version": "c67-usepuindxm-uncertainty-impulse-result-v1",
        "research_id": RESEARCH_ID,
        "stage": stage,
