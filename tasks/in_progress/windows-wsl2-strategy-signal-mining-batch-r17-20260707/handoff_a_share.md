...
- RTX 5090 power policy: prior 400W cap revoked by user; proceed under host/driver default policy and report observed telemetry.
...
Run focused validation appropriate to changed files, including py_compile, focused pytest, JSON parse, `git diff --check`, safety/overclaim scans, full-frame guard check, and GPU power-policy telemetry for GPU work.

## Boundary

Research-only. No recommendation/advice, ticket, eligibility candidate, strategy candidate promotion, product route, readiness, broker/order/paper/live/auto, secrets, raw-data migration, unapproved network/provider fetch, unapproved DB/cache write, full-frame wide3068, or privileged power-limit changes.
