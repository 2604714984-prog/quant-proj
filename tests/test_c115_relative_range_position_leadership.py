...
from research.adapters import c115_relative_range_position_leadership as adapter
from research.adapters.c115_relative_range_position_leadership import (
...
from scripts import run_c115_relative_range_position_leadership_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 11_500_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch68_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            spy_position = 0.6
            qqq_position = 0.75 if index % 2 == 0 else 0.45
            difference = qqq_position - spy_position
            ...
            row: dict[str, object] = {
                ...
                "qqq_range_position": qqq_position,
                "relative_range_position_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_range_position": spy_position,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c115-relative-range-position-leadership-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "(final adjusted close - minimum adjusted close) divided "
                    "by (maximum adjusted close - minimum adjusted close) "
                    "over exactly 127 sessions"
                ),
                "target_rule": (
                    "QQQ range position strictly greater than SPY selects "
                    "QQQ; equality or lower selects SPY"
                ),
            },
            "signals": rows,
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_range_position_difference"] = 2.0
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (23, 5)
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (29, 5)
