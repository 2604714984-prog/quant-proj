...
from research.adapters import c72_spy_qqq_12_1_relative_momentum_rotation as adapter
from research.adapters.c72_spy_qqq_12_1_relative_momentum_rotation import (
...
from scripts import run_c72_spy_qqq_12_1_relative_momentum_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 7_200_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch25_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            skip_session = signal_session - timedelta(days=30)
            base_session = signal_session - timedelta(days=365)
            ...
            spy_return = 0.10
            qqq_return = 0.12 if index % 2 == 0 else 0.08
            target = "QQQ" if qqq_return > spy_return else "SPY"
            targets.append(target)
            source_rows = {
                symbol: [
                    hashlib.sha256(
                        f"{stage}-{index}-{symbol}-{slot}".encode()
                    ).hexdigest()
                    for slot in range(3)
                ]
            ...
            rows.append(
                {
                    ...
                    "skip_session": skip_session.isoformat(),
                    "base_session": base_session.isoformat(),
                    "spy_12_1_return": spy_return,
                    "qqq_12_1_return": qqq_return,
                    "target_symbol": target,
    ...
    return json.dumps(
        {
            "schema_version": "c72-spy-qqq-12-1-relative-momentum-input-v1",
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "QQQ 12-1 return strictly greater than SPY means QQQ; "
                    "otherwise SPY"
                ),
                "tie_break": "SPY",
            },
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (6, 28)
