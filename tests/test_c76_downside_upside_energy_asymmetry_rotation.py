...
from research.adapters import c76_downside_upside_energy_asymmetry_rotation as adapter
from research.adapters.c76_downside_upside_energy_asymmetry_rotation import (
...
from scripts import run_c76_downside_upside_energy_asymmetry_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 7_600_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch29_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            formation_start = signal_session - timedelta(days=90)
            ...
            spy_downside = 0.01
            spy_upside = 0.02
            qqq_downside = 0.008 if index % 2 == 0 else 0.012
            qqq_upside = 0.02
            spy_ratio = spy_downside / spy_upside
            qqq_ratio = qqq_downside / qqq_upside
            target = "QQQ" if qqq_ratio < spy_ratio else "SPY"
            ...
            row = {
                ...
                "return_count": 63,
                "spy_downside_energy": spy_downside,
                "spy_upside_energy": spy_upside,
                "spy_asymmetry_ratio": spy_ratio,
                "qqq_downside_energy": qqq_downside,
                "qqq_upside_energy": qqq_upside,
                "qqq_asymmetry_ratio": qqq_ratio,
    ...
    return json.dumps(
        {
            "schema_version": "c76-downside-upside-energy-asymmetry-input-v1",
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "QQQ asymmetry ratio strictly below SPY means QQQ; "
                    "equality or greater means SPY"
                ),
                "tie_break": "SPY",
            },
            "signals": rows,
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (17, 11)
