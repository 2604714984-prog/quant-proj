...
from research.adapters import c118_qqq_14_session_rsi_state as adapter
from research.adapters.c118_qqq_14_session_rsi_state import (
...
from scripts import run_c118_qqq_14_session_rsi_state_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 11_800_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch71_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_average_gain = 0.7 if index % 2 == 0 else 0.3
            qqq_average_loss = 0.3 if index % 2 == 0 else 0.7
            qqq_rsi = 100.0 - 100.0 / (
                1.0 + qqq_average_gain / qqq_average_loss
            )
            target = "QQQ" if qqq_rsi > 50.0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "formation_session_count": 15,
                "formation_start": (signal_session - timedelta(days=30)).isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "qqq_average_gain_14": qqq_average_gain,
                "qqq_average_loss_14": qqq_average_loss,
                "qqq_rsi_14": qqq_rsi,
    ...
    return json.dumps(
        {
            "schema_version": "c118-qqq-14-session-rsi-state-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "RSI from arithmetic mean gains and losses over exactly "
                    "14 QQQ adjusted-close differences; zero average loss "
                    "sets RSI to 100"
                ),
                "target_rule": (
                    "QQQ RSI strictly greater than 50 selects QQQ; equality "
                    "or lower selects SPY"
                ),
            },
            "signals": rows,
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_rsi_14"] = 42.0
