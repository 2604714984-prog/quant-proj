...
from quant_system.research.us_cpi_release_intraday import (
    DailyBar,
    ResearchInputError,
    build_observations,
...
DEFINITION = (
    ...
    / "research"
    / "definitions"
    / "us_cpi_release_day_spy_intraday_premium_v1.json"
)


def test_frozen_monthly_release_dates_selects_later_revision_date() -> None:
    text = "\n".join(
        [
            "Release Dates:",
            "2024-01-11",
...
def test_definition_freezes_one_complete_event_list() -> None:
    ...
    assert events[0] == "2016-01-20"
    assert events[-1] == "2024-12-11"
