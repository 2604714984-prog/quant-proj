...
from research.adapters import c67_fred_us_economic_policy_uncertainty_impulse_rotation as adapter
from research.adapters.c67_fred_us_economic_policy_uncertainty_impulse_rotation import (
...
from scripts import run_c67_fred_us_economic_policy_uncertainty_impulse_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 6_700_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch20_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    rows = []
    state_support = {}
    raw_sha = "9bb7dcce5ad4b5e528bc5bc0f085219c37822aa6440143c0ebc271e7fb800b00"
    url = (
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            current_reference = _month(start, -2)
            previous_reference = _month(start, -3)
            current_observation = current_reference
            previous_observation = previous_reference
            current_value, previous_value = (
                (100.0, 110.0) if index % 2 == 0 else (120.0, 110.0)
            )
            ...
            def source(value: float, observed: date, reference: date) -> dict:
                identity = {
                    ...
                    "available_at_conservative": (
                        f"{_month(reference, 1).replace(day=15).isoformat()}T20:00:00-05:00"
                    ),
                    "availability_rule": (
    ...
    return json.dumps(
        {
            "schema_version": "c67-usepuindxm-uncertainty-impulse-input-packet-v1",
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "strict policy-uncertainty decline selects QQQ; equality or increase selects SPY"
                ),
