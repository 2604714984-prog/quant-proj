...
from research.adapters import c18_spy_halloween_seasonality as adapter
from research.adapters.c18_spy_halloween_seasonality import (
...
from scripts import run_c18_spy_halloween_seasonality_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.0000003814697265625
    assert BOOTSTRAP_RESAMPLES == 8_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 6
    assert BOOTSTRAP_SEED == 1_500_601
...
def test_bootstrap_index_sequence_has_a_literal_golden_vector() -> None:
    ...
    assert circular_block_indices(8, block_length=6, rng=rng) == (
        2,
        3,
        4,
        5,
...
def test_signal_packet_reproduces_frozen_support_without_price_access() -> None:
    ...
    assert sum(row.target == 1.0 for row in groups["validation"]) == 15
    assert sum(row.target == 0.0 for row in groups["validation"]) == 13
    assert sum(row.target == 1.0 for row in groups["retrospective_holdout"]) == 18
    assert sum(row.target == 0.0 for row in groups["retrospective_holdout"]) == 16
...
def test_signal_packet_rejects_state_tampering() -> None:
    ...
    record["signals"][0]["target"] = (
        "CASH" if record["signals"][0]["target"] == "SPY" else "SPY"
    )
...
def test_real_one_use_state_is_pristine_or_terminal_without_holdout() -> None:
    ...
    if runner.HOLDOUT_CLAIM.exists():
        assert runner.HOLDOUT_RESULT.exists()
