...
from research.adapters import c145_nyfed_treasury_settlement_fails_stress as adapter
from research.adapters.c145_nyfed_treasury_settlement_fails_stress import (
    HOLDOUT_INTERVALS,
    InputContractError,
...
from scripts import run_c145_nyfed_treasury_settlement_fails_stress_once as runner
...
def test_real_signal_packet_is_exact_and_state_supported() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in validation) == 61
    assert sum(row.target_symbol == "SPY" for row in validation) == 68
    assert sum(row.target_symbol == "QQQ" for row in holdout) == 79
    assert sum(row.target_symbol == "SPY" for row in holdout) == 77
...
def test_signal_packet_rejects_target_and_identity_tampering() -> None:
    ...
    record = json.loads(runner.SIGNAL_INPUT.read_bytes())
    record["signals"][0]["fails_impulse_usd_millions"] += 0.01
    with pytest.raises(runner.InputBlockedError, match="fails impulse is invalid"):
        runner._signals(json.dumps(record).encode())
...
def test_circular_block_indices_are_frozen_and_wrap() -> None:
    observed = circular_block_indices(
        7,
        block_length=4,
        rng=random.Random(14500101),
    )
    assert observed == (5, 6, 0, 1, 6, 0, 1)
...
def test_holdout_bootstrap_and_seven_gates_are_deterministic() -> None:
    ...
    assert decision.inference.seed == 14_500_101
