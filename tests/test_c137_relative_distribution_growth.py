...
from research.adapters import c137_relative_distribution_growth as adapter
from research.adapters.c137_relative_distribution_growth import (
...
from scripts import run_c137_relative_distribution_growth_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_700_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch81_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_distribution_growth = 0.10 if index % 2 == 0 else -0.10
            spy_distribution_growth = 0.0
            qqq_minus_spy_distribution_growth = (
                qqq_distribution_growth - spy_distribution_growth
            )
            target = "QQQ" if qqq_minus_spy_distribution_growth > 0.0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "formation_end": signal_session.isoformat(),
                "formation_session_count": 16,
                "formation_start": (signal_session - timedelta(days=730)).isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "qqq_distribution_growth": qqq_distribution_growth,
                "qqq_minus_spy_distribution_growth": qqq_minus_spy_distribution_growth,
                "spy_distribution_growth": spy_distribution_growth,
    ...
    return json.dumps(
        {
            "schema_version": "c137-relative-distribution-growth-input-v1",
            ...
            "volume_values_used_for_signal": False,
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    record = json.loads(_synthetic_signal_packet())
    record["signals"][0]["qqq_distribution_growth"] = -1.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="distribution-growth identity mismatch"):
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (9, 19)
