...
from research.adapters import c21_rsafs_retail_demand_rotation as adapter
from research.adapters.c21_rsafs_retail_demand_rotation import (
...
from scripts import run_c21_rsafs_retail_demand_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.0000000476837158203125
    assert BOOTSTRAP_RESAMPLES == 32_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_800_901
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            current_value, previous_value = (
                (390_000.0, 400_000.0) if qqq else (410_000.0, 400_000.0)
            )
            ...
            def source(value: float, cutoff: date, observation: date) -> dict:
                identity = {
                    ...
                    "source_class": "OFFICIAL_ALFRED_RSAFS",
                }
                return {
                    **identity,
    ...
    return json.dumps(
        {
            "schema_version": "c21-rsafs-retail-demand-rotation-spy-qqq-input-packet-v1",
