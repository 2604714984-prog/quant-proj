...
from research.adapters import c23_umcsent_behavioral_rotation as adapter
from research.adapters.c23_umcsent_behavioral_rotation import (
...
from scripts import run_c23_umcsent_behavioral_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.000000011920928955078125
    assert BOOTSTRAP_RESAMPLES == 128_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 2_000_101
...
def _synthetic_signal_packet() -> bytes:
        ...
        for index in range(count):
            ...
            def source(value: float, cutoff: date, observation: date) -> dict:
                identity = {
                    ...
                    "source_class": "OFFICIAL_ALFRED_UMCSENT",
                }
                return {
                    **identity,
    ...
    return json.dumps(
        {
            "schema_version": "c23-umcsent-behavioral-rotation-spy-qqq-input-packet-v1",
