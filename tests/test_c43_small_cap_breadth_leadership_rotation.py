...
from research.adapters import c43_small_cap_breadth_leadership_rotation as adapter
from research.adapters.c43_small_cap_breadth_leadership_rotation import (
...
from scripts import run_c43_small_cap_breadth_leadership_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_300_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch6_mechanism_count"] == 3
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "IWM", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "IWM", "100", previous_reference, previous_session
            )
    ...
    return json.dumps(
        {
            "schema_version": "epoch6-intermarket-leadership-input-packet-v1",
