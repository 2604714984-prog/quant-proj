...
from research.adapters import c47_copper_gold_growth_rotation as adapter
from research.adapters.c47_copper_gold_growth_rotation import (
...
from scripts import run_c47_copper_gold_growth_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_700_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "CPER", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "CPER", "100", previous_reference, previous_session
            )
