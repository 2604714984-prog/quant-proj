...
from research.adapters import c154_baa10y_spread_trailing_median as adapter
from research.adapters.c154_baa10y_spread_trailing_median import (
...
from scripts import run_c154_baa10y_spread_trailing_median_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 15_400_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            trailing_median = 2.0
            current_spread = 1.8 if qqq else 2.2
            distance = current_spread - trailing_median
            ...
            reference_month = _month(start, -1)
            trailing_start = _month(reference_month, -12)
            trailing_end = _month(reference_month, -1)
            identity = {
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c154-baa10ym-spy-qqq-full-input-v1",
...
def test_real_input_and_runtime_bundles_preflight_before_any_claim() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 19
    assert sum(row.target_symbol == "SPY" for row in groups["validation"]) == 9
    assert (
        sum(
