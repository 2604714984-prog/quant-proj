...
from research.adapters import c120_qqq_obv_direction as adapter
from research.adapters.c120_qqq_obv_direction import (
...
from scripts import run_c120_qqq_obv_direction_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_000_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch73_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_obv = 10_000.0 if index % 2 == 0 else -10_000.0
            target = "QQQ" if qqq_obv > 0.0 else "SPY"
    ...
    return json.dumps(
        {
            "schema_version": "c120-qqq-obv-direction-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "OBV starts at zero over exactly 63 QQQ adjusted close and "
                    "adjusted volume observations; each of the final 62 volumes "
                    "is added after an up close, subtracted after a down close, "
                    "and ignored on a tie"
                ),
                "target_rule": (
