...
from research.adapters import c141_finra_relative_short_sale_flow as adapter
from research.adapters.c141_finra_relative_short_sale_flow import (
    HOLDOUT_INTERVALS,
    InputContractError,
...
from scripts import run_c141_finra_relative_short_sale_flow_once as runner
...
def test_real_signal_packet_is_exact_and_state_supported() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in validation) == 95
    assert sum(row.target_symbol == "SPY" for row in validation) == 34
    assert sum(row.target_symbol == "QQQ" for row in holdout) == 120
    assert sum(row.target_symbol == "SPY" for row in holdout) == 36
...
def test_circular_block_indices_are_frozen_and_wrap() -> None:
    observed = circular_block_indices(
        7,
        block_length=4,
        rng=random.Random(14100101),
    )
    assert observed == (1, 2, 3, 4, 5, 6, 0)
...
def test_holdout_bootstrap_and_seven_gates_are_deterministic() -> None:
    ...
    assert decision.inference.seed == 14_100_101
