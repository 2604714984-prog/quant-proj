...
from research.adapters import c50_high_beta_low_vol_risk_appetite_rotation as adapter
from research.adapters.c50_high_beta_low_vol_risk_appetite_rotation import (
...
from scripts import run_c50_high_beta_low_vol_risk_appetite_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 5_000_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch8_mechanism_count"] == 3
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "SPHB", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "SPHB", "100", previous_reference, previous_session
            )
            right_current = source(
                "SPLV", "100", current_reference, current_session
            )
            right_previous = source(
                "SPLV", "100", previous_reference, previous_session
            )
    ...
    return json.dumps(
        {
            "schema_version": "epoch8-style-leadership-input-packet-v1",
