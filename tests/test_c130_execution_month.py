...
from research.adapters import c130_execution_month as adapter
from research.adapters.c130_execution_month import (
...
from scripts import run_c130_execution_month_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_000_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            target = "QQQ" if execution_month in {11, 12, 1, 2, 3, 4} else "SPY"
    ...
    return json.dumps(
        {
            "schema_version": "c130-execution-month-input-v1",
...
def test_synthetic_packet_reproduces_frozen_rule_without_outcomes() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 15
    assert sum(
