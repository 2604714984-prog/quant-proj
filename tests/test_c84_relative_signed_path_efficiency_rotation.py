...
from research.adapters import c84_relative_signed_path_efficiency_rotation as adapter
from research.adapters.c84_relative_signed_path_efficiency_rotation import (
...
from scripts import run_c84_relative_signed_path_efficiency_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 8_400_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch37_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            formation_start = signal_session - timedelta(days=200)
            ...
            spy_numerator = 0.10
            spy_denominator = 0.50
            spy_efficiency = spy_numerator / spy_denominator
            qqq_numerator = 0.20 if index % 2 == 0 else 0.00
            qqq_denominator = 0.50
            ...
            row = {
                ...
                "return_count": 126,
                "spy_log_return_sum": spy_numerator,
                "spy_absolute_log_return_sum": spy_denominator,
                "spy_signed_path_efficiency": spy_efficiency,
                "qqq_log_return_sum": qqq_numerator,
                "qqq_absolute_log_return_sum": qqq_denominator,
                "qqq_signed_path_efficiency": qqq_efficiency,
                "relative_efficiency_difference": difference,
                "formation_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-formation".encode()
                ).hexdigest(),
    ...
    return json.dumps(
        {
            "schema_version": "c84-relative-signed-path-efficiency-input-v1",
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (22, 6)
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (21, 13)
