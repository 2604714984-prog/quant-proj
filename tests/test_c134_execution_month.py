...
from research.adapters import c134_execution_month as adapter
from research.adapters.c134_execution_month import (
...
from scripts import run_c134_execution_month_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_400_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch78_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            target = "QQQ" if execution_month in {1, 4, 7, 10} else "SPY"
    ...
    return json.dumps(
        {
            "schema_version": "c134-execution-month-input-v1",
...
def test_synthetic_packet_reproduces_frozen_rule_without_outcomes() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 9
    assert sum(
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["execution_month"] = 13
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="execution-month range mismatch"):
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (19, 9)
