...
from research.adapters import c122_qqq_short_long_volatility as adapter
from research.adapters.c122_qqq_short_long_volatility import (
...
from scripts import run_c122_qqq_short_long_volatility_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_200_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch75_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_long_volatility = 0.02
            qqq_short_volatility = 0.01 if index % 2 == 0 else 0.03
            target = (
                "QQQ"
                if qqq_short_volatility <= qqq_long_volatility
                else "SPY"
            )
            ...
            row: dict[str, object] = {
                ...
                "qqq_long_volatility": qqq_long_volatility,
                "qqq_short_volatility": qqq_short_volatility,
    ...
    return json.dumps(
        {
            "schema_version": "c122-qqq-short-long-volatility-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "sample standard deviation with ddof 1 of the final 21 QQQ "
                    "returns versus sample standard deviation with ddof 1 of all "
                    "126 returns formed from exactly 127 adjusted closes"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_short_volatility"] = 42.0
