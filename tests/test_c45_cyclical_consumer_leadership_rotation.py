...
from research.adapters import c45_cyclical_consumer_leadership_rotation as adapter
from research.adapters.c45_cyclical_consumer_leadership_rotation import (
...
from scripts import run_c45_cyclical_consumer_leadership_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_500_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "XLY", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "XLY", "100", previous_reference, previous_session
            )
            right_current = source(
                "XLP", "100", current_reference, current_session
            )
            right_previous = source(
                "XLP", "100", previous_reference, previous_session
            )
