...
from research.adapters import c34_cftc_relative_equity_positioning_rotation as adapter
from research.adapters.c34_cftc_relative_equity_positioning_rotation import (
...
from scripts import run_c34_cftc_relative_equity_positioning_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 3_400_101
...
def _synthetic_signal_packet() -> bytes:
    def contract(
        ...
        identity = {
            "market_and_exchange_name": market_name,
            "report_date": report_date.isoformat(),
            "cftc_contract_market_code": code,
            "open_interest_all": 1000,
            "asset_manager_long_all": long_value,
            "asset_manager_short_all": short_value,
    ...
    def selection(report_date: date, positive_relative: bool) -> dict:
        sp500 = contract(
            report_date,
            runner.CFTC_SP500_CODE,
        ...
        nasdaq100 = contract(
            report_date,
            runner.CFTC_NASDAQ100_CODE,
            "NASDAQ-100 Consolidated - CHICAGO MERCANTILE EXCHANGE",
            650 if positive_relative else 550,
            350 if positive_relative else 450,
        )
        sp500_ratio = (
            sp500["asset_manager_long_all"] - sp500["asset_manager_short_all"]
        ) / sp500["open_interest_all"]
        nasdaq100_ratio = (
            nasdaq100["asset_manager_long_all"]
            - nasdaq100["asset_manager_short_all"]
        ) / nasdaq100["open_interest_all"]
        ...
        identity = {
            ...
            "sp500_contract_row_sha256": sp500["row_sha256"],
            "nasdaq100_contract_row_sha256": nasdaq100["row_sha256"],
            "sp500_asset_manager_net_pct_oi": sp500_ratio,
            "nasdaq100_asset_manager_net_pct_oi": nasdaq100_ratio,
            "relative_asset_manager_net_pct_oi": nasdaq100_ratio - sp500_ratio,
        }
        return {
            **identity,
            "sp500_contract_row": sp500,
            "nasdaq100_contract_row": nasdaq100,
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            delta = (
                current["relative_asset_manager_net_pct_oi"]
                - previous["relative_asset_manager_net_pct_oi"]
            )
            ...
            identity = {
                ...
                "delta_relative_asset_manager_net_pct_oi": delta,
    ...
    return json.dumps(
        {
            ...
            "signal_contract": {
                "nasdaq100_contract_code": runner.CFTC_NASDAQ100_CODE,
                "sp500_contract_code": runner.CFTC_SP500_CODE,
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (23, 11)
