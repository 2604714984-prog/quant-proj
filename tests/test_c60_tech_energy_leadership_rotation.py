...
from research.adapters import c60_tech_energy_leadership_rotation as adapter
from research.adapters.c60_tech_energy_leadership_rotation import (
...
from scripts import run_c60_tech_energy_leadership_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 6_000_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch13_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "XLK", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "XLK", "100", previous_reference, previous_session
            )
            right_current = source(
                "XLE", "100", current_reference, current_session
            )
            right_previous = source(
                "XLE", "100", previous_reference, previous_session
            )
    ...
    return json.dumps(
        {
            "schema_version": "c60-tech-energy-leadership-input-packet-v1",
