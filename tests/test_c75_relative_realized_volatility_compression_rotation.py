...
from research.adapters import c75_relative_realized_volatility_compression_rotation as adapter
from research.adapters.c75_relative_realized_volatility_compression_rotation import (
...
from scripts import run_c75_relative_realized_volatility_compression_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 7_500_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch28_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            older_qqq_vol = 0.012
            recent_spy_vol = 0.01
            recent_qqq_vol = 0.011 if index % 2 == 0 else 0.013
            older_ratio = older_qqq_vol / older_spy_vol
            recent_ratio = recent_qqq_vol / recent_spy_vol
            target = "QQQ" if recent_ratio < older_ratio else "SPY"
            ...
            row = {
                ...
                "ddof": 1,
                "older_spy_realized_volatility": older_spy_vol,
                "older_qqq_realized_volatility": older_qqq_vol,
                "recent_spy_realized_volatility": recent_spy_vol,
                "recent_qqq_realized_volatility": recent_qqq_vol,
                "older_relative_volatility_ratio": older_ratio,
                "recent_relative_volatility_ratio": recent_ratio,
                "relative_volatility_ratio_change": recent_ratio - older_ratio,
    ...
    return json.dumps(
        {
            "schema_version": "c75-relative-realized-volatility-compression-input-v1",
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "strict recent ratio compression means QQQ; equality or "
                    "expansion means SPY"
                ),
                "tie_break": "SPY",
            },
