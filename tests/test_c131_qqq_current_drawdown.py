...
from research.adapters import c131_qqq_current_drawdown as adapter
from research.adapters.c131_qqq_current_drawdown import (
...
from scripts import run_c131_qqq_current_drawdown_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_100_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_current_drawdown = -0.02 if index % 2 == 0 else -0.08
            target = "QQQ" if qqq_current_drawdown > -0.05 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "qqq_current_drawdown": qqq_current_drawdown,
    ...
    return json.dumps(
        {
            "schema_version": "c131-qqq-current-drawdown-input-v1",
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_current_drawdown"] = 1.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="current-drawdown range mismatch"):
