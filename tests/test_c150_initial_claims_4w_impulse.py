...
from research.adapters import c150_initial_claims_4w_impulse as adapter
from research.adapters.c150_initial_claims_4w_impulse import (
...
from scripts import run_c150_initial_claims_4w_impulse_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 15_000_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            previous_mean = 250_000.0
            current_mean = 240_000.0 if qqq else 260_000.0
            impulse = current_mean - previous_mean
            ...
            current_end = decision_date - timedelta(days=6)
            current_start = current_end - timedelta(days=21)
            previous_end = current_start - timedelta(days=7)
            previous_start = previous_end - timedelta(days=21)
            identity = {
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c150-icsa-spy-qqq-full-input-v1",
...
def test_real_input_and_runtime_bundles_preflight_before_any_claim() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 17
    assert sum(row.target_symbol == "SPY" for row in groups["validation"]) == 11
    assert (
        sum(
