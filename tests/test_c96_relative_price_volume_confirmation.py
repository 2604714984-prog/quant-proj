...
from research.adapters import c96_relative_price_volume_confirmation as adapter
from research.adapters.c96_relative_price_volume_confirmation import (
...
from scripts import run_c96_relative_price_volume_confirmation_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 9_600_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch49_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_confirmation = 0.1 if index % 2 == 0 else -0.1
            spy_confirmation = 0.0
            difference = qqq_confirmation - spy_confirmation
            ...
            row: dict[str, object] = {
                ...
                "correlation_pair_count": 63,
                "qqq_return_volume_change_correlation": qqq_confirmation,
                "relative_price_volume_confirmation_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_return_volume_change_correlation": spy_confirmation,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c96-relative-price-volume-confirmation-input-v1",
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_price_volume_confirmation_difference"] = 0.2
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (11, 17)
