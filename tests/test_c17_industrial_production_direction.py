...
from research.adapters.c17_industrial_production_direction import (
...
from scripts import run_c17_industrial_production_direction_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.000000762939453125
    assert BOOTSTRAP_RESAMPLES == 4_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_500_501
...
def test_constant_positive_active_holdout_passes_exact_inference() -> None:
    ...
    assert decision.inference.centered_null_one_sided_p == pytest.approx(1 / 4000001)
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            current, prior = ((100.0, 100.0) if qqq else (101.0, 100.0))
            identity = {
                "series_id": "INDPRO",
                "reference_month": start.isoformat()[:7],
                "reference_industrial_production_index": current,
                "prior_month": _month(start, -1).isoformat()[:7],
                "prior_industrial_production_index": prior,
                "fred_indpro_response_sha256": "a" * 64,
            }
            rows.append(
                {
                    "stage": stage,
                    ...
                    "signal_session": _month(start, -1).isoformat(),
                    "decision_at": f"{start.isoformat()}T09:00:00-05:00",
                    "reference_month": identity["reference_month"],
                    "monthly_change": current / prior - 1.0,
                    "comparison": "indpro_gt_prior_month_selects_SPY_else_QQQ",
                    "state": "QQQ" if qqq else "SPY",
    ...
    return json.dumps(
        {
            "schema_version": "c17-industrial-production-direction-spy-qqq-input-packet-v1",
...
def test_signal_packet_rejects_state_tampering() -> None:
    ...
    record["signals"][0]["state"] = "QQQ"
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="state arithmetic"):
