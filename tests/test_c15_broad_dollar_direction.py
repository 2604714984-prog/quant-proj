...
from research.adapters.c15_broad_dollar_direction import (
...
from scripts import run_c15_broad_dollar_direction_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.0000030517578125
    assert BOOTSTRAP_RESAMPLES == 1_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_500_301
...
def test_constant_positive_active_holdout_passes_exact_inference() -> None:
    ...
    assert decision.inference.centered_null_one_sided_p == pytest.approx(1 / 1000001)
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            identity = {
                ...
                "reference_broad_dollar_index": current,
                "prior_month": _month(start, -1).isoformat()[:7],
                "prior_broad_dollar_index": prior,
                "fred_broad_dollar_response_sha256": "a" * 64,
            }
            rows.append(
                {
                    "stage": stage,
                    ...
                    "comparison": "broad_dollar_gt_prior_month_selects_SPY_else_QQQ",
    ...
    return json.dumps(
        {
            "schema_version": "c15-broad-dollar-direction-spy-qqq-input-packet-v1",
