...
from research.adapters import c48_emerging_market_risk_appetite_rotation as adapter
from research.adapters.c48_emerging_market_risk_appetite_rotation import (
...
from scripts import run_c48_emerging_market_risk_appetite_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_800_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch7_mechanism_count"] == 3
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "EEM", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "EEM", "100", previous_reference, previous_session
            )
    ...
    return json.dumps(
        {
            "schema_version": "epoch7-intermarket-leadership-input-packet-v1",
