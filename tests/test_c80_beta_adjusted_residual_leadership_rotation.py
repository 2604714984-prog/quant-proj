...
from research.adapters import c80_beta_adjusted_residual_leadership_rotation as adapter
from research.adapters.c80_beta_adjusted_residual_leadership_rotation import (
...
from scripts import run_c80_beta_adjusted_residual_leadership_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 8_000_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch33_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            evaluation_start = signal_session - timedelta(days=30)
            estimation_start = evaluation_start - timedelta(days=60)
            ...
            alpha = qqq_mean - beta * spy_mean
            residual_sum = 0.01 if index % 2 == 0 else -0.01
            target = "QQQ" if residual_sum > 0.0 else "SPY"
            ...
            row = {
                ...
                "estimation_block_start": estimation_start.isoformat(),
                "estimation_block_end": evaluation_start.isoformat(),
                "evaluation_block_start": evaluation_start.isoformat(),
                "evaluation_block_end": signal_session.isoformat(),
                "estimation_return_count": 42,
                "evaluation_return_count": 21,
                "ddof": 1,
                "estimation_spy_mean": spy_mean,
                "estimation_qqq_mean": qqq_mean,
                "estimation_spy_variance": spy_variance,
                "estimation_covariance": covariance,
                "ols_alpha": alpha,
                "ols_beta": beta,
                "evaluation_residual_sum": residual_sum,
                "estimation_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-estimation".encode()
                ).hexdigest(),
                "evaluation_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-evaluation".encode()
                ).hexdigest(),
    ...
    return json.dumps(
        {
            "schema_version": "c80-beta-adjusted-residual-leadership-input-v1",
            ...
            "signal_contract": {
                ...
                "estimator": (
                    "ordinary OLS with intercept fitted on the first 42 paired "
                    "returns using sample covariance and sample SPY variance "
                    "with ddof=1"
                ),
                "residual_statistic": (
                    "arithmetic sum over the final 21 observations of QQQ return "
                    "minus (frozen alpha plus frozen beta times SPY return)"
                ),
                "target_rule": (
                    "strictly positive evaluation residual sum means QQQ; zero "
                    "or negative means SPY"
                ),
                "tie_break": "SPY",
            },
            "signals": rows,
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (19, 15)
