...
from research.adapters import c110_relative_downside_risk_adjusted_trend as adapter
from research.adapters.c110_relative_downside_risk_adjusted_trend import (
...
from scripts import run_c110_relative_downside_risk_adjusted_trend_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 11_000_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch63_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            spy_score = 1.5
            qqq_score = 1.75 if index % 2 == 0 else 1.25
            difference = qqq_score - spy_score
            target = "QQQ" if difference > 0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "formation_session_count": 127,
                "formation_start": (signal_session - timedelta(days=180)).isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "qqq_downside_deviation": 0.012,
                "qqq_downside_risk_adjusted_trend_score": qqq_score,
                "qqq_total_return": qqq_score * 0.012,
                ...
                "spy_downside_deviation": 0.01,
                "spy_downside_risk_adjusted_trend_score": spy_score,
                "spy_total_return": spy_score * 0.01,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c110-relative-downside-risk-adjusted-trend-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    ...
                    "sqrt(mean(square(min(simple_daily_return,0)))) over "
                    "exactly 126 daily returns"
                ),
                "target_rule": (
                    "QQQ downside-risk-adjusted trend score strictly greater "
                    "than SPY selects QQQ; equality or lower selects SPY"
                ),
            },
            "signals": rows,
