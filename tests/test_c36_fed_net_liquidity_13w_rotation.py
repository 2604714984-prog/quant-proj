...
from research.adapters import c36_fed_net_liquidity_13w_rotation as adapter
from research.adapters.c36_fed_net_liquidity_13w_rotation import (
...
from scripts import run_c36_fed_net_liquidity_13w_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 3_600_101
...
def _synthetic_signal_packet() -> bytes:
    def component(
        ...
        identity = {
            ...
            "value": value,
            "unit": "USD_BILLIONS" if series_id == "RRPONTSYD" else "USD_MILLIONS",
            "vintage_date": vintage_date.isoformat(),
            "available_at_conservative": datetime.combine(
                vintage_date, datetime.max.time(), runner.NY
            ).isoformat(),
            "raw_response_sha256": "1" * 64,
            "source_class": "OFFICIAL_FRED_ALFRED_VINTAGE_GRAPH_EXPORT",
        }
        return {
            **identity,
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            vintage_date = decision_at.date() - timedelta(days=1)
            baseline_cutoff = vintage_date - timedelta(days=91)
            positive = index % 2 == 0
            current = {
                "WALCL": component(
                    "WALCL", vintage_date - timedelta(days=1), 1100.0 if positive else 900.0, vintage_date
                ),
                "WTREGEN": component("WTREGEN", vintage_date - timedelta(days=1), 0.0, vintage_date),
                "RRPONTSYD": component("RRPONTSYD", vintage_date, 0.0, vintage_date),
            }
            baseline = {
                "WALCL": component("WALCL", baseline_cutoff, 1000.0, vintage_date),
                "WTREGEN": component("WTREGEN", baseline_cutoff, 0.0, vintage_date),
                "RRPONTSYD": component("RRPONTSYD", baseline_cutoff, 0.0, vintage_date),
            }
            current_hashes = {
                key: value["row_sha256"] for key, value in current.items()
            }
            baseline_hashes = {
                key: value["row_sha256"] for key, value in baseline.items()
            }
            current_net = current["WALCL"]["value"]
            baseline_net = baseline["WALCL"]["value"]
            delta = current_net - baseline_net
            target = "QQQ" if delta > 0.0 else "SPY"
            ...
            identity = {
                ...
                "current_net_liquidity_usd_millions": current_net,
                "baseline_net_liquidity_usd_millions": baseline_net,
                "delta_net_liquidity_usd_millions": delta,
    ...
    return json.dumps(
        {
            ...
            "signal_contract": {
                "series_ids": list(runner.FED_SERIES),
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (12, 16)
