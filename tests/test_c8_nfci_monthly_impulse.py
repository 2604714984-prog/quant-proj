...
from research.adapters.c8_nfci_monthly_impulse import (
...
from scripts import run_c8_nfci_monthly_impulse_once as runner
...
def test_signal_packet_reproduces_frozen_support_without_price_access() -> None:
    ...
    assert sum(row.target == 1.0 for row in groups["validation"]) == 19
    assert sum(row.target == 0.0 for row in groups["validation"]) == 9
    assert sum(row.target == 1.0 for row in groups["retrospective_holdout"]) == 23
    assert sum(row.target == 0.0 for row in groups["retrospective_holdout"]) == 11
