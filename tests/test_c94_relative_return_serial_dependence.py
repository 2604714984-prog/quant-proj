...
from research.adapters import c94_relative_return_serial_dependence as adapter
from research.adapters.c94_relative_return_serial_dependence import (
...
from scripts import run_c94_relative_return_serial_dependence_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 9_400_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch47_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_correlation = 0.1 if index % 2 == 0 else -0.1
            spy_correlation = 0.0
            difference = qqq_correlation - spy_correlation
            ...
            row: dict[str, object] = {
                ...
                "lag_pair_count": 62,
                "qqq_lag_one_return_correlation": qqq_correlation,
                "relative_serial_dependence_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_lag_one_return_correlation": spy_correlation,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c94-relative-return-serial-dependence-input-v1",
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_serial_dependence_difference"] = 0.2
