...
from research.adapters import c151_dgs2_3m_policy_expectation as adapter
from research.adapters.c151_dgs2_3m_policy_expectation import (
...
from scripts import run_c151_dgs2_3m_policy_expectation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 15_100_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            comparison_yield = 4.0
            current_yield = 3.5 if qqq else 4.5
    ...
    return json.dumps(
        {
            "schema_version": "c151-dgs2-spy-qqq-full-input-v1",
...
def test_real_input_and_runtime_bundles_preflight_before_any_claim() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 6
    assert sum(row.target_symbol == "SPY" for row in groups["validation"]) == 22
    assert (
        sum(
