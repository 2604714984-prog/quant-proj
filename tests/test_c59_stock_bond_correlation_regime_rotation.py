...
from research.adapters import c59_stock_bond_correlation_regime_rotation as adapter
from research.adapters.c59_stock_bond_correlation_regime_rotation import (
...
from scripts import run_c59_stock_bond_correlation_regime_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 5_900_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch12_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "IPO", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "IPO", "100", previous_reference, previous_session
            )
            right_current = source(
                "SPY", "100", current_reference, current_session
            )
            right_previous = source(
                "SPY", "100", previous_reference, previous_session
            )
    ...
    return json.dumps(
        {
            "schema_version": "epoch10-factor-leadership-input-packet-v1",
            "research_id": runner.RESEARCH_ID,
            "strategy_outcomes_opened": False,
            "price_values_used_for_signal": True,
            "signal_price_values_used_for_materialization": True,
...
def test_synthetic_signal_packet_reproduces_frozen_price_signal_only() -> None:
    ...
    groups = runner._correlation_signals(payload)
    assert len(groups["validation"]) == VALIDATION_INTERVALS
    assert len(groups["retrospective_holdout"]) == HOLDOUT_INTERVALS
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 16
    assert sum(row.target_symbol == "SPY" for row in groups["validation"]) == 12
    assert sum(row.target_symbol == "QQQ" for row in groups["retrospective_holdout"]) == 9
    assert sum(row.target_symbol == "SPY" for row in groups["retrospective_holdout"]) == 25
...
def test_signal_packet_rejects_state_tampering() -> None:
    ...
    original = record["signals"][0]["target_symbol"]
    record["signals"][0]["target_symbol"] = "SPY" if original == "QQQ" else "QQQ"
    payload = (json.dumps(record, sort_keys=True, separators=(",", ":")) + "\n").encode()
    with pytest.raises(runner.InputBlockedError, match="chronology mismatch"):
        runner._correlation_signals(payload)
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    groups = runner._correlation_signals(runner.SIGNAL_INPUT.read_bytes())
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (25, 9)
