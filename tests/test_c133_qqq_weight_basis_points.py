...
from research.adapters import c133_qqq_weight_basis_points as adapter
from research.adapters.c133_qqq_weight_basis_points import (
...
from scripts import run_c133_qqq_weight_basis_points_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_300_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            row: dict[str, object] = {
                ...
                "formation_session_count": 0,
                "formation_start": signal_session.isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "qqq_weight_basis_points": qqq_weight_basis_points,
    ...
    return json.dumps(
        {
            "schema_version": "c133-qqq-weight-basis-points-input-v1",
...
def test_synthetic_packet_reproduces_frozen_rule_without_outcomes() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 28
    assert sum(
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_weight_basis_points"] = 13
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="fixed-weight mismatch"):
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (0, 28)
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (0, 34)
