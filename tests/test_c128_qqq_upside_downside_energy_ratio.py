...
from research.adapters import c128_qqq_upside_downside_energy_ratio as adapter
from research.adapters.c128_qqq_upside_downside_energy_ratio import (
...
from scripts import run_c128_qqq_upside_downside_energy_ratio_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_800_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch77_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_upside_downside_energy_ratio = 1.25 if index % 2 == 0 else 0.75
            target = "QQQ" if qqq_upside_downside_energy_ratio > 1.0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "formation_session_count": 64,
                "formation_start": (signal_session - timedelta(days=90)).isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "qqq_upside_downside_energy_ratio": qqq_upside_downside_energy_ratio,
    ...
    return json.dumps(
        {
            "schema_version": "c128-qqq-upside-downside-energy-ratio-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "Sum of squared positive returns divided by sum of squared negative "
                    "returns from 63 simple QQQ returns formed by exactly 64 adjusted closes"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_upside_downside_energy_ratio"] = -1.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="energy-ratio range mismatch"):
