...
from research.adapters.c13_m2_growth_acceleration import (
...
from scripts import run_c13_m2_growth_acceleration_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.00001220703125
    assert BOOTSTRAP_RESAMPLES == 200_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_500_101
...
def test_constant_positive_active_holdout_passes_exact_inference() -> None:
    ...
    assert decision.inference.centered_null_one_sided_p == pytest.approx(1 / 200001)
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            current, prior = ((0.02, 0.01) if qqq else (0.01, 0.01))
            identity = {
                "series_id": "M2SL",
                "reference_month": start.isoformat()[:7],
                "reference_m2_billions": 102.0 if qqq else 101.0,
                "reference_year_ago_m2_billions": 100.0,
                "prior_reference_month": _month(start, -1).isoformat()[:7],
                "prior_m2_billions": 101.0,
                "prior_year_ago_m2_billions": 100.0,
                "fred_m2sl_response_sha256": "a" * 64,
            }
            rows.append(
                {
                    "stage": stage,
                    ...
                    "current_yoy": current,
                    "prior_yoy": prior,
                    "comparison": "current_yoy_gt_prior_yoy_selects_QQQ",
    ...
    return json.dumps(
        {
            "schema_version": "c13-m2-growth-acceleration-spy-qqq-input-packet-v1",
