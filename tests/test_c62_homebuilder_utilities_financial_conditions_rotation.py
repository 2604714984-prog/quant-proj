...
from research.adapters import (
    c62_homebuilder_utilities_financial_conditions_rotation as adapter,
)
from research.adapters.c62_homebuilder_utilities_financial_conditions_rotation import (
...
from scripts import (
    run_c62_homebuilder_utilities_financial_conditions_rotation_once as runner,
)
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 6_200_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch15_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "XHB", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "XHB", "100", previous_reference, previous_session
            )
            right_current = source(
                "XLU", "100", current_reference, current_session
            )
            right_previous = source(
                "XLU", "100", previous_reference, previous_session
            )
