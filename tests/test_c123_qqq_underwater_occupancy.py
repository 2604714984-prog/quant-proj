...
from research.adapters import c123_qqq_underwater_occupancy as adapter
from research.adapters.c123_qqq_underwater_occupancy import (
...
from scripts import run_c123_qqq_underwater_occupancy_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_300_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch76_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_earlier_underwater_fraction = 0.50
            qqq_later_underwater_fraction = 0.25 if index % 2 == 0 else 0.75
            target = (
                "QQQ"
                if qqq_later_underwater_fraction <= qqq_earlier_underwater_fraction
                else "SPY"
            )
            ...
            row: dict[str, object] = {
                ...
                "formation_session_count": 126,
                "formation_start": (signal_session - timedelta(days=190)).isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "qqq_earlier_underwater_fraction": qqq_earlier_underwater_fraction,
                "qqq_later_underwater_fraction": qqq_later_underwater_fraction,
    ...
    return json.dumps(
        {
            "schema_version": "c123-qqq-underwater-occupancy-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "underwater-session fraction in the later 63-session half "
                    "versus the earlier 63-session half, resetting each half's "
                    "running high"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_later_underwater_fraction"] = 42.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="fraction contract mismatch"):
