...
from research.adapters import c136_qqq_large_loss as adapter
from research.adapters.c136_qqq_large_loss import (
...
from scripts import run_c136_qqq_large_loss_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_600_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch80_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_worst_daily_return = -0.02 if index % 2 == 0 else -0.04
            target = "QQQ" if qqq_worst_daily_return > -0.03 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "qqq_worst_daily_return": qqq_worst_daily_return,
    ...
    return json.dumps(
        {
            "schema_version": "c136-qqq-large-loss-input-v1",
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_worst_daily_return"] = -1.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="worst-return range mismatch"):
