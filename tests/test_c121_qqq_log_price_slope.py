...
from research.adapters import c121_qqq_log_price_slope as adapter
from research.adapters.c121_qqq_log_price_slope import (
...
from scripts import run_c121_qqq_log_price_slope_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_100_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch74_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_log_price_slope = 0.01 if index % 2 == 0 else -0.01
            target = "QQQ" if qqq_log_price_slope > 0.0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "qqq_log_price_slope": qqq_log_price_slope,
    ...
    return json.dumps(
        {
            "schema_version": "c121-qqq-log-price-slope-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "ordinary-least-squares slope of log QQQ adjusted close on "
                    "integer time 0 through 62 over exactly 63 completed sessions"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_log_price_slope"] = 42.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="signal identity mismatch"):
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (8, 26)
