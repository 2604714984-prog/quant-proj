...
from research.adapters import c108_relative_breakout_persistence as adapter
from research.adapters.c108_relative_breakout_persistence import (
...
from scripts import run_c108_relative_breakout_persistence_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 10_800_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch61_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            spy_count = 5
            qqq_count = 6 if index % 2 == 0 else 4
            difference = qqq_count - spy_count
            ...
            row: dict[str, object] = {
                ...
                "breakout_observation_count": 63,
                "qqq_breakout_count": qqq_count,
                "relative_breakout_count_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_breakout_count": spy_count,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c108-relative-breakout-persistence-input-v1",
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_breakout_count_difference"] = 2
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (26, 8)
