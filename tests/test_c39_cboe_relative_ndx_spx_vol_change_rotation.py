...
from research.adapters import c39_cboe_relative_ndx_spx_vol_change_rotation as adapter
from research.adapters.c39_cboe_relative_ndx_spx_vol_change_rotation import (
...
from scripts import run_c39_cboe_relative_ndx_spx_vol_change_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 3_900_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch4_mechanism_count"] == 3
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            current_ratio, baseline_ratio = (
                (1.01, 1.0) if index % 2 == 0 else (0.99, 1.0)
            )

            def source(ratio: float, observed: date) -> dict:
                components = {"VIX": 20.0, "VXN": 20.0 * ratio}
                identity = {
                    "series_id": "VXN_DIV_VIX",
                    "source_class": "OFFICIAL_CBOE_STATIC_HISTORY_RETROSPECTIVE_ONLY",
                    "source_urls": urls,
                    "raw_response_sha256": raw_sha,
                    "observation_date": observed.isoformat(),
                    "component_values_index_points": components,
                    "value_ratio": ratio,
                    "available_at_conservative": (
                        f"{observed.isoformat()}T20:00:00-05:00"
                    ),
                    "retrieved_at": {
                        "VIX": "2026-07-23T15:25:27.205733+08:00",
                        "VXN": "2026-07-23T15:25:31.218292+08:00",
                    },
                }
                return {
                    **identity,
            ...
            current = source(current_ratio, decision_date)
            baseline = source(baseline_ratio, baseline_cutoff)
    ...
    return json.dumps(
        {
            "schema_version": "c39-cboe-relative-ndx-spx-vol-change-spy-qqq-input-packet-v1",
