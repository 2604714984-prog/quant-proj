...
from research.adapters import c127_qqq_sign_persistence_share as adapter
from research.adapters.c127_qqq_sign_persistence_share import (
...
from scripts import run_c127_qqq_sign_persistence_share_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_700_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_sign_persistence_share = 0.6 if index % 2 == 0 else 0.4
            target = "QQQ" if qqq_sign_persistence_share > 0.5 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "qqq_sign_persistence_share": qqq_sign_persistence_share,
    ...
    return json.dumps(
        {
            "schema_version": "c127-qqq-sign-persistence-share-input-v1",
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_sign_persistence_share"] = 42.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="sign-persistence range mismatch"):
