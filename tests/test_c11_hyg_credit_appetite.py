...
from research.adapters.c11_hyg_credit_appetite import (
...
from scripts import run_c11_hyg_credit_appetite_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.000048828125
    assert BOOTSTRAP_RESAMPLES == 50_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_300_101
...
def test_constant_positive_active_holdout_passes_exact_inference() -> None:
    ...
    assert decision.inference.centered_null_one_sided_p == pytest.approx(1 / 50001)
...
def test_signal_packet_reproduces_frozen_support_without_price_access() -> None:
    ...
    assert sum(row.target == 1.0 for row in groups["validation"]) == 14
    assert sum(row.target == 0.0 for row in groups["validation"]) == 14
    assert sum(row.target == 1.0 for row in groups["retrospective_holdout"]) == 25
    assert sum(row.target == 0.0 for row in groups["retrospective_holdout"]) == 9
    record = json.loads(payload)
    assert record["strategy_outcomes_opened"] is False
    assert record["spy_price_values_used"] is False
    lowered = payload.lower()
    for forbidden in (b'"terminal_wealth"', b'"nav"', b'"sharpe"', b'"strategy_returns"', b'"spy_returns"'):
