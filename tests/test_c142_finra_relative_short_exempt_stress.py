...
from research.adapters import c142_finra_relative_short_exempt_stress as adapter
from research.adapters.c142_finra_relative_short_exempt_stress import (
    HOLDOUT_INTERVALS,
    InputContractError,
...
from scripts import run_c142_finra_relative_short_exempt_stress_once as runner
...
def test_real_signal_packet_is_exact_and_state_supported() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in validation) == 45
    assert sum(row.target_symbol == "SPY" for row in validation) == 84
    assert sum(row.target_symbol == "QQQ" for row in holdout) == 28
    assert sum(row.target_symbol == "SPY" for row in holdout) == 128
...
def test_signal_packet_rejects_target_and_identity_tampering() -> None:
    ...
    record = json.loads(runner.SIGNAL_INPUT.read_bytes())
    record["signals"][0]["observation_sessions"][-1] = (
        record["signals"][0]["decision_session"]
    )
...
def test_circular_block_indices_are_frozen_and_wrap() -> None:
    observed = circular_block_indices(
        7,
        block_length=4,
        rng=random.Random(14200101),
    )
    assert observed == (4, 5, 6, 0, 2, 3, 4)
...
def test_holdout_bootstrap_and_seven_gates_are_deterministic() -> None:
    ...
    assert decision.inference.seed == 14_200_101
