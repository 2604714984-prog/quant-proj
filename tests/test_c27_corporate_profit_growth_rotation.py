...
from research.adapters import c27_corporate_profit_growth_rotation as adapter
from research.adapters.c27_corporate_profit_growth_rotation import (
...
from scripts import run_c27_corporate_profit_growth_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.0000000007450580596923828125
    assert BOOTSTRAP_RESAMPLES == 2_048_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 2_400_101
...
def _synthetic_signal_packet() -> bytes:
        ...
        for index in range(count):
            ...
            def source(value: float, cutoff: date, observation: date) -> dict:
                identity = {
                    ...
                    "source_class": "OFFICIAL_ALFRED_BEA_CORPORATE_PROFITS_AFTER_TAX",
                }
                return {
                    **identity,
    ...
    return json.dumps(
        {
            "schema_version": "c27-corporate-profit-growth-spy-qqq-input-packet-v1",
