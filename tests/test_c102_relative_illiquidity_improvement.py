...
from research.adapters import c102_relative_illiquidity_improvement as adapter
from research.adapters.c102_relative_illiquidity_improvement import (
...
from scripts import run_c102_relative_illiquidity_improvement_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 10_200_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch55_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_score = 1.1 if index % 2 == 0 else 0.9
            spy_score = 1.0
            difference = qqq_score - spy_score
            target = "QQQ" if difference < 0.0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "formation_session_count": 84,
                "formation_start": (signal_session - timedelta(days=100)).isoformat(),
                "holding_end_execution_session": end.isoformat(),
                "holding_start_execution_session": start.isoformat(),
                "baseline_session_count": 63,
                "qqq_baseline_mean_illiquidity": 100.0,
                "qqq_recent_mean_illiquidity": 100.0 * qqq_score,
                "qqq_illiquidity_improvement_score": qqq_score,
                "recent_session_count": 21,
                "relative_illiquidity_improvement_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_session_count": 85,
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_baseline_mean_illiquidity": 100.0,
                "spy_recent_mean_illiquidity": 100.0,
                "spy_illiquidity_improvement_score": spy_score,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c102-relative-illiquidity-improvement-input-v1",
            ...
            "volume_values_used_for_signal": True,
            ...
            "signal_contract": {
                ...
                "daily_illiquidity_definition": (
                    "absolute simple adjusted-close return divided by raw close "
                    "times raw volume"
                ),
                "score_definition": (
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_illiquidity_improvement_difference"] = 0.2
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (13, 21)
