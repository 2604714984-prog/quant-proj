...
from research.adapters import c101_cboe_relative_oil_equity_volatility as adapter
from research.adapters.c101_cboe_relative_oil_equity_volatility import (
...
from scripts import run_c101_cboe_relative_oil_equity_volatility_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 10_100_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch54_mechanism_count"] == 1
    ...
    assert record["boundaries"]["provider_or_network_call"] is True
    assert all(
        value is False
        for key, value in record["boundaries"].items()
        if key != "provider_or_network_call"
    )
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            row: dict[str, object] = {
                ...
                "current_vix": 20.0,
                "current_ovx": current_ratio * 20.0,
    ...
    return json.dumps(
        {
            "schema_version": "c101-cboe-relative-oil-equity-volatility-input-v1",
