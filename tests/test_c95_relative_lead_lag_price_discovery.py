...
from research.adapters import c95_relative_lead_lag_price_discovery as adapter
from research.adapters.c95_relative_lead_lag_price_discovery import (
...
from scripts import run_c95_relative_lead_lag_price_discovery_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 9_500_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch48_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_leads_spy = 0.1 if index % 2 == 0 else -0.1
            spy_leads_qqq = 0.0
            difference = qqq_leads_spy - spy_leads_qqq
            ...
            row: dict[str, object] = {
                ...
                "lead_lag_pair_count": 62,
                "qqq_leads_spy_correlation": qqq_leads_spy,
                "relative_lead_lag_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_leads_qqq_correlation": spy_leads_qqq,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c95-relative-lead-lag-price-discovery-input-v1",
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_lead_lag_difference"] = 0.2
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (22, 12)
