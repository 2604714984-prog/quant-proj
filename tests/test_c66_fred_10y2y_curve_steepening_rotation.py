...
from research.adapters import c66_fred_10y2y_curve_steepening_rotation as adapter
from research.adapters.c66_fred_10y2y_curve_steepening_rotation import (
...
from scripts import run_c66_fred_10y2y_curve_steepening_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 6_600_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch19_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    rows = []
    state_support = {}
    raw_sha = "e4b22b0dbbc9fecbd3d0c2cf31d1fa8aece72924ac5605007fb28277adfbc3ae"
    url = (
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            current_observation = decision_date
            previous_observation = current_reference - timedelta(days=1)
            current_value, previous_value = (
                (0.5, 0.4) if index % 2 == 0 else (0.3, 0.4)
            )
            ...
            def source(value: float, observed: date, reference: date) -> dict:
                identity = {
                    ...
                    "last_observation_date": observed.isoformat(),
                    "observation_count": 20,
                    "mean_spread_pct_points": value,
                    "constituent_rows_sha256": "1" * 64,
                    "available_at_conservative": (
                        f"{observed.isoformat()}T20:00:00-05:00"
                    ),
                    "availability_rule": (
    ...
    return json.dumps(
        {
            "schema_version": "c66-t10y2y-curve-steepening-input-packet-v1",
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "strict 10Y2Y curve steepening selects QQQ; equality or decline selects SPY"
                ),
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (18, 10)
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (15, 19)
