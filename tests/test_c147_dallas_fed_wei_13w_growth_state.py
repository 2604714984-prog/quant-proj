...
from research.adapters import c147_dallas_fed_wei_13w_growth_state as adapter
from research.adapters.c147_dallas_fed_wei_13w_growth_state import (
    HOLDOUT_INTERVALS,
    InputContractError,
...
from scripts import run_c147_dallas_fed_wei_13w_growth_state_once as runner
...
def test_exact_definition_adapter_data_and_shared_core_identities() -> None:
    ...
    assert sha(runner.SOURCE_ROWS) == runner.SOURCE_ROWS_SHA256
...
def test_real_signal_packet_is_exact_and_state_supported() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in validation) == 31
    assert sum(row.target_symbol == "SPY" for row in validation) == 98
    assert sum(row.target_symbol == "QQQ" for row in holdout) == 113
    assert sum(row.target_symbol == "SPY" for row in holdout) == 43
...
def test_signal_packet_rejects_target_and_identity_tampering() -> None:
    ...
    record = json.loads(runner.SIGNAL_INPUT.read_bytes())
    record["signals"][0]["snapshot_cutoff_date"] = record["signals"][0][
        "decision_session"
    ]
    ...
    record = json.loads(runner.SIGNAL_INPUT.read_bytes())
    record["signals"][0]["trailing_13_week_mean"] = "invalid"
    with pytest.raises(runner.InputBlockedError, match="WEI value is invalid"):
        runner._signals(json.dumps(record).encode())
...
def test_circular_block_indices_are_frozen_and_wrap() -> None:
    observed = circular_block_indices(
        7,
        block_length=4,
        rng=random.Random(14700101),
    )
    assert observed == (2, 3, 4, 5, 2, 3, 4)
...
def test_holdout_bootstrap_and_seven_gates_are_deterministic() -> None:
    ...
    assert decision.inference.seed == 14_700_101
