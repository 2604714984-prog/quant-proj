...
from research.adapters import c132_spy_qqq_correlation as adapter
from research.adapters.c132_spy_qqq_correlation import (
...
from scripts import run_c132_spy_qqq_correlation_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 13_200_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            spy_qqq_correlation = 0.90 if index % 2 == 0 else 0.97
            target = "QQQ" if spy_qqq_correlation < 0.95 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "spy_qqq_correlation": spy_qqq_correlation,
    ...
    return json.dumps(
        {
            "schema_version": "c132-spy-qqq-correlation-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "Contemporaneous Pearson correlation of 63 SPY and QQQ simple "
                    "returns formed from exactly 64 common adjusted closes"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["spy_qqq_correlation"] = 42.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="correlation range mismatch"):
