...
from research.adapters import c139_cboe_equity_put_call_sentiment as adapter
from research.adapters.c139_cboe_equity_put_call_sentiment import (
    HOLDOUT_INTERVALS,
    InputContractError,
...
from scripts import run_c139_cboe_equity_put_call_sentiment_once as runner
...
def test_real_signal_packet_is_exact_and_state_supported() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in holdout) == 97
    assert sum(row.target_symbol == "SPY" for row in holdout) == 59
...
def test_circular_block_indices_are_frozen_and_wrap() -> None:
    observed = circular_block_indices(
        7,
        block_length=4,
        rng=random.Random(13900101),
    )
    assert observed == (1, 2, 3, 4, 0, 1, 2)
...
def test_holdout_bootstrap_and_seven_gates_are_deterministic() -> None:
    ...
    assert decision.inference.seed == 13_900_101
