...
from research.adapters import c149_nyfed_empire_current_business_state as adapter
from research.adapters.c149_nyfed_empire_current_business_state import (
...
from scripts import run_c149_nyfed_empire_current_business_state_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 14_900_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            diffusion_index = 3.5 if qqq else -2.0
    ...
    return json.dumps(
        {
            "schema_version": "c149-empire-spy-qqq-full-input-v1",
