...
from research.adapters import c41_finra_margin_debit_change_rotation as adapter
from research.adapters.c41_finra_margin_debit_change_rotation import (
...
from scripts import run_c41_finra_margin_debit_change_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_100_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            decision_at = f"{decision_date.isoformat()}T09:05:00+08:00"
            current_reference = _month(start, -3)
            previous_reference = _month(start, -4)
            current_value, previous_value = (
                (110, 100) if index % 2 == 0 else (90, 100)
            )

            def available_at(reference: date) -> str:
                following = _month(reference, 1)
                final_day = _month(following, 1) - timedelta(days=1)
            ...
            def source(value: int, reference: date) -> dict:
                identity = {
                    "series_id": "FINRA_CUSTOMER_MARGIN_DEBIT_BALANCE",
                    "source_class": "OFFICIAL_FINRA_MONTHLY_AGGREGATE",
                    "source_page": source_page,
                    "source_url": source_url,
                    "raw_xlsx_sha256": (
                        "821d0a247cb646d38312994ac9a709a7a7c5cac32b9aa6c6d05c7eda4007331c"
                    ),
                    "reference_month": reference.isoformat(),
                    "value_usd_millions": value,
                    "available_at_conservative": available_at(reference),
                    "availability_rule": (
                        "23:59:59 America/New_York on the final calendar day "
                        "of the month following the reference month"
                    ),
                }
                return {
                    **identity,
            ...
            current = source(current_value, current_reference)
            previous = source(previous_value, previous_reference)
    ...
    return json.dumps(
        {
            "schema_version": "c41-finra-margin-debit-change-spy-qqq-input-packet-v1",
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (10, 24)
