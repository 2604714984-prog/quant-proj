...
from research.adapters import c155_reserve_balances_13w_liquidity as adapter
from research.adapters.c155_reserve_balances_13w_liquidity import (
...
from scripts import run_c155_reserve_balances_13w_liquidity_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 15_500_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            comparison_balance = 3_000_000.0
            current_balance = 3_100_000.0 if qqq else 2_900_000.0
            impulse = current_balance - comparison_balance
            ...
            comparison_observation = current_observation - timedelta(days=91)
            identity = {
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c155-wresbal-spy-qqq-full-input-v1",
...
def test_real_input_and_runtime_bundles_preflight_before_any_claim() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 13
    assert sum(row.target_symbol == "SPY" for row in groups["validation"]) == 15
    assert (
        sum(
