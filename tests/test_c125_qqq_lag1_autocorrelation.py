...
from research.adapters import c125_qqq_lag1_autocorrelation as adapter
from research.adapters.c125_qqq_lag1_autocorrelation import (
...
from scripts import run_c125_qqq_lag1_autocorrelation_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_500_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_lag1_autocorrelation = 0.25 if index % 2 == 0 else -0.25
            target = "QQQ" if qqq_lag1_autocorrelation > 0.0 else "SPY"
            ...
            row: dict[str, object] = {
                ...
                "qqq_lag1_autocorrelation": qqq_lag1_autocorrelation,
    ...
    return json.dumps(
        {
            "schema_version": "c125-qqq-lag1-autocorrelation-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "Pearson correlation between the first 62 and following 62 values "
                    "of 63 simple QQQ returns formed by exactly 64 adjusted closes"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_lag1_autocorrelation"] = 42.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="autocorrelation range mismatch"):
