...
from research.adapters import c24_permit_housing_financing_rotation as adapter
from research.adapters.c24_permit_housing_financing_rotation import (
...
from scripts import run_c24_permit_housing_financing_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.0000000059604644775390625
    assert BOOTSTRAP_RESAMPLES == 256_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 2_100_101
...
def _synthetic_signal_packet() -> bytes:
        ...
        for index in range(count):
            ...
            def source(value: float, cutoff: date, observation: date) -> dict:
                identity = {
                    ...
                    "source_class": "OFFICIAL_ALFRED_PERMIT",
                }
                return {
                    **identity,
    ...
    return json.dumps(
        {
            "schema_version": "c24-permit-housing-financing-rotation-spy-qqq-input-packet-v1",
