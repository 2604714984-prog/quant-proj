...
from research.adapters import c143_nyfed_sofr_effr_spread_impulse as adapter
from research.adapters.c143_nyfed_sofr_effr_spread_impulse import (
    HOLDOUT_INTERVALS,
    InputContractError,
...
from scripts import run_c143_nyfed_sofr_effr_spread_impulse_once as runner
...
def test_real_signal_packet_is_exact_and_state_supported() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in validation) == 94
    assert sum(row.target_symbol == "SPY" for row in validation) == 35
    assert sum(row.target_symbol == "QQQ" for row in holdout) == 99
    assert sum(row.target_symbol == "SPY" for row in holdout) == 57
...
def test_signal_packet_rejects_target_and_identity_tampering() -> None:
    ...
    record = json.loads(runner.SIGNAL_INPUT.read_bytes())
    record["signals"][0]["current_effective_date"] = record["signals"][0][
        "decision_session"
    ]
    ...
    record = json.loads(runner.SIGNAL_INPUT.read_bytes())
    record["signals"][0]["rate_spread_impulse_percentage_points"] += 0.01
    with pytest.raises(runner.InputBlockedError, match="spread impulse is invalid"):
        runner._signals(json.dumps(record).encode())
...
def test_circular_block_indices_are_frozen_and_wrap() -> None:
    observed = circular_block_indices(
        7,
        block_length=4,
        rng=random.Random(14300101),
    )
    assert observed == (5, 6, 0, 1, 5, 6, 0)
...
def test_holdout_bootstrap_and_seven_gates_are_deterministic() -> None:
    ...
    assert decision.inference.seed == 14_300_101
