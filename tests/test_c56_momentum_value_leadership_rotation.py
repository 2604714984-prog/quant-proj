...
from research.adapters import c56_momentum_value_leadership_rotation as adapter
from research.adapters.c56_momentum_value_leadership_rotation import (
...
from scripts import run_c56_momentum_value_leadership_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 5_600_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch10_mechanism_count"] == 3
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "MTUM", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "MTUM", "100", previous_reference, previous_session
            )
            right_current = source(
                "VTV", "100", current_reference, current_session
            )
            right_previous = source(
                "VTV", "100", previous_reference, previous_session
            )
