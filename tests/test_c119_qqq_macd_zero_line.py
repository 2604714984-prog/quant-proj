...
from research.adapters import c119_qqq_macd_zero_line as adapter
from research.adapters.c119_qqq_macd_zero_line import (
...
from scripts import run_c119_qqq_macd_zero_line_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 11_900_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch72_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_ema26 = 100.0
            qqq_ema12 = 102.0 if index % 2 == 0 else 98.0
            qqq_macd = qqq_ema12 - qqq_ema26
            target = "QQQ" if qqq_macd > 0.0 else "SPY"
    ...
    return json.dumps(
        {
            "schema_version": "c119-qqq-macd-zero-line-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "EMA12 minus EMA26 after initializing both at the first of "
                    "exactly 126 QQQ adjusted closes and applying alphas 2/13 "
                    "and 2/27 sequentially"
                ),
                "target_rule": (
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_macd"] = 42.0
