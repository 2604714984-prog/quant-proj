...
from research.adapters import c98_relative_volume_expansion_attention as adapter
from research.adapters.c98_relative_volume_expansion_attention import (
...
from scripts import run_c98_relative_volume_expansion_attention_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 9_800_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch51_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            row: dict[str, object] = {
                ...
                "qqq_baseline_mean_raw_volume": 100.0,
                "qqq_recent_mean_raw_volume": 100.0 * qqq_score,
                "qqq_volume_expansion_score": qqq_score,
                "recent_session_count": 21,
                "relative_volume_expansion_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_baseline_mean_raw_volume": 100.0,
                "spy_recent_mean_raw_volume": 100.0,
                "spy_volume_expansion_score": spy_score,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c98-relative-volume-expansion-attention-input-v1",
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_volume_expansion_difference"] = 0.2
