...
from research.adapters import c97_relative_close_location_accumulation as adapter
from research.adapters.c97_relative_close_location_accumulation import (
...
from scripts import run_c97_relative_close_location_accumulation_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 9_700_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch50_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_accumulation = 0.1 if index % 2 == 0 else -0.1
            spy_accumulation = 0.0
            difference = qqq_accumulation - spy_accumulation
            ...
            row: dict[str, object] = {
                ...
                "qqq_close_location_money_flow": qqq_accumulation,
                "relative_close_location_accumulation_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_close_location_money_flow": spy_accumulation,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c97-relative-close-location-accumulation-input-v1",
