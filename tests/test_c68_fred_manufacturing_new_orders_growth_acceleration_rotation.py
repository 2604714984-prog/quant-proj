...
from research.adapters import c68_fred_manufacturing_new_orders_growth_acceleration_rotation as adapter
from research.adapters.c68_fred_manufacturing_new_orders_growth_acceleration_rotation import (
...
from scripts import run_c68_fred_manufacturing_new_orders_growth_acceleration_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 6_800_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch21_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    rows = []
    state_support = {}
    raw_sha = "70abe9ab8194d53df8ea899698227b179dd5b6e78a7c33e08f0a842618a4ecec"
    url = (
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            decision_at = f"{decision_date.isoformat()}T20:05:00-05:00"
            current_reference = _month(start, -3)
            previous_reference = _month(start, -4)
            current_observation = current_reference
            previous_observation = previous_reference
            current_value, previous_value = (
                (120.0, 110.0) if index % 2 == 0 else (100.0, 110.0)
            )

            def source(value: float, observed: date, reference: date) -> dict:
                identity = {
                    "series_id": "FRED_AMTMNO",
                    "source_class": "FEDERAL_RESERVE_FRED_OFFICIAL_CURRENT_HISTORY_RETROSPECTIVE_SECONDARY",
                    "source_url": url,
                    "raw_response_sha256": raw_sha,
                    "reference_month": reference.isoformat(),
                    "observation_date": observed.isoformat(),
                    "current_millions_usd": value,
                    "prior_year_reference_month": _month(reference, -12).isoformat(),
                    "prior_year_observation_date": _month(reference, -12).isoformat(),
                    "prior_year_millions_usd": 100.0,
                    "yoy_growth": value / 100.0 - 1.0,
                    "available_at_conservative": (
                        f"{_month(reference, 2).replace(day=15).isoformat()}T20:00:00-05:00"
                    ),
                    "availability_rule": (
            ...
            current = source(current_value, current_observation, current_reference)
            previous = source(
                previous_value, previous_observation, previous_reference
            )
            delta = current["yoy_growth"] - previous["yoy_growth"]
    ...
    return json.dumps(
        {
            "schema_version": "c68-amtmno-growth-acceleration-input-packet-v1",
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "strict manufacturing-orders growth acceleration selects SPY; equality or deceleration selects QQQ"
                ),
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (validation.count("SPY"), validation.count("QQQ")) == (10, 18)
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (14, 20)
