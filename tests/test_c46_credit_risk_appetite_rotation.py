...
from research.adapters import c46_credit_risk_appetite_rotation as adapter
from research.adapters.c46_credit_risk_appetite_rotation import (
...
from scripts import run_c46_credit_risk_appetite_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_600_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "HYG", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "HYG", "100", previous_reference, previous_session
            )
            right_current = source(
                "IEF", "100", current_reference, current_session
            )
            right_previous = source(
                "IEF", "100", previous_reference, previous_session
            )
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (11, 23)
