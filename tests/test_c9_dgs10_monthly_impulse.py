...
from research.adapters.c9_dgs10_monthly_impulse import (
...
from scripts import run_c9_dgs10_monthly_impulse_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.0001953125
    assert BOOTSTRAP_RESAMPLES == 10_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_199_101
...
def test_signal_packet_reproduces_frozen_support_without_price_access() -> None:
    ...
    assert sum(row.target == 1.0 for row in groups["validation"]) == 12
    assert sum(row.target == 0.0 for row in groups["validation"]) == 16
