...
from research.adapters import (
    c63_vix_term_structure_compression_rotation as adapter,
)
from research.adapters.c63_vix_term_structure_compression_rotation import (
...
from scripts import (
    run_c63_vix_term_structure_compression_rotation_once as runner,
)
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 6_300_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch16_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current_value = "90" if index % 2 == 0 else "110"
            ...
            left_current = source(
                "^VIX", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "^VIX", "100", previous_reference, previous_session
            )
            right_current = source(
                "^VIX3M", "100", current_reference, current_session
            )
            right_previous = source(
                "^VIX3M", "100", previous_reference, previous_session
            )
            ...
            identity = {
                ...
                "relative_ratio_cross_product": cross_product,
    ...
    return json.dumps(
        {
            ...
            "signal_contract": {
                ...
                "target_rule": (
                    "strict prior-month VIX/VIX3M ratio decline selects QQQ; "
                    "equality or increase selects SPY"
                ),
                "threshold": 0,
            },
            "signals": rows,
