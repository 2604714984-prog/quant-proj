...
from research.adapters import c156_us_epu_trailing_90d_median as adapter
from research.adapters.c156_us_epu_trailing_90d_median import (
...
from scripts import run_c156_us_epu_trailing_90d_median_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 15_600_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            current_epu = 90.0 if qqq else 110.0
            distance = current_epu - trailing_median
            ...
            trailing_start = current_observation - timedelta(days=90)
            trailing_end = current_observation - timedelta(days=1)
            identity = {
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c156-usepu-spy-qqq-full-input-v1",
