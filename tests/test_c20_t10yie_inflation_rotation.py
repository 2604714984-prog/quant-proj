...
from research.adapters import c20_t10yie_inflation_rotation as adapter
from research.adapters.c20_t10yie_inflation_rotation import (
...
from scripts import run_c20_t10yie_inflation_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert PROGRAM_ALPHA == 0.000000095367431640625
    assert BOOTSTRAP_RESAMPLES == 16_000_000
    assert BOOTSTRAP_BLOCK_MONTHS == 3
    assert BOOTSTRAP_SEED == 1_700_801
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            previous_start = _month(start, -1)
            previous_decision_at = f"{previous_start.isoformat()}T09:00:00-05:00"
            ...
            def source(value: float, cutoff: date, observation: date) -> dict:
                identity = {
                    ...
                    "value_percent": value,
                    "realtime_start_date": observation.isoformat(),
                    ...
                    "source_class": "OFFICIAL_ALFRED_T10YIE",
                }
                return {
                    **identity,
            ...
            current = source(current_value, start, start - timedelta(days=2))
            previous = source(
                previous_value,
                previous_start,
                previous_start - timedelta(days=2),
            )
            identity = {
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c20-t10yie-inflation-rotation-spy-qqq-input-packet-v1",
