...
from research.adapters import c159_cfnai_diffusion_state as adapter
from research.adapters.c159_cfnai_diffusion_state import (
...
from scripts import run_c159_cfnai_diffusion_state_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 15_900_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq = index % 2 == 1
            diffusion = 0.25 if qqq else -0.25
            ...
            current_observation = decision_date - timedelta(days=20)
            identity = {
                "actual_vintage_date": (decision_date - timedelta(days=5)).isoformat(),
                "current_cfnai_diffusion_index": diffusion,
    ...
    return json.dumps(
        {
            "schema_version": "c159-cfnaidiff-spy-qqq-full-input-v1",
...
def test_real_input_and_runtime_bundles_preflight_before_any_claim() -> None:
    ...
    assert sum(row.target_symbol == "QQQ" for row in groups["validation"]) == 24
    assert sum(row.target_symbol == "SPY" for row in groups["validation"]) == 4
    assert (
        sum(
