...
from research.adapters import c117_relative_positive_return_streak as adapter
from research.adapters.c117_relative_positive_return_streak import (
...
from scripts import run_c117_relative_positive_return_streak_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 11_700_101
...
def test_definition_is_one_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert multiplicity["epoch70_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            spy_streak = 3
            qqq_streak = 4 if index % 2 == 0 else 2
            difference = qqq_streak - spy_streak
            ...
            row: dict[str, object] = {
                ...
                "qqq_longest_positive_return_streak": qqq_streak,
                "relative_positive_return_streak_difference": difference,
                "signal_session": signal_session.isoformat(),
                "source_row_hash_set_sha256": hashlib.sha256(
                    f"{stage}-{index}-rows".encode()
                ).hexdigest(),
                "spy_longest_positive_return_streak": spy_streak,
                "stage": stage,
    ...
    return json.dumps(
        {
            "schema_version": "c117-relative-positive-return-streak-input-v1",
            ...
            "signal_contract": {
                ...
                "score_definition": (
                    "maximum number of consecutive simple daily returns "
                    "strictly greater than zero among exactly 62 returns"
                ),
                "target_rule": (
                    "QQQ longest positive-return streak strictly greater than "
                    "SPY selects QQQ; equality or lower selects SPY"
                ),
            },
            "signals": rows,
...
def test_signal_packet_rejects_arithmetic_and_target_tampering() -> None:
    ...
    record["signals"][0]["relative_positive_return_streak_difference"] = 2
...
def test_real_input_bytes_are_bound_and_support_matches() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (20, 14)
