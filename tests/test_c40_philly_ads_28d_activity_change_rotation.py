...
from research.adapters import c40_philly_ads_28d_activity_change_rotation as adapter
from research.adapters.c40_philly_ads_28d_activity_change_rotation import (
...
from scripts import run_c40_philly_ads_28d_activity_change_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 4_000_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch5_mechanism_count"] == 3
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            baseline_cutoff = decision_date - timedelta(days=28)
            current_value, baseline_value = (
                (0.1, 0.0) if index % 2 == 0 else (-0.1, 0.0)
            )
            vintage_date = decision_date
            vintage_header = f"ADS_INDEX_{vintage_date.strftime('%m%d%y')}"

            def source(value: float, observed: date) -> dict:
                identity = {
                    "series_id": "ADS_REALTIME_VINTAGE",
                    "source_class": "OFFICIAL_PHILADELPHIA_FED_REALTIME_VINTAGE",
                    "source_page": source_page,
                    "source_url": source_url,
                    "raw_zip_sha256": "fcb0317952ed9d0b83b47f653002e9bc0593583983ddf509a6e3a84ebe088b59",
                    "xlsx_sha256": "5d2f2801a3f8314cec157c9d244e064921b29b29e15614ef73a6f4b35da93be8",
                    "vintage_header": vintage_header,
                    "vintage_date": vintage_date.isoformat(),
                    "available_at_conservative": (
                        f"{vintage_date.isoformat()}T20:00:00-05:00"
                    ),
                    "observation_date": observed.isoformat(),
                    "value_index_points": value,
                }
                return {
                    **identity,
            ...
            current = source(current_value, decision_date)
            baseline = source(baseline_value, baseline_cutoff)
            ...
            identity = {
                ...
                "vintage_header": vintage_header,
                "vintage_date": vintage_date.isoformat(),
                "current_observation_date": decision_date.isoformat(),
                "baseline_cutoff_date": baseline_cutoff.isoformat(),
                "baseline_observation_date": baseline_cutoff.isoformat(),
    ...
    return json.dumps(
        {
            "schema_version": "c40-philly-ads-28d-change-spy-qqq-input-packet-v2",
