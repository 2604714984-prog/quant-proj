...
from research.adapters import c89_spy_five_percent_correction_regime_rotation as adapter
from research.adapters.c89_spy_five_percent_correction_regime_rotation import (
...
from scripts import run_c89_spy_five_percent_correction_regime_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 8_900_101
...
def test_definition_is_single_variant_outcome_blind_and_nontrading() -> None:
    ...
    assert record["program_multiplicity"]["epoch42_mechanism_count"] == 1
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            peak = 100.0
            final_close = 96.0 if index % 2 == 0 else 94.0
            drawdown = final_close / peak - 1.0
            target = "SPY" if drawdown <= -0.05 else "QQQ"
            ...
            row = {
                ...
                "spy_peak_adjusted_close": peak,
                "spy_final_adjusted_close": final_close,
                "spy_current_drawdown": drawdown,
    ...
    return json.dumps(
        {
            "schema_version": "c89-spy-five-percent-correction-regime-input-v1",
            ...
            "signal_contract": {
                "formation_close_count": 126,
                "current_drawdown": (
                    "final adjusted close divided by maximum adjusted close "
                    "in the 126-close window minus one"
                ),
                "correction_threshold": -0.05,
                "target_rule": (
                    "SPY current drawdown at or below -0.05 means SPY; "
                    "strictly above -0.05 means QQQ"
                ),
...
def test_real_input_bytes_are_hash_bound_and_signals_match_frozen_support() -> None:
    ...
    assert (holdout.count("SPY"), holdout.count("QQQ")) == (5, 29)
