...
from research.adapters import c52_treasury_duration_leadership_rotation as adapter
from research.adapters.c52_treasury_duration_leadership_rotation import (
...
from scripts import run_c52_treasury_duration_leadership_rotation_once as runner
...
def test_frozen_definition_adapter_and_program_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 5_200_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            left_current = source(
                "TLT", left_current_value, current_reference, current_session
            )
            left_previous = source(
                "TLT", "100", previous_reference, previous_session
            )
            right_current = source(
                "BIL", "100", current_reference, current_session
            )
            right_previous = source(
                "BIL", "100", previous_reference, previous_session
            )
    ...
    return json.dumps(
        {
            ...
            "gld_price_values_used_for_materialization": False,
