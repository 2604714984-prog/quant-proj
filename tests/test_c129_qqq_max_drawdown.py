...
from research.adapters import c129_qqq_max_drawdown as adapter
from research.adapters.c129_qqq_max_drawdown import (
...
from scripts import run_c129_qqq_max_drawdown_once as runner
...
def test_frozen_definition_adapter_core_and_constants_are_exact() -> None:
    ...
    assert BOOTSTRAP_SEED == 12_900_101
...
def _synthetic_signal_packet() -> bytes:
    ...
    for stage, first, count in (
        ...
        for index in range(count):
            ...
            qqq_max_drawdown = -0.05 if index % 2 == 0 else -0.15
            target = "QQQ" if qqq_max_drawdown > -0.10 else "SPY"
    ...
    return json.dumps(
        {
            "schema_version": "c129-qqq-max-drawdown-input-v1",
...
def test_signal_packet_rejects_signal_and_target_tampering() -> None:
    ...
    record["signals"][0]["qqq_max_drawdown"] = 1.0
    payload = json.dumps(record, sort_keys=True, separators=(",", ":")).encode()
    with pytest.raises(runner.InputBlockedError, match="maximum-drawdown range mismatch"):
