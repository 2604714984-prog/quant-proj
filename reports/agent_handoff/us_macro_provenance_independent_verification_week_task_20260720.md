...
- any applicable revision predates the cutoff;
- a later actual-release page is not used to backfill prior knowledge;
- cancellations are not treated as events;
...
If none exists, the row remains blocked.
...
- event day must be an accepted XNYS session;
...
- row counts by family, year, status, and collision group;
- duplicate event IDs;
- naive timestamps;
- nonfinite values;
...
- any revision accepted without prior availability evidence;
- any unresolved collision counted twice;
