# Database DB2 Lane A1 Dispatch

BATCH_ID: CENTRAL_DB_DB2_A1_20260713
LANE_ID: A1
PRIORITY: P0
TARGET_DATABASE: /home/rongyu/workspace/quant_data/quant_research.duckdb
TARGET_TABLES: a_share_daily_basic
...
SOURCE_POLICY: official or explicitly authorized structured source; relay is secondary/unqualified unless separately accepted
SYMBOL_SCOPE: A0 symbols
DATE_RANGE: A0 date range
REQUIRED_FIELDS: ts_code|trade_date|circ_mv|total_mv|turnover_rate|volume_ratio|pe|pe_ttm|pb|ps|ps_ttm|dv_ratio|source|available_at|ingested_at|quality_status
PRIMARY_KEY: must be frozen per physical table before execution; duplicate count must be zero
PIT_REQUIREMENTS: available by decision-date close; revision identity retained
ADJUSTMENT_REQUIREMENTS: not applicable
CORPORATE_ACTION_REQUIREMENTS: share-count changes must reconcile with corporate actions
QUALITY_THRESHOLDS: duplicate keys=0; raw join>=98%; circ_mv and total_mv eligible-symbol coverage>=98%; systemic missing clusters=0
DEPENDENCIES: A0
...
CALLBACK_SCHEMA: A_SHARE_DB_CALLBACK_P0_CIRC_MV
GITHUB_BRANCH: codex/central-db-db2-a1-20260713
