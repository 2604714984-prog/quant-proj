# Database DB2 Lane U1 Dispatch

BATCH_ID: CENTRAL_DB_DB2_U1_20260713
LANE_ID: U1
PRIORITY: P0
...
SOURCE_POLICY: official sponsor/SEC documents plus accepted authorized price source and independent reconciliation
SYMBOL_SCOPE: QQQ|GLD
DATE_RANGE: 2016-01-01 through latest completed session
REQUIRED_FIELDS: symbol|trade_date|open|high|low|close|adjusted_close|total_return_index|volume|dividend_amount|split_factor|available_at|source|quality_status
PRIMARY_KEY: must be frozen per physical table before execution; duplicate count must be zero
PIT_REQUIREMENTS: publication/availability identity required for actions
ADJUSTMENT_REQUIREMENTS: explicit dividend+split methodology; raw close is not total return
CORPORATE_ACTION_REQUIREMENTS: dividends/splits/expense treatment reconciled
QUALITY_THRESHOLDS: duplicate keys=0; expected sessions complete; action reconciliation pass
DEPENDENCIES: U0
...
CALLBACK_SCHEMA: US_DB_CALLBACK_P0A
GITHUB_BRANCH: codex/central-db-db2-u1-20260713
