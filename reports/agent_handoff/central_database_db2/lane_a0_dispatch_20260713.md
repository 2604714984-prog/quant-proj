# Database DB2 Lane A0 Dispatch

BATCH_ID: CENTRAL_DB_DB2_A0_20260713
LANE_ID: A0
PRIORITY: P0
TARGET_DATABASE: /home/rongyu/workspace/quant_data/quant_research.duckdb
TARGET_TABLES: a_share_trade_calendar|a_share_daily_raw|a_share_symbol_master_history
...
SOURCE_POLICY: official exchange calendars plus accepted public/authorized structured market source
SYMBOL_SCOPE: all SSE/SZSE common A shares
DATE_RANGE: full available history through latest completed session
REQUIRED_FIELDS: ts_code|trade_date|open|high|low|close|pre_close|volume|amount|pct_chg|source|source_timestamp|ingested_at|quality_status|name|exchange|board|list_date|delist_date|list_status|status_effective_from|status_effective_to
PRIMARY_KEY: must be frozen per physical table before execution; duplicate count must be zero
PIT_REQUIREMENTS: source_timestamp and immutable available_at no later than consumer cutoff
ADJUSTMENT_REQUIREMENTS: raw prices only; no adjusted or total-return label
CORPORATE_ACTION_REQUIREMENTS: listing/delisting identity required
QUALITY_THRESHOLDS: duplicate keys=0; expected sessions complete; no future rows; listing consistency
...
CALLBACK_SCHEMA: A_SHARE_DB_CALLBACK_P0_CORE
GITHUB_BRANCH: codex/central-db-db2-a0-20260713
