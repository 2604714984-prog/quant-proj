"""One-hypothesis CPI release-day SPY intraday research adapter."""
...
def frozen_monthly_release_dates(
    ...
    """Select the last ALFRED CPI date in each month to drop revision-only dates."""
    ...
    return tuple(max(by_month[key]) for key in sorted(by_month))
...
def execute(definition_path: Path, database_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "input_classification": "RETROSPECTIVE_SECONDARY_SOURCE_NOT_STRICT_PIT",
