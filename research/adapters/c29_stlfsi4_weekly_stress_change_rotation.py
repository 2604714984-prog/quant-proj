"""Frozen statistics for the removable Cycle 29 STLFSI4 SPY/QQQ lane."""
...
RESEARCH_ID = "C29_STLFSI4_WEEKLY_STRESS_CHANGE_ROTATION_SPY_QQQ_V1"
INITIAL_CAPITAL = 40_000.0
VALIDATION_INTERVALS = 77
HOLDOUT_INTERVALS = 104
PROGRAM_ALPHA = 0.000000000186264514923095703125
BOOTSTRAP_RESAMPLES = 8_192_000_000
BOOTSTRAP_BLOCK_WEEKS = 13
BOOTSTRAP_SEED = 2_500_101
...
def _returns(
    values: tuple[float, ...], field: str, expected: int
) -> tuple[float, ...]:
    ...
    for value in values:
        ...
        if not math.isfinite(number) or number <= -1.0:
            raise InputContractError(
                f"{field} values must be finite and greater than -1"
            )
...
def _paired(
    ...
    comparator = _returns(comparator_returns, "comparator_returns", expected)
    active = tuple(
...
def circular_block_indices(
    ...
    *,
    ...
    while len(indices) < sample_size:
        ...
        indices.extend(
            (start + offset) % sample_size for offset in range(block_length)
        )
...
def bootstrap_inference(active_returns: tuple[float, ...]) -> BootstrapInference:
    ...
    block_count = math.ceil(HOLDOUT_INTERVALS / BOOTSTRAP_BLOCK_WEEKS)
    ...
    active_full = block_sums(active, BOOTSTRAP_BLOCK_WEEKS)
    active_final = block_sums(active, final_block_length)
    centered_full = block_sums(centered, BOOTSTRAP_BLOCK_WEEKS)
...
def holdout_decision(
    ...
    gates = (
        ...
        ("exactly_104_paired_intervals", len(active) == HOLDOUT_INTERVALS),
        (
            "centered_null_one_sided_p_at_most_0_000000000186264514923095703125",
            inference.centered_null_one_sided_p <= PROGRAM_ALPHA,
        ),
        (
            "uncentered_type7_0_000000000186264514923095703125_lower_bound_strictly_positive",
