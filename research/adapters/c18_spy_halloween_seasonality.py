"""Frozen statistics for the removable Cycle 18 SPY Halloween lane."""
...
RESEARCH_ID = "C18_SPY_HALLOWEEN_SEASONALITY_V1"
INITIAL_CAPITAL = 40_000.0
VALIDATION_INTERVALS = 28
HOLDOUT_INTERVALS = 34
PROGRAM_ALPHA = 0.0000003814697265625
BOOTSTRAP_RESAMPLES = 8_000_000
BOOTSTRAP_BLOCK_MONTHS = 6
BOOTSTRAP_SEED = 1_500_601
...
def _paired(
    ...
    spy = _returns(spy_returns, "spy_returns", expected)
    active = tuple(left - right for left, right in zip(strategy, spy, strict=True))
    if not all(math.isfinite(value) for value in active):
        raise InputContractError("active returns must be finite")
    return strategy, spy, active
...
def validation_decision(
    ...
    strategy, spy, active = _paired(
        strategy_returns, spy_returns, VALIDATION_INTERVALS
    )
    strategy_performance = _performance(strategy)
    spy_performance = _performance(spy)
    mean_active = statistics.fmean(active)
    gates = (
        (
            "strategy_terminal_net_wealth_strictly_above_spy",
            strategy_performance.terminal_wealth > spy_performance.terminal_wealth,
        ),
        ("arithmetic_mean_paired_active_return_strictly_positive", mean_active > 0.0),
        (
            "strategy_maximum_drawdown_no_worse_than_spy",
            strategy_performance.maximum_drawdown >= spy_performance.maximum_drawdown,
        ),
    )
...
def bootstrap_inference(active_returns: tuple[float, ...]) -> BootstrapInference:
    ...
    weight = rank - lower
    lower_bound = (
        smallest[lower] * (1.0 - weight) + smallest[upper] * weight
    )
...
def holdout_decision(
    ...
    strategy, spy, active = _paired(strategy_returns, spy_returns, HOLDOUT_INTERVALS)
    strategy_performance = _performance(strategy)
    spy_performance = _performance(spy)
    ...
    gates = (
        ...
        (
            "centered_null_one_sided_p_at_most_0_0000003814697265625",
            inference.centered_null_one_sided_p <= PROGRAM_ALPHA,
        ),
        (
            "uncentered_type7_0_0000003814697265625_lower_bound_strictly_positive",
