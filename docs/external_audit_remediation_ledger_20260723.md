...
| QF-002 | P0 | CODE_FIXED_R2_REAL_DATA_EVIDENCE_PENDING | `DecisionArtifact`; `capture_decision_artifact`; `_execute_frozen_adapter`; `run_candidate_rebalance` | THIS_COMMIT_PENDING | `pytest -q tests/test_event_loop.py tests/test_source_capture.py`; focused Ruff | `70 passed`; Ruff passed; candidate API rejects caller weights and derives normalized weights by executing the sole frozen declarative adapter contract | 0 | 0 | Fixture proves computation and byte binding only; real frozen feature/dataset/strategy evidence remains external. |
...
| QF-011 | P1 | PARTIAL_R2_STAGE_BINDING_FIXED | `run_candidate_rebalance`; candidate/adverse identity and `_hashes`; `research/identity.py`; stage plan continuity | THIS_COMMIT_PENDING | `pytest -q tests/test_event_loop.py tests/test_source_capture.py`; focused Ruff | `70 passed`; Ruff passed; identical weights from different frozen definitions now produce different stage hashes | 0 | 0 | Candidate stage hash now binds decision artifact, dataset, split, and costs; semantic hash revalidation and durable no-restart stage continuity remain for a later atomic commit. |
