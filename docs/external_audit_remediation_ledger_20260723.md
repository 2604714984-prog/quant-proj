...
|---|---|---|---|---|---|---|---:|---:|---|
| QF-001 | P0 | CODE_FIXED_R2_PROVIDER_EVIDENCE_BLOCKED | `data/source_identity.py::SourceIdentity.capture_level`; `capture_github_release_asset`; `require_provider_qualified_source`; `event_loop._require_candidate_sources`; `run_candidate_rebalance` interface grade | 751b7d5e6536051d19d9bb238d1b78f8e7aa55bd | `pytest -q tests/test_source_capture.py tests/test_event_loop.py tests/test_public_api_boundaries.py`; focused Ruff | `76 passed`; Ruff passed; generic caller captures are explicitly experimental; concrete GitHub adapter derives provider/subject/url/revision/available/retrieved metadata from fetched responses; all-generic candidate mechanics return `GENERIC_CAPTURE_EXPERIMENT` | 0 | 0 | No provider-qualified market-data adapter/response receipt is available, so no tested run reaches `CONTROLLED_CANDIDATE_INPUT`; real PIT value binding remains `BLOCKED_EXTERNAL_EVIDENCE`, not fixture-closed. |
| QF-015 | P0 | CODE_FIXED_R2_PROVIDER_MIGRATION_EVIDENCE_BLOCKED | `SourceIdentity.capture_level`; capture receipt payload; `select_source_revision`; provider-qualified adapter boundary | 751b7d5e6536051d19d9bb238d1b78f8e7aa55bd | Same QF-001/QF-015 focused suite | `76 passed`; Ruff passed; generic self-asserted migration metadata cannot yield candidate grade; provider-qualified GitHub adapter derives a stable source family and does not accept migration SHA input | 0 | 0 | Any real URL-family migration still requires provider/independent migration evidence and remains external. |
| QF-002 | P0 | CODE_FIXED_R2_REAL_DATA_EVIDENCE_PENDING | `DecisionArtifact`; `capture_decision_artifact`; `_execute_frozen_adapter`; captured feature/definition/adapter sources; `run_candidate_rebalance` | 7c95259f19a404fbbf05a3851930ee5316de4c45; 03a5fcdd205cec1e15370e473397e84730ece06c | `pytest -q tests/test_event_loop.py`; focused Ruff | `61 passed`; Ruff passed; candidate API rejects caller weights, executes frozen declarative bytes, binds each byte set to a capture receipt and rejects artifact availability after `decision_at` | 0 | 0 | Fixture proves computation, byte and time-gate mechanics only; all fixture captures are generic so the run remains experimental until real provider-qualified artifacts exist. |
| QF-003 | P0 | CODE_FIXED_R2_REAL_UNIVERSE_EVIDENCE_PENDING | `markets/universe.py::materialize_universe_partition`; frozen `lifecycle_eligible` rule execution; `UniverseMaterialization` | 1cc5c49dbcbcc10371989330792b84871e76c472 | `pytest -q tests/test_event_loop.py`; focused Ruff | `61 passed`; Ruff passed; caller inclusion-decision parameter removed and every complete-partition symbol is included/excluded solely by the executed lifecycle rule | 0 | 0 | Fixture proves full-partition rule mechanics only; provider-qualified historical partition and lifecycle evidence remain external. |
...
| QF-004 | P1 | CODE_FIXED_R2_EXTERNAL_ANCHOR_RECEIPT_PENDING | `research/experiments.py::preregister_trial`; `record_holdout_result`; `persist_experiment_ledger`; `ExperimentLedgerReceipt`; `require_adjusted_holdout_for_candidate`; `event_loop.run_candidate_rebalance` | a08b11f64b8bc08309bcb8387ed8677d191f67ac; 4a83dcf967c4e538dab6c7e17c40b6182ff6a787 | `pytest -q tests/test_experiment_receipts.py tests/test_event_loop.py tests/test_public_api_boundaries.py`; focused Ruff | `71 passed`; Ruff passed; preregistration timestamps/anchor/alpha/family size, durable prefix verification, PASSED derivation, Holm/Bonferroni recomputation, complete family, split-evaluation receipt and candidate binding enforced | 0 | 0 | Fixture proves persistence and selection-control mechanics only; independent external publication receipt for the preregistration head remains `BLOCKED_EXTERNAL_EVIDENCE`. |
| QF-005 | P1 | CODE_FIXED_R2_REAL_RETURNS_PENDING | `research/splits.py::evaluate_split`; sealed `SplitEvaluation`; computed HAC long-run variance; deterministic moving-block bootstrap; `event_loop.run_candidate_rebalance` | 3ed04138b862f9fc401f7940eff63a47d74abd31; 4a83dcf967c4e538dab6c7e17c40b6182ff6a787 | `pytest -q tests/test_research_splits_identity.py tests/test_event_loop.py`; focused Ruff | `82 passed`; Ruff passed; caller effective-N removed; returns/estimator/evaluation hashes, statistic, standard error, bandwidth/block length and computed effective-N recorded and bound into candidate stage hash | 0 | 0 | Fixture return series proves estimator mechanics only; real frozen return/residual evidence remains external. |
| QF-011 | P1 | CODE_FIXED_R2_REAL_ARTIFACTS_PENDING | `research/identity.py::capture_dataset_manifest`; `DatasetManifest.verify_identity`; `ExperimentEvent.stage_plan_sha256`; `event_loop.run_candidate_rebalance`; candidate/adverse identity and `_hashes` | 7c95259f19a404fbbf05a3851930ee5316de4c45; 5f6e5b0b7428663d3bd6fa47133bcc4804f43c1e; 389ac356a0a62f8c50a803ca5ee115f034506aef | `pytest -q tests/test_research_splits_identity.py tests/test_event_loop.py tests/test_public_api_boundaries.py`; focused Ruff | `88 passed`; Ruff passed; feature/label/split/calendar/action/cost and partition hashes are internally computed and byte-revalidated, dataset costs equal execution costs, full preregistered stage plan required, and all displayed artifacts enter stage hash | 0 | 0 | Generic/free-hash manifests are permanently experimental; real captured partitions/policies, external preregistration anchor, and stage-run evidence remain external. |
...
- `external_audit_verdict=PENDING_UNIFIED_REVIEW`
...
- `live_trading_authorized=false`
- `automatic_execution_authorized=false`
- No fixture or generic capture reached `CONTROLLED_CANDIDATE_INPUT`; all such runs are explicitly `GENERIC_CAPTURE_EXPERIMENT`.
- Project/production database writes: `0`.
- Market-data provider requests: `0`.
- Package-index and GitHub control-plane requests used for build/PR administration are not market-data provider requests.
- Remaining evidence is recorded per finding and includes real provider-qualified PIT observations, real frozen datasets/returns/actions/cost calibration, an independent preregistration anchor receipt, URL-migration evidence where applicable, and QF-014's independent reviewer receipt.
